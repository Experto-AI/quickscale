...
**Correct shape.** Tenant-table membership should be **declarative and enumerable**, and RLS + the conformance check should derive from that one declaration. A model is tenant-owned by inheriting one `TenantModel` base (or being registered in a tenant-table registry); a single migration operation (`EnableTenantRLS(model)`) emits the policy from one source string; and a CI conformance test walks `apps.get_models()`, selects everything carrying the tenant marker, and asserts each has (a) an `organization_id` column or an explicit, declared parent-scoping policy and (b) a live FORCE-RLS policy in `pg_policies`. Child tables must be a *declared* category — either they carry `organization_id`, or they are registered with a parent-join RLS policy — never an implicit "the app will remember to join."

**Alternatives.**
- **(A — preferred) Declarative tenant-table registry + generated RLS + CI conformance test.** One `TenantModel`/registry marker is the single source of truth; a reusable `EnableTenantRLS` migration operation generates the policy; a conformance test fails the build when any registered table lacks a policy and when any child table is neither covered nor explicitly exempted with a parent-join policy. *Preferred: it converts "did someone remember?" into a property the build enforces, kills the six-way SQL duplication, and makes the child-table category impossible to forget silently — the failure mode moves from "silent prod leak" to "red CI."*
- **(B) Keep per-module migrations, add only the conformance test.** Smallest change; catches missing coverage at CI. But it leaves the duplicated policy SQL and the manual table lists, so the *write* tax and drift risk remain — the test tells you a table is uncovered without removing the reason tables get left uncovered.
- **(C) Add `organization_id` to every child table (denormalize the FK down) and drop parent-join scoping entirely.** Makes every tenant table uniformly RLS-able with the same policy, no parent-derived special case. Strong isolation story, but it denormalizes the schema, needs backfills + a trigger/constraint to keep child `organization_id` equal to the parent's, and is a larger data migration — worth doing under (A)'s registry, not instead of it.
