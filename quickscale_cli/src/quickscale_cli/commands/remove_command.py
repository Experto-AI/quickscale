...
def remove(module_name: str, force: bool, keep_data: bool) -> None:
    ...
    module_in_state = state and module_name in state.modules
    ...
    if state and module_name in state.modules:
    ...
    if _update_quickscale_yml(project_path, module_name):
        click.secho("  ✅ Updated quickscale.yml", fg="green")
    else:
        click.secho("  ⚠️  Failed to update quickscale.yml", fg="yellow")
