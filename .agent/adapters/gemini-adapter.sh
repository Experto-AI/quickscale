...
generate_gemini_md() {
    cat "$(template_path "gemini_header.md")" > "$GEMINI_MD"
...
generate_agents() {
    while IFS=$'\t' read -r name description path; do
        ...
        local source_file out_file skills workflows delegates body
        ...
        delegates=$( {
            ...
            get_frontmatter_list "$source_file" "parent_agents"
...

        {
            ...
            if [[ -n "$skills" || -n "$workflows" || -n "$delegates" ]]; then
            ...
            if [[ -n "$skills" ]]; then
                ...
                while IFS= read -r skill; do
                    [[ -n "$skill" ]] || continue
                    printf -- '- @.agent/skills/%s/SKILL.md\n' "$skill"
                done <<< "$skills"
                printf '\n'
            fi
...
main() {
    ...
    assert_capability_value "gemini_cli" "supports.commands" "toml_prompt"
    assert_capability_value "gemini_cli" "supports.settings" "json"
