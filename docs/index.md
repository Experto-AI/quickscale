...
- **Technical**
  ...
  - [v88 Ticket Context](technical/v88_ticket_context.md) - Current v88 ledger: eight open v88 ticket entries across eight open merge positions, lanes W1 3 · W2 4 · W3 1; SA170/TA70 and SA167c (EV-7) are archived after their accepted release campaigns; SA166 is runnable with `deps: none`; SA165 heads W1 with `deps: none` as an integrated retained partial with SA165-R1 still tracked and one replacement release verdict authorized as EV-8; SA171 is release-accepted after SA176 preserved the serialized acquisition key and restored the full `make ci` gate, so SA172 now heads W3 with `deps: none`; no open ticket remains on the release critical path and no maintainer decision is open
