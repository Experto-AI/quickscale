...
The v0.85.0 (billing), v0.86.0 (organizations/multi-tenancy), and v0.87.0 (hardening) releases moved billing, tenant isolation, fail-closed database access, and module configuration into the shipped contract. The release-prepared v0.88.0 line adds one canonical request identity across generated projects and moves module Django wiring into the manifests. The boundary still shows in the frontend: some billing entry points and browser-side surfaces respect the fresh-generation versus manual-adoption split for existing projects.
