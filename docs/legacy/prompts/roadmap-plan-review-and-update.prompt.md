...
2. Review the selected release's implementation plan in `roadmap.md` (apply plan.md guidelines):
	...
	- If the release introduces a new module:
		- Verify the plan includes a task to add the module to `AVAILABLE_MODULES` in module configuration/commands. So quickscale plan/apply can work with it.
		- Verify the plan includes a task to update the `showcase_html` default theme (adding it to the "Installed Modules" list in `index.html.j2`), in order to show it in the showcase. See other modules for examples.
		- Verify to finally publish the module with ./scripts/publish_module.sh
