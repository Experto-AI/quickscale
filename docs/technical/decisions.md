...
**Status (2026-07-07):** target state, partially landed. `OrgApiBaseView` exists and is now used by all eight `OrgApi*` views (SA50 complete). Billing's four plain-View endpoints haven't been migrated yet (SA56). Org-role-scoped JSON endpoints now use the sanctioned `OrgApiBaseView`; billing's plain-View + manual-CSRF idiom remains as the last unsanctioned pattern still in production code. The SA46 CI gate closes the silent-CSRF-hole failure mode in the meantime.
