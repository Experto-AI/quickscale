...
- Current boot guard: `orgs.QuickscaleOrgsConfig.ready()` asserts `rolbypassrls=false` in SaaS mode with `DEBUG=False`; `migrate` remains the only exempt command path until SA2.1 widens that guard
