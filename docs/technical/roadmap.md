...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: none` — SA125 ✓ closed) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2. Its authorized continuation is preserved as recorded partial delivery; SA121 remains open on `SA121-CR-003`, `SA121-CR-006`, and `SA121-QG-001`, but **carries no open decision** — `SA121-DEC-002` authorized the minimal parser correction, and QG-001 now has only Track 3's two complexity blockers.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Open findings and their owning child (task remains open; no decision pending).** `SA117A-PLAN-010` (**high**, completeness — final staging must preserve the exact four-file tree for full-scope Review B), `SA117A-PLAN-013` (**medium**, completeness — every phase needs fail-fast literal final-stage and pre-merge commands), `SA117A-PLAN-014` (**medium**, breaking-change safety — merge-back needs an armed rollback transaction), and `SA117A-CR-002` (**medium**, test gap — the update rollback regression does not prove `_sync_module_dependencies` stays uncalled) → **SA117a**. Its separate quality blocker remains `_perform_module_embed` CC 22 > 20 under `SA117-QG-REMED`. `SA117-CR-003` (**high**, security — blank direct-CLI origins bypass the pre-mutation public-source gate), `SA117-CONT-PR-003` (**high**, resilience — interruption leaks the locally owned PostgreSQL validation container), and `SA117-CONT-PR-006` (**medium**, completeness — no secure literal bootstrap; staged-index emptiness assertion is not filename-safe) → **SA117b**. `SA117-CR-006` (**high**, completeness — lock-drift route omits complete baseline lock semantics) → **SA117c**. `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker. Each child needs its own scoped plan-review `STATUS: ok` before any source edit, per the serial handoff contract below.
  ...
  **`SA117-QG-REMED` — quality-ceiling remediation. `Track 3` · `deps: none` (SA125 ✓ closed).** `SA121-QG-001` recorded the exceeded ceilings introduced by `43d9b8fc`. After `SA125-DEC-001` retired the file line-ceiling gate, **two complexity findings remain**, each already inside its child's declared allowlist:
  ...
  - **Rule:** each child must leave `make quality` no worse than it found it and clear its own share before its slice review; per SA121's shrink-only rule, **raising a complexity ceiling is not an acceptable remedy** — reduce the branching, or record a structured waiver once SA121 ships the waiver ledger.
  - Conflict surface: none beyond each child's existing allowlist. SA117a/SA117b do not touch `scripts/quality_baseline.json` at all, and SA121's continuation is prohibited from touching it under `SA121-DEC-001`; no current phase owns that file.
  ...
  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: none (SA125 ✓ closed) · blocks SA112b` — **critical path**
    ...
    - Verify: all twelve `module.yml` versions equal `VERSION`; padded/non-canonical variants are rejected with an explicit version-mismatch error naming both versions; a deliberately skewed embedded manifest is rejected before the downstream `KeyError`; an `apply` selecting all 12 modules reaches managed-wiring regeneration with no `KeyError`; **its `SA117-QG-REMED` share is cleared** — `_perform_module_embed` CC ≤ 20, with no baseline ceiling raised. (No line-count remediation is owed; do not reduce `module_commands.py` or `manifest/loader.py` for gate reasons.)
    *(why →* this is the release blocker proper, and the only part of SA117 that SA112b's evidence validity depends on*)*

    **Recorded partial checkpoints:** 2026-07-27 change-review cap (delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)); two 2026-07-28 plan-review caps (no source delta; task remains open).
    - **Done:** clean Track 3 sync to `v87` at `24919792`, environment preflight, and two bounded planner/reviewer cycles. The literal late-sync transaction now resolves `SA117A-CONT-PR-008` / `SA117A-PLAN-011` at plan-detail level, and the eight-case cleanup matrix resolves `SA117A-PLAN-012`; no implementation or completion evidence landed.
    - **Pending / blocking:** `SA117A-PLAN-010` (**high**, completeness — the final staging block must keep the exact four-file tree staged for Review B), `SA117A-PLAN-013` (**medium**, completeness — every phase needs fail-fast literal final-stage and pre-merge commands), and `SA117A-PLAN-014` (**medium**, breaking-change safety — merge-back needs an armed rollback transaction). Source work remains blocked on `SA117A-CR-002` (**medium**, test gap) and `_perform_module_embed` CC 22 > 20.
    - **Decisions needed:** none; the maintainer selected recorded partial delivery at the plan-review cap. **Next action:** begin a fresh scoped continuation with a literal plan that closes `SA117A-PLAN-010`, `-013`, and `-014`, obtain plan-review `STATUS: ok`, then implement the missing dependency-sync spy and CC reduction, run the focused/full quality chain, and obtain full-scope change-review `STATUS: ok`; only then may SA117a be checked and Track 3 advance to SA117b.
...
**Current status — `make quality` is RED (recorded 2026-07-27 as `SA121-QG-001`; rescoped 2026-07-28 by `SA125-DEC-001`).** The exit criteria above are not currently met. There are **zero line-count blockers**; the only remaining `make quality` blockers are two complexity findings, both owned by Track 3's `SA117-QG-REMED`:

- `_perform_module_embed` CC 22 > 20 (**SA117a**)
- `_update_single_module` CC 18 > 17 (**SA117b**)

Both originated with the SA117 preservation checkpoint `43d9b8fc` (recorded partial delivery, unapproved). Until Track 3 remediates them, no track may claim the green-gate milestone, and SA121 cannot close because its own acceptance requires `make quality` green.
...
SA121 (baseline monotonicity) ◄─ next SA115 (e2e xdist; deps: none) SA117a (exact lockstep)
  │  (partial; DEC-002; deps: none) │  (validation AUTHORIZED)     ▲   + CC 22→20 share only
  │                                │                             │   deps: none; unblocked
  ▼                                │                             ▼
...
**Track 1 is internally serial and has one downstream consumer.** SA121 is next, SA122a follows, and SA122b carries the SA112e merge bound. Track 1 no longer gates any Track 3 work: SA125's completed deletion was the last cross-track dependency, and it is closed.
...
- **Track 1's two runnable tickets stay on one worktree by choice, not by dependency.** SA121's continuation and SA122a are file-disjoint — SA121 touches `scripts/check_quality.sh` and its focused tests, SA122a adds a new registry + parity checker touching no SA112/SA117 allowlist and needing no PostgreSQL/Docker — so they *could* run concurrently. **They are not split**, because Track 1 is one worktree, Track 2 cannot host them (below), and a fourth worktree would buy no release time: neither ticket is on or feeds the critical path, so parallelizing them shortens nothing that matters. Only `decisions.md` is conditionally shared, and the Merge procedure covers it. SA122b is the single further ordering exception, bounded below.
...
**Conflict surface.** The shared closeout files are `CHANGELOG.md` and `docs/technical/roadmap.md`, plus `docs/technical/decisions.md` when policy or acceptance evidence changes (SA122a writes its publish-context disposition there; SA121's continuation touches quality-policy prose). All three are covered by the Merge procedure above: the `git merge v87` before every merge-back forces conflicts to resolve on the track branch, preserving both sides. Two executable surfaces are also shared and already bounded: `.github/workflows/e2e.yml`'s path list between SA112e and SA115-CI-001 (serialized by the `merge after SA112` bound), and `scripts/quality_baseline.json`, whose line-count section is gone (SA125 ✓) — SA117a/SA117b do not modify it, and SA121's continuation is contractually forbidden from touching it (`SA121-DEC-001`), so the tracks cannot collide and no additional procedure is required.

### Track readiness (2026-07-28)

> **Re-verified against code on 2026-07-28 (third pass).** The two green-gate blockers were measured, not carried forward from prose: `radon cc` reports `_perform_module_embed` **22** against a baseline ceiling of 20 and `_update_single_module` **18** against 17 (`scripts/quality_baseline.json:104-115`). `SA125-DEC-001` is confirmed implemented — `scripts/quality_baseline.json` contains **zero** `large_files` entries and `scripts/check_quality.sh` retains that data as report-only output (`:587` "intentionally ignored"), so no line-count finding can block. SA121's merged partial delivery is present and inert as recorded: `scripts/quality_waivers.json` exists with an empty `waivers` array. No completed work was found still sitting in this file, and no audit finding became closeable this pass.

- **Track 1 — DECISION-CLEAR AND RUNNABLE NOW.** The track carries open **SA121**, open **SA122a**, and merge-gated **SA122b**, run serially in that order. **Next action: SA121** — continue on the surface SA125 narrowed; it stays a partial checkpoint on CR-003/CR-006 with **no open maintainer decision**, and still cannot check its box until `make quality` is green — which needs Track 3's two complexity fixes. `SA122-DEC-001` already resolves SA122a's publish-context disposition. Nothing on this track needs PostgreSQL/Docker, so it runs concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e.
...
- **Track 3 — SA117a RECORDED PARTIAL CHECKPOINT; PLAN, TEST, AND QUALITY BLOCKERS; NO OPEN PRODUCT DECISIONS (on the critical path).** `43d9b8fc` remains recorded partial delivery, and SA117a's reviewed lockstep correction landed with `SA117-CR-001` resolved. The latest bounded plan cycle resolved the original synchronization-transaction design finding but stopped at the cap on `SA117A-PLAN-010` (**high**) and `SA117A-PLAN-013` / `SA117A-PLAN-014` (**medium**); no source delta landed. The task also stays unchecked on `SA117A-CR-002` and its one-item `SA117-QG-REMED` share, `_perform_module_embed` CC 22 > 20; `make quality` remains red. **Next action: start a fresh scoped continuation, close the three literal plan findings, and obtain plan-review `STATUS: ok` before source edits**, then close the missing spy and complexity finding, rerun the full validation/quality chain, and obtain full-scope change-review `STATUS: ok`. **Do not reduce `module_commands.py` or `manifest/loader.py` line counts** — those ceilings were deleted, not met. SA117b must not start first. Do not publish splits (SA117e), start SA112, or treat the candidate as release-ready until the serial SA117 chain and SA117e's full-scope review complete; SA96-PUBLISH (human-only) holds until SA112f. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Net.** One critical path — **SA117a → SA117b → SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1 is runnable (SA121 then SA122a), and Track 2 may run SA115's authorized heavy validation while yielding infra and holding its merge behind SA112. Track 3 remains blocked at SA117a's plan gate after the maintainer selected recorded partial delivery at the cap; a fresh scoped continuation must close `SA117A-PLAN-010`, `-013`, and `-014` before source edits. Rebalancing is otherwise unchanged — Track 2 cannot accept new tickets while its branch carries merge-gated SA115 commits, and the SA117/SA112 chains are causally serial.
