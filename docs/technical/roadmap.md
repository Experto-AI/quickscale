...
**Status:** on the critical path and **executable today** — `SA117E-VAL-001` is resolved (baseline `43d9b8fc` approved 2026-08-01). **Head is SA117e-1**, the evidence-only first child of the SA117e umbrella after its 2026-08-01 Tier 3 split. `F-002` remains medium/blocking for SA117e-3's host-PostgreSQL lifecycle plan, but does not block SA117e-1 or SA117e-2. **SA112a now runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not start SA117e-3 before its scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
    ...
    - [ ] **SA117e-2 — Complete the rebaselined ordered source validation.** `Tier 2 · deps: SA117e-1` — local, service-free
      Resume the ordered sequence at command 4 with `SA117_BASELINE_REF=43d9b8fc0abd1c3193a3358f41dee0e09a3c106c`, then run `make sa117-capture`, `make sa117-verify`, `make check`, and `make quality`. Evidence-first — **may complete with no source delta**. Record every command, exit, and artifact. If a command fails for a reason other than the retired baseline, stop and ticket it rather than repairing in place.
      - Verify: `make sa117-lock-diff SA117_BASELINE_REF=43d9b8fc…` exits 0; `sa117-capture`, `sa117-verify`, `make check`, `make quality` each exit 0; checker semantics unchanged (no tolerance/exception logic added).
...
**No track-sequencing or worktree-assignment question is open. One standing implementation-authorization decision is open:** `SA112A-HANDOFF-001` requires explicit maintainer authorization for one fresh Adaptive implementation attempt before Track 1 can dispatch SA112a. The two earlier authorization decisions were resolved on 2026-08-01:
...
Track 3 is startable: SA117e-1 is its evidence-only head, while `F-002` blocks only SA117e-3's host-service mutation pending that child's scoped plan-review `STATUS: ok`. Track 1 has completed SA112a planning but **cannot currently start implementation** until `SA112A-HANDOFF-001` is authorized. That standing authorization decision is distinct from SA117e-4's public split-push confirmation and later SA96-PUBLISH, which are **execution-time** gates obtained at the moment of the outward-facing action rather than pre-granted decisions.
