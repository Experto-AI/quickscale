...
(closed; no open        (closed to new work)    SA117e-4 (publication ceremony)
 ticket, no edge to                              │  SA147 plan gate closed
 any Track 3 ticket)                             │  human-gated; outward-facing
                                                 │
                                                 ├──────────────► SA117e-5 (off path)
                                                 ▼
                                                 SA112b → c → d → f
...
**Critical path:** `SA117e-4 → SA112b → SA112c → SA112d → SA112f → SA140 → SA96-PUBLISH` — **seven** legs. SA147 is closed: the corrected-source plan is rebound to current `v87`, owns/reaps every service-backed apply process group on every outcome, propagates Compose cleanup/query/removal failures, and requires fresh exact-label zero proofs for containers, volumes, and networks before cleanup is disarmed or fixtures are deleted. `F-003` and the plan-completeness blocker `F-020` are resolved. SA117e-4 is now the head, but Phase 1 still requires its separate explicit execution authorization and the two later human gates remain ungranted. Nothing immutable or outward-facing exists yet: no split tag, remote core tag, teams deletion, PyPI action, or seal. **SA140 is on the path**: per-ticket acceptance of the `apply_command.py` complexity overrun does not satisfy the green gate, which requires an exit-0 `make quality`, so the repair is v87 scope sequenced after SA112f. **SA117e-5 is closeout and sits *off* the critical path**: SA112b's precondition is the *sealed* splits delivered by SA117e-4, not the umbrella's closure.
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-4 (head) | **no** — its corrected-source plan gate is closed, but Phase 1 needs separate explicit execution authorization | **no** — the pre-seal and later teams-deletion confirmations are execution-time gates | **yes** — no merge-order edge | **no** | ✅ yes |
...
**Truly green today: none.** SA147 is closed and the corrected-source plan gate no longer blocks SA117e-4, but no SA117e-4 execution authorization or human confirmation is implied. `SA117E4-DRIFT-003`, `F-003`, `F-020`, and the earlier publication-plan security findings are resolved; all twelve mutable split branches are refreshed; and `SA117E3-PUBLIC-ANALYTICS-001` remains post-seal evidence. The release-wide green gate remains unclaimed.

**Next action: SA117e-4.** Obtain explicit authorization to execute Phase 1 of the corrected-source plan; review/SA147 closure is not execution authorization. The historical [SA117e-4 release plan](../planning/sa117e-4-release-plan.md) remains evidence for the earlier source-bound ceremony only; its prior frozen-source SHA does not govern. The twelve-row seal confirmation, teams deletion confirmation, and SA96-PUBLISH remain separate execution-time gates.
...
**Status:** on the critical path at **SA117e-4**. SA147's corrected-source plan gate is closed with `F-003` and `F-020` resolved. SA117e-4 cannot start until its own Phase 1 execution is explicitly authorized; the exact-SHA branch loop already completed for all twelve modules, and nothing immutable has been created.
...
- [x] **SA147 — Write the text-complete corrected-source SA117e-4 plan.** `Tier 1 · deps: none` · **unblocks SA117e-4** · documentation only — no production code, no remote state
  ...
  **Deliverable.** One new file, `docs/planning/sa117e-4-corrected-source-plan.md`, that is readable and gradeable standing alone.
  ...
  - Allowlist: `docs/planning/sa117e-4-corrected-source-plan.md`; after the deliverable is complete, this task's authorized closeout also includes `docs/technical/roadmap.md` and `CHANGELOG.md`. Read-only inspection of the repository and remote is permitted as evidence-gathering; **no other file may be modified, and no ref may be created, moved, or deleted.**
  - Verify: the plan carries every mechanism inline with no mechanism-bearing cross-reference; it binds to current `v87` rather than any frozen SHA; the five resumption steps, the two standing facts, and the prohibitions above are all present; focused review confirms the F-003 process/cleanup invariant. This ticket's acceptance discharges `F-020` for SA117e-4.
  - Rollback: revert this task's three allowlisted documentation files. Nothing else is touched.
  ...
  **Closeout (2026-08-16; supersedes the recorded partial checkpoint).**
  - **Done:** the maintainer authorized the focused F-003 revision/review round and the shared closeout files. The standalone plan is rebound to current `v87` source `28a894704954f456d18f9851c25fdb16c7e65a5f`. One inline Linux child-subreaper/process-group owner now governs the branch proof, approved semantic harness, and final prompt proof on success, ordinary nonzero, timeout, exception, and parent signal. Its strict cleanup propagates every Compose down/query/removal failure and requires fresh exact-label zero proofs for containers, volumes, and networks before cleanup is disarmed or fixtures are deleted. F-003 is resolved on its frozen criterion; the complete inline plan discharges F-020. SA147 is checked.
  - **Findings/blockers:** none for SA147. The two execution-time human confirmations remain ungranted by design, and SA117e-4 Phase 1 still requires separate explicit execution authorization; neither is an SA147 blocker.
  - **Decisions needed:** none. No ceremony command, environment setup, local/remote ref mutation, seal, teams deletion, PyPI action, publication, or release claim occurred.
  - **Plan carryover:** `status: complete`, `reusable: false-as-execution-authorization`. The file is the complete procedure for the next SA117e-4 handoff, but review and task closure do not authorize running Phase 1.
  *(why →* `F-020` blocked Phase 1 until this complete plan closed it; an irreversible outward-facing workflow must be gradeable in one reading*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
      ...
      **Recorded partial checkpoint (2026-08-15; corrected-source planning only).**
      ...
      - **Superseded 2026-08-16:** SA147 is now checked; its current-source standalone plan resolves F-003 and discharges F-020. SA117e-4 remains unchecked only because Phase 1 has not received separate execution authorization and the execution-time confirmations remain future gates.
...
- **SA147 closed on Track 3.** Its corrected-source plan is consumed by the next leg on the same track, with no cross-track merge edge introduced.
