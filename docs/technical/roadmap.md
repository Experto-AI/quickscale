...
3. **SA114** (v87 gate re-verification & fix sweep, Track 1, `deps: none`) — ordered gates are green and no source fix was required; SA114-CR-001's documentation half is reconciled, so the task remains open only for one authorized full-scope re-review.
...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  **Recorded partial delivery (2026-07-25; review cap reached; task remains open).**
  ...
  - **Decisions needed:** one — whether to authorize a fresh scoped re-review continuation now that the documentation half is reconciled (see Track readiness).
...
```
Track 1 (awaiting review)  Track 2 (blocked — merge order)   Track 3 → release (CRITICAL PATH)
────────────────────────   ───────────────────────────────   ─────────────────────────────────
SA114 / SA116              SA115 (e2e xdist; deps: none)     SA112 ── installed-wheel
  (source+gates done;        │                                 │        plan→apply→up e2e
   awaiting re-review)       └── merge after SA112 ◄───────────┤
                                                               ▼
...
**Net.** One critical path — **SA112 → SA96-PUBLISH (human)** — with Track 3 clean to continue behind a single plan-review gate. Track 1 has green ordered-gate evidence and a now-reconciled documentation surface; SA114/SA116 stay unchecked pending one authorized full-scope re-review. Track 2 is correctly idle, waiting on SA112 by design.

**Open maintainer decision — Track 1 re-review authorization.** SA114-CR-001 was a *consistency* finding, not a source defect: the gates were green and the ticket was blocked purely because the roadmap described completed work as pending. That half is now fixed. The options are (a) authorize a fresh scoped continuation whose only job is a full-scope independent review of SA114/SA116 — closes both tickets and empties Track 1, cheap because no source work remains; (b) defer the re-review until SA112 lands and close SA114/SA116 in the same pass as the release closeout — saves one review cycle but leaves two tickets open across the release gate; (c) leave Track 1 parked indefinitely. Option (a) fits the project's established discipline most organically — every prior ticket in this cycle closed on an explicit independent `STATUS: ok`, and the recorded checkpoints repeatedly state the checkpoint "is not a waiver". It is also off the critical path, so it cannot delay SA112. Neither (a) nor (b) is a pure speedup, so this is left to the maintainer rather than applied. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
