...
Both tracks are now startable in parallel: SA117e-1 is Track 3's evidence-only head, while `F-002` blocks only SA117e-3's host-service mutation pending that child's scoped plan-review `STATUS: ok`. The one remaining human gate on the board — SA117e-4's public split-push confirmation, and later SA96-PUBLISH — is an **execution-time** gate obtained at the moment of the push, not a standing decision.
