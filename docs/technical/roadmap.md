...
**Truly green today: none** — but every *decision-shaped* blocker is now cleared, so both open heads are executable. **SA136d remains on the critical path and review-blocked** by six implementation findings; its retained partial tooling is not a completion or release claim. **SA139 is now fully unblocked to attempt**: its scope expansion is granted, and the remaining work is ordinary implementation. It stays off-path but remains the board's one unblocking move, converting Track 2's SA115a from "cannot finish" to finishable once it merges and SA115a reruns its two umbrella commands. SA122b-5's merge bound is a **hard upstream dependency** only SA112e can clear; no decision affects it.

**Open maintainer decisions: none.** The board's last two landed on 2026-08-08 and are applied below: `SA136D-QG-001` is dispositioned as **reviewed acceptance** under the inherited "no worse than found" rule, with the repair ticketed separately as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); and SA139 is granted a **scope expansion** to disambiguate the import at its source. Neither is a release authorization. All prior topology decisions are settled and recorded in [CHANGELOG.md](../../CHANGELOG.md).
...
**Open track-topology decisions: none**, and as of 2026-08-08 **no open maintainer decision of any kind** — `SA136D-QG-001` and SA139's scope expansion are both dispositioned in their tickets above, and neither alters topology. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
