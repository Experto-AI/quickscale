...
**CI-gate remediation from the 2026-07-19 sweep (SA102, SA103).** Two audit findings land on the CI/release plumbing this track owns. Both are file-disjoint from the Track 2 frontend workstream (they touch `ci.yml`/`publish.yml`/`Makefile` umbrellas + `scripts/check_ci_locally.sh`, not the template tree) and from SA101 (quality baseline), so they can run whenever this track is between critical-path steps — but **SA103 must land before the SA96-GATE four-command join re-runs**, since it tightens what "green" means. Sequence within the track: SA102 (trivial) first, then SA103.

- [ ] **SA102 — Add `--devtools` to every CI lint/typecheck invocation (TA61).** `Tier 1 · Track 3 · deps: none · assistant-executable`
...
TP2b ⧗ → TP4 (off crit.; TP1/TP2 ✓)   TP3a ✓ → TP3b (E2E concurrency;          SA93 ✓ (e2e in green-gate)
 (test-parallelization; off crit.)     rebalanced from Track 1; off crit.)    SA100 ✓ (TA58/TA59 theme preflight)
                                    SA104 ── FE static-source move (Fin.10   SA102 ── devtools CI gate (TA61, Tier 1)
                                      stage 1; safe now, byte-identical)      SA103 ── frontend-proof gate (TA60/red
                                       ├─ SA105 ── module de-spec (2a;          flag; feeds SA96-GATE) ── depends on
                                       │    deps SA104 + dormant-files decision)  node, file-disjoint from SA104
...
**Open work only:** Track 1 `TP2b` (validation-blocked) → `TP4`, Track 2 `TP3b` (after completed `TP3a`) + the frontend chain `SA104 → SA105/SA106` (arch Finding 10), Track 3 `SA101` (clears BLK-002; on the critical path) + the CI-gate remediation `SA102`/`SA103` (2026-07-19 audit sweep), and the release join
