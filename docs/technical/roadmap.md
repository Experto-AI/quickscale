...
1. **SA112a → SA112f** (installed-wheel full-lifecycle e2e `plan → apply → up`, Track 3) — six serial, handoff-sized tasks covering provisioning, diagnosis, the traceback-selected fix, permanent lifecycle coverage, CI triggers, and closeout. **Critical path.** Deps: SA110 ✓ + SA111a ✓ + SA113 ✓, **plus SA117e (transitively SA117b → SA117c) from SA112b onward** (see the evidence-validity dependency below — the binding node is the *pushed* splits, not the local stamp/assert). Next: **SA117b documentation closeout**, then SA117c, SA117e, and SA112a.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  Umbrella acceptance only: SA117a is closed; SA117b's executable implementation is committed and reviewed on `wt-track3`, pending merge through this recorded-partial checkpoint, and its child remains open on `SA117B-REVB-003`; SA117c and SA117e remain open and required. Child tickets own all implementation; **do not execute this parent block as a monolithic handoff.**

  **Split (`SA117-DEC-003`, 2026-07-27): SA117 was Tier 3 and was split into the four serial children SA117a → SA117b → SA117c → SA117e. SA117a closed 2026-07-28 (`SA117-DEC-005`); SA117b's implementation is complete but its documentation closeout remains open, leaving SA117b closeout → SA117c → SA117e.** SA117a carried what SA112b's evidence validity depends on and has delivered it; SA117b made pushing refreshed splits safe; SA117c proves the published splits match the core; SA117e reviews and pushes. The former SA117d (scope meta-tooling) is **deferred to v88 as SA124** (`SA117-DEC-004`). Sizing rationale and the measured candidate breakdown are in [CHANGELOG.md](../../CHANGELOG.md).

  **The original executable candidate is already merged.** `43d9b8fc` is on `v87` as recorded partial delivery, unapproved; SA117b's reviewed corrections in `efd86d21` remain pending merge through this checkpoint. The children are therefore **correction-and-review handoffs over merged-but-unapproved code**, not greenfield builds: each child corrects its named findings on the existing candidate and obtains independent review for its own slice. No child may check its box on a review narrower than its own file set, and SA117e still requires one full-scope review over the complete delta including `43d9b8fc`.
  ...
  **Open findings and their owning child (no decision pending).** `SA117A-CR-002`, `SA117-CR-003`, `SA117-CONT-PR-003`, and `SA117-CONT-PR-006` were resolved by the SA117b implementation and final Review A. `SA117B-REVB-003` (**medium**, consistency — the dependency diagram duplicates SA117e) remains on **SA117b documentation closeout**. `SA117-CR-006` (**high**, completeness — lock-drift route omits complete baseline lock semantics) → **SA117c**. `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker.
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - **SA117a — CLOSED 2026-07-28 (`SA117-DEC-005`).** Its load-bearing lockstep comparison (`SA117-CR-001`) is landed and verified in code — the canonical parser in `manifest/loader.py`, and all twenty-four source + bundled `module.yml` versions equal `VERSION` `0.87.0`, which is the whole of what SA112b's evidence validity depends on. Residue: `SA117A-CR-002` only, folded into SA117b with a pre-written literal diff. Closure rationale and the full review-cap ledger are in [CHANGELOG.md](../../CHANGELOG.md).

  - [ ] **SA117b — Close the split-publication safety and resilience gaps.** `Tier 2 · Track 3 · deps: none (SA117a ✓ closed)` — **implementation reviewed; recorded partial delivery 2026-07-29**
    ...
    **Absorbed from SA117a (`SA117-DEC-005`, 2026-07-28): `SA117A-CR-002` — the dependency-sync non-continuation spy.** Include it in this ticket's delta and review; it is two lines in a different file from the three findings above and needs no separate handoff. **The literal diff is pre-written below — this is transcription, not planning, and it requires no plan-review cycle of its own.**

    Target: `quickscale_cli/tests/commands/test_module_commands.py`, in the existing test `test_update_rolls_back_noncanonical_manifest_version` (declared at `:1560`). That test already patches two spies with `assert_not_called()` in the same `with` block; this adds a third in the identical shape.

    1. In the `with` block, immediately after the `_commit_module_update` patch that ends `) as spy_commit,` (`:1632-1634`), insert:
    ...
    2. Immediately after `spy_sync_state.assert_not_called()` (`:1651`), insert at the same indentation:

       ```python
        spy_sync_deps.assert_not_called()
       ```

    - **Why the assertion holds.** `_read_embedded_module_version` (`module_commands.py:1302`) raises `ModuleVersionMismatchError` before control reaches `_sync_module_dependencies` (`:1318`), so the spy must never fire. Production sequencing is already correct; this pins it against regression, which is exactly what `SA117A-CR-002` asked for.
    - Verify (add to this ticket's focused run): `poetry run pytest quickscale_cli/tests/commands/test_module_commands.py -k test_update_rolls_back_noncanonical_manifest_version` passes, and the assertion is proven non-tautological by temporarily moving the `_sync_module_dependencies` call above the version read and observing the test fail.

    **Recorded partial delivery (2026-07-29; functional commit `efd86d21`; final executable Review A `STATUS: ok`; task remains open).** All four executable task findings are resolved: `SA117-CR-003`, `SA117-CONT-PR-003`, `SA117-CONT-PR-006`, and `SA117A-CR-002`. Advisory `SA117-META-001` is satisfied by preserving the exact validation command below.
    - **Done:** origin fail-close for blank, unset, and whitespace-only direct-CLI origins; uniquely scoped verifier-owned Compose cleanup across SIGINT, SIGTERM, and timeout, with exact container/volume/network labels and resource-specific listing; publication-only trusted Git bootstrap, exact-origin assertions, and NUL-safe staged-index assertions; caller parity across the five consumers; and the dependency-sync non-continuation spy.
    - **Pending / blocking:** `SA117B-REVB-003` (**medium**, consistency) — the dependency diagram duplicates SA117e and must be corrected to one SA117c → SA117e → SA112a chain, then receive a fresh full-scope documentation review before SA117b may close.
    - **Decisions needed:** none.
    - **Not implemented / out of scope:** SA117c lock-drift completeness; SA117e split push and full-scope umbrella closeout; the SA112 chain; tag/PyPI publication. These remain open and are not implied by this closeout.
    - **Validation metadata:** Review A final `STATUS: ok`. Post-sync chain result: **656 passed**; Ruff check, Ruff format-check, typecheck, quality, and diff-check all exited 0; quality baseline regressions 0; MyPy configuration notes and existing quality warnings were non-blocking. The exact command was:
      ...
      Live Docker evidence: SIGINT returned 130; SIGTERM returned 143 with forced-cleanup failure; timeout failure was captured; the exact owned container, volume, and network were removed; unrelated controls survived; and no residuals remained. Final Review B hit its cap with `STATUS: partial` only on `SA117B-REVB-003`; no executable blocker remains.

    **Historical checkpoint (2026-07-28; superseded by the completion closeout above; documentation only; task was then open).**
    - **Done:** Track 3 was clean and synced to `v87` at `76462b13`; Poetry 2.4.1, the worktree `.venv`, Docker 29.1.3, and the GitHub origin were ready. Focused discovery mapped the publication, verifier, Compose-owner, shared-Git, and caller-parity seams. Two bounded review-driven re-plan cycles resolved `SA117B-PLAN-001` (exclusive verifier-owned Compose identity and signal/timeout cleanup), `SA117B-PLAN-002` (publication-only trusted Git bootstrap and NUL-safe staged-index controls), `SA117B-PLAN-003` (five-consumer caller parity), and `SA117B-PLAN-004` (exact reviewed base/tip and merge identity) at plan-detail level.
    - **Pending / blocking:** `SA117B-PLAN-005` (**medium**, completeness) — the implementation plan must add non-mutating Ruff check and Ruff format-check over every Python write target plus the repository type-check gate for the changed core/CLI packages before executable Review A, after any `v87` resynchronization, and after merge-back. The underlying task findings remain **unimplemented**: `SA117-CR-003` (**high**, security), `SA117-CONT-PR-003` (**high**, resilience), `SA117-CONT-PR-006` (**medium**, completeness), and absorbed `SA117A-CR-002` (**medium**, test gap). No source or test file changed, no task validation ran, and no executable review occurred.
    - **Decisions needed:** none. A fresh explicitly authorized focused plan/review continuation must carry `SA117B-PLAN-005`; plan-review `STATUS: ok` is still required before source edits. SA117b stays unchecked, SA117c remains blocked, and no split push or publication is authorized.
...
- [x] **SA121 — Enforce the shrink-only baseline against the merge base.** `Tier 2 · Track 1 · deps: none (SA125 ✓ closed) · recorded partial delivery 2026-07-28 · CLOSED 2026-07-28`
  ...
  **Recorded partial delivery (2026-07-28; functional commit `9448a0f7`; detail in [CHANGELOG.md](../../CHANGELOG.md)).**
  - **Done:** The reviewed candidate carries the merge-base monotonicity helper, a separate empty waiver ledger, `make quality` integration, policy/operator documentation, focused coverage, and the minimal strict-key parser correction, with `scripts/quality_baseline.json` unchanged and `scripts/quality_waivers.json` empty. QG run 2 passed **153 focused tests in 6.68s**; the helper against `v87` reported **0 violations / 0 waivers**; `bash -n` passed; and `make quality` exited 0 with **0 baseline regressions**. Formatter-normalized full-scope functional review returned **STATUS: ok**; `SA121-CR-003` and `SA121-CR-006` are resolved.
  - **Pending / blocking:** (resolved — see closure checkpoint below.) The historical ledger is preserved: `SA121-DOC-CR-002` (**medium**, consistency) — the substance was corrected 2026-07-28 in the roadmap-cleaning pass; the duplicated SA122a node and the "no cross-track edge" claim were replaced by the explicit two-edge statement. What remained was verification by a full-scope documentation review returning `STATUS: ok`, now obtained. `SA121-CR-007` remains **low/advisory, non-blocking** for stale historical numeric test prose.
  - **Closure checkpoint (2026-07-28):** `SA121-DOC-CR-002` resolved by full-scope documentation review returning **STATUS: ok**. The corrected topology (SA122b→SA112e, SA115→SA112) is verified consistent across all dependency, ordering, and diagram surfaces in this file. `SA121-CR-007` remains **low/advisory, non-blocking**: historical CHANGELOG prose says 156/156 tests while final accepted evidence says 153; no cleanup required for closure. **No blocker remains.** All executable scope (`9448a0f7`) was already green and independently reviewed; this checkpoint closes the documentation-only gap. SA121 is marked `[x]` and the detailed closeout entry is in CHANGELOG.md.
  - **Decisions needed:** none.
...
The two former complexity blockers were closed by SA126 (Track 1), so the green-gate unblock no longer sits behind the SA117 chain's plan gates. SA121's functional remediation is green with `SA121-QG-001`, `SA121-CR-003`, and `SA121-CR-006` resolved; `SA121-DOC-CR-002` resolved by full-scope documentation review (SA121 closed); SA122a follows.
...
─────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA121 ✓ CLOSED                      SA115 (e2e xdist; deps: none)  SA117c (lock-drift completeness) ◄─ next
  │  SA121-DOC-CR-002 resolved ✓      │  validation AUTHORIZED       │  deps: SA117b ✓
  │  functional ✓ (9448a0f7)          │  cannot finish → SA112f      │  SA117b ✓ (efd86d21)
  ▼                                   │  cannot merge  → SA112e      ▼
SA122a (gate registry) ◄─ in process │                             SA117e (review + push splits)
  │  deps: none                     │                              │
  ▼                                 │                              ▼
...
**Diagram checkpoint:** `SA117B-REVB-003` remains open because the diagram above duplicates SA117e. The authoritative intended sequence is SA117b documentation closeout → SA117c → SA117e → SA112a; the checkbox stays open until a fresh continuation corrects and reviews the diagram.

**Cross-track edges (authoritative).** Two, both merge-order only: **SA122b merges after SA112e** (shared `.github/workflows/e2e.yml` `pull_request.paths`) and **SA115 merges after SA112** (same file, plus `SA115-CI-001`). No cross-track edge blocks any track from *starting* or *working*; the detailed bounds stated below and in each ticket govern. *(This reconciles the substance of `SA121-DOC-CR-002` — the duplicated SA122a node and the former “no cross-track edge” claim are both gone. SA121 is closed; full-scope documentation review resolved SA121-DOC-CR-002.)*
...
**Critical path:** `SA117b documentation closeout → SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. SA117b's executable publication-safety work is complete, but its documentation review remains open; SA117c follows as the lock-drift gate before SA117e.

**Parallelism.** The open tickets occupy all three tracks: SA117b closeout → SA117c–SA117e → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and SA122a (in process) → SA127 → SA122b on Track 1 (SA121 closed); SA126 is complete. **The repository runs exactly three worktrees; a fourth is not created** (`SA112A-TRACK-001`, resolved 2026-07-28 — see Standing decisions).
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now contains release blocker SA127 behind in-process SA122a, while Track 3 must close SA117b's documentation finding before SA117c. Reusing Track 1 still requires no fourth worktree, but it would delay SA127 as well as the governance queue.
...
> **Re-verified on 2026-07-29 (SA117b recorded partial delivery).** Functional commit `efd86d21` and all executable findings are reviewed and green. SA117b remains open only on documentation finding `SA117B-REVB-003`; SA117c is blocked until that fresh documentation continuation closes.
>
> **Fifth-pass code measurements, carried.** `make quality` was **rerun directly: exit 0**, with 0 warning and 0 critical baseline regressions. `radon cc` confirms SA126's closeout — `_perform_module_embed` **18** (ceiling 20) and `_update_single_module` **13** (ceiling 17), down from 22 and 18. `QUARANTINE_TICKETS` in `scripts/test_integration.sh` is **empty** (declared, no entries). `scripts/quality_baseline.json` still carries **zero** `large_files` entries and `scripts/quality_waivers.json` is still an empty ledger. The historical fifth-pass branch state predated this checkpoint; current `wt-track3` carries the reviewed SA117b commits pending merge to `v87`. `wt-track2` remains closed with its merge-gated SA115 commits.
...
- **Track 3 (SA117b closeout → SA117c → SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path.**
  - **Can start — YES.** SA117b's next action is the documentation-only `SA117B-REVB-003` correction and full-scope review; no product decision is open.
  - **Can finish — YES.** The remaining blocker is inside `docs/technical/roadmap.md`; all executable findings are resolved.
  - **Can merge — YES.** This reviewed partial implementation merges to preserve the green executable delta; SA117c starts only after SA117b documentation closes.
...
- **Track 3 detail — SA117b EXECUTABLE COMPLETE; DOCUMENTATION CLOSEOUT OPEN.** `efd86d21` resolved all four executable findings and final Review A returned `STATUS: ok`; Review B remains partial only on `SA117B-REVB-003`. **Next action: correct and fully review the dependency diagram; then SA117c.** Do not publish splits, start SA112, or treat the candidate as release-ready.

**Net.** The Track 3 critical path is **SA117b documentation closeout → SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1 independently carries release blocker **SA127** between SA122a and SA122b; both chains must close for release. Track 2 remains merge-gated.

**Current state after the SA117b review cap.** SA117b's executable implementation and four task findings are complete and Review A is green, but the child remains unchecked on `SA117B-REVB-003`. Track 3 must close that documentation finding before SA117c; no product decision is open.

**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but delays in-process SA122a and release blocker SA127) or leave it behind SA117e. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one — `SA112A-TRACK-002`** (stated in full in the Parallelism section): move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying in-process SA122a and release blocker SA127. It is a sequencing decision and changes no ticket's technical content or any track's three states. **Default while undecided: SA112a stays on Track 3.** Every individual ticket still records **Decisions needed: none**.
