...
**Open maintainer decisions: two, and both are the same kind — spent continuation budgets.** Every prior authorization has been consumed: on 2026-08-09 the maintainer merged SA136d's recorded-partial checkpoint with F-007 unresolved; on 2026-08-10 the maintainer authorized SA136d's correction cycle and SA141's parallel start, and both ended at their caps in recorded-partial state. **SA136d and SA141 each now need fresh explicit authorization to open another implementation/validation/review cycle** — nothing technical blocks either one, and nothing else can advance them. These are the *only* pending choices on the board; authorizing them clears "can start" for both tracks and nothing else. No authorization here extends to a seal command, remote ref mutation, publication, or completion claim. Every other blocker is a hard upstream dependency or review finding, not a pending choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); SA139's scope expansion is implemented, reviewed, and closed. The remaining human gates are execution-time gates obtained at outward-facing actions, not pending decisions.
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed`
    ...
    **Seal-atomicity requirement re-scoped (settled; rationale in [CHANGELOG.md](../../CHANGELOG.md)).** Server-transactional atomicity is **withdrawn as a requirement**. The seal is enforced client-side as check-then-act with a fail-closed compensating verification, and the residual window is documented rather than engineered away. This unblocks the ticket on stock Git against GitHub.com; no platform migration is required.
    ...
    - **Retained tip and open ledger (task remains unchecked).** The corrected implementation tip is `355f65a1`; what landed, what passed, and the review history are in [CHANGELOG.md](../../CHANGELOG.md). Four blockers remain open, and no release claim is made.
      ...
      - **Authorization state.** `SA136D-QG-001` stays **accepted** (reviewed, 2026-08-08) under the inherited "no worse than found" rule; no ceiling is raised and SA140 owns the repair. The 2026-08-10 continuation authorization was **consumed** by two capped plan-review rounds without implementation, so a further attempt needs **fresh explicit authorization**. No authorization here covers a seal command, remote ref mutation, tag push, publication, or completion claim.
      - **Carry-forward plan (next attempt starts here; needs fresh authorization).** Implement the items below on the retained tip, revalidate, and obtain review. Because items 1–3 change production behaviour, [Proportionality](#rules-every-ticket-inherits) requires a scoped plan review over this slice before implementation — and that plan must first close plan-review F-004 (item 0). Order matters: fix F-007 first among the code items, since it is the only one that can mint a wrong-identity irreversible tag.
...
- [ ] **SA141 — Diagnose and repair the two Core E2E Poetry-install failures.** `Tier 2 · deps: none` · **blocks SA112f, SA115a acceptance, and the green gate**
  ...
  - **Retained repair and open ledger (task remains unchecked).** The diagnosis (ambient `VIRTUAL_ENV`/`POETRY_ACTIVE` inheritance under `poetry run pytest`), the isolation seam it landed in `poetry_env.py` plus its `apply_command.py`/`module_commands.py` callers, and the evidence that passed are recorded in [CHANGELOG.md](../../CHANGELOG.md). `_run_repo_poetry_command` remains the intentional ambient repository-only exception. Two blockers remain, and no green-gate claim is made.
    ...
    - **Next attempt (needs fresh explicit authorization; `qg_runs=2/2`, `review_cycles=2/2` are spent).** Fix F-002, rerun the complete full gate with a raised watchdog, execute the final root oracle, and obtain one full-scope review. No plan carryover exists — the medium-tier plan was not plan-reviewed and every planned slice landed. Advisory F-003 is discharged by recording `apply_command.py` as serialized SA141-then-SA140 work.
...
**Open track-topology decisions: none.** The two open maintainer decisions are continuation authorizations for SA136d and SA141 (see [Dependency & parallelization overview](#dependency--parallelization-overview)), not topology choices — SA136d and SA141 share no file and sit on different tracks, which is exactly the two-track concurrency this topology was built to allow. Every other “no” in the readiness table is a hard dependency, not a pending choice. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
