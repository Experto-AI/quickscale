...
- [ ] **SA122a — Declarative gate registry + parity checker.** `Tier 2 · Track 1 · deps: none`
  ...
  **Recorded partial delivery (2026-07-29; functional commit `73c153b2`; explicit maintainer-authorized preservation; task remains open).** **Done:** the additive groundwork now includes a seven-entry JSON gate registry, standalone parity checker and `make check-gate-parity` entrypoint, maintainer documentation, and a focused regression suite; the current tree deterministically reports exactly the five expected publish omissions, while Ruff/format, `make typecheck`, `make quality`, and `make check` are green and the latest focused suite reports 112 passing tests. **Pending / blocking:** `SA122A-CR-003` (**high**, security-boundary) — deceptive or malformed workflow/Bash forms can still false-green and E2E parsing is not uniformly structural; `SA122A-CR-002` (**high**, correctness) — exact binding/stage, arbitrary dependency-cycle, Make delegation, and E2E ownership invariants remain incomplete; `SA122A-QG-001` (**medium**, completeness) — required adversarial/exact-stream proof is incomplete and the focused MyPy command reports two errors in the E2E extractor; `SA122A-CR-004` remains **low/advisory** for `.PHONY` wording consistency. Terminal post-cap full-scope review returned `STATUS: partial`; this merge is preservation only, not a waiver, completion claim, or release-ready evidence. **Decisions needed:** before clean continuation, a maintainer must explicitly authorize another narrowly scoped remediation/review cycle or choose replacement/revert of the partial checker; independent full-scope review `STATUS: ok` is required before checking SA122a, starting SA122b, or treating the registry as authoritative.
