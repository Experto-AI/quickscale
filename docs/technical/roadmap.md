...
SA128d → SA122b-1 → … → -5             SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  ...
  ▼  Make · Bash · YAML · contracts   │  validation AUTHORIZED      │  recorded partial; UNBLOCKED
                                      │  cannot finish → SA112f     │  Phases 2–3 accepted; revised
                                      │  cannot merge  → SA112e     │  Phase 5 awaits plan review
SA122b-1 → -2 → -3 → -4 → -5          │                             │
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **yes** — the acceptance-boundary decision is recorded and a revised Phase 5 is drafted; the next action is a scoped plan-review dispatch, executable today | **yes** — the revised Phase 5 and the outstanding Phase 6 are both Track 3's own work | **yes** — no order gate | ✅ **yes** | ✅ **yes** — on the critical path |
...
**Track 3 is truly green on the critical path at SA117e-3.** The corrected ephemeral harness and lifecycle wrapper received full-file pre-use approval at their exact current hashes, and the interruption proof restored the frozen PostgreSQL container after a TERM in the stop-effect-before-bookkeeping window. The later local-CI negative control could not reach its intended stage-11 PostgreSQL gate: with host PostgreSQL stopped, stage 10's `make test-cov REQUIRE_BACKUPS_COVERAGE=1` fails first. **The maintainer recorded acceptance boundary (a) on 2026-08-05** — an alternate task-owned PostgreSQL endpoint keeps stage 10 green while the frozen host endpoint stays unavailable for stage 11, retaining the negative control — and a revised Phase 5 is drafted in the ticket. SA117e-3 remains **unchecked**: the revised plan needs a scoped plan review returning `STATUS: ok` before any host-service mutation, and `make ci-e2e` still has not run at the current tip. **Track 1 is truly green at SA128d**, off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear. SA117e-4 still needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path, **executable at SA117e-3 after recorded partial delivery**. Phases 2–3 are accepted locally: exact harness/wrapper bytes were approved and the interruption proof restored the original PostgreSQL state/readiness. Phase 4's public-split observation was diagnostic only and transfers to human-gated SA117e-4. Phase 5 stopped at local-CI stage 10 before the intended stage-11 host-down gate; the maintainer recorded acceptance boundary (a) on 2026-08-05 and a **revised Phase 5 is drafted, pending a scoped plan review returning `STATUS: ok`**. `make ci-e2e` has not run at current tip `2eae365a`, so SA117e-3 remains unchecked. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4. Do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Revised Phase 5 (candidate — requires a scoped plan review returning `STATUS: ok` before any host-service mutation).** Supersedes the superseded plan's Phase 5 only; Phases 1–4 and 6–9 stand.
      ...
      **Open parameters for the executing continuation:** the spare port and the scratch container name (`quickscale-sa117e3-scratch`) are placeholders to bind at execution. One pre-check before starting: `pg_dump --version` on the host must report **18**, or 5a's image pin needs to match whatever the host binaries are.

      **Phase 6 remains outstanding independently:** `make ci-e2e` has not run at current tip `2eae365a` (evidence was gathered at `9f5092f7`), and it needs exclusive Docker/PG capacity per the [infra-serialization rule](#dependency--parallelization-overview).
...
- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Umbrella · deps: none`
  ...
  **SA128a, SA128b, and SA128c are closed and merged** (evidence in [CHANGELOG.md](../../CHANGELOG.md)). One finding survives them: **SA128a/F-003**, a medium/advisory `_communicate_bounded` resilience item owned by **SA128d**, separate from SA128b's F-003.
...
**Open decisions — two, both the maintainer's, neither blocking today.** SA117e-3's acceptance-boundary decision **closed on 2026-08-05** (boundary (a): alternate task-owned PostgreSQL endpoint for stage 10, stage-11 negative control retained). Both remaining decisions are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
