...
The green-gate join (SA96-GATE), the installed-wheel discovery/resolver chain (SA109 ✓, SA110 ✓, SA113 ✓, SA111a ✓, SA111b ✓), the Track 2 frontend de-specialization chain (SA104 → SA108 ✓), the Track 1 re-verification chain (SA114 ✓, SA116 ✓), SA120 (quiet `check` parity), and SA125 (file line-ceiling retirement) are closed; detail lives in [CHANGELOG.md](../../CHANGELOG.md). Seven tickets remain open (grouped as the six entries below) — SA117 (now the four serial children SA117a, SA117b, SA117c, SA117e, split on 2026-07-27 as `SA117-DEC-003`) is a release blocker from the 2026-07-26 diagnostic spike, and SA121–SA122b are the remaining gate-governance chain on Track 1.
...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: SA125 ✓` — shared files, must be serial) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2. Its authorized continuation is preserved as recorded partial delivery; SA121 remains open on `SA121-CR-003`, `SA121-CR-006`, and `SA121-QG-001`, but **carries no open decision** — `SA121-DEC-002` authorized the minimal parser correction, and QG-001 now has only Track 3's two complexity blockers.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **`SA117-QG-REMED` — quality-ceiling remediation. `Track 3` · `deps: SA125 ✓` (satisfied 2026-07-28).** `SA121-QG-001` originally recorded five exceeded ceilings introduced by `43d9b8fc`. **SA125 retired the file line-ceiling gate entirely, dissolving three of the five**; only the two complexity findings remain, and each is already inside its child's declared allowlist.
  - **SA117a** owns `_perform_module_embed` (CC 22 > 20).
  - **SA117b** owns `_update_single_module` (CC 18 > 17).
  - **Dissolved by SA125, not by remediation:** `module_commands.py` (1642 > 1608), `manifest/loader.py` (686 > 562), and unbaselined `git_utils.py` (509). No child reduces these; the ceilings that flagged them cease to exist. **Do not spend a line-reduction refactor on them** — see `SA125-DEC-001` for why the target was unsound.
  - **Ordering:** SA125 landed on Track 1 and merged to `v87` **before** SA117a's continuation, so Track 3 can now sync a tree where only the CC finding remains. Each child must still leave `make quality` no worse than it found it and clear its own share before its slice review; per SA121's shrink-only rule, **raising a complexity ceiling is not an acceptable remedy** — reduce the branching, or record a structured waiver once SA121 ships the waiver ledger.
  - Conflict surface: none beyond each child's existing allowlist. SA117a/SA117b no longer touch `scripts/quality_baseline.json` at all (the former `git_utils.py` line-baseline entry is unnecessary because `large_files` is gone), and SA121's continuation remains prohibited from touching it under `SA121-DEC-001`; no current phase owns it for line-count remediation.

  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: SA125 ✓ (continuation dependency satisfied) · blocks SA112b` — **critical path**
    > **Deps note.** SA117a's *original* scope had `deps: none` and its already-merged partial delivery was executed under that. The `SA125` dependency applied **only to the remaining continuation** and is now satisfied by the merge to `v87`; after syncing, the continuation may start with the three line-ceiling blockers removed.
    ...
    - Verify: all twelve `module.yml` versions equal `VERSION`; padded/non-canonical variants are rejected with an explicit version-mismatch error naming both versions; a deliberately skewed embedded manifest is rejected before the downstream `KeyError`; an `apply` selecting all 12 modules reaches managed-wiring regeneration with no `KeyError`; **its `SA117-QG-REMED` share is cleared** — `_perform_module_embed` CC ≤ 20, with no baseline ceiling raised. (Line-count remediation was dissolved by `SA125-DEC-001`; do not reduce `module_commands.py` or `manifest/loader.py` for gate reasons.)
    *(why →* this is the release blocker proper, and the only part of SA117 that SA112b's evidence validity depends on*)*

    **Recorded partial checkpoint (2026-07-27 — change-review cap; task remains open; delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)).**
    - **Pending / blocking:** `SA117A-CR-002` (**medium**, test gap) — `test_update_rolls_back_noncanonical_manifest_version` asserts no state sync or commit but lacks an explicit `_sync_module_dependencies` spy and `assert_not_called()`. The production sequence is correct; this non-continuation property is not regression-proven. Plus SA117a's `SA117-QG-REMED` share, **narrowed by `SA125-DEC-001` to a single item**: `_perform_module_embed` CC 22 > 20. The three line-ceiling findings are dissolved by SA125, not remediated here.
    - **Next action (no decision pending; `deps: SA125 ✓`):** sync to `v87` with SA125's merge now present, then a fresh scoped continuation adds the missing dependency-sync spy, reduces `_perform_module_embed` branching to CC ≤ 20, reruns the focused and full quality chain, and obtains full-scope independent `STATUS: ok`; only then may SA117a be checked and Track 3 advance to SA117b.
  ...
  - [ ] **SA117b — Close the split-publication safety and resilience gaps.** `Tier 2 · Track 3 · deps: SA117a` — **critical path**
    ...
    - Verify: blank, unset, and whitespace-only origins each fail closed with a named error; a SIGINT injected between container creation and first use leaves no container or volume behind; a staged path containing spaces/newlines/quotes is handled correctly by the emptiness assertion; the bootstrap refuses an untrusted source; **its `SA117-QG-REMED` share is cleared** — `_update_single_module` CC ≤ 17, with no baseline ceiling raised. (`git_utils.py`'s 509-line finding is dissolved by `SA125-DEC-001`; no baseline entry is needed for it.)
...
Track 1 holds the gate-governance work: it changes how "green" is decided, not what the generator emits, and touches **no** Track 3 release-path file. `TA62`/SA120 and **SA125** are closed (see [CHANGELOG.md](../../CHANGELOG.md)), leaving **SA121** and **SA122a/SA122b** — the latter two from [arch-audit.md](../../arch-audit.md) Findings 12 and 11.

**Ordering on this track (one worktree, serial):** **SA125 completed → SA121 → SA122a**, with SA122b merge-gated behind SA112e.
- **SA125 is complete.** It retired the file line-ceiling gate (`SA125-DEC-001`), dissolved three of the five `SA121-QG-001` regressions outright, narrowed `SA121-CR-003`'s remaining test matrix, and unblocked Track 3's SA117a continuation.
- **SA121 next.** It shares four files with SA125 and now proceeds on the narrowed surface; the shared worktree keeps the phases serial. Per the arch-audit fix-order section, Finding 12 landed **before any further `quality_baseline.json` adjustment** — SA125 was precisely that adjustment, with its own decision record rather than an unexplained baseline edit.
- **SA122a third.** Finding 11 should be centralized **before or as part of** the SA112e/`SA115-CI-001` path-list edits, so SA122a should land early enough that those edits have a registry to write into; SA122b's consumer migration waits for SA112e so the e2e path list is rewritten once.

#### SA125 — Retire the file line-ceiling gate (keep the complexity gates)

- [x] **SA125 — Drop `large_files` from the quality gate; keep `complexity`.** `Tier 1 · Track 1 · deps: none · unblocks SA121 continuation + SA117a continuation`

  **Decision (`SA125-DEC-001`, 2026-07-28): per-file line ceilings are retired; per-function cyclomatic-complexity ceilings stay.** The two metrics sit in the same `scripts/quality_baseline.json` under the same enforcement and have been treated as equally authoritative. They are not.

  - **The ceilings are measurement fossils, not targets.** `manifest/loader.py`'s 562 ceiling is exactly the file's size at `43f49350`, the commit that happened to be HEAD when `76c5cc55` captured the baseline. No one judged that this file should be 562 lines. The shrink-only rule then promoted an accident into a binding contract — the mirror image of arch Finding 12, which names the same artifact acting as its own waiver authority.
  - **The gate taxes documentation.** `loader.py` is 686 lines: 348 code, 171 docstring, 134 blank, 33 comment. The cheapest legal route to compliance is deleting ~124 of the 204 documentation lines — including the docstring that specifies SA117a's fail-hard rejection contract. The metric pays you to make a release-blocking seam less legible while reading green.
  - **It is aimed against the project's own direction.** `loader.py` grew 187 → 686 across its life because it absorbs the declarative-manifest migration that [decisions.md §module-derivation-schema](./decisions.md#module-derivation-schema) mandates. A shrink-only ceiling on the file the architecture designates as the growth point is a rule pointed at the roadmap.
  - **It cannot distinguish mandated growth from decay.** The 124-line overage is almost exactly the 123-line region SA117a added for the reviewed exact-lockstep correction — required release-blocker work. A line delta reads identically whether it came from sloppiness or from a mandated validator.
  - **Complexity ceilings survive because they earn it.** Per-function cyclomatic complexity is a local, actionable claim about branching that ignores docstrings and blank lines and names a specific function. `_perform_module_embed` at CC 22 > 20 is a real finding. That half of the gate is kept unchanged, and SA121's monotonicity rule continues to govern it.
  - **Not a weakening of fail-hard policy.** [§fail-hard-principle](./decisions.md#fail-hard-principle) governs runtime behavior on absent or invalid configuration; this is a governance-metric retirement, not a runtime fallback. The advisory `LARGE_FILE_WARNING_LINES`/`LARGE_FILE_ERROR_LINES` reporting is **retained as a non-blocking diagnostic** — a tripwire that prompts a review question — and must not gate `make quality`.

  1. Removed the `large_files` section from `scripts/quality_baseline.json` (41 entries) and its blocking enforcement in `scripts/check_quality.sh`, leaving `complexity` (151 entries), `dead_code`, and `duplication` intact.
  2. Narrowed `scripts/check_quality_baseline_monotonicity.py` and `scripts/test_quality_baseline_monotonicity.py` to the surviving sections; the SA121 helper no longer parses or diffs `max_lines` keys.
  3. Updated `docs/technical/quality_tools.md` and recorded the rule in [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy), cross-referenced from the shrink-only policy so the two do not contradict.
  4. **Decision recorded:** retain the advisory large-file *report* as non-blocking output; it does not gate `make quality`.

  **Completion evidence (2026-07-28; functional commit `05d0f2b8`; independent review `STATUS: ok`).** The `large_files` gate and its baseline entries were removed, the monotonicity helper/tests were narrowed to the surviving sections, and the advisory large-file report was **retained as non-blocking output**. Focused pytest: **149 passed**; SA125 subset: **4 passed / 145 deselected**; Bash syntax, `make lint`, `make typecheck`, and `git diff --check` passed. `make quality` exited 2 with exactly `_perform_module_embed` CC **22 > 20** and `_update_single_module` CC **18 > 17**, both pre-existing Track 3 complexity findings, and **zero line-count blockers**. `SA125-CR-ADV-001` remains a low/advisory consistency finding for stale comments/docstrings in the monotonicity helper/tests; behavioral semantics are correct, the finding is non-blocking, and it does not reopen SA125. No full-suite or green `make quality` claim is made.
  **Advisory-report scope:** The retained large-file diagnostics do not affect the baseline, monotonicity checks, waivers, or exit status.
  *(why →* the retired half of the gate is a measurement fossil that penalizes mandated growth and rewards deleting documentation; keeping it would spend a 124-line refactor of `loader.py` on a number nobody chose*)*

  **Sequencing (maintainer decision, 2026-07-28; executed).** SA125 ran first on Track 1, and Track 3's SA117a continuation waited for it. SA125 was a deletion sized Tier 1. Landing it before SA117a shrank that child's quality share from three line reductions plus one complexity fix to the complexity fix alone, and avoided a 124-line `loader.py` refactor against a ceiling being deleted. It also shrank SA121's own remaining work, since `SA121-CR-003`'s large-file separator-key cases stopped existing. The cost was a short pause at the head of the critical path; the alternative — Track 3 proceeding CC-only in parallel — was rejected because it would have left two tracks mid-change on the same gate definition and kept `make quality` red longer.

  **File overlap with SA121 — serial handoff executed on Track 1.** SA125 and SA121's continuation both edit `scripts/check_quality.sh`, `scripts/check_quality_baseline_monotonicity.py`, `scripts/test_quality_baseline_monotonicity.py`, and `quality_tools.md`. They are one coherent unit of gate-policy work and **must not** be split across worktrees or run concurrently. SA125 landed first; its deletion is complete, and SA121's continuation now proceeds on the narrowed surface. `SA121-DEC-001`'s prohibition on SA121 touching `scripts/quality_baseline.json` remains in force; the completed SA125 phase has no active ownership claim over that file.

#### SA121 — Quality baseline can authorize its own growth (arch Finding 12)

- [ ] **SA121 — Enforce the shrink-only baseline against the merge base.** `Tier 2 · Track 1 · deps: SA125 ✓ · blocks any further baseline edit`
  ...
  **Recorded partial delivery (2026-07-27, two merged checkpoints — `3485e209` plus the tests/documentation continuation; delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)). Task remains open.** The merged candidate carries the merge-base monotonicity helper, a separate empty waiver ledger, `make quality` integration, policy/operator documentation, and focused coverage, with `scripts/quality_baseline.json` unchanged and `scripts/quality_waivers.json` empty. The merges are recorded partial delivery only — not a waiver, review approval, or permission to check SA121.
  ...
  - **Pending / blocking:** `SA121-CR-003` (**high**, completeness) — `complexity:file.py:::func` is accepted as an `orphan` (the extra colon is swallowed into the symbol) instead of rejected as `malformed`, so the new test is false-green; unknown-prefix, empty-component, missing/extra `::`, nonliteral duplication key, and unsafe-waiver-path cases still need exact malformed-waiver exit-1 and malformed-baseline exit-2 envelope assertions. `SA121-CR-006` (**medium**, test gap) — acceptance-critical sites still use filtered/subset, `>= 1`, `any(...)`, or non-empty-message assertions instead of complete ordered records, exact counts, and exact envelopes. `SA121-QG-001` (**high**, validation) — `make quality` still exits 2 on the two pre-existing Track 3 complexity findings outside Track 1. **Resolution is now split:** `SA125-DEC-001` dissolved the three line-ceiling findings by retiring that gate, and the two surviving complexity findings stay with Track 3's `SA117-QG-REMED`. SA121 needs no disposition decision either way. The remaining `SA121-CR-003` matrix is now narrowed because the large-file separator-key cases no longer exist. `SA121-REV-META-001` (**low**, advisory) asks future evidence to distinguish commands run separately after an `&&` chain stops.
...
**Current status — `make quality` is RED (recorded 2026-07-27 as `SA121-QG-001`; rescoped and partly cleared 2026-07-28 by `SA125-DEC-001`).** The exit criteria above are not currently met. SA125 removed all three file line-ceiling blockers; the only remaining `make quality` blockers are the two pre-existing Track 3 complexity findings:
- **Cleared by SA125.** `module_commands.py` 1642 > 1608, `manifest/loader.py` 686 > 562, and unbaselined `git_utils.py` at 509 were file line ceilings. No code was reduced for them, and no line-count blocker remains.
- **Still open for Track 3 remediation.** `_perform_module_embed` CC 22 > 20 (SA117a) and `_update_single_module` CC 18 > 17 (SA117b) are complexity findings, kept and owned by Track 3's `SA117-QG-REMED`; they are not SA125 failures.

All five originated with the SA117 preservation checkpoint `43d9b8fc` (recorded partial delivery, unapproved). Until Track 3 remediates the two surviving complexity findings, no track may claim the green-gate milestone, and SA121 cannot close because its own acceptance requires `make quality` green.
...
SA121 (baseline monotonicity) ◄─ next SA115 (e2e xdist; deps: none) SA117a (exact lockstep)
  │  (partial; DEC-002; SA125 ✓)    │  (validation AUTHORIZED)     ▲   + CC 22→20 share only
  │                                │                             │   SA125 ✓; unblocked
  ▼                                │                             ▼
SA122a (gate registry) ──┐         │                             SA117b (publish-path safety)
  ...
  ▼                      │         │                             SA117c (lock-drift completeness)
                         │         │                              ▼
SA122b (migrate) ◄───────┴─────────┼───── merge after SA112e ◄────┤  SA117e (review + push splits)
                                    │                              ▼   (scope tooling → v88 SA124)
                                                                   SA112a → b → c → d → e → f
                                                                   │  ▲   serial reviewed handoffs
                                                                   └── SA117e required from b on (evidence validity)
                                                           SA96-PUBLISH ── build → publish
                                                           (human-only; hold until SA112f)
```

**Track 1 is now internally serial and has one downstream consumer.** SA125 (`deps: none`) is complete; SA121 is next because it shares four files with it; SA122a follows; SA122b carries the SA112e merge bound. SA125's completed Track 1 work unblocked the Track 3 critical-path child SA117a by deleting three of its five quality blockers, which was strictly cheaper than remediating them.
...
**Completed temporary upstream node (2026-07-28).** `SA125` (Track 1) briefly preceded `SA117a` as `SA125 → SA117a → …`. It is not a permanent member of the chain: its Tier 1 deletion removed three of SA117a's five quality blockers, so it shortened the chain's remaining work by more than the pause it cost. The critical path has now reverted to the sequence above, with SA117a unblocked and still carrying its one complexity finding.
...
- **Track 1 is serial by file overlap, and SA125 has unblocked the critical path.** SA125 and SA121's continuation shared `scripts/check_quality.sh`, both monotonicity scripts, and `quality_tools.md` — one coherent unit of gate-policy work that was not split across worktrees. SA125 is complete; SA121 now proceeds on the narrowed surface. SA122a is independent of both (a new, purely additive registry + checker touching no SA112/SA117 allowlist and needing no PostgreSQL/Docker), so in principle it could run concurrently — but Track 1 is one worktree, and a fourth worktree would buy no release time since SA122a is not on the critical path. Only `decisions.md` is conditionally shared, and the Merge procedure covers it. SA122b is the single further ordering exception, bounded below.
...
- **SA112 is split only as a serial Track 3 chain.** SA112a–SA112f are separate reviewed handoffs on the same worktree, each merged to `v87` before the next starts. Provisioning, traceback evidence, the root fix, lifecycle coverage, triggers, and closeout are causally ordered, so no child is eligible to move and parallel branches would recreate the stale-evidence hazard the split removes.
...
**Conflict surface.** The shared closeout files are `CHANGELOG.md` and `docs/technical/roadmap.md`, plus `docs/technical/decisions.md` when policy or acceptance evidence changes (SA122a writes its publish-context disposition there; SA121's continuation touches quality-policy prose). All three are covered by the Merge procedure above: the `git merge v87` before every merge-back forces conflicts to resolve on the track branch, preserving both sides. Two executable surfaces are also shared and already bounded: `.github/workflows/e2e.yml`'s path list between SA112e and SA115-CI-001 (serialized by the `merge after SA112` bound), and `scripts/quality_baseline.json`, whose line-count section was removed by completed SA125; SA117a/SA117b no longer modify it, and SA121's continuation is contractually forbidden from touching it (`SA121-DEC-001`) — so the tracks cannot collide and no additional procedure is required.

### Track readiness (2026-07-28)

- **Track 1 — DECISION-CLEAR AND RUNNABLE NOW; SA125 complete and SA117a unblocked.** The track carries completed **SA125**, open **SA121**, open **SA122a**, and merge-gated **SA122b**, with the remaining work run serially after SA125. **Next action: SA121** — continue on the narrowed surface (it shares four files with SA125 and must not run concurrently); it stays a partial checkpoint on CR-003/CR-006 with **no open maintainer decision**, and still cannot check its box until `make quality` is green — which now needs Track 3's two complexity fixes. `SA122-DEC-001` already resolves SA122a's publish-context disposition. Nothing on this track needs PostgreSQL/Docker, so it runs concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e.
- **Track 2 — VALIDATION AUTHORIZED AND RUNNABLE; merge queued behind SA112; CLOSED TO NEW WORK (off the critical path).** SA115 is committed and reconciled with current `v87` (`5193f198`, `5b5de830`), with `SA115-DEC-001` steps 1–2 done and post-merge focus checks green. **Step 3 heavy validation was authorized on 2026-07-27**, together with implementing the ratified `SA115-DEC-002` guard clamp; validation was human-authorization-gated, never SA112-gated, and only the *merge* is order-gated. **Next action:** run the serial-baseline vs. parallel comparison, `make ci-e2e`, and local `check_ci_locally.sh --e2e`, scheduled around Track 3's exclusive-infra legs. **Closed to new tickets** until SA115 merges — any ticket homed here would drag SA115 onto `v87` early (see the Parallelism section).
- **Track 3 — SA117a RECORDED PARTIAL CHECKPOINT; TEST AND QUALITY BLOCKERS; NO OPEN DECISIONS (on the critical path).** `43d9b8fc` remains recorded partial delivery, and SA117a's reviewed lockstep correction landed with `SA117-CR-001` resolved. The task stays unchecked on `SA117A-CR-002` and its `SA117-QG-REMED` share, now narrowed by `SA125-DEC-001` to `_perform_module_embed` CC 22 > 20; `make quality` remains red. **Next action: sync to `v87`; SA125 is satisfied, then run a fresh scoped SA117a continuation** closing the missing spy and the one complexity finding, rerunning the full validation/quality chain, and obtaining full-scope `STATUS: ok`. **Do not reduce `module_commands.py` or `manifest/loader.py` line counts** — those ceilings were deleted, not met. SA117b must not start first. Do not publish splits (SA117e), start SA112, or treat the candidate as release-ready until the serial SA117 chain and SA117e's full-scope review complete; SA96-PUBLISH (human-only) holds until SA112f. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Net.** One critical path — **SA117a → SA117b → SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. All three tracks are runnable with **no open maintainer decision**: Track 1's SA125 is complete, so SA121 is next and SA122a follows; Track 3 syncs the satisfied SA125 dependency and continues SA117a with a one-item quality share; Track 2 runs SA115's authorized heavy validation while yielding infra to Track 3 and holding its merge behind SA112. Rebalancing is otherwise unchanged — Track 2 cannot accept new tickets while its branch carries merge-gated SA115 commits, and the SA117/SA112 chains are causally serial.

**Standing decisions.** `SA117-DEC-002` (2026-07-26) — **SA117 runs first on Track 3, before SA112a**, and is a hard prerequisite for SA112b onward; rationale in the SA117 section. `SA117-DEC-003` (2026-07-27) — **SA117 was Tier 3 and is split into the serial children SA117a, SA117b, SA117c, SA117e**; the parent is umbrella-acceptance only and must not be executed as one handoff. `SA117-DEC-004` (2026-07-27) — **the SA117 scope meta-tooling is deferred to v88 as SA124**; the merged code stays in place, unadvertised, and is not a v87 gate. `SA121-DEC-001` (2026-07-27) — SA121's focused **tests/documentation-only** continuation is **authorized**, scoped to `SA121-CR-003`/`-005`/`-006` and required to preserve the reviewed runtime delta. `SA121-DEC-002` (2026-07-27) — **the strict-key parser is corrected, not the policy amended** (Option A): `complexity:file.py:::func` must be rejected as `malformed`, which **widens `SA121-DEC-001` to permit that minimal runtime edit and no other**; `scripts/quality_baseline.json`, the waiver ledger, Make targets, and thresholds all stay untouched. `SA115-DEC-001` step 3 — **AUTHORIZED** (2026-07-27, reversing the 2026-07-26 hold), with Track 3 holding infra priority; `SA115-DEC-002` (guard clamps workers) — **ratified**, implemented as part of that now-authorized validation phase. `SA122-DEC-001` (2026-07-27) — **publish is a full-coverage gate context**; see [decisions.md §publish-path-gate-coverage](./decisions.md#publish-path-gate-coverage). `SA125-DEC-001` (2026-07-28) — **per-file line ceilings are retired; per-function complexity ceilings are kept.** The `large_files` ceilings were measurement fossils that taxed documentation and could not distinguish mandated growth from decay; complexity ceilings are local, actionable, and stay under SA121's monotonicity rule. SA125 completed this decision on Track 1 before SA121's continuation (shared files) and SA117a's continuation (three of whose five quality blockers it dissolved); the policy is recorded in [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy). Each is stated in full in its own section above; no roadmap surface may contradict them. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
