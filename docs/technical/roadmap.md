...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; F-002 closed;        SA115a ◐ (acceptance-blocked   SA117e-4 (head)
...
**Critical path:** `SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA140 → SA96-PUBLISH` — eight remaining legs. **SA136f is closed** after full-scope review approved the normative loop/seal policy and resolved F-003, F-004, F-005, and F-006; SA117e-4 is now the Track 3 head and the only outward-facing split publication step. **SA141 is a parallel green-gate prerequisite** that must merge before SA112f but is not itself a link in that chain, so it runs concurrently on Track 1 rather than lengthening the path. **SA140 is on the path**: accepting `SA136D-QG-001` per-ticket does not satisfy the green gate, which requires an exit-0 `make quality`, so the repair is v87 scope sequenced after SA112f. **SA117e-5 is closeout and sits *off* the critical path**: SA112b's precondition is the *sealed* splits delivered by SA117e-4, not the umbrella's closure. Track 2 and SA122b-5 are entirely off-path filler; SA141 is off-path but not filler — the green gate cannot be claimed without it. The green-gate milestone is governed by the four-command join below and is not claimed here. No seal command, remote-ref mutation, tag push, or publication is authorized anywhere on this board; those sit behind SA117e-4's execution-time human gate.
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-4 (head) | **yes** — SA136f is closed and the sealed-publication execution scope is defined | **yes** — no other track's output is required to start the human-gated step | **yes** — no merge-order gate | **yes** | ✅ **yes** |
...
**Truly green today: Track 3 (SA117e-4)** — and it is *on* the critical path, so it is real progress rather than filler. Nothing else is truly green: **SA141** is start-blocked on a maintainer decision, **SA122b-5** is merge-gated by SA112e (a hard dependency only SA112e clears), and **SA115a** cannot finish until SA141 closes (also hard). The release-wide green gate remains unclaimed.

**Open maintainer decision: authorize a third SA141 full-gate cycle after the Poetry/package caches are restored.** SA136f's focused continuation is complete and reviewed; F-003, F-004, F-005, and F-006 are resolved. SA141's second continuation grant is consumed: F-002 is closed and the full gate ran twice, but neither run exited 0 because external package access failed at two different sites (first two CLI registry failures, then one Core Poetry-install timeout). Guidance for that decision is in [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance). `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). The remaining human gates (the twelve-row seal confirmation, SA96-PUBLISH) are execution-time gates obtained at the outward-facing action, not pending decisions.
...
**Status:** on the critical path and **live at SA117e-4**, the human-gated outward split-publication step. SA136a–f are closed, so the seal machinery and its normative ordering are merged and reviewed. SA112b's provisioning precondition is already satisfied; its split-publication precondition is the sealed output of SA117e-4.

**Standing order constraints.** Do not seal or push splits before SA136f is merged and SA117e-4's human gate is satisfied, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`. The loop/seal contract is normative in `decisions.md`; SA117e-4 is the outward publication step and SA117e-5 retains the SA117/SA136 closeout semantics.
...
1. **Branch publication is an idempotent republish; sealing is an immutable boundary.** Split *branches* are mutable working artifacts and may be republished as many times as verification requires. The split tags created by the seal are immutable under the normal transport contract and are not reversible. The core-tag push is a separate release trigger that may start PyPI publication. This distinction breaks the circular dependency that capped two SA117e-4 plan rounds — you cannot verify published state without publishing, so the branch loop must not be modelled as a one-shot mutation.
...
| PyPI release | irreversible | triggered by the pushed core tag; separate from split sealing |
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — acceptance umbrella; its final closeout remains with SA117e-5 and does not block SA117e-4
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [x] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓, SA136b ✓` — closed; seal predicate, `--seal`/`--seal-all`/`--status` and the `seal-*` Make targets are merged and reviewed. Retained here because SA117e-4 executes them. Evidence in [CHANGELOG.md](../../CHANGELOG.md).

  - [x] **SA136f — Ratify the loop/seal contract in `decisions.md` and close SA119.** `Tier 1 · deps: SA136a–SA136e ✓ all closed` — closed after full-scope review; no publication implied.
    **Reconciles SA136e's two advisory findings** (`F-003` medium and `F-004` low, both consistency): the closeout updates the named documentation surfaces and retains serial-handoff terminology consistently. Neither is a waiver or a release claim; evidence is in [CHANGELOG.md](../../CHANGELOG.md).
    Rewrite [§module-version-lockstep](./decisions.md#module-version-lockstep) Rule 3 as the six-step ordering: bump/stamp/commit → create the core tag **locally only** → loop `publish-module` and test installed `apply` with `--split-ref` → `seal-modules` → verify twelve of twelve sealed and a clean no-override `apply` → push the core tag with `git push origin X.Y.Z`, the PyPI publication trigger. Add Rule 4 (branch publication is idempotent up to the immutable seal; name the circular dependency it breaks), Rule 5 (embed consumes an immutable tag resolved by identity; no mapping table exists because none is needed; absence is a hard error; `--split-ref` is an explicit override), and Rule 6 (tags follow content identity). Replace the "Known limitation" paragraph — SA119 is resolved, not deferred — recording the residual: immutability rests on convention plus transport, so a force-privileged maintainer can still move a tag. Add Rule 7 (the seal is enforced client-side as check-then-act with sampled-tip, immediate branch-reread, tag-conflict, explicit-refspec, and fail-closed post-push checks; the narrow reread-to-push window is accepted and detected, not eliminated).
    - Allowlist: `docs/technical/decisions.md`, this roadmap, `CHANGELOG.md`.
    - Verify: no roadmap or decisions text defers immutable-ref pinning to v88; the recorded ordering matches what SA136d's targets actually do. **Closeout:** final full-scope review returned `STATUS: ok`; `F-003`, `F-004`, `F-005`, and `F-006` are resolved. SA119 is closed by design because default embeds consume identity-derived immutable split tags. No seal, tag, push, publication, SA117/SA136 umbrella closure, or release-readiness claim occurred.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · deps: none · blocks SA96-PUBLISH`
  ...
  **Approach — stamp + assert, then seal (all in v87).** Rules are in [decisions.md §module-version-lockstep](./decisions.md#module-version-lockstep), which is the SSOT; this ticket only tracks the work.
  ...
  3. **Seal** — owned by [SA136](#sa136--tag-sealed-split-publication) (2026-08-06). Stamping gives observability, not prevention: the embed ref was a moving branch, so a matched version was not a guaranteed-matched artifact. SA136 makes embed consume an immutable `splits/<m>-module/<version>` tag resolved by identity, which is the prevention half formerly deferred to SA119. **Consumer and producer tooling are done** (SA136c and SA136d, merged); SA117e-4 executes the outward publish/seal cycle under its fresh human gate.
     ...
     **Release ordering (mandatory):** bump/stamp/commit → create the core tag locally only → repeat the mutable branch publish/test loop → create and push immutable split tags → verify twelve of twelve seals and a clean no-override `apply` → push the core tag, which is the PyPI publication trigger. Publishing core before the split tags carry matching manifests ships a `quickscale apply` that fails for every user.

   **State.** SA117a/b/c are closed; the local stamp/assert and the comparison gate SA117e-4's acceptance consumes are merged. **SA117e is the sole open child**, and its head `-4` follows the closed SA136f policy child. The original executable candidate at `43d9b8fc` is on `v87` as recorded partial delivery, **unapproved at umbrella scope** — SA117e is a correction-and-review effort over merged-but-unapproved code, not a greenfield build. The former SA117d (scope meta-tooling) is deferred to v88 as **SA124**.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
     ...
     - [ ] **SA117e-4 — Loop, seal, and human-confirmed publication.** `Tier 2 · deps: SA136a–f ✓` · **HUMAN-GATED — outward-facing** — satisfies SA112b's precondition
       Executes the split-publication portion of the six-step ordering recorded by SA136f, using the machinery SA136a–e ship. The branch loop is repeatable, the split tags created by the seal are immutable, and the later core-tag push is the separate PyPI publication trigger. Before the seal, obtain fresh maintainer confirmation of the complete twelve-module pre-state; this is an execution-time gate, not a seal command input.
    ...
    - [ ] **SA117e-5 — Closeout review and close SA117 and SA136.** `Tier 1 · deps: SA117e-4` — documentation only; SA136f and SA119 are already closed, while SA117e-5 retains the final SA117/SA136 umbrella closeout semantics.
...
- **SA136 is homed on Track 3 as a sibling umbrella, not folded into SA117e-4.** The machinery is production CLI/publish code, a different risk class from the push ceremony; folding it in would make `-4` Tier 3 — the sizing violation that forced the SA117e split. Its `-f` documentation child is closed; the umbrella's final acceptance semantics remain with SA117e-5.
- **SA117e-4 is now Track 3's live head** after SA136f made the loop/seal contract normative. SA136a–f are closed as children, while SA136 remains an acceptance umbrella until the retained SA117e-5 closeout.
...
**Rebalancing verdict: no relocation available; one new ticket (SA141) placed on the idle track.** Nothing existing is movable — SA117e-4 is now the active Track 3 head after SA136f's normative closeout, SA112b–f are one serial evidence chain, SA122b-5 carries its `merge after SA112e` bound, SA140 is evidence-bound behind SA112b, and Track 2 stays closed to unrelated work. The one available parallelism gain was extracting the two Core E2E failures — previously carried as unowned prose on two tickets that were forbidden to fix them — into **SA141** and homing it on Track 1. Docker/PostgreSQL remains serialized with Track 3 priority whenever a service-backed critical-path leg is active.

**Open track-topology decisions: none. One non-topology maintainer decision remains:** whether to authorize a third SA141 quality-gate cycle after cache remediation. That is an authorization choice, not a topology one. Every other remaining “no” in the readiness table is a hard dependency that only the named upstream ticket can clear. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
