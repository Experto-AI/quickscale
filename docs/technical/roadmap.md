...
- [ ] **SA98 — Consolidate the `_sanitize_href`/`_sanitize_rendered_html` sanitizer copy-pair.** `Tier 2 · Track 2 · deps: SA97 (commons rule) — soft`
  **Blocked checkpoint (2026-07-17; maintainer-selected stop-and-merge; not complete).** Track 2 assessment confirmed SA98 is the first unchecked item for this track; no implementation files were changed. The maintainer selected **complete SA97 first** rather than choosing a provisional sanitizer home or skipping ahead to SA99.

  **Completed assessment:** the existing blog/listings sanitizer copies and their regression suites were identified, and the continuation boundary is clear: shared-home placement must follow the authoritative commons rule rather than be inferred independently on Track 2.

  **Pending/Blocking:**
  - **SA98-BLOCK-001 (blocking):** SA97 remains unchecked, and its commons-rule decision has not yet landed on `v87`. Under the maintainer-selected sequence, complete SA97 on Track 1 and merge its decision checkpoint into `v87` before SA98 changes begin. If that rule names a home applicable to this sanitizer, use it; otherwise record an explicit architecture/maintainer decision on the sanitizer home before implementation.
  - **Decision state:** no Track 2 placement choice is required until the merged SA97 rule is assessed. A sanitizer-home decision remains conditionally pending if that rule does not cover this runtime helper.

  **Clean continuation:** after SA97 lands, start in `/home/victor/code/quickscale-wt-track2` with a clean `git status`, merge `v87`, confirm or explicitly decide the sanitizer home as described above, then move both sanitizer functions there, update blog/listings consumers, run both existing sanitizer regression suites, confirm no second copy remains, and complete independent review before marking SA98 done.
  ...
  - After SA97 lands, use its sanctioned runtime home if the rule applies to this sanitizer; otherwise record an explicit architecture/maintainer placement decision. Then move the sanitizer to that home and have blog and listings both consume it.
...
**Net — no release-path maintainer decisions pending.** Both pre-publish module sweeps are complete (SA96-T1 and SA96-T2); SA93 (+ SA100, riding its review) continues on the release path. The audit-remediation tickets SA97/SA98/SA99 run in the idle Track 1/2 capacity, independent of the SA93 → SA96-GATE → SA96-PUBLISH chain. For the selected Track 2 sequence, SA98 waits for SA97; a sanitizer-home decision is needed only if the merged SA97 rule does not cover that helper. Rerun exact `make ci-e2e`, independently review the full SA93 delta, and prove E2E success on `v87`; then SA96-GATE can run the four-command publishability join and SA96-PUBLISH can proceed. The squash-migrations decision and bounded guardrail strategy are recorded in [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92); reasoning trail in [CHANGELOG.md](../../CHANGELOG.md).
