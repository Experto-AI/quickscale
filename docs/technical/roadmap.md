...
**One open maintainer decision exists on the whole board:** `SA136D-QG-001` — repair-versus-acceptance for the out-of-allowlist `apply_command.py` complexity overrun (see [SA136d](#sa136--tag-sealed-split-publication)). It gates Track 3's **can finish**, not its **can start**. All prior topology decisions are settled and recorded in [CHANGELOG.md](../../CHANGELOG.md).
...
**Rebalancing verdict: no relocation available.** Nothing is movable — SA136f/SA117e-4 are dependency-bound to SA136d, SA112b–f are one serial evidence chain, SA139 → SA122b-5 is in-track ordered, and Track 2 stays closed to unrelated work. Docker/PostgreSQL remains serialized with Track 3 priority when SA115a reruns its umbrella gates.

**Open track-topology decisions: none.** The only open maintainer decision on the board is `SA136D-QG-001` (repair versus reviewed acceptance), recorded in SA136d's checkpoint above; it does not alter topology. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
