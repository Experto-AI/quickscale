...
> **Track status (2026-07-11, SA65–SA70 opened; SA63/SA69 closed; SA67 blocked):** Track 1 — **3 open items** (SA59, SA60, SA70) — SA59 is **blocked**: investigation and gate-split planning are complete, but no implementation is integrated into v87. The Track 1 worktree holds unmerged implementation files (structural-split artifacts, CI role wiring, test scripts) that are not safe to merge due to open blockers (see SA59 checklist below). The restricted-role integration has not been successfully executed end-to-end and known technical gaps remain. SA59's publish.yml blocker is resolved via a 2026-07-10 decision (split release/CI validation into a DB-free unit gate plus a separate PostgreSQL integration gate; see SA59's Decision note), raising it from Tier 1 to Tier 2; implementation is still open. Track 2 — **0 open items**, fully closed this pass.   Track 3 — **1 open item** (SA67 blocked). SA65 is complete. SA66 is complete — the conformance gate passes (zero unclassified gaps). CR-SA66-005 remaining (low, advisory): prose says common and mode-specific required specs are classification sources, but test iterates only mode-specific specs and rationale test does not inspect decisions.md; manual parity verified. SA67 is blocked — no repo or deployment access to the external beta sites (`experto-ai-web`, `bap-web`) to perform the required verification. SA68 is complete — the migrate-path launcher env-pair contract is finished; `start.sh.j2`/`production.py.j2` use the bridge exclusively; `orgs/apps.py` no longer inspects `sys.argv`. CR-SA68-004 (rollout-guidance for Redis-absent createcachetable) remains as accepted blocking follow-up — see SA68 entry below.
...
- [ ] **SA59 — Stop auto-priming `QUICKSCALE_ALLOW_BYPASSRLS=1` in the test-unit path — blocked.** `Tier 2 · Track 1 · deps: none`
  ...
  *Target files:* `Makefile:321-327`; `scripts/test_unit.sh:365-366` (split into unit/integration paths); `.github/workflows/ci.yml:234-351` (`test` job — add the NOBYPASSRLS role, matching `isolation-conformance`'s existing postgres-service shape); `.github/workflows/publish.yml` (`test` job — add a postgres service + NOBYPASSRLS role for the new integration gate); `scripts/test_isolation_conformance.sh:50` (existing role-creation pattern to reuse, not duplicate); `docs/technical/decisions.md` (record the unit/integration gate split as the testing-pipeline shape, alongside the existing Database Policy section).
  *Acceptance:* the blanket `QUICKSCALE_ALLOW_BYPASSRLS=1` export is removed from the unit-only path; the integration path runs module suites against a NOBYPASSRLS role in both `ci.yml` and `publish.yml`; developers set the SA14.4 hatch explicitly per-suite when they need it; `make test-unit` (and any new integration target) documents the split in its help text; decisions.md records the split.
  - [ ] **Investigation and planning (2026-07-10):** The initial blanket-removal draft was rolled back before landing; the unit/integration gate-split decision above was adopted as the path forward. Confirmed that `publish.yml` invokes `scripts/test_unit.sh` without a PostgreSQL service or a NOBYPASSRLS/NOSUPERUSER role; removing the blanket export would fail the publish workflow. Worktree-only implementation artifacts were retained but remain unmerged, unreviewed, and blocked (see Unmerged worktree state below).

  **Unmerged worktree state (Track 1 only — not integrated into v87):** The following implementation artifacts exist only in the Track 1 worktree. They have not been merged into v87, are unreviewed, and are not safe to merge due to the open blockers below. No SA59 implementation is integrated into any release.

  - CI `quickscale_test_role` (LOGIN CREATEDB NOBYPASSRLS NOSUPERUSER) added to ci.yml's `test` job, wired through every module's QS_*_DB_USER env var, with database ownership + public schema grants on 11 module test databases. `QUICKSCALE_ALLOW_BYPASSRLS=0` prevents the hatch from being overridden. Publish.yml role setup, Makefile/test_unit.sh split, and decisions.md record are worktree-only.
  - Blanket `QUICKSCALE_ALLOW_BYPASSRLS=1` removed from Makefile `test-unit` target and `scripts/test_unit.sh`. Module test suites stripped from `test-unit` (now DB-free core + CLI only). `scripts/test_integration.sh` created. `make test-integration` target added. `make test` updated to run both unit and integration. ci.yml `test` job split into separate unit/integration steps. PostgreSQL 18 service, database creation, and `quickscale_test_role` setup added to publish.yml `test` job. Gate-split recorded in decisions.md §Unit/Integration Gate Split.
    *(Validation findings from worktree integration gate: the integration gate runs under NOBYPASSRLS and exposes pre-existing RLS test failures — 15 in billing (quickscale_modules/billing) and 45 in social (quickscale_modules/social) — plus 77.55% mean module coverage across the 12 integration suites. These are discovered blockers requiring separately scoped follow-up; they are not resolved by SA59 itself.)*

  - [ ] **Blocking: backups integration tests use SQLite and lack a `QS_BACKUPS_DB_*` PostgreSQL/RLS seam (F-SA59-DB-007).** Manifests/settings fall back to SQLite; no integration run exercises backups against a real PostgreSQL restricted role. Backups must connect as a restricted PostgreSQL role with RLS active before the integration gate validates tenant-boundary safety.
  - [ ] **Blocking: integration runner must exclude e2e tests from normal CI (F-SA59-E2E-008).** `scripts/test_integration.sh` includes `pytest -m e2e`, pulling Playwright-based end-to-end tests into the CI integration gate. E2E tests belong under a dedicated `make test-e2e` target, not the module integration run.
  - [ ] **Blocking: `make ci` / local validation parity (F-SA59-PARITY-009).** The `make ci` target and `scripts/check_ci_locally.sh` run `make test-unit` only, missing the integration gate entirely. A developer running `make ci` locally sees a different validation surface than CI. Either wire `make test-integration` into the local CI flow or stop documenting it as CI-local parity.
  - [ ] **Blocking: CI coverage thresholds, XML artifacts, and Codecov upload missing (F-SA59-COV-004).** Neither `make test-unit` nor `scripts/test_integration.sh` produces `coverage.xml` output; no CI step uploads coverage data to Codecov; no per-module coverage thresholds are enforced. Core, CLI, and each module must produce persisted coverage artifacts with enforced minimum thresholds.
  - [ ] **Blocking: administrator-side least-privilege schema/table grants required; Forms and Listings test helpers create/grant inner RLS role via runtime DDL (F-SA59-ROLE-006).** Forms and Listings Django-created test databases have test helpers that currently create and grant the inner RLS testing role via runtime `CREATE ROLE ...` DDL. This runtime role DDL must be removed — the inner role must be pre-provisioned by the database administrator using explicit least-privilege schema/table grants plus `has_schema_privilege`/`has_table_privilege` assertions in each test-database setup path. The outer test connection role must retain `NOCREATEROLE` so the runtime DDL path is structurally impossible. Additionally, every test-database role creation path must assert `LOGIN CREATEDB NOINHERIT NOBYPASSRLS NOSUPERUSER` explicitly. The CI worktree `quickscale_test_role` currently uses `LOGIN CREATEDB NOBYPASSRLS NOSUPERUSER` but does not explicitly assert `NOINHERIT` — without explicit `NOINHERIT`, a test role that inherits privileges from a parent role could bypass the intended restricted-surface assertion.
  - [ ] **Fix: direct-connection role uses `LOGIN`, not `NOLOGIN`; correct matching docs and command matrix (F-SA59-DOC-002, F-SA59-CMD-010).** Role descriptions in scripts, workflow docs, CHANGELOG, and the test-command reference matrix use `NOLOGIN`, but the CI `quickscale_test_role` requires `LOGIN` to establish test-database connections. All documentation and the command matrix must reflect `LOGIN CREATEDB NOINHERIT NOBYPASSRLS NOSUPERUSER` consistently.
  - [ ] **Blocking: execute full restricted-role PostgreSQL 18 module gate end-to-end with coverage evidence (F-SA59-VALID-003).** The restricted-role integration pipeline has not been successfully executed through its full path. The ci.yml integration step provides partial signal, but the publish.yml restricted-role workflow remains unverified. A successful end-to-end run must produce per-module coverage artifacts and confirm the restricted role fired correctly across all suites.
  - [ ] **Finding: agent-handback provenance discrepancy (advisory; user accepted).** The previous implementation handback omitted `decisions.md` and the new `scripts/test_integration.sh` from its changed-files listing, so those changes were not independently reviewed and the closeout snapshot was incomplete. Treat as advisory; any future SA59 continuation should include these files in the handback manifest.
  - **Blocked state:** Implementation is held in the Track 1 worktree only. Not safe to merge into v87 until all blockers above are resolved. User selected stop rather than authorize the required administrator-side grant path for F-SA59-ROLE-006.
