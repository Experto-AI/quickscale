...
SA141 ◐ (head; Core E2E             SA115a ◐ (acceptance-blocked   SA136d ◐ (corrected seal tooling;
  │  poetry-install repair;          │   by SA141; only ticket      │  review+validation-blocked)
...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **no autonomous cycle remains** — the carry-forward plan for F-007/F-008/F-004 and `SA136D-QG-002` is recorded on the ticket, but executing it requires explicit continuation authorization | **no** — F-004/F-007/F-008 remain blocking and `SA136D-QG-002` blocks final validation | **yes, recorded partial only** — the maintainer explicitly selected the checkpoint merge with F-007 unresolved; this grants no seal or release authorization | no — review+validation-blocked | ✅ **yes** |
...
**Truly green today: SA141 only**, and it is **off the critical path but not filler** — the green gate is unreachable without it. Every other track is blocked: **SA136d** is on the critical path and **review+validation-blocked** by F-004/F-007/F-008 plus `SA136D-QG-002`; **SA122b-5**'s merge bound is a **hard upstream dependency** only SA112e can clear; and **SA115a** cannot finish until SA141 lands. The release-wide green gate remains unclaimed and no full E2E greenness is claimed anywhere.

**Open maintainer decisions: none.** On 2026-08-09 the maintainer explicitly selected SA136d's recorded-partial checkpoint merge despite unresolved high-severity F-007; SA136d remains unchecked, the decision grants no seal or release authorization, and a future correction cycle requires fresh authorization. Every other blocker is a hard upstream dependency, not a pending choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); SA139's scope expansion is implemented, reviewed, and closed. The remaining human gates are execution-time gates obtained at outward-facing actions, not pending decisions.
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed`
    ...
    **Seal-atomicity requirement re-scoped (settled; rationale in [CHANGELOG.md](../../CHANGELOG.md)).** Server-transactional atomicity is **withdrawn as a requirement**. The seal is enforced client-side as check-then-act with a fail-closed compensating verification, and the residual window is documented rather than engineered away. This unblocks the ticket on stock Git against GitHub.com; no platform migration is required.
    ...
    - **Recorded-partial checkpoint (2026-08-08; task remains unchecked).** `F-001`–`F-008` are the stable executable review ledger; F-006 is resolved by independent review, while the open findings below remain blocking and no release claim is made.
      ...
      - **Decisions needed: none.** On 2026-08-09 the maintainer explicitly selected “Stop here, record Done / Pending-Blocking / Decisions-needed, and merge the checkpoint” with high-severity F-007 still unresolved. This authorizes only the recorded-partial branch merge: SA136d remains unchecked, every blocker above remains open, and no seal or publication authorization is implied. Any future correction cycle needs fresh authorization. `SA136D-QG-001` remains **resolved 2026-08-08 as reviewed acceptance** under the inherited "no worse than found" rule; no ceiling is raised and SA140 still owns the repair.
      - **Carry-forward plan for the next hand-off (not yet plan-reviewed; needs fresh authorization before any mutation).** No slice of the *approved* plan remains unimplemented — this is the derived remediation direction for the three blockers, recorded so the next attempt starts from it rather than re-deriving it. Order matters: fix F-007 first, since it is the only one that can mint a wrong-identity irreversible tag.
...
**Open track-topology decisions: none, and no open maintainer decision remains.** The maintainer explicitly selected SA136d's recorded-partial checkpoint merge with unresolved F-007 while withholding all seal/release authorization; a future correction cycle requires fresh authorization. Every other “no” in the readiness table is a hard dependency, not a pending choice. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
