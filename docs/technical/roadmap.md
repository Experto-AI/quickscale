...
> **Track status (2026-07-10, SA65–SA70 opened):** Track 1 — **3 open items** (SA59, SA60, SA70). Track 2 — **0 open items**. Track 3 — **4 open items** (SA65, SA66, SA67, SA68).
...
- [ ] **SA59 — Stop auto-priming `QUICKSCALE_ALLOW_BYPASSRLS=1` in the test-unit path.** `Tier 1 · Track 1 · deps: none`
  ...
  *Acceptance:* the blanket export is removed; CI's test-unit job runs against a NOBYPASSRLS role (create one via `QS_*_DB_USER` if the current CI role has BYPASSRLS/superuser — verify against the same container SA58's test targets), and developers set the SA14.4 hatch explicitly per-suite when they need it. `make test-unit` documents the opt-in in its help text if it doesn't already.
  - [x] **Investigation (2026-07-10):** No implementation changes were retained or committed — the removal draft was rolled back before landing. Confirmed that `publish.yml` invokes `scripts/test_unit.sh` without a PostgreSQL service or a NOBYPASSRLS/NOSUPERUSER role; removing the blanket export would fail the publish workflow. Before SA59's acceptance criteria can be met, decide whether `publish.yml` must provision a PostgreSQL service with a NOBYPASSRLS role (retaining module test coverage in the release pipeline) or whether release validation should be restructured into a DB-free unit gate plus a separate PostgreSQL integration gate. SA59's source acceptance criteria remain pending until this publish/role decision is resolved.
...
- [x] **SA69 — Record the `ImproperlyConfigured` exception-identity change in decisions.md.** `Tier 1 · Track 2 · deps: none`
  ...
  *Acceptance:* decisions.md states which `ImproperlyConfigured` a contracts-layer catcher should import vs. which a Django-runtime catcher should import; the stale adapter docstring references are corrected; consider whether a lint/naming guard is warranted if a fourth housekeeping-labeled behavioral commit recurs (note it as a watch item if not addressed now, rather than scope-creeping this task into tooling work).
  Follow-up (impl): decisions.md §ImproperlyConfigured-Exception-Identity records the split and watch item. The three named adapters' docstrings are corrected from `django.core.exceptions.ImproperlyConfigured` to `quickscale_core.contracts.module_discovery.ImproperlyConfigured`. No stale references were found outside the three named adapters via grep. The lint/naming guard is deferred as a watch item per the decision record.
