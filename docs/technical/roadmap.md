...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  │  ON THE CRITICAL PATH             │  validation AUTHORIZED      │  TRULY GREEN — RESET-001
  │  PARTIAL — HANDOFF-004 consumed    │  cannot finish → SA112f     │  Umbrella, split by risk class
  ▼  F-005 still blocking — reset      │  cannot merge  → SA112e     │  review · source · infra ·
                                      │                             │  PUSH(human) · closeout
...
**Critical path:** `SA117e-2 → -3 → -4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path), with **SA112a on Track 1 at a recorded-partial checkpoint** (the `SA112A-HANDOFF-004` attempt ended without a checked box — see the SA112a ticket) and remaining a merge-order precondition of SA112b. **SA117e-5 is closeout and sits *off* the critical path** — SA112b's other precondition is the *pushed* splits, delivered by SA117e-4, so SA112b does not wait for the umbrella to close. Track 2 is entirely off it; Track 1 is on it only for SA112a.

**Cross-track edges — three, all merge-order only.** SA122b-5 merges after SA112e, and SA115 merges after SA112; both share `.github/workflows/e2e.yml`'s `pull_request.paths` list. **SA112a (Track 1) must complete and merge before SA112b starts (Track 3)** — the recorded-partial checkpoint already on `v87` preserves work but does not satisfy that edge; only the completion merge after `F-005` closes does. SA112b consumes the independently approved provisioning seam, so the edge is satisfied only by SA112a's later completion merge-back to `v87`, not by co-scheduling.
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-2 | **yes** — `SA117E2-RESET-001` granted 2026-08-03; review budget restored | **yes** — one fresh full-scope closeout review over its own two documents | **yes** — no order gate | ✅ **yes** | ✅ **yes** — real progress |
| **Track 1** — SA112a | **no** — `SA112A-RESET-001`; the control plane remains at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1` | **no** — `F-005` lacks an independent QG result and an approving `STATUS: ok` full-scope review | **yes** — recorded-partial merge authorized; no order gate | no | ✅ **yes** — real progress, blocked on control-plane reset |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is truly green (`SA117E2-RESET-001`, granted 2026-08-03) and on the critical path; Track 1's parallel leg is paused at a recorded-partial checkpoint** — the maintainer granted `SA112A-HANDOFF-004`, DC-48 produced a narrower `F-005` candidate and implementation-agent evidence, but the required independent QG dispatch was refused before execution because the persisted control-plane scope still reports `qg_runs=3/2`, `review_cycles=2/2`, and `authorized_retries=1/1`. No independent QG result or approving full-scope review exists, so the attempt ended without a checked box and the maintainer selected recorded partial delivery. Track 1 is blocked on `SA112A-RESET-001`, a supported reset that actually clears those per-scope counters; another authorization-only retry cannot. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path and **truly green** — head remains SA117e-2, and `SA117E2-RESET-001` (granted 2026-08-03) restores the review budget needed to close it, so the child can start, finish, and merge on Track 3's own work. SA117e-1 closed 2026-08-03 (evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). SA117e-2's complete five-command ordered validation passes against the maintainer-approved `1333eb4a…` replacement baseline; `SA117E-VAL-002` stays open only until the granted fresh full-scope closeout review returns `STATUS: ok`. Track 1 is not running a live parallel leg: its `SA112A-HANDOFF-004` attempt ended at a recorded-partial checkpoint with a narrower but unapproved `F-005` candidate after the persisted control plane refused independent QG at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1`; `SA112A-RESET-001` now blocks continuation, so Track 3 is the only truly green track. `SA117E1-REV-001` (high/blocking, explicit-absence lease safety) and `SA117E1-REV-002` (medium/blocking, unbound publication authorization) remain owned by SA117e-4; plan-review `F-002` remains the separate medium/blocking host-PostgreSQL lifecycle requirement on SA117e-3. **SA117e-2 remains unchecked until that review returns `STATUS: ok`; SA117e-3 is not authorized to start before it does.** **SA112a runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not run SA117e-3's host-PostgreSQL work before SA117e-2 closes and SA117e-3's scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 closed 2026-08-03** (full-scope historical review, evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). Two of its `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. `SA117E1-REV-003`, `CR-SA117E-005`, and `CR-SA117E-006` are resolved.
    ...
    - [ ] **SA117e-2 — Complete the rebaselined ordered source validation.** `Tier 2 · deps: SA117e-1 ✓ closed` — local, service-free
      **Recorded-partial checkpoint (2026-08-03) — the complete five-command ordered validation is green against `SA117_BASELINE_REF=1333eb4a…` with no source delta; per-command evidence, digests, and test counts are in [CHANGELOG.md](../../CHANGELOG.md).** `SA117E-VAL-002` (**high**, validation) has direct green evidence and needs only the mandatory full-scope independent closeout review over the completion record.
      **`SA117E2-RESET-001` — CONTROL PLANE RESET, GRANTED 2026-08-03.** The maintainer reset the control plane for SA117e-2, clearing the spent `review_cycles=3/2` / `authorized_retries=1/1` ledger and granting **one fresh full-scope independent closeout review** over this child's unchanged documentation scope, to be passed to the control plane as `user_authorization`. The stop was a consumed per-attempt ledger, not a design objection; the validation itself is unchanged and is **not re-run**. **The grant waives no gate** — `SA117E-VAL-002` closes only if that review returns `STATUS: ok`, the changed-file scope stays exactly the two closeout documents, and a review ending without a checked box records its retrospective and needs fresh authorization. Track 3 is therefore **can start: yes, can finish: yes, can merge: yes — truly green**.
      - Verify: a full-scope independent review of `CHANGELOG.md` and this roadmap returns `STATUS: ok`, explicitly closes `SA117E-VAL-002`, and confirms no completion language exceeds the green evidence.
      - Rollback: revert the two documentation files; validation itself changed no tracked file.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **`SA112A-HANDOFF-001` and `SA112A-HANDOFF-002` — CONSUMED.** The first attempt produced the exact four-file candidate; the second corrected the original `OUTPUT_DIR` shape gap, resolved executable `F-003`, and exposed the remaining lexical-alias defect below. Full delivery narrative and validation evidence are in [CHANGELOG.md](../../CHANGELOG.md).

    **`SA112A-HANDOFF-003` — GRANTED and CONSUMED 2026-08-03.** The maintainer granted **one fresh implementation attempt, one fresh quality-gate run, and one fresh full-scope independent review** over SA112a's unchanged four-file scope, to be passed to the control plane as `user_authorization`. The prior stop was a consumed per-attempt ledger (`qg_runs=2/2`, `review_cycles=2/2`), not a design objection; the approved continuation plan above carries plan review `STATUS: ok` and is **reused verbatim**. **The grant is per-attempt and waives no gate** — the executable slice still needs `STATUS: ok`, closeout still needs its own final full-scope review, and an attempt ending without a checked box records its retrospective and needs fresh authorization. **That attempt has now run and ended without a checked box — see the attempt retrospective below.** The candidate is unapproved and unmerged, Track 1 is again awaiting fresh authorization, and SA112a's *completion* merge remains SA112b's precondition; neither the existing recorded-partial merge nor this checkpoint satisfies it.

    **Scope of the attempt — the one remaining blocker.** `F-005` (**medium**, correctness; `scripts/_installed_wheel_venv.sh` / `scripts/test_installed_wheel_venv.py`): after a nonexistent path component sets probe state false, `..` returns to an existing prefix without restoring probing, so a later directory symlink is treated as nonexistent and receives provisioning output. Cover reverse-order/repeated cancellation, root cancellation, root spellings, accepted `link/../sibling` symlink-ancestor identity, status 2, empty stdout, native caller-trap parity for `missing/../link` rejection, and target preservation. `F-006` remains **low/advisory** unreadable-directory hardening and is **not** part of this correction. Delivered work, validation evidence, `F-003`'s resolution, and the reviewed-tip identity gap from the post-review Ruff hooks are in [CHANGELOG.md](../../CHANGELOG.md).

    **Pre-dispatch findings (2026-08-03; investigation only, no delta retained — reproduce before trusting).** Two facts were established against the merged checkpoint and are recorded so the attempt does not re-derive them:
    1. **`F-005` is confirmed exploitable, and a negative control exists.** At the merged tip, `provision_installed_venv.sh REPO /tmp/X/missing/../link` (where `link → real/`) exits **0**, begins staging, and writes into the unowned symlink target; `/tmp/X/missing/../link/.` already exits 2 but on the *wrong* diagnostic (`must not exist or must be empty`, not `must not be a symlink`). That pair is a ready-made before/after control for the correction.
    2. **The obvious fix is wrong — do not restore a saved parent.** Pushing the pre-component walk state and restoring it on `..` repairs `missing/../link` but **breaks `link/../sibling`**: `mkdir -p` resolves `..` through the *symlink target*, so `link/..` is the target's parent, not the link's. The correction must keep the existing canonical `%/*` trim (which matches filesystem semantics) and recompute **only** the probe flag after the pop — resuming probing iff the popped-to path exists. Any candidate that changes `..` resolution for an existing symlinked component is a regression, and `link/../sibling` belongs in the test matrix as its guard.

    **`SA112A-HANDOFF-003` attempt — recorded-partial checkpoint (2026-08-03; task remains unchecked; no completion claim).** **Done:** Track 1 synced cleanly to `v87` `952806c13f03c7899dbdc40aa09e1f0f4fceda42`; DC-41 implemented the `F-005` correction candidate in `scripts/_installed_wheel_venv.sh` and `scripts/test_installed_wheel_venv.py`, self-reporting reproduction, six added cases, focused tests, positive and negative smoke controls, parity baseline, lint, typecheck, diff, and scope checks — implementation-agent evidence only. **Pending-Blocking:** no independent QG result exists and no approving `STATUS: ok` full-scope review exists — the independent full-scope review (DC-44) over the two scripts plus roadmap and changelog returned `STATUS: partial`. The authorized QG dispatch (DC-42) incremented `qg_runs` to 3/2 but returned empty with no ResultEnvelope or evidence; `record_failed_handback` marked an infrastructure failure, and a fresh shortened retry was refused because `qg_runs` remained 3/2. `F-005` (**medium**, correctness) therefore remains open and unapproved — the required repeated-cancellation, root-cancellation, accepted `link/../sibling` symlink-ancestor identity, and native caller-trap parity for `missing/../link` rejection tests remain missing; SA112a remains unchecked and does not satisfy SA112b; `F-006` (**low/advisory**) remains outside this correction. **Decisions needed:** fresh maintainer authorization (`SA112A-HANDOFF-004`) restoring **both** one QG dispatch **and** one full-scope review dispatch — the consumed shared override ("one extra cycle on blocking findings only") cannot supply both; the approved continuation plan is reused verbatim. Full narrative is in [CHANGELOG.md](../../CHANGELOG.md).

    **`SA112A-HANDOFF-004` attempt — recorded-partial checkpoint (2026-08-03; task remains unchecked; no completion claim).** **Done:** the maintainer granted `SA112A-HANDOFF-004`; Track 1 started clean, merged `v87` as `c678a42c`, reused Poetry 2.4.1 / Python 3.14.6, and DC-48 updated only `scripts/_installed_wheel_venv.sh` and `scripts/test_installed_wheel_venv.py`. The candidate guards the empty component stack/root spellings and adds twelve F-005 matrix cases; DC-48 self-reported the pre-fix exploit reproduced, 67 focused tests, syntax, smoke/negative-control/parity/lint/typecheck/diff/scope evidence — implementation-agent evidence only. **Pending-Blocking:** `F-005` (**medium**, correctness) remains open and unapproved because no independent QG result and no approving `STATUS: ok` full-scope review exist. The required QG dispatch was refused before execution even with the exact user authorization because the persisted scope remains at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1`; no validation or review allowance was restored. SA112a remains unchecked and does not satisfy SA112b; `F-006` (**low/advisory**) remains outside this correction. **Decisions needed:** `SA112A-RESET-001` — before any continuation, use a supported control-plane reset that clears the SA112a changed-file scope's QG, review, and authorized-retry counters, then rerun the complete ordered QG and obtain full-scope `STATUS: ok`. Another authorization-only retry cannot buy either dispatch. The maintainer selected recorded partial delivery for this attempt.

    The merged checkpoint preserves progress only. It is not a waiver, completion claim, executable approval, or satisfaction of SA112b's dependency.
...
**Status:** **head is SA112a and the track is at a recorded-partial checkpoint** — the `SA112A-HANDOFF-004` attempt ran and ended without a checked box: DC-48's two-file `F-005` candidate and twelve added matrix cases have implementation-agent evidence only; the required independent QG dispatch was refused before execution at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1`, so no independent QG result or approving full-scope review exists. The maintainer selected recorded partial delivery. Track 1 is blocked on `SA112A-RESET-001`, a supported control-plane reset that clears all three persisted counters before the focused validation/review cycle can resume. It is on the critical path, and neither the earlier partial merge nor this checkpoint satisfies SA112b's dependency — only SA112a's completion merge does. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**Open decisions — three, all the maintainer's; one blocks Track 1 today.** `SA112A-RESET-001` (**blocks Track 1 today**): `SA112A-HANDOFF-004` was granted and consumed, but its exact QG dispatch was refused because the persisted scope remained at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1`; clean continuation requires a supported control-plane reset that actually clears all three allowances before the ordered QG and full-scope review run. The other two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. `SA117E2-RESET-001` was **granted 2026-08-03** and is no longer open. All other blockers on the board are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are **execution-time** human gates obtained at the outward-facing action and never pre-granted.

Earlier resolved authorizations (`SA117E-VAL-001`, the `SA117E-VAL-002` replacement-baseline decision, `SA112A-AUTH-006`, `SA112A-HANDOFF-001`, `SA112A-HANDOFF-002`, `SA112A-HANDOFF-003`, `SA112A-HANDOFF-004`, `SA112A-TRACK-003`, SA132's three remediation decisions) are recorded in [CHANGELOG.md](../../CHANGELOG.md) and are not restated here.
