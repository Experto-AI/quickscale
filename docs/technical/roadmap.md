...
SA122b-1 → -2 → -3 → -4 → -5          SA115 (e2e xdist; deps: none) SA117e-3 ⛔ → -4 → -5
  ...
  │  Make · sh · ci · publish · e2e   │  validation AUTHORIZED      │  ORDER-001 decision needed
  │  checker authoritative (SA128 ✓)  │  cannot finish → SA112d     │  correction + Phase 5 green
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      Execute the reviewed installed-wheel harness against the public splits, then run `make ci-e2e` in exclusive Docker/PostgreSQL capacity. The harness is **ephemeral and untracked**: a rematerialized artifact whose hash does not match the approved SHA is **re-reviewed before use, not trusted by provenance**. Track 3 holds infra priority ([Infra serialization](#dependency--parallelization-overview)) — an SA115 run is abandoned or restarted rather than queued ahead of this.
      - Verify: harness hash matches the approved SHA or a fresh review returns `STATUS: ok`; the harness executes successfully including installed all-module `apply`; `make ci-e2e` exits 0 with `QUARANTINE_TICKETS` empty; the host-PG negative control reproduces the expected failure; the frozen host PostgreSQL identity/configuration/readiness is restored on every exit path.
      ...
      **Blocked (2026-08-05; recorded partial delivery at correction commit `ffd97e66`; no tag, push, or publication).** Everything local is green — correction reviewed `STATUS: ok`, the negative control passed with the literal stage-11 banner and host restoration, and the approved harness ran with 12/12 immutable public split-SHA parity (evidence in [CHANGELOG.md](../../CHANGELOG.md)).
      - **`SA117E3-PUBLIC-ANALYTICS-001`** (**high**, external contract) — installed `apply` stopped before internal-DB and all-module acceptance because public `splits/analytics-module` reports manifest version `0.80.0` while core and the source manifest require `0.87.0`. The harness produced no pass artifact and `make ci-e2e` has not run at `ffd97e66`.
      - **Decision needed — `SA117E3-ORDER-001` (maintainer's, not upstream work).** The acceptance order is circular: SA117e-3 requires a successful public-split harness *before* SA117e-4, while SA117e-4 owns the human-gated split refresh that would make the public manifests match `0.87.0`. Either (a) authorize a reviewed, human-gated split refresh before SA117e-3 closes — without bypassing `SA117E1-REV-001`, `SA117E1-REV-002`, or the twelve-row confirmation — or (b) move the public-harness success gate into SA117e-4 and revise the dependency contract explicitly, leaving SA117e-3 to close on `make ci-e2e` alone. Nothing here authorizes a push.
    ...
    **Carried plan-review ledger.** Only `F-001` remains, resolved at plan-detail level and carried by SA117e-4. `F-002`, the `F-003`–`F-010` pre-use ledger, and the correction findings are resolved; `SA117E3-EXEC-001` was granted and consumed. SA117e-3 waits on `SA117E3-ORDER-001`; full history is in [CHANGELOG.md](../../CHANGELOG.md).
