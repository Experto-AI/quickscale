...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: none`) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2, independent. Its authorized tests/documentation-only continuation is preserved as recorded partial delivery; SA121 remains open on the parser-contract, exact-assertion, and full-quality blockers recorded below.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Split (`SA117-DEC-003`, 2026-07-27): SA117 was Tier 3 and is now four serial children.** The roadmap rule at the top of this file requires every phase to be Adaptive Tier 1–2. SA117 failed that gate on three independent counts, and the failure was empirical, not theoretical — two plan-review cycles hit the non-convergence cap without ever reaching `STATUS: ok`:
  ...
  - **Consequence.** Six open findings (`SA117-CR-001`, `-003`, `-005`, `-006`, `SA117-CONT-PR-003`, `-006`) sat in three unrelated subsystems, so every review pass had to hold all three objectives at once. That is the mechanism that stalled the ticket; splitting the review working set is the fix, and `SA117-DEC-004` removed the third objective from v87 scope entirely.
  ...
  **The chain is SA117a → SA117b → SA117c → SA117e.** SA117a is what SA112b's evidence validity actually depends on; SA117b is what makes pushing refreshed splits safe; SA117c proves the published splits match the core; SA117e reviews and pushes. The former SA117d (scope meta-tooling) was **deferred to v88 as SA124** by `SA117-DEC-004` and is no longer part of this ticket.
...
- [ ] **SA121 — Enforce the shrink-only baseline against the merge base.** `Tier 2 · Track 1 · deps: none · blocks any further baseline edit`
  ...
  **Blocked checkpoint (2026-07-27 — recorded partial delivery after the change-review cap; task remains open).**
  ...
  - **Decisions needed at authorization time:** none. No product or architecture decision was open when the authorization above cleared that cycle to start; the later checkpoint below records what the cycle discovered.

  **Recorded partial continuation checkpoint (2026-07-27 — explicit maintainer merge authorization; task remains open).**
  - **Done:** The tests/documentation continuation corrects `quality_tools.md` so `waiver_evaluations` are accurately described as retained in the policy artifact but omitted from stdout, expands strict-key and canonical-diagnostic coverage, and isolates shell success tests from unrelated live-repository `find`/`wc` drift. Accepted evidence for the landed slice: the unfiltered focused suite passed **156/156** with zero skips or deselections; the standalone monotonicity helper against `v87`, `bash -n scripts/check_quality.sh`, and `make check` exited 0. Review passed the CR-005 documentation correction, deterministic shell fixture, and tests/docs-only scope protection. No runtime helper, baseline, waiver ledger, Make target, or threshold changed.
  - **Pending / blocking:** `SA121-CR-003` (**high**, completeness) remains open because `complexity:file.py:::func` is accepted as an `orphan` (the extra colon becomes part of the symbol) instead of rejected as `malformed`; the new test is false-green, and unknown-prefix, empty-symbol, unsafe-waiver-path, plus exact malformed-baseline envelope coverage remain incomplete. `SA121-CR-006` (**medium**, test gap) remains open because acceptance-critical sites still use filtered/subset, `>= 1`, `any(...)`, or non-empty-message assertions instead of complete ordered records and exact envelopes. `SA121-QG-001` (**high**, validation) remains open because `make quality` exits 2 on five pre-existing `v87` runtime regressions outside Track 1: `module_commands.py` 1642 > 1608 lines, `manifest/loader.py` 624 > 562 lines, unbaselined `git_utils.py` at 509 lines, `_perform_module_embed` CC 22 > 20, and `_update_single_module` CC 18 > 17. `SA121-REV-META-001` (**low**, advisory) asks future evidence to distinguish commands run separately after an `&&` chain stops.
  - **Decisions needed:** Before clean continuation, a maintainer must choose whether to authorize the minimal parser correction or amend the strict-key policy, and the owning runtime track must remediate or formally disposition the five quality regressions. The maintainer explicitly chose to preserve these unresolved findings and merge this checkpoint; that instruction is **recorded partial delivery only**, not a waiver, independent review approval, release-gate approval, or permission to check SA121.
...
SA121 (baseline monotonicity)     SA115 (e2e xdist; deps: none)  SA117a (exact lockstep) ◄─ next
  (recorded partial; blocked)       │  (validation AUTHORIZED)     ▼
...
SA121 and SA122a both have `deps: none`, but SA121 is now a recorded partial checkpoint with blocking parser, assertion, and quality findings. SA122a is Track 1's next clean runnable ticket; only SA122b carries an ordering bound.
...
- **Track 1 has one clean runnable ticket and one blocked checkpoint.** SA121 and SA122a share no files (SA121's preserved continuation is tests/documentation over the focused quality suite and `quality_tools.md`; SA122a is a new registry + checker), but SA121 now waits on the parser-contract and quality dispositions recorded above. SA122a is therefore next. A fourth worktree would not accelerate the blocked decision, while Track 2 remains closed and Track 3 remains the critical path. Only `decisions.md` is conditionally shared by future SA121 policy work and SA122a, and that surface is covered by the Merge procedure.

- **Track 1 remains off the critical path.** SA121's recorded partial continuation is blocked from clean closure, while SA122a is purely additive (new registry + checker) and independently unblocked. None of SA122a's files appear in any SA112 or SA117 child allowlist, and it needs no PostgreSQL or Docker, so it can run fully concurrent with Track 3. SA122b is the single exception and is bounded below.
...
- **Track 2 — VALIDATION AUTHORIZED AND RUNNABLE; merge queued behind SA112; CLOSED TO NEW WORK (off the critical path).** SA115 is committed (`5193f198`) and reconciled with current `v87` (`5b5de830`) as of 2026-07-25 — `SA115-DEC-001` steps 1–2 are done, and the `scripts/test_e2e.sh` drift (memory guard, heartbeat, provenance, swap-veto correction) is resolved with post-merge focus checks green. The branch is clean and current, and **`SA115-DEC-001` step 3 heavy validation was authorized on 2026-07-27**, together with implementing the ratified `SA115-DEC-002` guard clamp. Validation was human-authorization-gated, never SA112-gated; only the *merge* is order-gated behind SA112. **Next action:** run the serial-baseline vs. parallel comparison, `make ci-e2e`, and local `check_ci_locally.sh --e2e`, scheduled around Track 3's exclusive-infra legs. Legacy compatibility finding stands: the SA105 dormant-file guarantee covers only fresh current-theme recipients; legacy pre-SA105 recipients get no retroactive dormant guarantee, and the shipped continuation adopts only missing forms/social surfaces (no blog/crm/listings backfill). **Closed to new tickets** until SA115 merges — `wt-track2` carries unmerged merge-gated commits (`5193f198`, `5b5de830`), so any ticket homed here would drag SA115 onto `v87` early (see the Parallelism section).
- **Track 1 — SA121 RECORDED PARTIAL; SA122a RUNNABLE (off the critical path).** SA114/SA116/SA120 are closed; the track carries **SA121**, **SA122a**, and **SA122b**. SA121's authorized tests/documentation continuation is merged only as a blocked checkpoint: CR-003, CR-006, and QG-001 remain blocking, and clean continuation needs the parser-contract and quality-regression dispositions above. **Next action:** SA122a. It needs no PostgreSQL/Docker and may run concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e. `SA122-DEC-001` already resolves SA122a's publish-context disposition.
- **Track 3 — SA117 RESIZED AND UNBLOCKED AS FOUR CHILDREN; SA117a IS RUNNABLE; NO OPEN DECISIONS (on the critical path).** The executable candidate at `43d9b8fc` is merged to `v87` by explicit maintainer instruction as a blocked preservation checkpoint, not as review approval. It was Tier 3 (77 files / 7,424 lines / three objectives) and stalled two plan-review cycles because every pass had to hold all three objectives at once; `SA117-DEC-003` (2026-07-27) splits it into **SA117a, SA117b, SA117c, SA117e**, each a Tier 1–2 correction-and-review handoff over the merged candidate with its own scoped plan review, and `SA117-DEC-004` deferred the third objective (scope meta-tooling) to v88 as SA124. **Next action: SA117a** (exact lockstep comparison, resolving `SA117-CR-001`) — it needs its own scoped plan-review `STATUS: ok` before source edits, but no longer needs a maintainer decision to start, because the working set now fits a single review pass. Do not publish splits (SA117e), start SA112, or treat the candidate as release-ready until SA117e's full-scope review returns `STATUS: ok`. SA96-PUBLISH (human-only) still holds until SA112f closes the umbrella. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Net.** One critical path — **SA117a → SA117b → SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 3's next action is **SA117a**; the rest of the SA117 chain follows, then SA112a, and no SA112 umbrella implementation is authorized. Track 2's SA115 is implementation-ready and **now authorized for heavy validation** (`SA115-DEC-001` step 3 granted 2026-07-27); it must yield exclusive Docker/PostgreSQL to Track 3, and its merge is still ordered after SA112. Track 1 preserves **SA121** as recorded partial delivery with three blocking findings and moves its clean lane to independently unblocked **SA122a**; **SA122b** still merges after SA112e. The 2026-07-27 rebalancing result is unchanged: Track 2 cannot accept SA122a while its branch carries merge-gated SA115 commits.

**Standing decisions.** `SA117-DEC-002` (2026-07-26) — **SA117 runs first on Track 3, before SA112a**, and is a hard prerequisite for SA112b onward; rationale in the SA117 section. `SA117-DEC-003` (2026-07-27) — **SA117 was Tier 3 and is split into the serial children SA117a, SA117b, SA117c, SA117e**; the parent is umbrella-acceptance only and must not be executed as one handoff. `SA117-DEC-004` (2026-07-27) — **the SA117 scope meta-tooling is deferred to v88 as SA124**; the merged code stays in place, unadvertised, and is not a v87 gate. `SA121-DEC-001` (2026-07-27) — SA121's focused **tests/documentation-only** continuation is **authorized**, scoped to `SA121-CR-003`/`-005`/`-006` and required to preserve the reviewed runtime delta. `SA115-DEC-001` step 3 — **AUTHORIZED** (2026-07-27, reversing the 2026-07-26 hold), with Track 3 holding infra priority; `SA115-DEC-002` (guard clamps workers) — **ratified**, implemented as part of that now-authorized validation phase. `SA122-DEC-001` (2026-07-27) — **publish is a full-coverage gate context**; see [decisions.md §publish-path-gate-coverage](./decisions.md#publish-path-gate-coverage). Each is stated in full in its own section above; no roadmap surface may contradict them. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
