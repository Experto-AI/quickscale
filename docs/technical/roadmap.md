...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA139 ◐ (recorded partial;             SA115a ◐ (local delta green;   SA136d ◐ (partial seal tooling;
  │  validation-blocked)                  blocked by SA139-QG-001)
  │  ── unblocks ──────────────────►  │  no shared file, no gate     │  review-blocked)
...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **yes** — start from a fresh focused plan carrying F-001–F-006; the prior plan is stale-unverified for reuse | **no** — six in-scope review blockers remain, and the unchanged `make quality` baseline needs a separately owned repair or an explicit reviewed acceptance rule | **yes, partial only** — no merge-order gate, but retained tooling is not release-authorized | no — review-blocked | ✅ **yes** |
| **Track 1** — SA139 (head) | **yes** — reviewed root-anchor implementation is in place; the remaining validation blocker is recorded as `SA139-QG-001` | **no** — parity and both umbrella commands remain blocked before E2E by `SA139-QG-001` | **yes** — no merge-order gate; recorded-partial preservation writes no shared executable file | no — validation-blocked | no — off-path, but it is the only ticket that unblocks another track |
...
**Truly green today: none.** **SA136d remains on the critical path but is review-blocked**; its retained partial tooling is not a completion or release claim. SA139 has reviewed executable progress but remains validation-blocked by `SA139-QG-001`; it is off-path and remains the board's one unblocking move, converting Track 2's SA115a from "cannot finish" to finishable only after the owning repair and exact reruns. SA122b-5's merge bound is a **hard upstream dependency** only SA112e can clear; no decision of the maintainer's affects it.
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed`
    ...
    **Seal-atomicity requirement re-scoped (2026-08-07 maintainer decision; see [decision 2](#track-topology--settled)).** Server-transactional atomicity is **withdrawn as a requirement**. The seal is enforced client-side as check-then-act with a fail-closed compensating verification, and the residual window is documented rather than engineered away. This unblocks the ticket on stock Git against GitHub.com; no platform migration is required.
    ...
    - **Prior plan-finding ledger:** the carried planning findings from the pre-implementation attempts were superseded by the fresh plan and `STATUS: ok` plan review completed on 2026-08-08; the retired server-transactional `F-003` remains retired by the re-scope. The `F-001`–`F-006` identifiers in the recorded-partial checkpoint below are a later **executable change-review ledger**, not those historical plan findings.
    - **Recorded-partial checkpoint (2026-08-08; task remains unchecked).**
      - **Done / partial improvements:** Track 3 retained the test-first seal mechanics, CLI/status/batch wiring, and `seal-module` / `seal-modules` / `seal-status` Make surfaces in commits `1e9e909c`, `26d5fdd6`, `ed503c1c`, and `f6157f4c`. Hermetic focused validation reached 25 passed; Ruff, format, lint, typecheck, and full tests passed (Core 2709, CLI 2060, integration 2462; coverage 90.36% / 90.94% / 94.40%). No real remote ref, GitHub publication, or PyPI publication occurred. Full-scope executable review returned `STATUS: partial`; therefore these commands are **not release-authorized** and this is not a completion claim.
      - **Pending-Blocking:** `F-001` (**high**, correctness) — local tag lookup must distinguish absence and peel lightweight/annotated tags to commits through the publication runner. `F-002` (**high**, resilience) — annotated-tag creation must always supply a non-interactive message. `F-003` (**high**, correctness) — equal-tree reuse must seal the prior commit while branch race checks remain bound to the current branch commit. `F-004` (**medium**, resilience) — local cleanup must be pre-armed across creation, push, remote-probe, conflict, ambiguity, and cleanup-error precedence. `F-005` (**medium**, completeness) — every pair of status/seal/seal-all/publish-outdated actions must fail before Git bootstrap. `F-006` (**medium**, test gap) — hermetic real-Git fixtures must exercise production helpers and all success/failure/race/Make-dispatch paths instead of mocking them away. `SA136D-QG-001` (**medium**, validation) — `make quality` exits 2 on unchanged `apply_command.py::_execute_apply_steps_locked` complexity 56 versus allowed 55, outside SA136d's allowlist.
      - **Decisions needed:** before the next attempt, decide whether `SA136D-QG-001` is repaired under separately owned scope or whether SA136d may use the inherited “no worse than found” rule as an explicit reviewed acceptance criterion. No seal or publication authorization is implied by that decision.
