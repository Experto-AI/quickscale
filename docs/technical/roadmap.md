...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **`SA117-QG-REMED` — quality-ceiling remediation. `Track 3` · `deps: none` (SA125 ✓ closed).** `SA121-QG-001` recorded the exceeded ceilings introduced by `43d9b8fc`. After `SA125-DEC-001` retired the file line-ceiling gate, **two complexity findings remain**, each already inside its child's declared allowlist:
  ...
  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: none (SA125 ✓ closed) · blocks SA112b` — **critical path**
    ...
    **Recorded partial checkpoints:** 2026-07-27 change-review cap (delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)); 2026-07-28 plan-review cap (no source delta; task remains open).
    - **Done:** clean Track 3 sync and environment preflight, scoped discovery, and plan findings `SA117A-CONT-PR-007` / `SA117A-CONT-PR-009` resolved; no new implementation or completion evidence landed. **Pending / blocking:** `SA117A-CR-002` (**medium**, test gap), `_perform_module_embed` CC 22 > 20, and `SA117A-CONT-PR-008` (**medium**, plan completeness) — Phase 3/5 synchronization classification remains outside its rollback transaction and can strand a completed merge.
    - **Decisions needed:** none. **Next action:** start from a fresh `v87` sync, correct only `SA117A-CONT-PR-008`, obtain scoped plan-review `STATUS: ok`, then implement the missing dependency-sync spy and CC reduction, run the focused/full quality chain, and obtain full-scope change-review `STATUS: ok`; only then may SA117a be checked and Track 3 advance to SA117b.
