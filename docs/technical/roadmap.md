...
SA128d ✓ → SA122b-1 → … → -5           SA115 (e2e xdist; deps: none) SA117e-3 ⛔ → -4 → -5
  ...
  ▼  F-001 resolved; checker authoritative │ validation AUTHORIZED      │  ORDER-001 decision needed
                                      │  cannot finish → SA112d     │  correction + Phase 5 green
                                      │  cannot merge  → SA112e     │  public analytics blocks harness
SA122b-1 → -2 → -3 → -4 → -5          │                             │
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **no** — **maintainer decision** `SA117E3-ORDER-001`; the public analytics split still reports `0.80.0` | **no** — the reviewed installed-wheel harness fails before Phase 6 against that unchanged public input | **yes** — the recorded-partial checkpoint has no merge-order gate | no | ✅ **yes** — blocked on the critical path |
...
**Track 3 is blocked on the critical path.** The `SA117E3-EXEC-001` round landed and independently approved the narrow correction and cleared the revised Phase 5 negative control, but the exact reviewed installed-wheel harness then failed against the unchanged public analytics split: SHA parity was 12/12 while analytics still reported version `0.80.0` against core/source `0.87.0`. `SA117E3-ORDER-001` must reconcile that harness-before-push / push-in-SA117e-4 ordering before another attempt. **Track 1 is now truly green at SA122b-1** after SA128d's accepted full-scope review; it is off the critical path and can proceed without competing for Track 3's PG/Docker capacity. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear. Track 3 and Track 1 can merge; Track 2 remains merge-gated. SA117e-4 still carries `SA117E1-REV-001` and `SA117E1-REV-002`; neither is bypassed or resolved by the new ordering decision.
...
**Status:** on the critical path and **blocked at SA117e-3** by `SA117E3-ORDER-001`; see the [readiness table](#dependency--parallelization-overview) and that child's current checkpoint. The three-file correction and revised Phase 5 are green, but the reviewed installed-wheel harness proved that the unchanged public analytics split remains at `0.80.0`; Phase 6 has not run. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Current checkpoint (2026-08-05; recorded partial delivery; task unchecked; correction commit `ffd97e66`; no tag, push, or publication).**
      - **Done:** the exact three-file correction was validated and received full-scope review `STATUS: ok`: the restore assertion now derives `PORT` from Django settings, and `SA117E3-BLK-001` plus `SA117E3-EVID-001` are resolved. Revised Phase 5 passed at `.quickscale/sa117e3/evidence/phase5-20260805T130413Z-1182566`: stage 10 passed, stage 11 emitted the literal `PostgreSQL Not Available` banner and exited 1, the frozen host PostgreSQL identity/configuration/readiness was restored, and scratch/FIFO/port residue was absent. The current-tip harness was rematerialized for `ffd97e66`, independently approved at SHA-256 `6757f901ba765af6cc7b398596fb9bb4356ed4120255b879b6e35717fb81cff3`, and executed under the approved wrapper; plan and immutable public split-SHA parity passed 12/12, and host/resource cleanup passed.
      - **Pending-Blocking:** `SA117E3-PUBLIC-ANALYTICS-001` (**high**, external contract) — installed `apply` stopped before internal-DB and all-module acceptance because public `splits/analytics-module` at the frozen approved SHA reports manifest version `0.80.0`, while core and the source analytics manifest require `0.87.0`. Evidence: `.quickscale/sa117e3/runs/sa117e-harness.FNO2Eq/evidence/` and wrapper restoration evidence `.quickscale/sa117e3/evidence/harness-wrapper.20260805T132328Z.1206300/`. The harness did not produce a pass artifact, and Phase 6 `make ci-e2e` did not run. No completion, release-readiness, tag, split push, installed public apply, or publication is claimed.
      - **Decisions needed:** `SA117E3-ORDER-001` — reconcile the circular acceptance order: SA117e-3 currently requires a successful public-split harness before SA117e-4, while SA117e-4 owns the human-gated split refresh that can make the public manifests match `0.87.0`. Either authorize a reviewed, human-gated split refresh before SA117e-3 closes (without bypassing `SA117E1-REV-001`, `SA117E1-REV-002`, or the twelve-row confirmation), or move the public-harness success gate after SA117e-4 and revise the dependency contract explicitly. **This checkpoint authorizes neither option and no push.** `SA117E3-EXEC-001` is consumed.
      ...
      **Revised Phase 5 (plan review `STATUS: ok`; completed 2026-08-05).** Supersedes the superseded plan's Phase 5 only; Phases 1–4 and 6–9 stand. The accepted evidence is recorded in the current checkpoint above.
      ...
      **Phase 6 remains outstanding independently:** `make ci-e2e` has not run at current tip `ffd97e66`; Phase 5 is accepted, but the failed public-split harness and `SA117E3-ORDER-001` now block advancement. A later authorized run still needs exclusive Docker/PG capacity per the [infra-serialization rule](#dependency--parallelization-overview).
    ...
    **Carried plan-review ledger.** From the superseded monolithic execution plan: `F-001` is resolved at plan-detail level and carried by SA117e-4; `F-002` was resolved at plan-detail level by SA117e-3's scoped plan review (DC-25). The separate `F-003`–`F-010` pre-use ledger and the later correction findings are resolved; `SA117E3-EXEC-001` was granted and consumed. SA117e-3 now waits on `SA117E3-ORDER-001`, with full history in [CHANGELOG.md](../../CHANGELOG.md).
...
**One track-sequencing question is open: `SA117E3-ORDER-001`; no worktree-assignment question is open, and the latest rebalancing review (2026-08-05) applied no move — the sixth consecutive pass to reach that conclusion.** Every open v87 ticket carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), and SA112a — the one node that was worth parallelizing onto Track 1 — has closed and merged. The remaining off-path work (SA122b-1…5) is a single serial chain, now unblocked at its head; moving any of it onto Track 3 would push filler ahead of the critical path. `SA117E3-ORDER-001` changes the order of children already on Track 3, not their worktree placement.
...
**Open decisions — three, all the maintainer's; one blocks the critical path.** `SA117E3-ORDER-001` must reconcile SA117e-3's public-harness acceptance with SA117e-4's ownership of the human-gated split refresh; `SA117E3-EXEC-001` was granted and consumed without resolving that ordering contradiction. The other two are owned by **SA117e-4**: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. SA128d is complete, so `SA128D-EXEC-001` is resolved and no longer counted. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
