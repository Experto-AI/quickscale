...
**Status:** on the critical path and **blocked at SA117e-4**. SA112b's provisioning precondition is satisfied by SA112a's merge; its split-publication precondition waits on SA117e-4. `SA117E1-REV-001`, `SA117E1-REV-002`, and `SA117E3-PUBLIC-ANALYTICS-001` are owned by SA117e-4; the first two require maintainer decisions before implementation, and the twelve-row confirmation is obtained fresh at push time.
...
**Handoff entry point.** SA117e-4 carries a [recommended disposition and four-step sequencing](#recommended-disposition--proposal-not-ratified) for its two open decisions. It is a **proposal, not a ratification**: Step 1 (read-only lease research) may begin immediately, Steps 2–4 require written maintainer ratification of both decisions in `CHANGELOG.md` first.
    ...
    **SA117e-1, SA117e-2, and SA117e-3 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-4` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-4 — Tag, human-confirmed split push, and post-push parity.** `Tier 2 · deps: SA117e-3` · **HUMAN-GATED — outward-facing, irreversible** — satisfies SA112b's precondition
      ...
      *(why →* the only irreversible, outward-facing step on the umbrella, isolated so it carries exactly one gate and no unrelated work*)*
      ...
      #### Recommended disposition — **PROPOSAL, not ratified**

      > A handoff may execute **Step 1 only** without further authorization: it is read-only research that produces the evidence decision (b) needs, and it commits the board to nothing. **Steps 2–4 require the maintainer to have ratified both decisions in writing first** — a handoff that finds no such ratification in `CHANGELOG.md` stops after Step 1 and reports.

      **Recommendation on (b) — `SA117E1-REV-001`: ratify an explicit-absence contract; do not accept bare `--force-with-lease`.** Bare `--force-with-lease` derives its expected value from the *local tracking ref*, so it compares the remote against this machine's cached idea of the remote. A branch that genuinely does not exist and a branch this clone has simply never heard of are indistinguishable to it — the check passes in both cases, and in one of them a real branch is clobbered. The failure is silent: no error, no warning, surfacing later as a broken `apply` for users. Converting silent failures into loud ones is precisely what SA117 exists to do, so accepting a check that passes when it should not is the one shape this ticket cannot take.

      **Recommendation on (a) — `SA117E1-REV-002`: bind and consume; do not defer to SA124.** The verifier and the publisher are joined only by maintainer attention today, so "I verified" and "I pushed" are not provably about the same twelve branches at the same instant — remote state moves, and attention is interrupted. Deferring is defensible on its own terms, but **not into SA124**: SA124 is by definition the home for meta-tooling that *gates nothing*, and moving a gate that is still relied upon into it silently redefines that ticket. If the maintainer does prefer to defer, the honest form is an explicit de-advertisement recorded in `decisions.md` stating that the twelve-row human confirmation is the sole protection — not a quiet relocation.

      **Compounding note the handoff must not lose.** Taking the weak option on *both* leaves the release's one irreversible step protected by exactly one control: a human reading twelve rows correctly, once. That may be an acceptable posture for a solo maintainer, but it must be chosen explicitly, because deciding the two separately hides that they collapse onto the same single point of failure.

      **Handoff sequencing.**
      1. **Ratify the lease contract (read-only; no ratification required to begin).** Establish whether `--force-with-lease=<ref>:` with an empty expected value is the supported spelling for "must not exist" on the pinned Git version, and prove the behaviour against a bare remote, a concurrent-writer race, and a stale tracking ref. If Git cannot express it cleanly, the fallback is an explicit `ls-remote` absence assertion inside the same operation — less atomic, but honest and still strictly stronger than bare `--force-with-lease`. Produces evidence only; **no source delta, no push.**
      2. **Implement `SA117E1-REV-001` and `SA117E1-REV-002` together** — one coherent review unit, not two. They touch the same push path, and splitting them costs a second full-scope review over overlapping files for no isolation benefit. The interlock is small: digest the verified twelve-row matrix, pass that digest to the push operation, have the push recompute remote state and refuse on mismatch, and burn the digest on use.
      3. **One full-scope independent review** over that combined slice.
      4. **Then** the fresh twelve-row human confirmation, then the push, then the post-push obligations above.
...
**Open decisions — two, both the maintainer's, and both block Track 3 at SA117e-4 today.** They are the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. They must be answered before the current critical-path head implements; a **recommended disposition for both — bind-and-consume, and ratify explicit absence — is recorded on SA117e-4 as an unratified proposal**, whose read-only first step is startable today. `SA117E3-ORDER-001` is resolved (option b), `SA117E3-EXEC-001` was granted and consumed, and SA117e-3 is closed; all are recorded in [CHANGELOG.md](../../CHANGELOG.md). All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
