...
- [x] Move `config_schema.py`, `state_schema.py`, and `delta.py` from `quickscale_cli` into `quickscale_core` (Finding 6)
- [x] Extract `beta_migration.py` from the end-user CLI package to `scripts/` or a dev-only package (Finding 6)
- [x] Unify `.quickscale/state.yml` and `.quickscale/config.yml` under one schema/owner (Finding 3)
- [x] Add drift detection to apply: hash managed files at apply time, warn on mismatch at next run (Finding 3)
- [x] Surface module version drift as a warning at `apply`/`update` time (Finding 2, partial)
- [x] Fix `scripts/publish.sh` to use Poetry or uv workspace mechanisms instead of sed-rewriting `pyproject.toml` at publish time (Finding 9)
- [x] Derive DR env-var portability from module contract declarations instead of name-pattern heuristics (Finding 17)
- [x] Conditionally include per-module React components and TypeScript interface entries in generated apps based on selected modules (Finding 15)
- [x] Consolidate Django templates duplicated at identical relative paths across `showcase_html` and `showcase_react` themes (Finding 15)
- [x] Wire `selected_modules` through the real `quickscale plan` / `quickscale apply` generation path so `ProjectGenerator(selected_modules=...)` is fed from the declared module set in `quickscale.yml` instead of remaining an unused generator capability; verify the initial scaffold only emits React interface keys, imports, routes, nav items, and optional page files for selected modules while preserving current behavior for legacy/default generation paths
- [x] Normalize corrupted-config error handling between `.quickscale/state.yml` and `.quickscale/config.yml`: wrap `yaml.YAMLError` from `quickscale_core/config/module_config.py:load_config()` in the same style of domain-specific exception used by state loading (`StateError` or a unified project-state error) so apply/status/update surface consistent operator-facing diagnostics when the config file is malformed
- [x] Harden `quickscale_devtools/src/quickscale_devtools/beta_migration.py` TOML rewriting helpers, especially `_replace_toml_section`, by adding coverage for nested tables / unusual section layouts and either tightening the current text-replacement safety checks or replacing that path with a safer structured writer strategy if the current approach proves fragile
- [x] Document the maintainer-only publishing contract for `quickscale_devtools`: make it explicit in `scripts/publish.sh`, `scripts/prepare_publish.py`, and/or package docs that `quickscale-devtools` is intentionally excluded from the public publish workflow, including what maintainers should do if they ever need to distribute it separately
- [x] Add an inline explanation in `quickscale_devtools/pyproject.toml` for the current packaging pattern where Poetry owns the dependency graph while the PEP 621 `[project]` table keeps dynamic metadata; include the intended maintenance rule so future edits do not accidentally diverge the two metadata surfaces
- [x] Tighten `quickscale_devtools/src/quickscale_devtools/beta_migration.py:_locate_toml_section_bounds` so unrelated top-level `[[...]]` array-of-tables headers terminate the current section instead of being silently absorbed into it; preserve the current behavior for true child tables of the active section, and add regression coverage showing the distinction with realistic `pyproject.toml` layouts that place `[[tool.poetry.source]]` or similar sibling array blocks between `[tool.poetry.dependencies]` and later sections
- [x] Harden `quickscale_devtools/src/quickscale_devtools/beta_migration.py:_replace_toml_section` against silent loss of nested child tables / array-of-tables that belong under the replaced section (for example Poetry source declarations or other advanced dependency metadata): either preserve those blocks explicitly during replacement or document and guard-fail on unsupported layouts, and add tests proving the chosen behavior is deliberate rather than accidental
- [x] Clarify the best-effort managed-file-hash capture contract in `quickscale_cli/src/quickscale_cli/commands/apply_command.py`: either consume the boolean result from `_capture_managed_file_hashes_after_apply()` to emit a precise warning when the hash ledger was not recorded, or simplify the helper to return `None` if no caller action is intended; keep the current non-fatal apply behavior, but remove the present ambiguity between "result matters" and "result is ignored"
- [x] Expand beta-migration integration coverage beyond the current mocked success path in `quickscale_cli/tests/test_beta_migration.py`: add a higher-fidelity test (or clearly scoped E2E follow-up if full integration is too heavy for this phase) that exercises the in-place beta migration flow against a real generated project/apply path closely enough to validate the TOML rewrite helpers and managed apply handoff together, not just through monkeypatched stubs
- [x] Document and/or harden recipient-only non-path dependency handling in `quickscale_devtools/src/quickscale_devtools/beta_migration.py:_execute_merge_pyproject_and_frontend_package`: make the current donor-authoritative merge contract explicit in the function docstring and roadmap context, and decide whether the maintainer workflow should keep silently dropping recipient-only non-path dependencies or instead surface them in the report/checkpoint output for manual review before continuation. If behavior remains donor-authoritative, add targeted tests proving the intended drop behavior is deliberate and visible rather than accidental.
...
- [ ] **Wiring-derivation migration — re-scoped from the single "delete `module_wiring_specs.py`" item (Finding 1).** Context for the hand-off: the earlier Finding-1 slices migrated only per-module *option resolution* (defaults, normalization, legacy-key handling, validation) into the `*_manifest.py` adapters. The actual *wiring derivation* in `quickscale_cli/src/quickscale_cli/commands/module_wiring_specs.py` — `INSTALLED_APPS`/dependency-app ordering, `MIDDLEWARE`, Django-settings projection (including computed/conditional settings), URL includes, and managed-file code generation — is still hand-coded there for all 11 modules. `module.yml` cannot yet express this, and `quickscale_core/src/quickscale_core/manifest/derivation.py` ships the `DerivedSetting`/`OptionDerivation` dataclasses but is explicitly *not* wired into the loader or wiring generation (its docstring defers YAML/loader wiring to "a later roadmap phase"). The migrated modules still call `resolve_*_module_options(...)` for options only, then build the `ModuleWiringSpec` imperatively. Deleting the file now would break Django wiring for every module in every generated project. So the file must be migrated one module per slice (mirroring the option-resolution migration), then deleted. The single deletion checkbox is replaced by the capability + per-module slices below.

  **Wiring-expression capability (prerequisite slices):**
  - [ ] Extend the manifest-driven path so `module.yml` can express module *wiring* (not just option resolution): `django_apps` with dependency-app ordering, `middleware`, computed/conditional Django-settings projection (conditional gating of settings on option values, plus composite/dict settings like `STORAGES` — beyond today's flat `django_setting` pass-through), URL includes (including pre-home vs post-home placement), and managed-file code generation. Wire `derivation.py`'s `DerivedSetting`/`OptionDerivation` into the loader/resolver so the new manifest sections round-trip through `yaml.safe_load`. Conditional/computed cases to cover: auth `ACCOUNT_LOGIN_METHODS`/`ACCOUNT_SIGNUP_FIELDS` derived from `authentication_method`; storage `STORAGES`/`AWS_*` block only when `backend` is `s3`/`r2`; notifications conditional `anymail` app + `EMAIL_BACKEND`/`DEFAULT_FROM_EMAIL`/`SERVER_EMAIL`; analytics empty spec when `enabled` is false; orgs `pre_home_url_includes` vs `url_includes` by `mode` (solo/saas); social managed Python files (`quickscale_managed/social_*.py`).
  - [ ] Add a manifest-driven wiring builder API in `quickscale_core` that produces a `ModuleWiringSpec` from a module's manifest, runnable *alongside* the legacy `module_wiring_specs.py` builders without deleting them yet (parallel path, per the established migration pattern).
  - [ ] Add `*_manifest.py` adapters for the four modules that never received one (`blog`, `listings`, `orgs`, `storage`) so every module has a manifest adapter before its wiring slice.

  **Per-module wiring slices** (one module per slice; for each: move that module's `_<name>_wiring` derivation onto the manifest-driven builder, delete its `_<name>_wiring` function and registry entry from `module_wiring_specs.py`, and add parity tests proving the manifest-driven `ModuleWiringSpec` matches the prior hand-coded output for representative option sets). Ordered low-coupling first:
  - [ ] `analytics` wiring slice — pilot; app + settings, conditional empty spec when disabled (Finding 1)
  - [ ] `backups` wiring slice — app + settings (Finding 1)
  - [ ] `billing` wiring slice — apps + settings + URL include (Finding 1)
  - [ ] `crm` wiring slice — apps + settings + URL include (Finding 1)
  - [ ] `blog` wiring slice — apps + settings + `markdownx` + URL includes; requires the new `blog` adapter (Finding 1)
  - [ ] `listings` wiring slice — apps + settings + URL includes; requires the new `listings` adapter (Finding 1)
  - [ ] `forms` wiring slice — apps + settings + URL include (Finding 1)
  - [ ] `notifications` wiring slice — conditional `anymail` app + `EMAIL_BACKEND` + URL include (Finding 1)
  - [ ] `auth` wiring slice — computed `login_methods`/`signup_fields`, static settings, middleware, two URL includes (high coupling) (Finding 1)
  - [ ] `orgs` wiring slice — mode-based pre-home vs post-home URL placement + middleware; requires the new `orgs` adapter (high coupling) (Finding 1)
  - [ ] `storage` wiring slice — backend-conditional `STORAGES`/`AWS_*` block; requires the new `storage` adapter (high coupling) (Finding 1)
  - [ ] `social` wiring slice — managed-file code generation + URL include (high coupling) (Finding 1)

  **Capstone:**
  - [ ] Delete `quickscale_cli/src/quickscale_cli/commands/module_wiring_specs.py` and switch `quickscale_cli/src/quickscale_cli/utils/module_wiring_manager.py` off `build_module_wiring_specs` to the manifest-driven wiring builder, once every per-module wiring slice above has landed with parity coverage (Finding 1)
...
- [ ] Define the DR boundary contract that separates embeddable Django surfaces from the centrally-owned backup/restore engine before moving implementation code (Finding 5)
- [ ] Extract snapshot/archive primitives from the backups module into a CLI/core-owned engine library while preserving current behavior (Finding 5)
- [ ] Extract restore/orchestration flow, verification, and rollback-pin handling into the centrally-owned engine layer (Finding 5)
- [ ] Replace the hidden CLI↔module management-command/env-var protocol with a smaller explicit internal engine boundary or adapter layer (Finding 5)
- [ ] Shrink the embeddable backups module to thin Django-facing surfaces only (models, policy/admin, scheduler-facing hooks) after the engine extraction is complete (Finding 5)
- [ ] Document the migration/compatibility contract for existing generated projects adopting the split DR architecture (Finding 5)
- [ ] Define the authoritative module provenance fields to persist in state (embedded version, commit SHA, and any required release identifier) before changing subtree workflows (Finding 2)
- [ ] Update subtree release tooling so module split branches are cut only from tagged/versioned source states rather than arbitrary commits (Finding 2)
- [ ] Persist module commit SHA + version during embed/update/apply flows so project state records what was actually embedded (Finding 2)
- [ ] Validate embedded module provenance during `apply`/`update` and surface mismatches between recorded version, commit SHA, and release source (Finding 2)
- [ ] Add clear operator-facing diagnostics for untagged split provenance or version/SHA mismatches so module versions become actionable rather than decorative (Finding 2)
- [ ] Inventory which Python/Django/PostgreSQL constraints belong to the generator runtime versus generated-project templates before changing pin ownership (Finding 7)
- [ ] Split configuration ownership so generator runtime pins and generated-project runtime pins are managed independently (Finding 7)
- [ ] Update generation so emitted project templates use generated-project-owned runtime pins instead of inheriting generator package constraints accidentally (Finding 7)
- [ ] Add validation coverage for intentionally diverged generator-vs-generated-project runtime pin sets (Finding 7)
- [ ] Align documentation and operator messaging with the decoupled runtime-pin model once generator and generated-project constraints no longer share one compatibility window (Finding 7)
