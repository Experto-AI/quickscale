...
- [ ] **SA96-GATE — Green-gate join (cross-track).** `Tier 1 · v87 integration · deps: SA96-T1 ✓ + SA96-T2 ✓ + SA93` · *assistant-executable*
  ...
  - **Finding `SA96-GATE-BLK-001` (2026-07-18; pending):** Clean Track 3 was synced to v87 with Python 3.13.14 in the Poetry environment, PostgreSQL 18 roles provisioned, Docker ready, and no `QUARANTINE_TICKETS` entries. The first default `make check` was tool-terminated at 99% after one hour amid concurrent Track 1/2 jobs. Retry `PYTEST_XDIST_WORKERS=4 make check` ran 2,353 core tests passed + 1 skipped but failed the configured coverage floor at 80.39% < 90%. TP2 records the identical serial/xdist core 80.39% as waived/non-blocking, but SA96-GATE explicitly requires `make check` exit 0 and TP work must not weaken coverage thresholds. `make quality`, `make ci`, and `make ci-e2e` were not run because the sequence stopped at the first failed gate. Required resolution: either restore core coverage to policy or reconcile the gate/policy through an explicitly approved owning task; no silent threshold weakening.
