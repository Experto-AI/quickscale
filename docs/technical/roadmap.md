...
- [ ] **SA167d — Complete the CLI wiring-drain acceptance.** `Band B · Tier 2 · W1 · merge #18 · deps: none · blocks SA165`
  ...
  **State (measured 2026-08-27): phases A-D accepted, phase E outstanding.** The retained product
  tip is `1743871f`, carried under the docs-only checkpoint at `467714cb`. The required pre-task
  synchronization merged `v88` at `8a8f364b` into `wt-track1` while preserving that retained
  product delta. A-D acceptance evidence is archived in [CHANGELOG.md](../../CHANGELOG.md). Keep
  this item unchecked: the retained product delta is unmerged and its terminal review produced no
  grade. **This is not a merge-back-only ticket.**
  ...
  **Blocking.** The terminal review was blocked because it did not receive the complete authoritative
  diff from the pre-run base through `1743871f`; therefore the product delta is ungraded. The
  required pre-task synchronization has now merged `v88` at `8a8f364b` and resolved the standing
  roadmap conflict. Close the remaining blocks by accepting E0, completing C1, synchronizing any
  newer `v88` successor before closeout, running the complete final campaign on one unchanged tip,
  and supplying that tip's complete diff to an independent reviewer before merge-back.
