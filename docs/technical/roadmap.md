...
**Critical path:** `SA117e → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path), with **SA112a on Track 1 running in parallel** and joining as a merge-order precondition of SA112b. Track 2 is entirely off it; Track 1 is on it only for SA112a.
...
**Status:** on the critical path but **execution-blocked at plan review** — `SA117E-VAL-001` is resolved (baseline `43d9b8fc` approved 2026-08-01), while `F-002` remains medium/blocking at the two-pass plan-review cap. **Head is SA117e**; ordered command 4 is its restart point only after a fresh plan-review continuation returns `STATUS: ok`. The gate precondition (`make ci-e2e` exit 0) is satisfied. **SA112a now runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not execute SA117e, publish splits, start SA112b, or treat anything as release-ready until the plan gate opens and SA117e closes.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  *(why →* `apply` with any module has zero end-to-end coverage — `test_e2e_development_workflow.py:276` plans with modules skipped — so this skew class has never been exercised*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Tier 2 · deps: none` — critical path; contains a human-only step
    ...
    **Plan-review-cap checkpoint (2026-08-01; recorded partial delivery; task remains unchecked).** **Done:** Track 3 started clean at `074061ca`, fast-forwarded to `v87` `6d0da500`, and discovery rebound the full SA117e continuation to current repository evidence. Two bounded plan-review passes reduced the blocking ledger from high+medium (2) to medium (1). `F-001` is resolved at plan-detail level: every protected split push is bound to the exact SHA/`ABSENT` value frozen in the human-authorized twelve-row Decision Brief; an immediate reread must match byte-for-byte, only the frozen value may be supplied as `EXPECTED_REMOTE_SHA`, and any mismatch stops before mutation and requires a complete rebrief plus fresh authorization. No source file, tag, split ref, package registry, Docker resource, or PostgreSQL service was changed. **Pending-Blocking:** `F-002` (**medium**, resilience) — the host-PostgreSQL lifecycle plan must arm a restoration-obligation marker *before* invoking the stop command, reconcile an in-flight or uncertain stop outcome to the captured original state on every exit path, and prove the stop-effect-before-bookkeeping interruption window restores service readiness. **Decisions needed:** none; the correction is deterministic, but implementation remains forbidden until a fresh plan-review continuation returns `STATUS: ok`. Advisories `CR-SA117E-005` and `CR-SA117E-006` remain open and non-blocking for execution evidence. Full checkpoint history is in [CHANGELOG.md](../../CHANGELOG.md).
...
1. **`SA117E-VAL-001` — RESOLVED, baseline `43d9b8fc` approved.** The baseline decision is cleared, but Track 3's *can execute* remains blocked by `F-002`; ordered command 4 is the restart point only after plan review returns `STATUS: ok`.
...
Track 1 is startable while Track 3 remains plan-review-blocked on `F-002`; no track-sequencing or worktree-assignment decision is open. The remaining human gate on SA117e — public split-push confirmation, and later SA96-PUBLISH — is an **execution-time** gate obtained at the moment of the push, not a standing decision, and does not override the earlier plan-review gate.
