...
───────────────────────────────   ─────────────────────────────────
SA115 (e2e xdist; deps: none)     SA112 ── installed-wheel
  │                                 │        plan→apply→up e2e
  └── merge after SA112 ◄───────────┤
                                     ▼
...
**Parallelism.** The three open tickets occupy two tracks (SA112 → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2), and no rebalancing move is available that would speed the critical path:
...
- **SA115 must stay on Track 2.** It edits `scripts/test_e2e.sh` (shared with SA112, hence `merge after SA112`). Moving it to Track 3 would collide with SA112 on that file. Track 2 sitting idle is the correct outcome, not waste. (Its infra contention is now with SA112 only — SA114's heavy legs are done.)
...
**Net.** One critical path — **SA112 → SA96-PUBLISH (human)** — with Track 3 clean to continue behind a single plan-review gate. Track 2 is implementation-ready but validation-paused pending explicit future maintainer approval (SA115-DEC-001 step 3: **Keep unauthorized**), merge after SA112.
