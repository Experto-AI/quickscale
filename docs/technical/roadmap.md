...
> **Track status (2026-07-07):** Track 1 — **2 open items, clear to continue** (SA41 ready now, SA47 soft-sequenced after; SA35/SA39/SA45 complete). Track 2 — **0 open items, complete** (SA21.2 complete; SA37/SA38/SA40 complete). Track 3 — **3 open items, clear to continue** (SA42, SA44 ready now; SA46's CR-SA46-001 decision is resolved (Option B) — the literal-only-evaluator follow-up can proceed; SA34/SA36 complete).
...
SA41 (deps: none)                         SA21.2 (deps: SA21.1 complete, SA36 complete) — complete SA46 (deps: none) — partial, CR-SA46-001 decision resolved (Option B)
...
- [x] **SA21.2 — Wire forms/blog throttles and IP logging to the new canonical-IP and cache infrastructure.** `Tier 2 · Track 2 · deps: SA21.1 (complete), SA36 (complete)`
...
> **SA21.2 — complete.** Added a shared importable `get_client_ip(request)` utility to `quickscale_modules_orgs.current_org` that mirrors the generated-settings helper (SA21.1/SA36) — reads `USE_X_FORWARDED_FOR`/`TRUSTED_PROXY_COUNT` from Django settings, resolves the canonical client IP from `X-Forwarded-For` when configured, and falls back to `REMOTE_ADDR` otherwise. Wired both residual `REMOTE_ADDR` callsites in `FormSubmitAPIView.create()` (`forms/views.py:231,257`) and `_get_blog_api_rate_limit_ident()` (`blog/views.py:320-326`) to the shared helper. `FormSubmitThrottle.get_ident()` now delegates to `get_client_ip()` for fail-closed IP resolution, eliminating the short‑XFF‑chain divergence with DRF's built‑in `get_ident()` (CR‑SA21.2‑001). Form submission tests verify `ip_address` uses X-Forwarded-For when configured (three new tests in `test_views.py`). Blog rate-limit tests verify X-Forwarded-For-based buckets when `USE_X_FORWARDED_FOR` is enabled (one new test in `test_api.py`). Forms throttle regression tests cover short-chain fail-closed, sufficient-chain resolution, and `USE_X_FORWARDED_FOR=False` ignoring XFF (three new tests in `test_throttles.py`). The shared `get_client_ip` itself is covered by 8 focused unit tests in `test_public_context.py` (default fallback, XFF disabled, proxy-count zero, single/two-proxy resolution, empty XFF, short chain fail-closed, missing META). Shared cache backend was already proved in SA21.1 (generated settings ship RedisCache/DatabaseCache; module tests use LocMemCache which is sufficient for per-process isolation). Closes SA21.2. Full detail in [CHANGELOG.md](../../CHANGELOG.md).
...
- [ ] **SA46 — Add a CI gate pairing every `csrf_exempt` callsite with a signature check or `_enforce_csrf`.** `Tier 1 · Track 3 · deps: none — partial, CR-SA46-001 decision resolved, follow-up ready to implement`
  ...
  *Acceptance:* the gate fails CI on a `csrf_exempt` view missing `_enforce_csrf`/a signature check; existing billing, notifications, and blog callsites pass; wired into `ci.yml` alongside the other AST gates. Full consolidation onto one base view (arch-audit's fold/migration options) remains out of scope.
  > **CR-SA46-001 (high, security-boundary):** The reachability evaluator uses ad-hoc truthiness checks that miss some compile-time literal branch cases (non-zero float, empty bytes, literal tuple/list/dict). Unreachable helper code can still falsely satisfy the gate through those missed branches.
  >
  > **Decision (2026-07-07, resolved):** Option **(B)** — replace the ad-hoc truthiness checks with a safe literal-only evaluator, plus focused regressions for the remaining literal-branch cases (non-zero float, empty bytes, literal tuple/list/dict), then close SA46. Chosen because the gate protects a security boundary (CSRF-exemption correctness) rather than being a one-time migration convenience — this project's precedent is to close security-boundary gaps rather than carry an accepted residual on a newly-introduced gate. SA46 remains open (not yet unblocked for merge) until this follow-up lands.
