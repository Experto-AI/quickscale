...
The two former complexity blockers were closed by SA126 (Track 1), so the green-gate unblock no longer sits behind the SA117 chain's plan gates. SA121 is closed and its merge-base monotonicity gate is live in `make quality`; SA127 is closed at `a2eb7a6c` and SA128 follows on Track 1.
...
**Track 1 is internally serial and partly release-critical.** SA121 and SA127 are closed (SA127 at `a2eb7a6c`); SA128 is next, and SA122b then carries the SA112e merge bound. The SA122 governance chain remains off-path. SA127 has landed independently of Track 3, removing the zero-module release blocker.
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now leads with SA128 (SA127 ✓ closed), while Track 3 has SA117c and SA117e ahead of SA112a. Reusing Track 1 still requires no fourth worktree, but it would delay SA128 as well as the governance queue.
  ...
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* Track 1's queue head. That head is now **SA128**, so the move would delay the governance chain to advance a critical-path node; it no longer trades against a release blocker.
...
- **Track 1 (SA128 → SA122b; SA127 ✓, SA121 and SA122a closed) — start: YES · finish: YES · merge: YES. TRULY GREEN.**
  - **Can start — YES.** SA128 carries `deps: none` and service-free validation now that `SA122A-AUTH-001` is resolved. SA122b follows with its stated SA112e merge bound. SA127 ✓ completed ahead of schedule.
  - **Can finish — YES.** SA128's acceptance is self-contained — adversarial fixtures and the existing registry, no other track's output. SA127 already delivered; SA122b retains its stated SA112e merge bound.
  - **Can merge — YES.** SA128 carries no merge-order gate. SA122b alone is order-gated behind **SA112e** (shared `.github/workflows/e2e.yml` path list) — a hard dependency that only that upstream work clears.
  - **On the critical path? NO — the Track 1 release blocker (SA127) is closed.** SA128 and SA122b remain governance filler. `SA112A-TRACK-002` weighs a critical-path node (SA112a) against Track 1's governance queue now that the release blocker is removed.
...
**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying SA128; it changes no track's three states. **Default while undecided: SA112a stays on Track 3; Track 1 works SA128 → SA122b.** Every individual ticket records **Decisions needed: none**.
