...
**Track 3 is truly green (`SA117E2-RESET-001`, granted 2026-08-03) and on the critical path; Track 1's parallel leg is paused at a recorded-partial checkpoint** whose `F-005` candidate has implementation-agent evidence only. Track 1 is blocked on `SA112A-RESET-001` — a supported control-plane reset that actually clears the persisted `qg_runs=3/2` / `review_cycles=2/2` / `authorized_retries=1/1` counters; another authorization-only retry cannot, having now failed twice (detail on the SA112a ticket). Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **Authorization history — `SA112A-HANDOFF-001` … `-004` all GRANTED and CONSUMED, none producing a checked box.** Each grant was per-attempt and waived no gate; delivery narratives, validation evidence, `F-003`'s resolution, and the post-review Ruff reviewed-tip identity gap are in [CHANGELOG.md](../../CHANGELOG.md). **A further authorization-only grant cannot restart this ticket** — see `SA112A-RESET-001` below. SA112a's *completion* merge remains SA112b's precondition; neither the recorded-partial merges nor the current checkpoint satisfies it.

    **The one remaining blocker.** `F-005` (**medium**, correctness; `scripts/_installed_wheel_venv.sh` / `scripts/test_installed_wheel_venv.py`): after a nonexistent path component sets probe state false, `..` returns to an existing prefix without restoring probing, so a later directory symlink is treated as nonexistent and receives provisioning output. Cover reverse-order/repeated cancellation, root cancellation, root spellings, accepted `link/../sibling` symlink-ancestor identity, status 2, empty stdout, native caller-trap parity for `missing/../link` rejection, and target preservation. `F-006` remains **low/advisory** unreadable-directory hardening and is **not** part of this correction.
    ...
    **Current checkpoint (`SA112A-HANDOFF-004`, 2026-08-03; task unchecked; no completion claim).** The merged candidate — `scripts/_installed_wheel_venv.sh` and `scripts/test_installed_wheel_venv.py` only — guards the empty component stack and root spellings and carries twelve `F-005` matrix cases, with **implementation-agent evidence only**. It supersedes the narrower six-case `HANDOFF-003` candidate; reuse it rather than restarting from the merged tip's pre-correction shape. **`F-005` remains open and unapproved**: no independent QG result and no approving `STATUS: ok` full-scope review exist, because the required QG dispatch was refused before execution at `qg_runs=3/2`, `review_cycles=2/2`, `authorized_retries=1/1` — the exact user authorization restored no executable allowance. `F-006` (**low/advisory**) remains outside this correction.

    **`SA112A-RESET-001` — the blocking decision.** Before any continuation, a supported control-plane reset must clear the SA112a changed-file scope's QG, review, and authorized-retry counters; then the complete ordered QG reruns and full-scope review must return `STATUS: ok`. Another authorization-only retry cannot buy either dispatch — that has now failed twice. The merged checkpoint preserves progress only: it is not a waiver, completion claim, executable approval, or satisfaction of SA112b's dependency.
...
**Status:** **head is SA112a and the track is at a recorded-partial checkpoint** — the `SA112A-HANDOFF-004` attempt ended without a checked box, leaving a two-file `F-005` candidate with implementation-agent evidence only. Track 1 is blocked on `SA112A-RESET-001`, a supported control-plane reset that clears all three persisted counters before the focused validation/review cycle can resume. It is on the critical path, and neither the earlier partial merge nor this checkpoint satisfies SA112b's dependency — only SA112a's completion merge does. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
