...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Tier 2 · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Blocked checkpoint (2026-07-27 — review cap reached; task remains open).**
  - **Done:** A Track 3 candidate stamps the twelve module metadata sets to `0.87.0`, adds embed/wiring lockstep enforcement and lease-safe split-publication tooling, and has repeatedly passed the repository quality gates (including smoke-install, coverage, and PostgreSQL integration). It is **committed on `wt-track3` as preservation checkpoint `43d9b8fc`** (2026-07-27, 69 files + 8 new scripts) to remove the uncommitted-work hazard that previously required recovering the SA115 and SA112 candidates. The checkpoint is **not a review approval and not mergeable**: it stays off `v87` because independent review has not approved the executable delta.
