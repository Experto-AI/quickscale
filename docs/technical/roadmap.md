...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; Core E2E             SA115a ◐ (acceptance-blocked   SA136d ◐ (plan-review capped;
  │  poetry-install repair;          │   by SA141; only ticket      │  no correction code landed)
  │  green-gate prerequisite)        │   on this track)             ▼
  │                                  │                          SA136f
  ▼                                  │                              │
...
| **Track 1** — SA141 (head) | **yes** — evidence-first diagnosis is executable today; no decision, authorization, or plan gate | **yes** — the failing tests and their install seam are in its own allowlist; no other track's output is required | **yes** — no merge-order gate; must merge *before* SA112f, which is a downstream ordering constraint on SA112f, not a gate on SA141 | **yes** | no — off-path, but a hard green-gate prerequisite |
...
| **Track 2** — SA115a | **yes** — no merge-order dependency remains | **no** — both umbrella commands reach E2E but each hits a distinct out-of-scope Core E2E Poetry-install failure ([SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance)); no SA115a defect is identified | **yes** — no merge-order gate or shared executable-file writer | no — acceptance-blocked by SA141 | no — off-path filler |

**Truly green today: SA141 only.** SA136d remains the critical-path head but is plan-gated after the 2026-08-10 recorded-partial decision; SA141 on Track 1 is **off-path but not filler**, since the green gate is unreachable without it. The two remaining tracks stay blocked by hard upstream dependencies: **SA122b-5**'s merge bound is clearable only by SA112e, and **SA115a** cannot finish until SA141 lands. The release-wide green gate remains unclaimed and no full E2E greenness is claimed anywhere.
...
**Status:** off the critical path and **closed to new work**. **SA115a is the track's only remaining ticket**, and the track carries **no merge-order gate and no shared executable file**. Its local implementation and direct E2E evidence are retained and reviewed `STATUS: ok`. It cannot finish only because both umbrella commands fail on the two out-of-scope Core E2E Poetry-install defects now ticketed as [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance) on Track 1; no SA115a defect is asserted.
...
**The milestone is unclaimed, and has two named prerequisites.** `make ci-e2e` most recently exited 0 with empty quarantine at `v87` `c4b6e354` (SA117e-3), but currently exits 1 on the two Core E2E Poetry-install failures owned by [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance); `make quality` exits 2 pending [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). No single clean rerun has covered all four commands, and `make ci` in particular has not been rerun. **SA112f owns the clean closeout rerun** and requires SA140 and SA141 to have landed; SA96-PUBLISH requires that evidence before release.
