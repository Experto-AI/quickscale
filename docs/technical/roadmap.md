...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-1 → -2 → -3 → -4 → -5
  ...
  │  PARTIAL — second attempt capped  │  cannot finish → SA112f     │  review · source · infra ·
  ▼  fresh dispatch grant needed      │  cannot merge  → SA112e     │  PUSH(human) · closeout
...
| **Track 1** — SA112a | **no** — `SA112A-HANDOFF-002` was consumed by the second capped attempt; a fresh dispatch grant is required | **no** — `F-005` remains medium/blocking at the review cap | **yes** — this recorded-partial checkpoint was explicitly authorized for merge | no | ✅ **yes** — blocked critical-path node |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is the only truly green track.** Track 1 remains on the critical path but is blocked after `SA112A-HANDOFF-002` ended at a second recorded-partial review cap: `F-003` is resolved, while `F-005` still needs one focused correction plus fresh validation/review under a new dispatch grant. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **Second recorded-partial checkpoint (2026-08-03) — the task remains unchecked and is not SA112b-ready.**
    - **Done:** Track 1 synced cleanly to `v87` `0caf490a`; the helper now rejects direct non-directory/disallowed-symlink shapes and the first dot-component alias; both complete service-free QG chains passed, including 49 focused tests, two positive smoke runs per chain with six markers/20 probes, the missing-manifests negative control, lint, typecheck, diff, and scope checks. Independent review resolved `F-003` (early `OUTPUT_DIR` ownership, cleanup, and caller-trap behavior).
    - **Pending-Blocking:** `F-005` (**medium**, correctness; `scripts/_installed_wheel_venv.sh` / `scripts/test_installed_wheel_venv.py`) — after a nonexistent path component sets probe state false, `..` can return to an existing prefix without restoring probing; a later directory symlink in `missing/../link` or `missing/../link/.` can be treated as nonexistent and receive provisioning output. Recompute existence/probe state after cancellation and cover reverse-order/repeated cancellation, root spellings, symlink-ancestor output identity, status 2, empty stdout, native caller traps, and target preservation. `F-006` remains **low/advisory** unreadable-directory hardening and is not part of this correction.
    - **Decisions needed:** a later continuation requires a fresh `SA112A-HANDOFF-003` authorization for one focused correction/validation/review cycle. Review pass 1 and pass 2 both ended at highest blocking severity **medium**, count **1**; the maintainer selected recorded partial delivery and merge at the cap.

    **Post-review hook note:** the full checkpoint review returned `STATUS: ok` for preservation only, then mandatory Ruff hooks reformatted two F-008 docstrings and required two D205 blank lines. Quality-gate evidence proved AST identity, word-identical docstrings, all eight hooks green, and no semantic change. Because those mechanical bytes post-date the reviewed tip, the maintainer explicitly selected checkpoint merge without a fresh post-cap tip review; this is a reviewed-tip identity gap, not executable approval.

    This checkpoint preserves progress only. It is not a waiver, completion claim, executable approval, or satisfaction of SA112b's dependency.
...
**Status:** **head is SA112a at its second recorded-partial checkpoint and is not truly green.** `SA112A-HANDOFF-002` is consumed; `F-003` is resolved, but `F-005` remains medium/blocking and a fresh `SA112A-HANDOFF-003` dispatch grant is required before another correction/validation/review cycle. It is on the critical path, and the partial merge does **not** satisfy SA112b's dependency — only SA112a's completion merge does. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**Open decisions — three, all the maintainer's.** Two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. The third is **`SA112A-HANDOFF-003`**, a fresh dispatch authorization required after the second SA112a attempt ended at its review cap; it blocks Track 1 today. All other blockers on the board are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are **execution-time** human gates obtained at the outward-facing action and never pre-granted.
