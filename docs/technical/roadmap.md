...
**Truly green today: Track 3 (SA117e-4)** — and it is *on* the critical path, so it is real progress rather than filler. **SA141 is now startable** on its third grant but is not truly green until that cycle's gate exits 0. **SA122b-5** stays merge-gated by SA112e and **SA115a** cannot finish until SA141 closes; both are hard dependencies. The release-wide green gate remains unclaimed.

**Open maintainer decisions: none.** SA136f's focused continuation is complete and reviewed; F-003, F-004, F-005, and F-006 are resolved. The third SA141 full-gate cycle was **granted on 2026-08-12 under option 1 (warm the caches, then rerun)**; scope is in [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance). Every remaining "no" on this board is now a hard upstream dependency, not a choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). The remaining human gates (the twelve-row seal confirmation, SA96-PUBLISH) are execution-time gates obtained at the outward-facing action, not pending decisions.
...
- [ ] **SA141 — Diagnose and repair the two Core E2E Poetry-install failures.** `Tier 2 · deps: none` · **blocks SA112f, SA115a acceptance, and the green gate**
  ...
  - **Retained repair and open ledger (task remains unchecked).** The diagnosis (ambient `VIRTUAL_ENV`/`POETRY_ACTIVE` inheritance under `poetry run pytest`), the isolation seam in `poetry_env.py`, and the complete second-continuation evidence are recorded in [CHANGELOG.md](../../CHANGELOG.md). `_run_repo_poetry_command` remains the intentional ambient repository-only exception. No green-gate claim is made.
    ...
    - **Decision resolved (2026-08-12):** the maintainer authorized the third quality-gate cycle under option 1 (warm the caches, then rerun). Changing the out-of-allowlist E2E timeout remains unauthorized and would require separate scope review.
    - **Unblock options (both failures were external package access, not code).**
      1. **Warm the caches, then rerun (authorized).** Pre-fetch the Poetry/pnpm/Docker Hub artifacts the E2E lane needs into a persistent local cache before the run, so no gate step performs a cold network fetch. Cheapest, changes no product code, and directly targets the observed cause. Authorized under the resolved 2026-08-12 decision.
