...
- [ ] **SA102 — Add `--devtools` to every CI lint/typecheck invocation (TA61).** `Tier 1 · Track 1 (rebalanced from Track 3, 2026-07-19) · deps: none · assistant-executable · HAND-OFF-READY`
  **The clean, unblocked task for Track 1 right now** — no pending decision, no dependency, no cross-track wait. SA99 brought `quickscale_devtools` under the Makefile *default* lint/mypy sections, but all five CI invocation sites pin explicit `-- --core --cli --modules` flags that exclude it, so devtools runs in no gate anywhere (empirically it passes both today — this is drift that will silently admit the first future regression). Add `--devtools` to the five sites: `.github/workflows/ci.yml:336,348` and `scripts/check_ci_locally.sh:188,253` (serial) + `375,390` (parallel) — or drop the explicit flags where the default sections are the intent.
  - File-disjoint from Track 1's own blocked TP2b (which edits the `Makefile` `test-cov` target only) and from the in-flight Track 3 SA101 (quality-baseline source) / Track 2 TP3b (`scripts/test_e2e.sh`), so it can start immediately in parallel.
  - Verify: a temporary ruff violation in `quickscale_devtools/src` must fail both `make ci` and `ci.yml`; revert.
  *(why →* tech-audit TA61 — recorded SA99 coverage claim vs. actual gate coverage*)*
...
**CI-gate remediation from the 2026-07-19 sweep (SA103; SA102 rebalanced to Track 1).** Two audit findings landed on the CI/release plumbing. **SA102** (TA61, trivial) was **rebalanced to Track 1 on 2026-07-19** as a hand-off-ready task while Track 3 stays busy on the SA101 critical path — it is file-disjoint from SA101 (quality-baseline source) and from SA103 (SA102 edits lint/typecheck *invocation flags*; SA103 adds a *frontend* job), so the split is conflict-free. **SA103** stays here: it touches `ci.yml`/`publish.yml`/`Makefile` umbrellas + `scripts/check_ci_locally.sh`, is file-disjoint from the Track 2 frontend workstream (template tree) and from SA101, and **must land before the SA96-GATE four-command join re-runs** since it tightens what "green" means.

- [ ] **SA103 — Wire the rendered-frontend proof into a blocking gate (TA60 + arch red flag).** `Tier 2 · Track 3 · deps: none · feeds SA96-GATE · assistant-executable`
...
SA102 ── devtools CI gate (TA61,      TP3a ✓ → TP3b (E2E concurrency;          SA93 ✓ (e2e in green-gate)
  Tier 1, HAND-OFF-READY;               rebalanced from Track 1; off crit.)    SA100 ✓ (TA58/TA59 theme preflight)
  rebalanced from Track 3, 2026-07-19)  SA104 ── FE static-source move (Fin.10   SA103 ── frontend-proof gate (TA60/red
TP2b ⧗ → TP4 (off crit.; TP1/TP2 ✓)     stage 1; safe now, byte-identical)      flag; feeds SA96-GATE; node-gated,
 (test-parallelization; off crit.)     ├─ SA105 ── module de-spec (2a;          file-disjoint from SA104)
                                       │    deps SA104 + dormant-files decision)
        ...
        SA101 ── clear BLK-002 quality regressions (Option A: remediate, no re-baseline)  Track 3 · feeds SA96-GATE
        SA103 ── frontend-proof gate (arch red flag: land before the join re-runs)        Track 3 · feeds SA96-GATE
                       ▼
...
**Open work only:** Track 1 `SA102` (hand-off-ready; rebalanced from Track 3) + `TP2b` (validation-blocked) → `TP4`, Track 2 `TP3b` (after completed `TP3a`) + the frontend chain `SA104 → SA105/SA106` (arch Finding 10), Track 3 `SA101` (clears BLK-002; on the critical path) + `SA103` (2026-07-19 audit sweep; feeds SA96-GATE), and the release join
`SA96-GATE → SA96-PUBLISH`. The two TP chains run in parallel with no source-file overlap
(`Makefile`/`pyproject.toml` vs `scripts/test_e2e.sh` + `quickscale_cli/tests/test_e2e_development_workflow.py`); the frontend chain (SA104+) owns the `quickscale_core` generator + `showcase_react` template tree only, disjoint from both the TP work and the SA102/SA103 CI surfaces, so all four workstreams run concurrently. All of TP, the frontend chain, and SA102 are off the release critical path; **SA103 feeds the gate** (arch red flag — land before the SA96-GATE join re-runs). None may regress any gate's pass/fail set or coverage thresholds. `TP4` is docs-only and lands after
`TP1` (done) and `TP3b` (Track 2) merge to `v87`.

**Critical path.** Both pre-publish module sweeps are complete: **SA96-T1** (Track 1) and **SA96-T2** (Track 2). **SA93** is complete and its dependency is met; the remaining critical-path chain is **SA101** (clear the BLK-002 quality regressions, Track 3) → **SA96-GATE** (green-gate join) → **SA96-PUBLISH** (human-only). **SA98** and **SA100** are complete independent audit-remediation tickets; neither blocks SA96-GATE. SA97 and SA99 are also complete (see [CHANGELOG.md](../../CHANGELOG.md)).
...
- **Track 1 — ACTIVE; a hand-off-ready task is now queued (SA102) alongside the blocked TP2b and pending TP4.** **Rebalance (2026-07-19):** because TP2b is validation-blocked and TP4 is soft-gated on TP3b merging, **SA102** (TA61 devtools CI gate, Tier 1, trivial) was moved here from Track 3 to give Track 1 an immediately startable, unblocked, decision-free task while Track 3 stays on the SA101 critical path. SA102 is file-disjoint from TP2b (`Makefile` `test-cov`) and from the in-flight Track 2/Track 3 work, so it runs in parallel with no conflict. All prior release tickets closed (SA92, SA84, SA86, SA96-T1) and both audit-remediation tickets — **SA97** (arch Finding 9 test-plumbing half) and **SA99** (arch Finding 7 devtools→ruff/mypy) — completed 2026-07-17. **TP1** (static-gate fan-out) and **TP2** (unit xdist) were accepted 2026-07-18; TP2 retains the waived/non-blocking pre-existing core coverage finding. **TP2b** has implementation fixes, focused structural/behavioral evidence, and bounded-xdist evidence, but **QG-TP2B-001** blocks the exact post-fix default and required-backups validation; **CR-TP2B-001/002** were resolved by independent change-review pass 2. **TP4** (AI fast-loop docs) remains pending. The E2E sub-chain **TP3a/TP3b** was rebalanced to Track 2 (2026-07-18); it is file-disjoint from these (TP3a owns `scripts/test_e2e.sh` plus `quickscale_cli/tests/test_e2e_development_workflow.py`; TP3b retains `scripts/test_e2e.sh` for concurrent-lane orchestration). All Tier 1–2, none on the SA96 release critical path — pure SDLC cycle-time work; must not regress any gate's pass/fail set or coverage thresholds. **TP2b is not clean to close out; no design decision is pending.** Evidence for closed work in [CHANGELOG.md](../../CHANGELOG.md).
- **Track 2 — E2E-concurrency sub-chain (TP3a ✓ → TP3b) + the arch Finding 10 frontend chain (SA104 → SA105/SA106).** Release tickets and audit remediation are closed (SA94, SA88b, SA86, SA95, SA96-T2, and SA98; SA97+SA98 close arch-audit Finding 9). Track 2 carries the **TP3a → TP3b** sub-chain (rebalanced off Track 1) plus the newly-cut **frontend-theme de-specialization** workstream (arch Finding 10, the frontend counterpart to this track's SA94 react-only work): **SA104** (stage 1, byte-static source move — safe now, SA90-fixture-verified), then **SA105** (stage 2a, module de-spec — deps SA104 + a maintainer dormant-files decision) and **SA106** (stage 2b, identity de-spec — deps SA104). **TP3a** is complete; **TP3b** (concurrent Core/CLI E2E lanes) remains. TP3b retains `scripts/test_e2e.sh` and reuses the read-only `_qs_jobs.sh` helpers; the frontend chain owns the `quickscale_core` generator + `showcase_react` template tree — no source overlap with the TP work, with Track 1, or with Track 3's SA102/SA103 CI surfaces, so all run in parallel (only the closeout files are shared, covered by the Merge procedure). All off the release critical path; must not regress any gate or coverage threshold. **Carried execution gates:** do not start TP3b or claim integrated CI green until `make ci-e2e` is rerun with more than a 20-minute allowance and the final CLI E2E result is retained (TP3a evidence gap); **SA105 has a pending maintainer decision** (dormant module files in generated trees — record in decisions.md before implementing). Evidence for closed work in [CHANGELOG.md](../../CHANGELOG.md).
- **Track 3 — SA93 COMPLETE; SA96-GATE blocked on SA96-GATE-BLK-002, now with the decision made and SA101 ready to execute.** Finding 1, all four GATEs, SA91, SA100, and SA93 are complete. SA93's local gate and verified hosted evidence are recorded above; **SA96-GATE is the next release-path step but is not currently clean** — canonical coverage is restored and `make check` exits 0, but `make quality` exits 2 with 9 critical + 10 warning baseline regressions, so the four-command join cannot pass yet. Independent change-review pass 2 returned **STATUS ok** with **SA96-CR-001 (blocking)** and **SA96-CR-002 (advisory)** resolved and no new findings. **The maintainer decision on BLK-002 is made (2026-07-19): Option A — remediate against the existing baseline, do not re-baseline** — cut as the standalone assistant-executable ticket **SA101** (Track 3, on the critical path). Track 3 is therefore hand-off-ready on the release path: execute SA101 → re-run the SA96-GATE join. **This track also carries the two CI-gate remediations from the 2026-07-19 audit sweep — SA102** (TA61 devtools gate, Tier 1, trivial, off the critical path) **and SA103** (TA60 / arch red flag frontend-proof gate, Tier 2), both file-disjoint from SA101 and from the Track 2 frontend chain. **SA103 feeds SA96-GATE** — it tightens what the four-command join proves, so land it before the join re-runs; SA102 can slot in whenever the track is between critical-path steps. The PyPI publish (SA96-PUBLISH) remains explicitly excluded from assistant work (human-only). SA91 retains CR-SA91-REV-006 (low/advisory); SA89B-CR-004, SA93-REV-005, and SA93-ADV-001..004 remain non-gating advisories.

**Net — all three tracks have assigned work; the 2026-07-19 audit sweep added five tickets (SA103 on Track 3, SA104→SA105/SA106 on Track 2, and SA102 rebalanced onto Track 1 as a hand-off-ready task) with one pending maintainer decision on SA105 (dormant frontend files).** Track 1 carries **SA102** (unblocked, startable now) plus the off-critical-path TP test-parallelization suite, with **TP2b pending/blocked** on exact post-fix validation and final independent re-review, and **TP4** pending; Track 2 carries the rebalanced E2E-concurrency sub-chain (**TP3a** complete, **TP3b** remaining) plus the new arch-Finding-10 frontend chain (**SA104** safe-now, then **SA105/SA106**). Both are off the release critical path and run in parallel with no source-file overlap. Both pre-publish module sweeps and SA93 are complete, so **SA96-GATE** is the next release-path step — coverage is restored and `make check` is green, but the gate is **blocked on SA96-GATE-BLK-002** (`make quality` exit 2 with 9 critical + 10 warning baseline regressions). Independent change-review pass 2 returned **STATUS ok** with **SA96-CR-001 (blocking)** and **SA96-CR-002 (advisory)** resolved and no new findings. The user approved merging this blocked checkpoint without claiming completion; **the BLK-002 decision is now made (Option A — remediate against the existing baseline, no re-baseline) and cut as ticket SA101 (Track 3, critical path)**, so no maintainer decision is pending — SA101 executes, then the four-command join re-runs. Once green, a coding assistant runs the join and reports, then hands off to the human-only **SA96-PUBLISH** step. **SA98** and **SA100** are complete independent audit remediation, while SA97 and SA99 are also complete. No TP2b design decision is pending; the remaining work is validation and independent review. The squash-migrations decision and bounded guardrail strategy are recorded in [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92); reasoning trail in [CHANGELOG.md](../../CHANGELOG.md).
