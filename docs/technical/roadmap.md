...
SA128d ⛔ → SA122b-1 → … → -5           SA115 (e2e xdist; deps: none) SA117e-3 ⛔ → -4 → -5
  ...
  ▼  F-001 blocked; needs budget      │  validation AUTHORIZED      │  ORDER-001 decision needed
...
**One track-sequencing question is open: `SA117E3-ORDER-001`; no worktree-assignment question is open, and the latest rebalancing review (2026-08-05) applied no move — the sixth consecutive pass to reach that conclusion.** Every open v87 ticket carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), and SA112a — the one node that was worth parallelizing onto Track 1 — has closed and merged. The remaining off-path work (SA128d, SA122b-1…5) is a single serial chain whose head is dependency-blocked on its predecessor, so it cannot be spread further; moving any of it onto Track 3 would push filler ahead of the critical path. `SA117E3-ORDER-001` changes the order of children already on Track 3, not their worktree placement.
...
**Open decisions — four, all the maintainer's; one blocks the critical path.** `SA117E3-ORDER-001` must reconcile SA117e-3's public-harness acceptance with SA117e-4's ownership of the human-gated split refresh; `SA117E3-EXEC-001` was granted and consumed without resolving that ordering contradiction. `SA128D-EXEC-001` remains open and blocks off-path Track 1 only — one fresh execution/review budget round for a correction whose design is already approved. The other two are owned by **SA117e-4**: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
