...
SA96-T1 ── module sweep (open)      SA96-T2 ── module sweep (completed)          SA91 ✓ (parallel loop, non-gating)
...
**Critical path.** The three tracks now run in parallel again: **SA96-T1** (Track 1 module sweep), **SA96-T2** (Track 2 module sweep — completed 2026-07-17), and **SA93** (Track 3 e2e closeout) share no files and can proceed independently. All three feed the **SA96-GATE** cross-track join; **SA96-PUBLISH** follows the join. Longest pole is whichever of the two remaining tracks finishes last — SA93 (deterministic CR-SA93-REV-002/004 fixes + `make ci-e2e` rerun) or SA96-T1.
...
**Net — no maintainer decisions pending.** SA96-T2 is complete; SA96-T1 and SA93 continue in parallel. They join at **SA96-GATE** (the green-gate: `make check`/`quality`/`ci`/`ci-e2e` all green with empty quarantine), which is the definition of "publishable"; **SA96-PUBLISH** then runs the staged PyPI ladder. The squash-migrations decision and bounded guardrail strategy are recorded in [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92); reasoning trail in [CHANGELOG.md](../../CHANGELOG.md).
