...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-1 → -2 → -3 → -4 → -5
  ...
  │  PARTIAL — review cap reached     │  cannot finish → SA112f     │  review · source · infra ·
...
| **Track 1** — SA112a | **no** — executable candidate is preserved, but fresh correction/review capacity is required | **no** — `F-003` and `F-005` remain blocking | **yes** — recorded-partial merge explicitly authorized | no | ✅ **yes** — critical-path checkpoint |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is the only truly green track.** Track 1 remains on the critical path but is stopped at SA112a's recorded-partial review checkpoint; its completion merge is still SA112b's other precondition. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **`SA112A-HANDOFF-001` — CONSUMED, 2026-08-01.** The authorized fresh implementation attempt produced the exact four-file candidate. The historical documentation-consistency `F-003` remains closed by the four-file `Files` line above; the current executable finding with the reused ID is distinguished below by category and location.

    **Recorded-partial checkpoint (2026-08-01; task remains unchecked; not SA112b-ready).**
    - **Done:** Track 1 started clean at `ee8754d8`, fast-forwarded to `v87` `b43a57ce`, and implemented the approved helper/wrapper/smoke/test slice. Two full service-free quality-gate passes covered shell syntax, focused tests (18 before the final correction), exact six-marker order, all 20 smoke probes, terminal stdout, the missing-manifests negative control followed by a green rerun, lint, typecheck, diff, and exact scope; the parity checker's exit 2 was byte-identical to baseline and remains the declared publish-workflow gap. Review pass 2 resolved `F-001` (repeated-invocation reset) and `F-002` (all-path caller-trap semantics). The user-authorized post-cap implementation moved validated output ownership ahead of prerequisite/allocation work; 9 targeted and 27 focused tests passed, and reverting that correction failed 8 of 9 targeted cases.
    - **Pending-Blocking:** `F-003` (**medium**, correctness; `scripts/_installed_wheel_venv.sh` early `OUTPUT_DIR` ownership and `scripts/test_installed_wheel_venv.py`) remains open because the control plane refused a third quality-gate and third independent-review dispatch at `qg_runs=2/2` and `review_cycles=2/2`; its correction exists and is test-backed but lacks fresh independent executable approval. `F-005` (**medium**, correctness; the same helper/test pair) was found by the final checkpoint review: an existing regular file, broken symlink, or other non-directory `OUTPUT_DIR` can pass validation and fail later with status 1 instead of the documented argument-error status 2. Reject non-directory/disallowed-symlink shapes before trap arming, check emptiness only on a real directory, and cover regular-file, broken-symlink, symlink-policy, and other-node cases while preserving caller data and empty stdout.
    - **Decisions needed:** none — both contracts and corrections are explicit. Clean continuation starts from this exact checkpoint, fixes `F-005`, reruns the literal focused and service-free validation chain, and obtains a fresh full-scope independent review that closes both `F-003` and `F-005`; only then may the approved completion closeout run. After the final review returned `STATUS: partial`, the maintainer explicitly instructed this Done / Pending-Blocking / Decisions-needed checkpoint to be committed and merged. The merge preserves work; it is not a waiver, completion claim, executable approval, or satisfaction of SA112b's dependency.
...
**Status:** **head is SA112a at a recorded-partial review checkpoint — the tested four-file candidate may merge for preservation, but the task cannot finish and Track 1 is not truly green until `F-005` is fixed and a fresh full-scope independent review closes executable `F-003` and `F-005`.** It remains on the critical path, and this partial merge does not satisfy SA112b's dependency. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**No track-sequencing or worktree-assignment question is open, and the 2026-08-01 rebalancing review applied no move:** every open v87 ticket already carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), Track 3's children are one coherent serial umbrella, and SA112a is already the parallelized critical-path node. **Two implementation decisions remain open**, both the maintainer's and both owned by SA117e-4: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind/consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. Neither is needed until SA117e-4 implementation, so neither blocks any track today. The authorization decisions were resolved on 2026-08-01:
...
3. **`SA112A-HANDOFF-001` — CONSUMED; SA112a stays Track 1's head.** The fresh implementation attempt produced the four-file candidate, but the executable review cap ended with `F-003` pending independent verification. The **reordering question remains resolved in favour of Option A**: off-path SA128a stays queued behind SA112a rather than crossing the one-child-at-a-time boundary.

Track 1 has implemented SA112a and is **stopped at recorded partial delivery** pending the `F-005` invalid-output-path correction and a fresh full-scope independent review of executable `F-003` and `F-005`; it may not start SA128a or satisfy SA112b first. Track 3 is preserved at **SA117e-1**, whose historical review ran but whose documentation closeout remains open. Plan-review `F-002` blocks SA117e-3's host-service mutation pending that child's scoped plan-review `STATUS: ok`; `SA117E1-REV-001` and `SA117E1-REV-002` separately block SA117e-4's public split push. Those standing decisions are distinct from the push confirmation and later SA96-PUBLISH, which remain **execution-time** human gates obtained at the outward-facing action rather than pre-granted authorization.
...
- **Current state.** SA112a's scoped plan passed independent plan review and produced the recorded-partial four-file candidate above. The candidate is service-free tested, but executable `F-005` requires correction and `F-003`/`F-005` remain blocking pending a fresh full-scope independent review; SA112a stays unchecked and SA112b remains blocked on it.

**Reordering question — RESOLVED 2026-08-01, Option A.** Track 1's queue is unchanged (**SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**); SA128a was not promoted ahead of SA112a. The rejected Option B would have converted idle time into off-path progress at the cost of delaying a critical-path node by one child under the one-child-at-a-time contract.
