...
| **Track 1** — SA112a | **no** — `SA112A-HANDOFF-001`, cleared by a **maintainer decision** | yes | yes | no | yes (SA112a only) |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

Every "no" is classified above as either *maintainer-decision-clearable* or *hard upstream dependency*; none is ambiguous. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs two maintainer decisions recorded in its task block.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **Pending-Blocking — `SA112A-HANDOFF-001` (medium, orchestration).** The Adaptive control plane refused the exact four-file implementation handoff because a persisted prior block already consumed `blocked_retries=1/1`, and the maintainer declined the one human-authorized override in that run. **Decision needed (maintainer's, not upstream work):** explicitly authorize one fresh `Adaptive-implement` attempt for this exact four-file scope so it can be passed as `user_authorization`; until then SA112a cannot start, and it is the only thing stopping it. No executable/test file has changed; no syntax, focused, smoke, negative-control, parity, lint, type, executable-review, or completion-review evidence is claimed. Reuse the approved plan above — do not re-plan unless `VERSION`, `scripts/smoke_install.sh`, or its mapped caller topology changes. `F-003` (documentation consistency) is **closed** by the four-file `Files` line above and the Track-topology allowlist. Checkpoint detail is in [CHANGELOG.md](../../CHANGELOG.md).
...
**Status:** **head is SA112a. Can start: no — blocked solely by `SA112A-HANDOFF-001`, a maintainer authorization, not upstream work. Can finish: yes once started — `deps: none`, four-file disjoint allowlist, service-free validation. Can merge: yes — no merge-order gate (SA112a is itself the upstream of SA112b).** Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**No track-sequencing or worktree-assignment question is open, and the 2026-08-01 rebalancing review applied no move:** every open v87 ticket already carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), Track 3's children are one coherent serial umbrella, and SA112a is already the parallelized critical-path node. **Four implementation decisions are open**, all the maintainer's: `SA112A-HANDOFF-001` (authorize one fresh Adaptive attempt before Track 1 can dispatch SA112a); the explicit-absence Git lease contract for `SA117E1-REV-001` and the bind/consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`, both before SA117e-4 implementation; and the **reordering question** below. The earlier authorization decisions were resolved on 2026-08-01:
...
**Open reordering question — Track 1's head while SA112a is blocked.** Track 1 is one serial worktree under the one-child-at-a-time handoff contract, so it is currently idle. Off-path SA128a is `deps: none` with a disjoint allowlist and could start today. **Option A (recommended): authorize `SA112A-HANDOFF-001` and keep SA112a as the head** — SA112a is on the critical path and SA128a's consumer SA122b-1 is independently gated behind SA128d, so nothing is lost by leaving SA128a queued. **Option B: start SA128a now** — converts idle time into off-path progress, but under the serial contract it delays SA112a's start by one child once authorization arrives, pushing a critical-path node behind filler. This is surfaced rather than applied because it trades critical-path latency for filler throughput.
