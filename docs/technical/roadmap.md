...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  *(why →* `apply` with any module has zero end-to-end coverage — `test_e2e_development_workflow.py:276` plans with modules skipped — so this skew class has never been exercised*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Tier 2 · deps: none` — critical path; contains a human-only step
    ...
    **SA117e recorded partial delivery (2026-07-31; functional correction `b11c24c5`; SA117e/SA117 remain unchecked; no public mutation).**
    **Done.** Track 3 started clean and synced to current `v87`; the SA117e plan and its independent plan review returned `STATUS: ok`. The two-file verifier fallback portability/correctness correction is committed as `b11c24c5` and received full independent correction review `STATUS: ok` with no findings — that reviewed slice covers the correction only, not the still-missing full-scope historical SA117 executable review over the complete delta including `43d9b8fc`. The disposable installed-wheel harness `/tmp/sa117e-harness.HnXjQA/harness.reviewed.sh` (exact SHA `53762f9dd837679808754094d0b4f199665f801ca91f2d69710b2115651c552b`) received full review `STATUS: ok` after `CR-SA117E-001`–`004` resolved, but was **not executed** and is **ephemeral/untracked**. Ordered source validation: **32 focused correction tests passed** and **929 broader tests passed**; `make version-check` passed; `make check-manifest-sync` passed 12/12.
    **Pending-Blocking.** **SA117E-VAL-001** (high, completeness): ordered command 4 exited 2 because `SA117_BASELINE_REF=97404f3e…` is internally inconsistent (VERSION `0.87.0` with 60 module values pre-lockstep); the continuation must revise/reapprove the lock-diff baseline — recommended immutable baseline `43d9b8fc`, the first internally consistent 0.87.0 lockstep checkpoint — without weakening checker semantics. `sa117-capture`, `sa117-verify`, `make check`, `make quality`, and `make ci-e2e` were not run (stop-on-first-failure). Advisories `CR-SA117E-005` (evidence wording) and `CR-SA117E-006` (handoff command exactness) remain. After the baseline resolves, continuation still needs: the full historical executable-delta review including `43d9b8fc`; the remaining ordered validation; a rematerialized/revalidated harness if the temp artifact/hash is unavailable or changed; exclusive Docker/PG state with a host-PG negative control; fresh human public-push confirmation; a local-only tag; the protected 12 split pushes; public/source/bundled parity; an installed all-module apply; and the final closeout review.
    **Decisions-needed.** The user chose to stop this run, not to approve a future baseline change: continuation requires explicit revision/reapproval of ordered command 4 to use an internally consistent immutable baseline, with `43d9b8fc0abd1c3193a3358f41dee0e09a3c106c` (the first internally consistent 0.87.0 lockstep checkpoint) recommended; alternatively the task remains blocked. Checker weakening is not an accepted option. Fresh human split-push confirmation remains a separate execution-time gate to be obtained at the push, not already granted by this stop.
    *(SA117e and SA117 remain unchecked; this checkpoint records partial delivery truthfully and claims no completion, tag, push, or release readiness.)*
