...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ✓ (closed; warm-cache         SA115a ◐ (SA141 dependency      SA117e-4 (head)
  │  gate + full review green)         │   cleared; only ticket)        │
  ▼                                  │                          SA117e-4 → -5
...
**Critical path:** `SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA140 → SA96-PUBLISH` — eight remaining legs. **SA136f is closed** after full-scope review approved the normative loop/seal policy and resolved F-003, F-004, F-005, and F-006; SA117e-4 is now the Track 3 head and the only outward-facing split publication step. **SA141's parallel green-gate prerequisite is closed after its already-merged executable repair passed final validation and review**, so SA112f and SA115a no longer wait on its Core E2E repair. **SA140 is on the path**: accepting `SA136D-QG-001` per-ticket does not satisfy the green gate, which requires an exit-0 `make quality`, so the repair is v87 scope sequenced after SA112f. **SA117e-5 is closeout and sits *off* the critical path**: SA112b's precondition is the *sealed* splits delivered by SA117e-4, not the umbrella's closure. Track 2 and SA122b-5 are entirely off-path filler. The green-gate milestone is governed by the four-command join below and is not claimed here. No seal command, remote-ref mutation, tag push, or publication is authorized anywhere on this board; those sit behind SA117e-4's execution-time human gate.

**Cross-track edges — one merge-order edge, no unserialized co-writes.** **SA122b-5 merging after SA112e is the only merge-order edge on the board**, and the two tickets are the only writers of `.github/workflows/e2e.yml`'s `pull_request.paths` list. **SA115a has no merge-order dependency and no shared executable-file writer.** Track 2 remains gate-free. `publish.yml` has no open writer.
...
| **Track 2** — SA115a | **yes** — no merge-order dependency | **yes** — acceptance dependency cleared by the closed SA141; no SA115a defect is identified | **yes** — no merge-order gate or shared executable-file writer | **yes** | no — off-path filler |

**Truly green today: Track 2 (SA115a) and Track 3 (SA117e-4).** Track 3 is *on* the critical path, so it is real progress rather than filler. SA141 is closed after its authorized warm-cache cycle passed E2E and full review; that clears SA115a's sole acceptance dependency. **SA122b-5** is now Track 1's head and stays merge-gated by SA112e. The release-wide green gate remains unclaimed.
...
Single-writer or unowned: `apply_command.py` (SA140 alone), `test_e2e_full_workflow.py` (no open writer), `docs/technical/decisions.md` (SA136f closed; SA117e-5 remains the in-track closeout writer). `scripts/publish_module.py`, `module_commands.py`, `module_output.py`, `git_utils.py`, `scripts/check_sa117_scope.py`, `scripts/version_tool.sh`, `scripts/quality_waivers.json`, and `scripts/quality_baseline.json` have no open v87 writer. No additional procedure is required.
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — remains unchecked until SA117e-5 closes the acceptance umbrella; it does not block SA117e-4.
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [x] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓, SA136b ✓` — closed; seal predicate, `--seal`/`--seal-all`/`--status` and the `seal-*` Make targets are merged and reviewed. Retained here because SA117e-4 executes them and the SA136f closeout ratified their ordering. Evidence in [CHANGELOG.md](../../CHANGELOG.md).
  ...
  - [x] **SA136f — Ratify the loop/seal contract in `decisions.md` and close SA119.** `Tier 1 · deps: SA136a–SA136e ✓ all closed` — closed after full-scope review; no publication implied.
    ...
    **Closeout record:** [§module-version-lockstep](./decisions.md#module-version-lockstep) now contains the normative six-step ordering: bump/stamp/commit → create the core tag **locally only** → loop `publish-module` and test installed `apply` with `--split-ref` → `seal-modules` → verify twelve of twelve sealed and a clean no-override `apply` → push the core tag with `git push origin X.Y.Z`, the PyPI publication trigger. Its Rules 4–6 establish idempotent branch publication, identity-resolved immutable tags without a mapping table, and content-identity tag reuse; the former "Known limitation" records SA119 resolved with the accepted force-privileged maintainer residual; Rule 7 records client-side check-then-act sealing with sampled-tip, immediate branch-reread, tag-conflict, explicit-refspec, and fail-closed post-push checks.
    - Allowlist: `docs/technical/decisions.md`, this roadmap, `CHANGELOG.md`.
    - Verify: no roadmap or decisions text still defers immutable-ref pinning to v88; the recorded ordering matches what SA136d's targets actually do. **Closeout:** final full-scope review returned `STATUS: ok`; `F-003`, `F-004`, `F-005`, and `F-006` are resolved. SA119 is closed by design because default embeds consume identity-derived immutable split tags. No seal, tag, push, publication, SA117/SA136 umbrella closure, or release-readiness claim occurred.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112f — Run ordered acceptance, review the complete delta, and close SA112.** `Tier 2 · deps: SA112e` (SA141's E2E repair is merged and closed)
...
- [ ] **SA115a — Guard clamp and local xdist validation.** `Tier 2 · deps: none` — items 1–3; **no `e2e.yml` edit**, no merge-order gate
  ...
  - **Retained implementation; task remains unchecked/recorded-partial.** All three phases are committed and reviewed `STATUS: ok`; the measured speedup, container/cleanup evidence, and review history are in [CHANGELOG.md](../../CHANGELOG.md). SA141's empty-quarantine E2E gate and final review clear the former acceptance dependency; no SA115a defect is asserted. The speedup is provisional and gets one confirmation re-measure at SA112f, not a re-review or merge gate. Advisory `F-003` (document `QS_E2E_XDIST_WORKERS` and the coupled clamp in `docs/technical/validation_policy.md`) is outside this allowlist and rides with a future docs pass.
...
**Status:** queue is **[SA122b-5](#sa122--release-assurance-is-four-hand-synchronized-gate-inventories-arch-finding-11)** (head; merge after SA112e). The SA128 parity checker is authoritative and is an input, not scope. (SA139 and SA141 are closed; see [CHANGELOG.md](../../CHANGELOG.md).)
...
**The milestone is unclaimed.** SA141's Core E2E environment-isolation repair is closed, so `make ci-e2e` is expected to exit 0 with empty quarantine. `make quality` separately exits 2 pending [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). No single clean rerun has covered all four commands, and `make ci` in particular has not been rerun. **SA112f owns the pre-SA140 installed-wheel/E2E acceptance; after SA140 lands, SA96-PUBLISH owns the final four-command join before any publication action.**
...
- **SA136f is closed on Track 3** as the documentation-only critical-path closeout: SA136a–f are all closed, and keeping the umbrella closeout beside the reviewed seal machinery avoided splitting its normative contract across tracks. SA117e-4 is now the live Track 3 head.
...
**Rebalancing verdict: the SA141 placement delivered its intended parallelism gain.** SA141 is closed after removing the two Core E2E Poetry-environment failures from the idle track. Nothing remaining is movable — SA136f is closed, SA117e-4 is the active Track 3 head, SA112b–f are one serial evidence chain, SA122b-5 carries its `merge after SA112e` bound, SA140 is evidence-bound behind SA112b, and Track 2 stays closed to unrelated work. Docker/PostgreSQL remains serialized with Track 3 priority whenever a service-backed critical-path leg is active.
