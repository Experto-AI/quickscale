...
- [x] **SA54 — Guard the stale-restore threshold default against drift.** `Tier 1 · Track 2 · deps: none (soft-sequence after SA53 — same file; also touches dr_engine/orchestration.py)`
  ...
  *Acceptance:* `orchestration.py`'s `restore_admin_uploaded_backup()` now accepts `stale_threshold_minutes: int = 30` as a backward-compatible keyword parameter, while the sole production caller (`admin.py`) injects `services.STALE_RESTORE_THRESHOLD_MINUTES` explicitly. A regression test uses `inspect.signature` to prove the default stays aligned with the module-owned constant and would fail on silent drift. The module-owned constant remains the single source of truth for the production value; the core default exists for backward compatibility and is tested for parity.
