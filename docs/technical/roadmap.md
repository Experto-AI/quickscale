...
- [ ] **SA117e-4 — Resume, seal, and verify split publication.** `Tier 2 · Track 3 · deps: none · AUTHORIZED end to end · REF-CEREMONY — NOT DELEGABLE`
  ...
  - **Run state (2026-08-17, in progress).** Evidence: `/tmp/quickscale-sa117e4-corrected-evidence-NAtJUx`; worktree `wt-track3` at `3c1b1b03`.
    - Phase 1 **passed** — `focused/version-check/check` exit 0, `quality` exit 2 with exactly the decision-1c tuple; drift gate classified 9/9 paths proceed.
    - Phase 2 **passed** — all twelve corrected-source module trees byte-identical to the published split roots, all twelve manifests `cmp`-equal at `0.87.0`, branch set exactly twelve plus teams, no pre-existing split tag, no remote core tag. **No republish is required.**
    - Phase 3 **partially passed** — local `0.87.0` rebound to `3c1b1b03` (tag object `4e7a8876`), backup `refs/sa117e4-backup/core-tag/0.87.0` = `4694483c` retained, tag **local and unpushed**. Installed plan proved the seven-prompt contract exactly once each (validating decisions 1d/1e against the real CLI). Branch apply accepted under decision 1f.
    - Phase 4 **passed** — twelve-row table frozen and inspected, digest `172bb2d00a7e4ac576a9c15d60eb439aec9d7f11685902ad703e42a480a06250`, confirmation record written non-interactively; `seal-status` reports all twelve `unsealed up to date` at the frozen SHAs.
    - Phase 5 **not started** — the seal aborted at module 1 with **zero tags pushed** (serial-abort worked as designed) on the `SA148` credential defect below. After the repo-local helper workaround, rerun `--seal-all` from the seal worktree. Remaining: twelve tag pushes, verification, default no-override proof, `splits/teams-module` deletion, closeout, merge-back.
    - Stale worktrees under `/tmp/quickscale-sa117e4-*` from earlier failed runs are still registered; prune them at closeout.
...
### SA148 — publication runner strips its own credential source

- [ ] **SA148 — Make publication Git credentials reachable, or say they are not.** `Tier 1 · Track: v88 backlog · deps: none`
  - `_publication_environment()` (`quickscale_core/src/quickscale_core/utils/git_utils.py:82`) sets `GIT_CONFIG_GLOBAL=os.devnull` and `GIT_TERMINAL_PROMPT=0`. A `credential.helper` configured in `~/.gitconfig` — the normal place — is therefore invisible to every publication push, which then fails with `could not read Username for 'https://github.com': terminal prompts disabled`. The docstring immediately above claims credential helpers "remain available"; that is false for a globally-configured helper.
  - Either preserve `credential.helper` (and its helper-specific config) explicitly through the sanitizer, or keep stripping it and state in the docstring, `--help`, and the seal runbook that publication requires repo-local or environment credentials. Do not weaken `GIT_CONFIG_NOSYSTEM`/`GIT_TERMINAL_PROMPT`, and never accept a token inlined in a URL or argv.
  - Add a regression proving a globally-configured helper either survives or produces an actionable error naming the repo-local requirement.
  - **Live workaround in place (2026-08-17):** `git config --local credential.helper store` in `/home/victor/code/quickscale/.git/config`. Repo-local config survives the sanitizer and reuses the existing `~/.git-credentials`. Reverse with `git config --local --unset credential.helper`.
  *(why → it blocked the SA117e-4 seal at module one, and the misleading docstring cost a full diagnosis cycle)*
