...
- **W1 — retain SA165 (#22).** A fresh terminal SA165-R1 completed over the narrowed four-file
  product candidate and returned **blocking**: `OPERATIONS.md.j2` changes generated bytes, while all
  three authoritative `OPERATIONS.md` hashes in `sa90_emission_manifests.json` still describe the
  pre-change output. The focused behaviour tests are green, but exact-manifest parity is not settled.
  The next action is a maintainer scope decision, followed by the bounded fixture/provenance repair
  and another fresh terminal review. `FROZEN-CHECK` and the single `EV-8` verdict remain prohibited.
  SA179 (#35) still owns documentation reconciliation and audit-note retirement after SA165 is green.
...
| **W1** | SA165 (#22) | **no — pending scope authority** — `deps: none`, but fresh SA165-R1 found stale exact-manifest hashes and the bounded repair must be authorized first | **no** — exact-manifest parity is unsettled, fresh SA165-R1 is blocking, and the `EV-8`-authorized verdict has not returned green | **yes after the bounded repair, fresh green review, and green verdict** — no cross-lane blocker remains | no |
...
**W2 and W3 are truly green; W1 is scope-blocked until the SA165 exact-manifest repair is authorized,
then cannot finish until a fresh SA165-R1 and SA165's `EV-8`-authorized final-candidate verdict are
green.** **No open ticket remains on the
...
**One maintainer decision is open: authorize the shape of SA165's exact-manifest repair.** A fresh
terminal SA165-R1 found that the product candidate changed rendered `OPERATIONS.md` bytes without
rebaselining the three authoritative manifest hashes. The required work cannot be omitted. Choose
between **A (recommended)** — widen SA165's bounded product repair to include only those three hashes
plus one provenance entry, keeping the later SA161 → SA160 rebaseline pair intact — and **B** — open
a separate W1 repair ticket immediately ahead of SA165 with the same frozen closure criterion. Either
choice stays on W1, preserves every unrelated fixture entry, requires fresh generation evidence and
another clean-byte terminal review, and grants no additional release run.

The previously settled decision remains granted as **`EV-8`**: exactly one replacement
...
|---|---|---|
| SA160, SA161 | generator templates + **SA90 emission-parity fixture**, `docs/others/tech-audit.md` | emitted output changes; their ordered pair remains intact after the bounded SA165 repair |
...
| SA165 | `quickscale_core/.../state_schema.py`, `scripts/test_isolation_conformance.sh`, `quickscale_core/tests/test_generator/test_generator.py`, `OPERATIONS.md.j2`, and the three `OPERATIONS.md` entries plus provenance in `sa90_emission_manifests.json` once repair scope is authorized | retained product discharge; fresh terminal review found the generated-output fixture stale. **Documentation surfaces moved to SA179 2026-09-05**, which keeps the candidate free of the files that record its own review |
...
- `sa90_emission_manifests.json` — the bounded SA165 repair, if authorized, updates only the three
  `OPERATIONS.md` hashes and appends its own provenance entry; SA161 then SA160 remain an ordered pair
  after it. Every rebaseline carries per-file rationale and preserves all prior entries.
...
- [ ] **SA165 — Discharge the tech-audit watch items that carry an action.** `Band C · Tier 3 · W1 · merge #22 · deps: none`
  ...
  **State (measured 2026-09-05): retained Phase D candidate; fresh product-only SA165-R1 returned
  blocking on stale exact-manifest hashes; bounded repair scope decision and final-candidate release
  verdict outstanding.**
  ...
  **Fresh terminal review result.** The product-only patch from pre-SA165 base
  `5a7ee9659e280c48a41eab176f3188bf3b6f59c9` to reviewed state
  `69f39e2b06654f3b80cd18bbad417736c7ef938f` received a blocking grade. The four intended behaviours
  and their focused tests passed, but `OPERATIONS.md.j2` adds rendered warning text while the
  `react_default`, `react_empty`, and `react_selected` manifest records retain the old
  `OPERATIONS.md` hash. The exact-manifest integration contract is therefore predictably red. A later
  integration sync did not alter the four reviewed product files or the fixture, so the finding
  remains the current blocker; the repair run must nevertheless establish a fresh tip binding.
  ...
  **Pending, in order.** (1) Resolve the open scope decision: authorize either the recommended bounded
  SA165 widening or a separate immediately preceding W1 repair ticket. (2) Sync W1 to current `v88`,
  re-establish the candidate binding, deliberately generate all three React variants, update only
  their `OPERATIONS.md` hashes in `sa90_emission_manifests.json`, append provenance describing only
  that emitted-file change, and preserve every unrelated path, mode, hash, and prior evidence entry.
  (3) Run the three `test_generated_tree_matches_manifest` variants and
  `test_operations_md_warns_generated_credentials_are_local_only`; retain the already-green state,
  isolation, and host-dependency evidence and fix any in-scope pre-existing defect the widened gate
  exposes. (4) Run completion-grade convergence over the repaired candidate, materialize a new
  base-to-tip patch, and obtain another fresh independent terminal SA165-R1; that review ends its
  root run. (5) In a later root run, revalidate the fresh review binding and execute `FROZEN-CHECK`.
  (6) Spend **`EV-8`** — the single
  ...
  **Constraints on the remainder.** Reviewed plan authority `EV-6` still governs `FROZEN-CHECK` and
  ...
  The next implementation run is limited to the authorized exact-manifest repair and must terminate
  at its fresh SA165-R1. Retain product object
  ...
  **Shared conflict surface:** `quickscale_core/src/quickscale_core/schema/state_schema.py`, `scripts/test_isolation_conformance.sh`, `quickscale_core/tests/test_generator/test_generator.py`, `quickscale_core/.../templates/OPERATIONS.md.j2`, the three `OPERATIONS.md` entries and provenance in `quickscale_core/tests/fixtures/sa90_emission_manifests.json`, `docs/others/tech-audit.md`.
