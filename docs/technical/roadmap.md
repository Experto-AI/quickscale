...
5. **SA117e** (embedded-manifest / split-branch version skew, Track 3) — **RELEASE BLOCKER, and Track 3's next action.** SA117a, SA117b, and SA117c are closed; SA117e covers the reviewed split push and is the sole open child. Its 2026-07-30 continuation is at a **recorded validation-blocked checkpoint**: no local release tag or public split was created, and the exact quiet/E2E gates must be remediated before publication may resume. `apply` embeds modules by git subtree from `splits/<module>-module` on the public remote, so embedded `module.yml` files are whatever was last published, not the working tree. The published splits predate the derivation sections v87's core requires, so `apply` fails for every module set. Approach decided (`SA117-DEC-001`): **stamp + assert in v87, pin in v88 (SA119)**; rules in [decisions.md §module-version-lockstep](./decisions.md#module-version-lockstep). Blocks SA96-PUBLISH **and SA112b–SA112f** (2026-07-26 sequencing decision: `SA117-DEC-002`). Next: **remediate `SA117E-QG-001`–`SA117E-QG-003`, then resume SA117e**.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Open findings and their owning child.** All SA117a/SA117b/SA117c findings are resolved (see [CHANGELOG.md](../../CHANGELOG.md)). `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker. SA117e now carries `SA117E-QG-001`–`SA117E-QG-003` plus open decision `SA117E-QG-DEC-001`; they must be remediated and validated before review/tag/push resumes.
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Tier 2 · Track 3 · deps: SA117b + SA117c` — **critical path; contains a human-only step**
    ...
    **RECORDED VALIDATION-BLOCKED CHECKPOINT (2026-07-30; status only; task remains open).**
    - **Done:** The resumed Track 3 worktree was clean and fast-forwarded to `v87` at `2466f738`; SA117b/SA117c and the human split-push authorization were confirmed. Discovery and the approved high-risk plan bound the complete inherited candidate to 50 included commits, 155 review paths, and twelve pending split refs. Before public mutation, two bounded quality-gate runs passed `make version-check`, manifest sync, the non-vacuous SA117 lock comparison, 660 focused tests, 9 mismatch/publication nodes, lint, format, MyPy, unit/integration coverage, `make smoke-install`, `make quality`, and `git diff --check`. The second run removed only stale test-owned Docker resources that had exhausted address pools. The worktree remained clean at the same HEAD/tree. **No source file, local release tag, origin tag, public split, TestPyPI/PyPI artifact, or package workflow was created or changed.**
    - **Pending / blocking:** `SA117E-QG-001` (**high**, validation infrastructure) — exact `make check QUIET=1` hangs in the redirected pytest/xdist recipe although the same component gates pass; `SA117E-QG-002` (**high**, completeness) — Core E2E collection cannot import `scripts.publish_module` under the runner's package-root `PYTHONPATH`; `SA117E-QG-003` (**high**, correctness) — CLI E2E `test_update_auto_commits_each_module_e2e` receives update exit 1. The two-run QG cap is exhausted. The mandatory full-scope source review, local tag, twelve split pushes, remote byte-parity proof, and installed all-module `apply` therefore did not run.
    - **Decisions needed:** `SA117E-QG-DEC-001` — a future continuation requires maintainer authorization for a separately discovered, planned, and reviewed remediation slice covering `SA117E-QG-001`–`SA117E-QG-003`; only after its exact quiet/E2E gates pass may SA117e resume. This run selected recorded checkpoint and merge rather than widening scope.
...
- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Tier 2 · Track 1 · deps: none · replaces SA122a's checker`
  ...
  **Why replacement rather than remediation (`SA122A-AUTH-001`, 2026-07-29).** The current extractors decide membership by regex over lines: `_extract_makefile_targets` re-implements Make's grammar by hand; `_extract_check_ci_serial_gates` / `_extract_check_ci_parallel_gates` and the workflow `run:` scrapers match `\bmake\s+([a-z][a-z0-9_-]+)` with prefix-based `echo`/`#` filters. That admits both false directions — a gate named inside a heredoc, a comment on a continuation line, or an uncalled function reads as covered, while `make $(GATES)`, a variable-composed target, an `include`d definition, or a gate reached through `check`'s delegation is invisible. Each remediation cycle would close one deceptive form and leave the next, which is the compounding shape arch Finding 11 exists to remove. The workflow-YAML half is already structural (`yaml.BaseLoader` with duplicate-key rejection) and is **kept**.
  ...
  2. **Shell** (`scripts/check_ci_locally.sh` serial and parallel lists, and workflow `run:` blocks) — parse to a syntax tree, or dry-run with a stubbed `make` on `PATH` that records argv. Quoting, heredocs, comments, and unreached code must resolve by construction, with no exception list.
  ...
  **File allowlist:** `scripts/check_gate_parity.py`, `scripts/test_gate_parity.py`, plus `pyproject.toml` only if a shell-AST dependency is chosen over the stubbed-`PATH` approach. `scripts/gate_registry.json` and the `Makefile` target are **inputs, not scope** — do not edit them.

  **Open decisions:** none. (Shell-AST library versus stubbed-`PATH` argv recording is an implementation choice for the scoped plan, not a maintainer decision — both satisfy the Verify line.)

  **RECORDED BLOCKED CHECKPOINT (2026-07-29; no executable delta retained; task remains open).**
  - **Done:** Track 1 was clean, synced to `v87`, and service-free prerequisites were ready. The maintainer fixed the implementation direction: no Bubblewrap or text/regex fallback; delegate semantics to GNU Make/Bash, keep all six inert adversarial forms, enforce repository-contained inputs, preserve direct exit 1 / Make exit 2 for the five publish omissions, and keep the checker standalone until SA122b. Two bounded fix/re-review rounds and one explicitly authorized post-cap re-plan were completed. Every rejected checker/test candidate was restored byte-identically before this checkpoint, so **no SA128 executable or test change is merged** and the existing checker remains non-authoritative.
...
**The milestone is still unclaimed; the 2026-07-30 Track 3 attempt found validation blockers as well as missing join evidence.** The exit criteria require `make check`, `make quality`, `make ci`, **and** `make ci-e2e` all green in one clean rerun. SA117e's two bounded QG runs left the non-quiet check components, `make quality`, unit/integration coverage, smoke-install, and focused SA117 gates green, but exact `make check QUIET=1` hung and `make ci-e2e` failed on `SA117E-QG-002`/`SA117E-QG-003` after stale Docker-resource exhaustion was cleared. The exact four-command join therefore remains unproved, and SA117e cannot proceed to public mutation until `SA117E-QG-001`–`SA117E-QG-003` are remediated and the exact gates pass. SA112f still owns the eventual clean four-command closeout rerun; SA96-PUBLISH still requires that evidence before release.

The two former complexity blockers were closed by SA126 (Track 1), so the green-gate unblock no longer sits behind the SA117 chain's plan gates. SA121 is closed and its merge-base monotonicity gate is live in `make quality`; SA127 is closed at `a2eb7a6c`; SA128's continuation is in process on Track 1, with SA129 then SA130 behind it and SA122b last.
...
──────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA128 (structural parity check)         SA115 (e2e xdist; deps: none)  QG authorize + remediate ◄─ next
  │  deps: none · replaces SA122a        │  validation AUTHORIZED       │  DEC-001 / QG-001–003
  ▼  CONTINUATION IN PROCESS             │  cannot finish → SA112f      ▼  exact quiet + E2E green
SA129 (superuser probe; Tier 2)         │  cannot merge  → SA112e      SA117e (review + push splits)
...
**Diagram checkpoint (2026-07-30, third pass).** Track 3's next action is `SA117E-QG-DEC-001` plus remediation of `SA117E-QG-001`–`SA117E-QG-003`; only a green exact quiet/E2E rerun returns to SA117e review/tag/push. SA117a/b/c are closed and merged, so no closed node remains on the diagram. Track 1 begins at SA128, whose continuation is **in process on `wt-track1`** (decision-recording commit `c4e6560d`, not yet merged to `v87`).
...
**Track 1 is internally serial and now entirely off the critical path.** SA128's continuation is in process, SA129 and SA130 remain queued behind it, and SA122b then carries the SA112e merge bound. The SA128/SA122b governance chain is off-path; SA129 and SA130 are also off-path, non-release-blocking product defects. Track 1's release blocker (SA127) is closed.

**Critical path:** `SA117E-QG-DEC-001 + SA117E-QG-001–003 remediation → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. Authorization/remediation is Track 3's immediate next action; SA117e review/tag/push resumes only after the exact quiet/E2E gates pass.

**Parallelism.** The open work occupies all three tracks: `SA117E-QG-DEC-001` + `SA117E-QG-001`–`003` remediation → SA117e → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and in-process SA128 → SA129 → SA130 → SA122b on Track 1. **The repository runs exactly three worktrees; a fourth is not created** (`SA112A-TRACK-001`, resolved 2026-07-28 — see Standing decisions).
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28; re-costed 2026-07-30) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 leads with **in-process** SA128, followed by SA129, SA130, and SA122b (SA127 ✓ closed), while Track 3 has SA117e ahead of SA112a. **The cost rose this pass:** SA128 is no longer merely blocked, it is being worked, so taking the move now preempts in-process work rather than displacing a stalled queue. **Recommendation: decline for now** and revisit if SA128 caps again. Reusing Track 1 still requires no fourth worktree, but it would delay the whole remaining Track 1 queue.
  ...
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* Track 1's queue head. That head is **in-process SA128**, with SA129 and SA130 queued before SA122b, so the move would preempt live work and delay the full off-path queue to advance a critical-path node; it no longer trades against a release blocker.
...
> **Re-verified on 2026-07-30 (third pass).** SA117a/b/c are closed and fully merged; SA117e reached the recorded validation-blocked checkpoint above before any public mutation. Before this checkpoint edit, `wt-track3` carried **zero** commits unmerged into `v87`; this docs-only checkpoint is the sole pending Track 3 delta. **`wt-track1` carries `c4e6560d` (SA128 decision-recording, documentation only) — Track 1 is in process**; `wt-track2` still carries `5193f198` + `5b5de830` and remains closed to new work.
>
...
- **Track 1 (SA128 → SA129 → SA130 → SA122b) — start: YES · finish: NO · merge: YES. NOT truly green. In process.**
  - **Can start — YES, and it has started.** SA128 has `deps: none`, no open product decision, and service-free validation; its fresh continuation is under way on `wt-track1` with the implementation decisions ratified there. SA129 and SA130 remain queued; SA122b remains last with its stated SA112e merge bound.
...
- **Track 3 (SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: NO · merge: YES (checkpoint only). NOT truly green, and on the critical path.**
  - **Can start — YES, and it started.** SA117a/b/c are closed and merged; SA117e's dependencies and human split-push authorization were satisfied before the validation checkpoint.
  - **Can finish — NO. Validation blockers, clearable only through a separately authorized remediation slice.** `SA117E-QG-001`–`SA117E-QG-003` keep the exact quiet/E2E gates red at the two-run QG cap; `SA117E-QG-DEC-001` must authorize remediation before SA117e can resume.
  - **Can merge — YES for this docs-only checkpoint, not for SA117e completion.** SA117e has no cross-track merge-order gate, but no executable approval, local tag, public split, parity proof, or installed all-module apply exists to merge or claim.
  - **Next action: decide and remediate `SA117E-QG-001`–`SA117E-QG-003`, then resume SA117e.** Do not publish splits, start SA112, or treat the umbrella candidate as release-ready until SA117e is closed.
...
**Net.** The Track 3 critical path is **`SA117E-QG-DEC-001` + `SA117E-QG-001`–`003` remediation and green exact gates → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1's release blocker is resolved; in-process SA128 is followed by SA129, SA130, and SA122b, all off-path, with SA122b retaining the SA112e merge bound. Track 2 remains merge-gated.

**Two open decisions:** `SA117E-QG-DEC-001` — whether to authorize a separately planned/reviewed remediation slice for the three exact-gate blockers before SA117e resumes; and `SA112A-TRACK-002` — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but at the cost of preempting in-process SA128 and delaying its successors) or leave it behind SA117e. `SA127-ORDER-001` and `SA122A-AUTH-001` are both resolved. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: two.** **`SA117E-QG-DEC-001`** — authorize or decline a separately discovered, planned, and reviewed remediation slice for `SA117E-QG-001`–`SA117E-QG-003`; default while undecided: SA117e remains blocked and no public mutation occurs. **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of preempting **in-process** SA128 and delaying its queued SA129 → SA130 → SA122b successors; it changes no track's three states. **Recommendation: decline the move while SA128 is live. Default while undecided: SA112a stays on Track 3; Track 1 continues SA128 → SA129 → SA130 → SA122b.**
