...
SA128d → SA122b-1 → … → -5             SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  ...
  ▼  Make · Bash · YAML · contracts   │  validation AUTHORIZED      │  BLOCKED — recorded partial
                                      │  cannot finish → SA112f     │  Phases 2–3 accepted; Phase 5
                                      │  cannot merge  → SA112e     │  stops at stage 10 before gate
SA122b-1 → -2 → -3 → -4 → -5          │                             │
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **no** — the maintainer selected recorded partial delivery after Phase 5 proved the stage-11 negative control unreachable; resumption needs the acceptance decision below | **no** — `make ci-e2e` has not run at the current tip and the task remains unchecked | **yes** — this documentation-only recorded-partial checkpoint is authorized; no order gate | no | ✅ **yes** — blocked on a maintainer acceptance decision |
| **Track 1** — SA128d | **yes** — `deps: SA128c ✓ closed`, no order gate, no persisted cap | **yes** — SA128d's own two-file scope depends on no other track's output | **yes** — no order gate | ✅ **yes** | no — Track 1's only critical-path node (SA112a) is closed |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is blocked on the critical path at SA117e-3.** The corrected ephemeral harness and lifecycle wrapper received full-file pre-use approval at their exact current hashes, and the interruption proof restored the frozen PostgreSQL container after a TERM in the stop-effect-before-bookkeeping window. The later local-CI negative control cannot reach its intended stage-11 PostgreSQL gate: with host PostgreSQL stopped, stage 10's `make test-cov REQUIRE_BACKUPS_COVERAGE=1` fails first. The maintainer selected recorded partial delivery, so `make ci-e2e` remains unrun at the current tip and SA117e-3 remains unchecked pending the acceptance decision in its checkpoint. **Track 1 is truly green at SA128d**, off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear. SA117e-4 still needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path, **blocked at SA117e-3 after recorded partial delivery**. Phases 2–3 are accepted locally: exact harness/wrapper bytes were approved and the interruption proof restored the original PostgreSQL state/readiness. Phase 4's public-split observation was diagnostic only and transfers to human-gated SA117e-4. Phase 5 stops at local-CI stage 10 before the intended stage-11 host-down gate; `make ci-e2e` has not run at current tip `2eae365a`, so SA117e-3 remains unchecked. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4. Do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Current checkpoint (`SA117E3-HANDOFF-003`, 2026-08-04; recorded partial delivery selected; task unchecked; no tag, push, or publication).** Acceptance evidence was gathered at source tip `9f5092f719e59a38ae6bed5f0bc6f27934cab7cf`; Track 3 and `v87` are now synchronized at `2eae365a490c8dbfe074aa8b3a3ab040b3eaee68`. The earlier nine-phase plan is superseded at Phase 5 because its stage-11 oracle is unreachable under the current local-CI ordering.
      - **Done:** Phase 2's worktree-local ignored candidates received fresh full-file approval at exact SHA-256 harness `68787fd487795b4c9dfae9e87448dc495eade65c0efdcb89b38364582ea9109e` and lifecycle wrapper `41741154a84e26f3944464e40c4eb297c0f9985b94fe0ff560c7351922213dfd`, resolving `F-003`–`F-010`. Phase 3 then proved the stop-effect-before-bookkeeping interruption: after `STOP_EFFECT_OBSERVED`, TERM produced exit 143 and restored the same immutable PostgreSQL container, configuration projection, running state, and readiness with no `RESTORE_FAILED`; evidence is `.quickscale/sa117e3/evidence/phase3-2026-08-04T164407Z-784108/`. Phase 4 confirmed all twelve source SHA checks but found the public analytics split still carries version `0.80.0` rather than core `0.87.0`; that was diagnostic only and belongs to SA117e-4's human-gated public mutation.
      - **Pending-Blocking:** Phase 5 evidence `.quickscale/sa117e3/evidence/phase5-20260804T185416Z/` shows `bash scripts/check_ci_locally.sh --e2e` fails in stage 10 while PostgreSQL is intentionally unavailable because `make test-cov REQUIRE_BACKUPS_COVERAGE=1` needs PostgreSQL. The source-owned stage-11 PostgreSQL-not-available branch and banner are therefore not reached, so the approved negative-control oracle is unproved. The wrapper restored the same host container and readiness successfully. Phase 6 `make ci-e2e` has not run at current tip `2eae365a`; no completion, release-readiness, tag, split push, installed public apply, or publication is claimed.
      - **Decisions needed:** before resuming, the maintainer must choose one acceptance boundary and obtain a revised scoped plan: either provide an alternate task-owned PostgreSQL endpoint so stage 10 remains green while the frozen host endpoint is unavailable for stage 11, or revise/remove the stage-11 negative control because the current command ordering makes its oracle unreachable. The recorded-partial merge authorizes preservation only and waives no validation or review gate.
    ...
    **Carried plan-review ledger.** From the superseded monolithic execution plan: `F-001` is resolved at plan-detail level and carried by SA117e-4; `F-002` was resolved at plan-detail level by SA117e-3's scoped plan review (DC-25). The separate `F-003`–`F-010` pre-use ledger is resolved at the exact hashes recorded in `SA117E3-HANDOFF-003`; SA117e-3 is now blocked by the Phase 5 acceptance-boundary decision, not by executable approval. Full history is in [CHANGELOG.md](../../CHANGELOG.md).
...
- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Umbrella · deps: none`
  ...
  **SA128a, SA128b, and SA128c are review-approved and accepted for integration** (Make semantics via `-qp`/`--dry-run`; shell inventories observed by actual Bash; hosted, publish, and E2E structure observed — evidence in [CHANGELOG.md](../../CHANGELOG.md)). This is the reviewed pre-integration status; no closeout commit or merge is claimed here. No SA128a, SA128b, or SA128c finding remains open except **SA128a/F-003**, a medium/advisory `_communicate_bounded` resilience item owned by **SA128d**, separate from SA128b's F-003.

  - [x] **SA128c — Extend structural YAML to hosted, publish, and E2E.** `Tier 2 · deps: SA128b`
  ...
  **Attempts and outcomes.** Both monolithic attempts were reverted before any commit, with nothing dangling in reflog or stash, so their designs are lost. Every SA128 child attempt appends a row here **before** its checkpoint, so a design rejected in one domain is visible to the others. The three closed children (SA128a, SA128b, SA128c) returned full-scope `STATUS: ok`; their evidence is in [CHANGELOG.md](../../CHANGELOG.md) and is not restated here.
  ...
  | 3 | structural hosted/publish/E2E parsing with bounded Bash observation for publish `run` blocks | F-001, F-002 | 155 tests, five focused regressions, strict MyPy, Ruff, `make typecheck`, `make quality`, `git diff --check` — all green; five publish omissions remain | review pass 1 found F-001 (medium) and F-002 (high); review pass 2 `STATUS: ok`, both resolved | accepted; SA128c complete and SA128d remains the next pending child |
...
**Open decisions — three, all the maintainer's.** **SA117e-3 is blocked today** on choosing an alternate task-owned PostgreSQL endpoint for stage 10 or revising/removing the unreachable stage-11 host-down negative control. The other two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
