...
1. **SA101** (quality-baseline remediation, Track 3) → **SA96-GATE** (green-gate join). SA93's dependency is met; SA96-GATE is the release-path join but **blocked** on quality-baseline regressions (SA96-GATE-BLK-002), whose decision is now made (Option A — remediate) and cut as **SA101**. SA101 implementation is complete (checkpoint `SA101-IMPL-001`: all 19 regressions cleared, `make quality` exit 0) and **validation is resolved** — `SA101-VAL-001` was closed via option (a) (2026-07-19 experiment: the restricted-role CRM/forms failures reproduce identically on clean pre-SA101 and are confirmed pre-existing; SA101 is green at HEAD). Track 3's remaining closeout is assistant-executable: `make check` + independent change-review, then mark SA101 `[x]`. The prior coverage blocker (SA96-GATE-BLK-001) is superseded by the restored canonical coverage evidence.
...
4. **2026-07-19 audit-sweep remediation** — the delta pass opened two tech-audit findings (**TA60** frontend-proof-ungated, **TA61** devtools-gates-absent) and promoted arch-audit **Finding 10** (frontend-source specialization). **SA102** (TA61) has landed on Track 1 (see [CHANGELOG.md](../../CHANGELOG.md); TA61 closed), and **SA103** (TA60 + the matching arch red flag) is complete with its feeder condition met for the **SA96-GATE** join; no SA103 blocker or open decision remains. The staged frontend workstream **SA104 → SA105/SA106** (Finding 10, Track 2) remains. The SA104 chain is off the release critical path. Arch **Finding 7** stays unscheduled (SA104 shrinks its surface — sequence any tuple-derivation work after it); Findings 2/4 remain **not ticketed** with teams.
...
- **Checkpoint `SA101-IMPL-001` (implementation complete — validation incomplete):** nine serial behavior-preserving phases cleared all 19 baseline regressions (ledger 19→0) through file-local extraction, direct-identity aliases, and CRM/forms migration compaction proven byte-parity (SHA-256-matched deconstruction/reapply/reverse-to-zero probes; SA92 squash guardrail + targeted CRM/forms migration tests pass). `make quality` reaches exit 0 with zero baseline regressions; no baseline/threshold/gate changes; the v87 shrink-only policy is recorded in [decisions.md](./decisions.md) §5. Plan-review pass 2 returned `STATUS: ok` (SA101-PR-001/002/003 resolved). Full detail in [CHANGELOG.md](../../CHANGELOG.md). **`make check` and the full implementation change-review have not yet run.**
  - **`SA101-VAL-001` — RESOLVED via option (a) (2026-07-19 experiment; no maintainer decision remains).** The reported restricted-role failures were reproduced on **clean pre-SA101** (`d7ccf24b`) and confirmed **pre-existing, not caused by the compaction**: CRM **3 failed / 281 passed / 3 errors** on clean pre-SA101 (exact match to the report; errors are `InsufficientPrivilege: new row violates RLS policy for crm_company` — the *old* migration test inserted rows under `NOBYPASSRLS` without org context) vs **284 passed / 0 failed / 0 errors** at HEAD with SA101. Forms is green in both states (212 passed). SA101's Phase 9 rewrite of `crm/tests/test_migrations.py` **fixed** the pre-existing fixture defect, so options (b) fixture-repair and (c) rollback are both moot. Method: same environment, restricted `quickscale_test_role` (`NOBYPASSRLS`), `QUICKSCALE_ALLOW_BYPASSRLS=1` to include the migration/RLS tests; trustworthy because the RLS-negative isolation tests pass only under a genuine restricted role.
  - **Hand-off — Track 3 closeout (assistant-executable, no decision pending):** (1) run `make check` on v87 (must exit 0 — behavior-preserving refactor, no gate change); (2) obtain an independent full implementation change-review (`STATUS: ok`); then (3) mark SA101 `[x]`, and (4) re-run the **SA96-GATE** four-command join with SA103's feeder condition met. Until (1)/(2) land, `make check`/`make ci`/`make ci-e2e` must not be represented as run.
...
Track 1 was idle and carries the **TP (test-parallelization)** suite plus the completed, rebalanced **SA103** frontend-proof gate. TP goal: shorten the SDLC feedback loop (and let an AI assistant run partial tests concurrently) by parallelizing the long-running quality gates. Motivation and full analysis: the parallelization audit summarized under *(why →* …*)* on each ticket. **The integration suite is already parallel (SA91, `QS_INTEGRATION_JOBS`)**; **TP1** (serial static-gate fan-out), **TP2** (unit-suite xdist), and **SA103** are complete (see [CHANGELOG.md](../../CHANGELOG.md)), while TP2b implementation and focused evidence are landed — its `Makefile` `test-cov` edits are already merged to v87 — but its closeout is validation-blocked pending independent review. The remaining Track 1 tickets are **TP2b** (make `test-cov` xdist-safe via `coverage combine`) and **TP4** (docs-only AI fast-loop recipes). **The E2E-concurrency sub-chain (TP3a/TP3b) was rebalanced to Track 2** (2026-07-18) and is complete. SA103 has met its release-feeding condition; TP2b/TP4 are pure cycle-time improvements and must not regress any existing gate's pass/fail set or coverage thresholds.

- [x] **SA103 — Wire the rendered-frontend proof into a blocking gate (TA60 + arch red flag).** `Tier 2 · Track 1 (rebalanced from Track 3, 2026-07-19) · deps: none · feeds SA96-GATE · complete`
  **Completion evidence (2026-07-19):** `ci.yml` has a blocking `lint-frontend` job with Node/pnpm/cache setup; the local `make ci` / `scripts/check_ci_locally.sh` frontend-lint path is guarded only when Node or pnpm is absent, while real failures block; and `publish.yml` runs `frontend-proof` before downstream build/publish. A deliberate injected TypeScript error made `make ci` exit 2 with `TS2322`/`TS6133`; `Dashboard.tsx.j2` was restored byte-identically; `make frontend-proof` passed. QG1's 15 focused tests and bounded `PYTEST_XDIST_WORKERS=2 make ci` passed (4,536 core/CLI tests, 323 backups tests, 2 expected skips, 92.88% mean coverage with every file ≥80%, integrations green); QG2's 19 focused tests plus six delta tests and Ruff/MyPy/YAML/workflow-order/Bash/Make/diff checks passed.
  - **Review and residual caveat:** Independent change-review pass 2 returned `STATUS: ok` and resolved **SA103-REV-001**, **SA103-REV-002**, and **SA103-ADV-001**. No blocker or open decision remains for SA103. Hosted GitHub Actions were not remotely observed; this is a non-blocking residual caveat.
  - **Chain (cross-track, non-blocking):** SA104 (Track 2) turns the 60 byte-static `.j2` files into real `.ts`/`.tsx` the repo's ESLint/tsc can gate directly, retiring this finding's structural cause. SA103 is complete and is not blocked on SA104.
...
**CI-gate remediation from the 2026-07-19 sweep — both tickets now on Track 1.** Two audit findings landed on the CI/release plumbing, and both were rebalanced off this track to run in parallel with SA101. **SA102** (TA61, trivial) landed on Track 1 on 2026-07-19 (TA61 closed). **SA103** (TA60, frontend-proof gate) was **rebalanced to Track 1 on 2026-07-19** and is now complete: with TP2b's `Makefile` `test-cov` edits already merged to v87, SA103's files (`ci.yml`/`publish.yml`/`Makefile` umbrellas + `scripts/check_ci_locally.sh`) were file-disjoint from live Track 1 work, and its release-feeding condition is met. Track 3 retains **SA101** and still owns the SA96-GATE join; the join must not be represented as run until its own closeout and re-verification occur.
...
the TP + frontend chains run in parallel, off the
        ...
        SA103 ✓ (Track 1) ── frontend-proof gate (feeder condition met; parallelized with SA101)  ┘ feed
                       │  SA101 impl complete SA101-IMPL-001: 19→0, make quality exit0, CRM/forms migration parity proven;
                       │  SA101-VAL-001 RESOLVED via (a) — failures reproduce on clean pre-SA101 (pre-existing), SA101 green at HEAD;
                       │  remaining closeout: make check + independent change-review (assistant-executable)
                       ▼
        SA96-GATE ── green-gate join (make check/quality/ci/ci-e2e)  deps: SA96-T1 ✓ + SA96-T2 ✓ + SA93 ✓ + SA101 + SA103 ✓
                       ▼                        (SA101 validated; once its make check + change-review land, the four-command join can re-run)
        SA96-PUBLISH ── build → publish-test → publish-prod          deps: SA96-GATE  (human-only)
```

**Open work only:** Track 1 `TP2b` (validation-blocked) → `TP4`, Track 2's frontend chain `SA104 → SA105/SA106` (arch Finding 10), Track 3 `SA101` (clears BLK-002; on the critical path), and the release join
`SA96-GATE → SA96-PUBLISH`. The frontend chain (SA104+) owns the `quickscale_core` generator + `showcase_react` template tree only, disjoint from the TP work and the SA103 CI surfaces, so the remaining workstreams run concurrently. All of TP and the frontend chain are off the release critical path; **SA103 is complete and its feeder condition is met**, while the SA96-GATE join still awaits SA101 closeout and re-verification. None may regress any gate's pass/fail set or coverage thresholds. `TP4` is docs-only and unblocked (TP1 done, TP3b merged to `v87`); it remains pending.

**Critical path.** Both pre-publish module sweeps are complete: **SA96-T1** (Track 1) and **SA96-T2** (Track 2). **SA93** is complete and its dependency is met; the remaining critical-path chain is **SA101** (clear the BLK-002 quality regressions, Track 3) → **SA96-GATE** (green-gate join) → **SA96-PUBLISH** (human-only). **SA103** (frontend-proof gate, Track 1) is complete and its gate-tightening feeder condition is met; it ran concurrently with SA101. **SA101 implementation is complete (`SA101-IMPL-001`, 19→0 regressions, `make quality` exit 0, CRM/forms migration parity proven) and validation is resolved — `SA101-VAL-001` was closed via option (a)** (2026-07-19 experiment: the restricted-role CRM/forms failures reproduce identically on clean pre-SA101 → confirmed pre-existing, not caused by the compaction; SA101 is green at HEAD, 284 CRM / 212 Forms passed). No maintainer decision remains; the assistant-executable closeout (`make check` + full change-review, then mark SA101 `[x]`) can proceed, after which the SA96-GATE join can re-run. **SA98** and **SA100** are complete independent audit-remediation tickets; neither blocks SA96-GATE. SA97, SA99, and SA102 are also complete (see [CHANGELOG.md](../../CHANGELOG.md)).
...
- **Track 1 — ACTIVE; carries completed SA103 alongside the blocked TP2b and pending TP4.** **Rebalance (2026-07-19):** two CI-gate tickets moved here from Track 3 — **SA102** (TA61 devtools CI gate, Tier 1) **landed** (TA61 closed; see [CHANGELOG.md](../../CHANGELOG.md)), and **SA103** (TA60 frontend-proof gate, Tier 2) was moved here and completed in parallel with Track 3's SA101 now that TP2b's `Makefile` `test-cov` edits are already merged to v87, leaving SA103 file-disjoint from all live Track 1 work (TP2b/TP4). SA103's **SA96-GATE** feeder condition is met; no blocker or open decision remains for SA103. **TP1** and **TP2** were accepted 2026-07-18; **TP2b** remains validation-blocked by **QG-TP2B-001** pending exact post-fix default and required-backups validation plus final independent review; **TP4** remains pending (docs-only, unblocked). The E2E sub-chain **TP3a/TP3b** was rebalanced to Track 2 and is complete. TP2b/TP4 are off the SA96 release critical path and must not regress any gate's pass/fail set or coverage thresholds. Evidence for closed work is in [CHANGELOG.md](../../CHANGELOG.md).
...
- **Track 3 — SA93 COMPLETE; SA101 validated (`SA101-VAL-001` resolved via (a)); hand-off-ready for assistant closeout.** Finding 1, all four GATEs, SA91, SA100, and SA93 are complete. SA101 implementation remediates all 19 BLK-002 regressions through nine serial phases (file-local extraction + direct-identity alias refactors + migration-local compaction) with `make quality` reaching exit 0 and exact migration parity proven for CRM/forms (SHA-256-matched operation/catalog/data/reverse/reapply probes). Independent plan review pass 2 returned `STATUS: ok` (SA101-PR-001/002/003 resolved). **`SA101-VAL-001` is resolved: the 2026-07-19 experiment reproduced the restricted-role CRM/forms failures identically on clean pre-SA101 (`d7ccf24b`) — confirming they are pre-existing, not caused by the compaction — while SA101 at HEAD is green (CRM 284 / Forms 212 passed under `NOBYPASSRLS`). SA101's Phase 9 rewrite of `crm/tests/test_migrations.py` fixed the pre-existing fixture defect.** No maintainer decision remains. **Remaining Track 3 closeout (assistant-executable, no cross-track dep):** run `make check` (must exit 0), obtain an independent full implementation change-review (`STATUS: ok`), then mark SA101 `[x]` and hand to the SA96-GATE four-command join (SA103 feeder condition met). **The 2026-07-19 audit-sweep CI-gate tickets were rebalanced off this track to Track 1** — SA102 landed, and SA103 (frontend-proof gate) now parallelizes SA101 rather than queuing behind it here; both still feed SA96-GATE. The PyPI publish (SA96-PUBLISH) remains explicitly excluded from assistant work (human-only). SA91 retains CR-SA91-REV-006 (low/advisory); SA89B-CR-004, SA93-REV-005, and SA93-ADV-001..004 remain non-gating advisories.


**Net — all three tracks have assigned work; the 2026-07-19 audit sweep added five tickets (SA102 landed + SA103 rebalanced onto Track 1, SA104→SA105/SA106 on Track 2). One maintainer decision remains open — SA105 (dormant frontend files); Track 3's `SA101-VAL-001` decision is now closed (resolved via option (a); failures confirmed pre-existing, SA101 green at HEAD).** Track 1 carries completed **SA103** (frontend-proof gate, rebalanced from Track 3 to parallelize SA101) plus the off-critical-path TP test-parallelization suite, with **TP2b pending/blocked** on exact post-fix validation and final independent re-review and **TP4** pending; Track 2 carries the completed E2E-concurrency sub-chain (**TP3a** and **TP3b** complete) plus the new arch-Finding-10 frontend chain (**SA104** safe-now, then **SA105/SA106**); Track 3 carries the critical-path **SA101**, whose implementation and validation are complete as recorded above, with remaining closeout the assistant-executable `make check` + full change-review. Both pre-publish module sweeps and SA93 are complete, so **SA96-GATE** is next but remains **blocked** — BLK-002 implementation is cleared and SA101 is validated, but SA101's `make check` + change-review closeout remains before the four-command join can re-run; **SA103's feeder condition is met**. **SA98**, **SA100**, SA97, SA99, and SA102 are landed; no TP2b design decision is pending. The squash-migrations decision, bounded guardrail strategy, and v87 shrink-only quality-maxima policy are recorded in [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92); detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
