...
**Truly-green open tickets:** none. `SA112f` remains the head of the longest dependency chain, but `SA112F-QG-001` blocks its start and finish states. No off-path filler ticket is assigned.
...
- [ ] **SA112f — Run ordered acceptance, review, and close SA112.** `Tier 2 · Track 3 · deps: SA112d + SA143 complete`
  ...
  - **`SA112F-QG-001` (blocking, hard acceptance dependency):** default two-worker runs reached green Core E2E but failed CLI E2E amid severe npm-registry stalls and Docker service pressure, including one frontend exit 137; serial umbrella attempts exceeded their watchdogs without a verdict. This evidence does not isolate a product defect, so no executable change is justified and the provisional xdist speedup remains unconfirmed.
