...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  ...
  - **Blocked checkpoint (2026-07-23; continued 2026-07-24 — implementation and partial final-sync validation recorded; task remains open):**
    ...
    - **Decisions needed:** None on implementation or acceptance. On 2026-07-24 the maintainer selected **continue iterating** — keep re-running the literal gate chain rather than waiving ORDER-001. **Handoff for the next executor:** the only unmet precondition is environmental, so (1) restore sustained `registry.npmjs.org` throughput for generated-project Docker builds *or* configure a maintainer-approved compatible mirror, verifying it can carry a full `pnpm install` (not just a metadata probe); (2) from a clean `wt-track1` synced to latest `v87`, run the exact chain `make check && make quality && make ci && make ci-e2e` in one window; (3) require all four exit 0 with quarantine empty; (4) obtain independent full-scope change review `STATUS: ok`; (5) only then mark SA114/SA116 `[x]` and add the completion CHANGELOG entry. The `scripts/test_e2e.sh` low-memory guard + `QS_E2E_HEARTBEAT_INTERVAL` mitigation (landed on `v87`, see CHANGELOG) is already in effect and targets the hang-mode failure; the remaining blocker is npm-registry throughput only.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Tier 2 · Track 3 · deps: SA110 ✓ · SA111a ✓ · SA113 ✓`
  ...
  *C. Runtime root-fix (`SA112` Phase 1 — pending traceback, not pre-selected).* The prior prototype's installed `apply` raised `NameError: name 'QUICKSCALE_BILLING_ENABLED' is not defined` during managed-wiring regeneration; the `apply` wrapper collapses the exception to `str(exc)` so the raising frame is not yet known. Static pointer for the continuation (do **not** treat as the fix): `QUICKSCALE_BILLING_ENABLED` is a manifest-derived setting (`spec.settings`, see `quickscale_cli/tests/test_manifest_entry_point_integration.py:350`); the bare-name evaluation is reached only through `_materialize_implied_module_configs` → managed-wiring regeneration from an **installed** context. Phase-1 capture procedure: run step B.2 once with the wrapper's `except` temporarily re-raising (or `logging.exception`) on a disposable branch, save the full traceback to an artifact file, select the root fix **only** from the actual raising frame, then revert the diagnostic branch before implementing. Surface a decision to the maintainer only if the frame admits multiple contract-valid fixes with materially different compatibility consequences; otherwise apply the traceback-selected technical fix (expected to follow the SA109/SA113/AF7 bundled-fallback precedent).
