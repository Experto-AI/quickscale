...
SA128d → SA122b-1 → … → -5             SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  ...
  ▼  Make · Bash · YAML · contracts   │  validation AUTHORIZED      │  revised plan approved; blocked
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **no** — the narrow `SA117E3-BLK-001` correction plan is approved, but its fresh execution/review budget was declined after plan review consumed the available round | **no** — no tracked correction ran, so the stage-11 oracle and Phase 6 remain unproved | **yes** — the recorded-partial checkpoint has no order gate | no | ✅ **yes** — on the critical path |
...
**Status:** on the critical path, **blocked at SA117e-3 after a third recorded partial delivery**. Phases 2–3 remain accepted locally: exact harness/wrapper bytes were approved and the interruption proof restored the original PostgreSQL state/readiness. The revised Phase 5 plan received scoped plan-review `STATUS: ok`, but its first bounded run stopped in stage 10 on `SA117E3-BLK-001`: one existing backups test hardcodes expected restore port `5432` instead of honoring process-local `QS_BACKUPS_DB_PORT`. The wrapper restored the exact host container/readiness and cleanup completed. A separately authorized narrow correction plan now exists and is independently approved, including default/nondefault test proof, exact digest provenance, review-before-commit byte identity, merge-back, and safe negative-control resumption; **none of it was executed** because the plan-review round consumed `plan_review_cycles=2/2` and the maintainer declined a new execution/review round. Clean continuation requires that fresh authorization, then implementation of the approved three-file correction before the stage-11 banner or `make ci-e2e` can be claimed. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4. Do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Continuation checkpoint (`SA117E3-HANDOFF-005`, 2026-08-05; recorded partial delivery; no tracked correction).** **Done:** the maintainer authorized the narrow fix; the planner fixed the exact three-file allowlist (`quickscale_modules/backups/tests/test_services.py`, this roadmap, and `CHANGELOG.md`), the one-expression dynamic-port expectation, default and redirected focused proofs, broader backups/Ruff gates, immutable `13bca454…` versus canonical `547b44b8…` provenance, pre-commit full-scope review, reviewed-patch digest identity, exact merge-back, and safe negative-control/`make ci-e2e` resumption. Fresh plan-review pass 1 returned `STATUS: ok`, resolving `F-001`/`F-002`; pass 2 returned `STATUS: ok`, resolving `ORCH-GATE-001`. **Pending-Blocking:** implementation did not start — the two successful passes consumed the user-authorized round's `plan_review_cycles=2/2`, and the control plane refused the first preflight/implementation dispatch because no review allowance remained for its future delta. `SA117E3-BLK-001` and `SA117E3-EVID-001` therefore remain open; no test, production, evidence, Docker, or host-service mutation followed this checkpoint. **Decisions needed:** `SA117E3-EXEC-001` — authorize one fresh execution/review budget round, then execute the already-approved plan verbatim. The maintainer selected stop-and-record for this session; that choice is not a finding waiver or completion claim.
...
**Open decisions — three, all the maintainer's; one blocks Track 3 today.** `SA117E3-EXEC-001` requires one fresh execution/review budget round for the already-approved narrow correction; the maintainer authorized and approved its design but selected stop-and-record when the plan-review allowance was spent before implementation. The other two decisions are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
