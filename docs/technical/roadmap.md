...
The green-gate join (SA96-GATE), the installed-wheel discovery/resolver chain (SA109 ✓, SA110 ✓, SA113 ✓, SA111a ✓, SA111b ✓), the Track 2 frontend de-specialization chain (SA104 → SA108 ✓), the Track 1 re-verification chain (SA114 ✓, SA116 ✓), SA120 (quiet `check` parity), SA121 (baseline monotonicity — **closes arch-audit Finding 12**), SA125 (file line-ceiling retirement), SA126 (module-command complexity remediation), SA127 (zero-module installed `apply`), and SA117b (split-publication safety) are closed; detail lives in [CHANGELOG.md](../../CHANGELOG.md). The remaining open work is grouped as the five entries below — SA117 (split into serial children on 2026-07-27 as `SA117-DEC-003`; SA117a/b/c are closed) remains a release blocker through its sole open child SA117e, while SA128 → SA122b are the remaining gate-governance chain. The Track 1 queue is **SA129 → SA130 → SA128 → SA122b** (reordered 2026-07-30, `SA131-ORDER-001`): **SA129** (superuser probe defeated by Django 5.2+ shell auto-imports) and **SA130** (dead Poetry timeout variable in the generated Dockerfile) are now the head — both found by the manual `apply` session on 2026-07-29, both Tier 2, `deps: none`, **neither a release blocker**; SA128 follows at a second recorded blocked checkpoint after a fresh continuation again exhausted its review cap with no executable delta retained; SA122b remains last, depends on SA128, and merges after SA112e. **No open ticket is Tier 3.**

1. **SA112a → SA112f** (installed-wheel full-lifecycle e2e `plan → apply → up`, Track 3) — six serial, handoff-sized tasks covering provisioning, diagnosis, the traceback-selected fix, permanent lifecycle coverage, CI triggers, and closeout. **Critical path.** Deps: SA110 ✓ + SA111a ✓ + SA113 ✓, **plus SA117e (transitively SA117c; SA117b ✓) from SA112b onward** (see the evidence-validity dependency below — the binding node is the *pushed* splits, not the local stamp/assert). Next: **SA117e**, then SA112a.
...
4. **SA128 → SA122b** (gate-topology registry, Track 1; now queued after SA129/SA130 per `SA131-ORDER-001`) — arch Finding 11. Release assurance is four hand-synchronized inventories; `TA62` and `SA115-CI-001` are its latest paid drift instances. SA122a delivered the registry and was **closed as superseded** on 2026-07-29 (`SA122A-AUTH-001` option (b)): the registry is kept, its regex-based checker is rejected and rebuilt by **SA128** on structural parsing (deps: none); SA122b then migrates the consumers and **merges after SA112e**, which owns the last manual e2e path-list append.
5. **SA117e** (embedded-manifest / split-branch version skew, Track 3) — **RELEASE BLOCKER, and Track 3's next action.** SA117a, SA117b, and SA117c are closed; SA117e covers the reviewed split push and is the sole open child. `apply` embeds modules by git subtree from `splits/<module>-module` on the public remote, so embedded `module.yml` files are whatever was last published, not the working tree. The published splits predate the derivation sections v87's core requires, so `apply` fails for every module set. Approach decided (`SA117-DEC-001`): **stamp + assert in v87, pin in v88 (SA119)**; rules in [decisions.md §module-version-lockstep](./decisions.md#module-version-lockstep). Blocks SA96-PUBLISH **and SA112b–SA112f** (2026-07-26 sequencing decision: `SA117-DEC-002`). Next: **SA117e**.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  Umbrella acceptance only: SA117a/b/c are closed; SA117e remains open and required. Child tickets own all implementation; **do not execute this parent block as a monolithic handoff.**

  **Split (`SA117-DEC-003`, 2026-07-27): SA117 was Tier 3 and was split into the four serial children SA117a → SA117b → SA117c → SA117e; the first three are closed (closeout detail in [CHANGELOG.md](../../CHANGELOG.md)) and SA117e remains.** SA117a delivered what SA112b's evidence validity depends on; SA117b made pushing refreshed splits safe; SA117c supplies the local comparison gate; SA117e pushes refreshed splits and proves the published ones match the core. The former SA117d (scope meta-tooling) is **deferred to v88 as SA124** (`SA117-DEC-004`). Sizing rationale and the measured candidate breakdown are in [CHANGELOG.md](../../CHANGELOG.md).

  **The original executable candidate and SA117b corrections are already merged.** `43d9b8fc` is on `v87` as recorded partial delivery, unapproved at umbrella scope; SA117b's reviewed corrections landed in `efd86d21`. The remaining child is therefore a **correction-and-review handoff over merged-but-unapproved umbrella code**, not a greenfield build: it corrects its named findings on the existing candidate and obtains independent review for its own slice. It may not check its box on a review narrower than its own file set, and SA117e still requires one full-scope review over the complete delta including `43d9b8fc`.
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - **SA117a, SA117b, SA117c — ALL CLOSED (2026-07-28 / 2026-07-29 / 2026-07-30, `46a2f186`).** Closeout evidence is in [CHANGELOG.md](../../CHANGELOG.md). Forward-binding residue: SA117c's **local** lock-comparison gate is the input SA117e's post-push verification consumes. No finding carried over; **SA117e is the sole remaining child.**
...
Track 1 holds the gate-governance work: it changes how "green" is decided, not what the generator emits. `TA62`/SA120, **SA121**, **SA125**, **SA126**, and **SA127** are closed (see [CHANGELOG.md](../../CHANGELOG.md)), followed by **SA128/SA122b** from [arch-audit.md](../../arch-audit.md) Finding 11 (SA122a closed as superseded, `SA122A-AUTH-001`).

**Ordering on this track (one worktree, serial):** **SA129 → SA130 → SA128 (second blocked checkpoint) → SA122b**, with SA122b merge-gated behind SA112e. **SA129 and SA130 moved ahead of SA128 on 2026-07-30 (`SA131-ORDER-001`, applied as a pure speedup):** all three are `deps: none` and file-disjoint from one another, from SA122b's registry consumers, and from every Track 3 surface (SA129 is `quickscale_cli` CLI code, SA130 is a `quickscale_core` generator template, SA128 is `scripts/check_gate_parity.py` plus its tests), so nothing forces a rebase in any direction. SA128 has retained no executable delta across two review caps and needs a fresh continuation regardless; its only downstream consumer, SA122b, is separately merge-gated behind SA112e — five Track 3 children away — so SA128 carries wide slack and reordering costs no path. SA129 before SA130 is arbitrary; they are independent and may swap.
- **SA129 is Track 1's current action.** Fully diagnosed, `deps: none`, two-file allowlist, service-free validation, no open decision — executable today. SA130 follows on the same terms.
- **SA128 remains open at its second blocked checkpoint, now third in the queue.** The 2026-07-30 fresh continuation completed discovery, an approved two-phase plan, two implementation/QG rounds, and two full-scope reviews; the second review remained `STATUS: partial` with severity/count unchanged at high and 9 → 9. Every executable/test candidate was restored byte-identically to the synced start before this checkpoint, so the merged SA122a checker remains non-authoritative. Finding 11 still needs centralization **before or as part of** the SA112e/`SA115-CI-001` path-list edits. The registry those edits write into is **already merged and usable** (SA122a's kept half); SA128 must make the checker trustworthy before SA122b may consume it.
...
- **SA122a — CLOSED as superseded 2026-07-29 (`SA122A-AUTH-001`, option (b)).** Forward-binding residue only: its registry half stays merged and usable (`scripts/gate_registry.json`, `make check-gate-parity`, maintainer docs, at `73c153b2`) and is an **input, not scope**, for everything below; its checker half is replaced wholesale by **SA128**. Closeout detail and the full finding list are in [CHANGELOG.md](../../CHANGELOG.md).

- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Tier 2 · Track 1 · deps: none · replaces SA122a's checker · queues after SA129 + SA130 (SA131-ORDER-001)`

  Replace `scripts/check_gate_parity.py`'s inventory extraction so each context's gate set is derived from the *semantics* of its source, not from text that resembles an invocation. The registry, its schema, the Make entrypoint, and `SA122-DEC-001`'s publish-as-full-coverage rule are unchanged inputs — this ticket changes only how an inventory is observed.

  **Why replacement rather than remediation (`SA122A-AUTH-001`, 2026-07-29).** The current extractors decide membership by regex over lines: `_extract_makefile_targets` re-implements Make's grammar by hand; `_extract_check_ci_serial_gates` / `_extract_check_ci_parallel_gates` and the workflow `run:` scrapers match `\bmake\s+([a-z][a-z0-9_-]+)` with prefix-based `echo`/`#` filters. That admits both false directions — a gate named inside a heredoc, a comment on a continuation line, or an uncalled function reads as covered, while `make $(GATES)`, a variable-composed target, an `include`d definition, or a gate reached through `check`'s delegation is invisible. Each remediation cycle would close one deceptive form and leave the next, which is the compounding shape arch Finding 11 exists to remove. The workflow-YAML half is already structural (`yaml.BaseLoader` with duplicate-key rejection) and is **kept**.

  1. **Make** — derive targets and recipe contents from Make itself (`make -qp` / `make --dry-run <target>`), so variable expansion, `include`, and target delegation are resolved by Make rather than inferred. `check`'s effective gate set must be observed, not reconstructed.
  2. **Shell** (`scripts/check_ci_locally.sh` serial and parallel lists, and workflow `run:` blocks) — execute with actual Bash and a recorder `make` on a controlled `PATH`. Quoting, heredocs, comments, short-circuiting, functions, and unreached code must resolve through Bash semantics by construction, with no syntax-tree alternative or exception list.
  3. **Workflow YAML** — keep the existing structural parsing and extend it to E2E path extraction, which `SA122A-CR-003` records as still non-uniform.
  4. **Fail hard on ambiguity.** An input the parser cannot resolve is an error, never a pass — [decisions.md §fail-hard-principle](./decisions.md#fail-hard-principle). This is the property the regex layer could not offer and the reason the finding is a security boundary rather than a nit.
  5. Carry forward the still-valid SA122a acceptance: the checker reproduces today's inventories for all five contexts with no false differences, and **keeps failing on the publish omission** — a declared-open gap owned by SA122b, not a narrowing to be dispositioned away.

  - Verify: an adversarial fixture set false-greens **none** of the six ratified inert forms — (1) an `echo`'d invocation, (2) a quoted heredoc containing an invocation, (3) a commented continuation line, (4) a gate inside an uncalled shell function, (5) a gate behind `if false;`, and (6) a short-circuited `false && make …` invocation are all reported absent; `make $(VAR)`-composed and delegated targets are reported present; an unparseable Makefile/shell/YAML input exits non-zero with a named error rather than an empty inventory; adding a fake gate to the registry fails every context that has not adopted it; the five expected publish omissions are still reported exactly. While those omissions remain, direct `poetry run python scripts/check_gate_parity.py` exits **1** and the unchanged `make check-gate-parity` wrapper exits **2** with the same records; `make quality` and `make typecheck` exit **0**, and the two focused MyPy errors in the E2E extractor from `SA122A-QG-001` must be gone, not baselined. This corrects the earlier contradictory exit-0 criterion: a fail-hard parity gate cannot both report semantic differences and return success, and SA122b—not exception logic—owns making it green.
  - Rollback: the registry, Make target, and documentation are untouched by this ticket; reverting it restores the current partial checker without disturbing them.
  *(why →* `SA122A-CR-003`/`SA122A-CR-002`; a parity checker that can assert coverage a context does not have is worse than none, because SA122b's consumer migration and every later gate would trust it*)*

  **Disposition of SA122a's findings.** `SA122A-CR-003` (security-boundary) and `SA122A-CR-002` (correctness) are **owned by this ticket** and close with it. `SA122A-QG-001`'s MyPy errors close here; its adversarial-proof requirement is folded into the Verify line above. `SA122A-CR-004` (low/advisory `.PHONY` wording) is carried, non-blocking.

  **Executable file allowlist (exact):** `scripts/check_gate_parity.py` and `scripts/test_gate_parity.py`. No parser dependency, `pyproject.toml`, or lockfile change is authorized. `scripts/gate_registry.json` and the `Makefile` target are **inputs, not scope** — do not edit them.

  **Ratified decisions — carry these forward unchanged on the next continuation:**
  - **`SA128-DEC-001` — tool-owned semantic observation, no sandbox or parser fallback (2026-07-29).** Remove Bubblewrap completely — it is neither a prerequisite nor an optional branch. Make inventory comes from GNU Make's own expansion/dry-run semantics; shell inventory comes from actual Bash execution with a recorder `make` on a controlled `PATH`; workflow structure and E2E paths come from duplicate-key-rejecting `yaml.BaseLoader`. No shell-AST dependency was selected, and text/regex extraction is never an authorized fallback. Cleared environment, restricted recorder `PATH`, timeouts, output bounds, and process-group cleanup are determinism/resilience measures, **not** an isolation boundary; the checker observes trusted repository sources that CI already executes.
  - **`SA128-DEC-002` — fail-hard trust and caller contract (2026-07-29).** Every consumed input, including CLI `--registry`, must be a canonical non-symlink file inside the repository. Keep all six inert adversarial forms exactly as enumerated in the Verify line, including the sixth short-circuit case `false && make …`. While SA122b's five publish omissions remain, direct execution exits **1** and the unchanged Make wrapper exits **2** with the same records; do not add tolerance or exception logic. SA122b makes the target green by fixing inventories.
  - **`SA128-DEC-003` — review-cap disposition and restart point (2026-07-29).** Do not preserve or patch any rejected candidate: all executable/test deltas were reverted. The next attempt starts from the unchanged SA122a baseline and the stable finding ledger below, first deriving a literal current-source oracle for all **16** publish `run` values and **10** bound hosted `run` values. No SA128 completion, SA129 start, or SA122b authorization occurs before focused/repository validation and full-scope review return `STATUS: ok`.

  **Open decisions:** none. The implementation architecture, trust boundary, exit behavior, adversarial matrix, and restart point are already decided by `SA128-DEC-001` through `SA128-DEC-003`; the next continuation must not ask or re-decide them.

  **SECOND RECORDED BLOCKED CHECKPOINT (2026-07-30; no executable delta retained; task remains open).** The first checkpoint (2026-07-29) is superseded by this one and its record lives in [CHANGELOG.md](../../CHANGELOG.md); the finding ledger below is the stable one carried forward.
  - **Done:** Track 1 started clean, merged current `v87`, and rebuilt its Poetry environment on Python 3.14.6. Fresh discovery confirmed `deps: none`, no task-local open decision, the exact two-file executable allowlist, all 16 publish `run` values, all 10 bound hosted `run` values, and the unchanged SA122a baseline. A new two-phase plan received independent plan-review `STATUS: ok`. Two implementation/QG rounds followed: the terminal rejected candidate passed 132 focused tests, strict MyPy, Ruff, exact direct-exit-1 / Make-exit-2 five-record parity, `make typecheck`, `make quality`, and `git diff --check`. Two full-scope reviews nevertheless remained `STATUS: partial`; highest unresolved severity stayed **high** and the unresolved count stayed **9 → 9**, so the review cap stopped further autonomous fixing. The checker and test candidates were restored byte-identically to the synced start commit `10ff5c89`; **no SA128 executable or test change is retained or merged**.
  - **Pending / blocking:** `SA128-CR-001` (**high**, correctness — effective Make recipes, delegation, and `check` aggregation remain unproved); `SA128-CR-002` (**high**, correctness — local serial/parallel order, join, failure, and complete-observation semantics remain unproved); `SA128-CR-003` (**high**, security-boundary — publish Bash and E2E YAML still need actual/structural semantics); `SA128-CR-004` (**high**, completeness — hosted commands, `needs`, and stage topology remain unproved); `SA128-CR-005` (**high**, completeness — arbitrary dependency cycles remain unproved); `SA128-SEC-006` (**high**, security-boundary — every consumed source/include/registry path still needs one no-bypass containment contract); `SA128-CR-007` (**high**, correctness — failed or ambiguous observations still need uniform controlled exit 2); `SA128-CR-008` (**high**, resilience — output/event bounds, readers, residual processes, and process-group cleanup still need literal failure semantics); and `SA128-TEST-009` (**high**, test-gap — the complete inert-form, Make delegation, Bash ordering/join/failure, hostile-CWD, lifecycle, and caller-parity matrix remains absent from the retained baseline).
  - **Decisions needed:** none. The next continuation starts again from the unchanged SA122a baseline and the ratified `SA128-DEC-001` through `SA128-DEC-003` constraints. It must resolve the complete stable ledger, run focused and repository validation, and obtain full-scope independent review `STATUS: ok` before checking SA128 or authorizing SA122b. **SA129 and SA130 no longer wait on this continuation** (`SA131-ORDER-001`); they run first.

- [ ] **SA122b — Migrate the consumers onto the registry.** `Tier 2 · Track 1 · deps: SA128 · merge after SA112e`

  Make each context derive its inventory from the registry instead of restating it: Make `check` aggregation, the serial and parallel lists in `check_ci_locally.sh` (whose current tests pin worker count/order and therefore protect each copy rather than derive it), hosted `ci.yml` jobs and `needs`, publish membership — which per `SA122-DEC-001` means **adding the five repository conformance gates to `.github/workflows/publish.yml`**, closing the gap the parity checker reports — and the `e2e.yml` `pull_request.paths` allowlist. Make the SA128 parity checker **blocking** in CI once every context derives.

  - **Merge-order bound.** SA112e appends the installed-wheel path tuple to `.github/workflows/e2e.yml`, and `SA115-CI-001` appends its own; both preserve exact ordered tuples with `yaml.BaseLoader` regression coverage. Migrating that list before those land would force rework of all three edits. **Land SA122b after SA112e** — the same coordination bound SA115 already carries. SA128 is unaffected by that bound; the registry itself is already merged, so SA112e and SA115-CI-001 register their paths as they go.
...
Found 2026-07-29 by the manual zero-module `plan`/`apply` session that also produced SA127 (closed; see [CHANGELOG.md](../../CHANGELOG.md)). `apply` completes the whole lifecycle, then ends with:
...
- [ ] **SA129 — Carry the superuser answer on an explicit sentinel line.** `Tier 2 · Track 1 · deps: none · Track 1 head (SA131-ORDER-001)`
...
- [ ] **SA130 — Use Poetry's actual timeout variable.** `Tier 2 · Track 1 · deps: none · queues behind SA129, ahead of SA128`
...
The two former complexity blockers were closed by SA126 (Track 1), so the green-gate unblock no longer sits behind the SA117 chain's plan gates. SA121 is closed and its merge-base monotonicity gate is live in `make quality`; SA127 is closed at `a2eb7a6c`; Track 1 now runs SA129 → SA130 → SA128 (second blocked checkpoint) → SA122b (`SA131-ORDER-001`).
...
──────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA129 (superuser probe; Tier 2) ◄─ next  SA115 (e2e xdist; deps: none)  SA117e (review + push splits) ◄─ IN PROCESS
  │  deps: none · not a blocker          │  validation AUTHORIZED       │  deps: SA117a/b/c ✓ (all closed)
  ▼                                      │  cannot finish → SA112f      │  human-confirmed public push
SA130 (dead poetry timeout; Tier 2)     │  cannot merge  → SA112e      │  (scope tooling → v88 SA124)
  │  deps: none · not a blocker         │                              │
  ▼                                     │                              │
SA128 (structural parity check)         │                              │
  │  deps: none · replaces SA122a       │                              │
  ▼  SECOND BLOCKED CHECKPOINT           │                              │
...
**Diagram checkpoint (2026-07-30, third pass).** Track 3 is **in process on SA117e** — SA117a/b/c are closed and merged, so no closed node remains on the diagram. Track 1 now begins at **SA129** (`SA131-ORDER-001`); SA128 keeps its second blocked checkpoint with no executable/test delta retained, and moves behind SA130.
...
**Track 1 is internally serial and now entirely off the critical path.** SA129 is the head, SA130 follows, SA128 sits third at its second blocked checkpoint, and SA122b is last with the SA112e merge bound. The SA128/SA122b governance chain is off-path; SA129 and SA130 are also off-path, non-release-blocking product defects. Track 1's release blocker (SA127) is closed.

**Critical path:** `SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. SA117e is Track 3's next action and the longest remaining dependency chain to the release.

**Parallelism.** The open tickets occupy all three tracks: SA117e → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and SA129 → SA130 → blocked SA128 → SA122b on Track 1. **The repository runs exactly three worktrees; a fourth is not created** (`SA112A-TRACK-001`, resolved 2026-07-28 — see Standing decisions).
...
- **`SA122A-AUTH-001` — RESOLVED 2026-07-29: option (b), replace the checker; ticketed as SA128.** Forward-binding rule only: the registry, the Make entrypoint, and `SA122-DEC-001`'s publish-as-full-coverage rule are unchanged inputs, and only *how an inventory is observed* is rebuilt. **SA128 owns `SA122A-CR-003`, `SA122A-CR-002`, and `SA122A-QG-001`.** Rationale and the declined options are in [CHANGELOG.md](../../CHANGELOG.md).
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28; re-costed 2026-07-30, third pass) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now leads with **executable** SA129, then SA130, then blocked SA128, then SA122b (SA127 ✓ closed), while Track 3 is **in process on SA117e** ahead of SA112a. The cost rose again with `SA131-ORDER-001`: taking the move would now displace two immediately-executable, fully-diagnosed Tier 2 defects rather than a stalled continuation. Reusing Track 1 still requires no fourth worktree. **Recommendation: decline while SA129/SA130 are the head** — take it only if Track 3's SA117e stalls and SA112a's critical-path start becomes the binding constraint.
  - **Independence checks pass.** SA112a's `deps: SA110 ✓ + SA111a ✓ + SA113 ✓` are satisfied and it does **not** carry the SA117e bound (that begins at SA112b). Its three-file allowlist — `scripts/smoke_install.sh`, `scripts/_installed_wheel_venv.sh`, `scripts/provision_installed_venv.sh` — is disjoint from SA121 (`scripts/check_quality.sh`), from SA128 (`scripts/check_gate_parity.py` and its tests), and from the entire live SA117 chain. Its validation is service-free (`bash -n`, focused tests, `make smoke-install`), so it never contends with Track 3 for PostgreSQL/Docker. It is a self-contained provisioning-seam extraction, not a piece of a larger logical change.
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* Track 1's queue head. That head is now **executable SA129**, followed by SA130, blocked SA128, and SA122b, so the move would delay two ready-to-run product-defect fixes and the full off-path queue to advance a critical-path node; it does not trade against a release blocker.
...
- **The registry is already merged, so SA112e and SA115-CI-001 can record their paths in it today.** That was the sequencing benefit SA122a was protecting, and it survives SA122a's closure. SA128 does not gate it — SA128 makes the *checker* over the registry trustworthy, which is what SA122b and any later gate need.
...
**Conflict surface.** The shared closeout files are `CHANGELOG.md` and `docs/technical/roadmap.md`, plus `docs/technical/decisions.md` when policy or acceptance evidence changes (the SA122 chain writes its publish-context disposition there; SA121's delivered functional scope touched quality-policy prose). All three are covered by the Merge procedure above: the `git merge v87` before every merge-back forces conflicts to resolve on the track branch, preserving both sides. Two executable surfaces are also shared and already bounded: `.github/workflows/e2e.yml`'s path list between SA112e and SA115-CI-001 (serialized by the `merge after SA112` bound), and `scripts/quality_baseline.json`, whose line-count section is gone (SA125 ✓) — **no open phase owns that file**: SA126 was forbidden from raising a ceiling in it and the closed SA117 children left it untouched; SA121's delivered functional scope also left it untouched (`SA121-DEC-001`). A third executable surface was **removed** in the fifth pass rather than bounded: `quickscale_cli/src/quickscale_cli/commands/module_commands.py` was shared between SA117a and SA117b through their split quality shares, and is now single-owner under SA126 (`SA126-DEC-001`). The tracks cannot collide and no additional procedure is required.

### Track readiness (2026-07-30)

> **Re-verified on 2026-07-30 (third pass).** SA117a/b/c are closed and fully merged; SA117e is Track 3's **in-process** ticket. Branch state re-measured this pass: `wt-track3` carries **zero** commits unmerged into `v87`; `wt-track1` carries **zero** commits unmerged (the documentation-only SA128 second blocked checkpoint merged at `347ad879`/`48579fe9`, with no executable/test delta retained); `wt-track2` still carries exactly `5193f198` + `5b5de830` and remains closed to new work.
>
...
**Tier census (re-verified 2026-07-30): no open ticket is Tier 3.** SA112e and SA96-PUBLISH are Tier 1; every other open ticket (SA117e, SA112a–d/f, SA115, SA128, SA129, SA130, SA122b) is Tier 2. The two historical Tier 3s were already split — SA117 into SA117a/b/c/e (`SA117-DEC-003`) and SA122 into SA122a/SA122b.

Each track reports **three independent states**. A track is **truly green** only when all three are yes.

- **Track 1 (SA129 → SA130 → SA128 → SA122b) — start: YES · finish: YES · merge: YES. TRULY GREEN at its head, but off the critical path (filler work).**
  - **Can start — YES.** SA129 is the head after `SA131-ORDER-001`: `deps: none`, fully diagnosed, exact two-file allowlist, service-free validation, no open decision. SA130 follows on identical terms.
  - **Can finish — YES (for SA129 and SA130).** Both reach a checked box using only Track 1 work; their acceptance criteria depend on no other track's output. **SA128 individually remains finish-NO** — it still carries nine high findings (`SA128-CR-001`…`SA128-TEST-009`) and the current checker is explicitly non-authoritative; that blocker is clearable on this track alone, by a fresh continuation, and needs no upstream work. SA122b cannot finish before SA128.
  - **Can merge — YES.** SA129, SA130, and a corrected SA128 carry no merge-order gate. SA122b alone is order-gated behind **SA112e** (shared `.github/workflows/e2e.yml` path list) — a hard dependency that only that upstream work clears.
  - **On the critical path? NO — Track 1's release blocker (SA127) is closed.** SA128/SA122b are governance work; SA129/SA130 are non-release-blocking product defects. All Track 1 work is therefore **off-path filler** relative to the release, however green it is. `SA112A-TRACK-002` weighs a critical-path node (SA112a) against the full Track 1 queue.
...
- **Track 3 (SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, on the critical path, and IN PROCESS on SA117e.**
  - **Can start — YES (already started).** SA117a/b/c are closed and merged; SA117e has no unmet dependency and no open product decision, and is the live in-process ticket.
  - **Can finish — YES.** SA117e carries explicit acceptance and no unresolved pre-start decision.
  - **Can merge — YES.** SA117e carries no merge-order gate from another track; it retains its own human-confirmed public-push step, which is an authorization inside the ticket, not a cross-track dependency.
  - **Next action: SA117e.** Do not publish splits, start SA112, or treat the umbrella candidate as release-ready until SA117e is closed.
  - **Process note (`SA117-DEC-005`, 2026-07-28).** The **proportionality rule** in the handoff contract binds the remaining child: documentation/test-only deltas go straight to implementation and one change review, while SA117e production work retains its scoped plan gates.

**Net.** The Track 3 critical path is **SA117e (in process) → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1's release blocker is resolved; its queue is now SA129 → SA130 → blocked SA128 → SA122b, all off-path, with SA122b retaining the SA112e merge bound. Track 2 remains merge-gated. **Truly green tickets: SA117e (Track 3, on the critical path — real progress), SA129 and SA130 (Track 1, off-path filler).** SA128, SA122b, and SA115 are each not truly green; SA96-PUBLISH is human-only and held behind SA112f.

**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, at the cost of displacing executable SA129/SA130 and the rest of the Track 1 queue) or leave it behind SA117e. `SA127-ORDER-001`, `SA122A-AUTH-001`, and `SA131-ORDER-001` are resolved. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of displacing SA129 → SA130 → SA128 → SA122b; it changes no track's three states. **Default while undecided: SA112a stays on Track 3; Track 1 runs SA129 → SA130 → SA128 → SA122b.** Every individual ticket records **Decisions needed: none**.

**Standing decisions.** `SA131-ORDER-001` (2026-07-30) — **Track 1 runs SA129 → SA130 → SA128 → SA122b.** Applied as a pure speedup, not surfaced as a maintainer decision, because it delays nothing: all three leading tickets are `deps: none` and mutually file-disjoint; SA128 retained no executable delta across two review caps and needs a fresh continuation whenever it is scheduled; and SA128's only consumer, SA122b, is independently merge-gated behind SA112e — five Track 3 children away — so SA128 carries wide slack. The move converts Track 1's head from a twice-capped governance rebuild into two fully-diagnosed, executable Tier 2 product fixes, flipping the head's *can-finish* from NO to YES without touching any track's merge order or the critical path. Reversible at any time by rescheduling SA128 first. `SA117-DEC-005` (2026-07-28) — **SA117a is closed; its residue folds into SA117b; gates are matched to change size.** SA117a's load-bearing lockstep delivery was verified present in code, while its remaining scope had shrunk to one `assert_not_called()` spy in one existing test — which nonetheless consumed one change-review cap, three plan-review caps, and a full-scope documentation review while producing **zero lines**. The `SA117A-CR-002` spy moves to SA117b with its **literal diff pre-written** so no further planning is required; `SA117A-PLAN-010` (reviewed-tip identity) and `SA117A-PLAN-013` (shared-document conflict reconciliation) are **retired as ticket blockers** and restated once as standing merge-procedure guidance, since they described generic ceremony rather than SA117a's content. The accompanying **proportionality rule** binds every remaining Track 3 child: a delta confined to test assertions or documentation, with no production-behavior change, goes straight to implementation and one change review — scoped plan review is reserved for production behavior, security boundaries, and external surfaces. Splitting SA117a further was explicitly rejected as the wrong remedy: the ticket was already two lines, and additional handoffs were the cost, not the cure. `SA126-DEC-001` (2026-07-28) — **the two `module_commands.py` complexity ceilings are split out of SA117a/SA117b into SA126, Tier 1 on Track 1.** Both blocking functions live in one file, so splitting them across two serial critical-path children was the only reason that file was shared; one owner removes the coupling. The move converts Track 1 from *cannot finish* to *can finish* (it clears `SA121-QG-001` itself), reduces SA117a — stalled at three review caps with no source delta — to a single test-file assertion, and unblocks the green-gate milestone without waiting on the SA117 chain's plan gates. `SA117-QG-REMED` is retired; neither SA117a nor SA117b carries a complexity criterion or touches `module_commands.py`. `SA112A-TRACK-001` (2026-07-28) — **the repository holds at three worktrees; no Track 4 is created and SA112a stays on Track 3.** SA112a is dependency-satisfied and file-disjoint, but a second concurrent review lane is not worth one critical-path node while SA117a is still converging; parallelism was obtained by splitting SA126 instead. `SA117-DEC-002` (2026-07-26) — **SA117 runs first on Track 3, before SA112a**, and is a hard prerequisite for SA112b onward; rationale in the SA117 section. `SA117-DEC-003` (2026-07-27) — **SA117 was Tier 3 and is split into the serial children SA117a, SA117b, SA117c, SA117e**; the parent is umbrella-acceptance only and must not be executed as one handoff. `SA117-DEC-004` (2026-07-27) — **the SA117 scope meta-tooling is deferred to v88 as SA124**; the merged code stays in place, unadvertised, and is not a v87 gate. `SA115-DEC-001` step 3 — **AUTHORIZED** (2026-07-27, reversing the 2026-07-26 hold), with Track 3 holding infra priority; `SA115-DEC-002` (guard clamps workers) — **ratified**, implemented as part of that now-authorized validation phase. `SA122A-AUTH-001` (2026-07-29) — **SA122a's registry is kept and its regex-based parity checker is replaced, not patched**; the rebuild is Tier 2 ticket **SA128** on Track 1, which owns `SA122A-CR-003`, `SA122A-CR-002`, and `SA122A-QG-001`, derives each context's inventory from Make/shell/YAML semantics rather than line matching, and fails hard on unparseable input. `SA122-DEC-001` (2026-07-27) — **publish is a full-coverage gate context**; see [decisions.md §publish-path-gate-coverage](./decisions.md#publish-path-gate-coverage). `SA125-DEC-001` (2026-07-28) — **per-file line ceilings are retired; per-function complexity ceilings are kept.** The `large_files` ceilings were measurement fossils that taxed documentation and could not distinguish mandated growth from decay; complexity ceilings are local, actionable, and stay under SA121's monotonicity rule. **SA125 implemented and closed this decision on 2026-07-28** (see [CHANGELOG.md](../../CHANGELOG.md)); the policy text is in [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy). Each is stated in full in its own section above; no roadmap surface may contradict them. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
...
- [ ] **SA123 — Add dependency-vulnerability and security static analysis to the gate set.** `Tier 2 · v88 · deps: SA128`
