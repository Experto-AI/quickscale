...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  ...
  - **Blocked checkpoint (2026-07-23; continued 2026-07-24 — implementation and partial final-sync validation recorded; task remains open):**
    ...
    - **Decisions needed:** None on implementation or acceptance. On 2026-07-24 the maintainer selected **restore network and rerun**, preserving ORDER-001 rather than waiving it. The only required human/environment action is to restore sustained npm-registry throughput or approve a compatible mirror before the fresh continuation.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Tier 2 · Track 3 · deps: SA110 ✓ · SA111a ✓ · SA113 ✓`
  ...
  **Decisions needed**
  - **Maintainer input required:** provide the authoritative literal command/helper/output/cleanup/validation/commit registry for `SA112-PR-002` and the fully expanded ordered repository-relative workflow path tuple for `SA112-CR-005`. The final authorized autonomous correction cycle has been consumed; do not run a fourth autonomous plan rewrite. Once the maintainer-supplied registry exists, submit that artifact to focused independent plan review. Product implementation and diagnostic instrumentation remain unauthorized until review returns `STATUS: ok`.
  - **User direction (2026-07-24):** stop here, record and merge this roadmap-only blocked checkpoint. This authorizes checkpoint integration, not implementation and not waiver or downgrade of either blocking finding.
...
- **Track 3 — open engineering work; SA112 final authorized post-cap plan review stopped at a mergeable blocked checkpoint (2026-07-24).** The green-gate join, SA109, SA110, SA113, SA111a, and SA111b are closed. SA113 resolved the installed-context implication resolver crash, and SA111a proves installed-wheel `plan` coverage with all 12 modules. SA111b (optional, now complete) adds a fast in-monorepo fallback regression test. **SA112** remains pre-diagnostic: the extra blocking-only plan/review cycle completed, but independent review stayed `STATUS: partial`; `SA112-PR-002` (high) and `SA112-CR-005` (medium) remain blocking. Continuation now requires a maintainer-supplied literal command/helper registry and fully expanded ordered workflow path tuple; no fourth autonomous plan correction, product implementation, or diagnostic instrumentation is authorized before focused plan review returns `STATUS: ok`. SA96-PUBLISH (human-only) holds until SA112 closes. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006 low; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).
