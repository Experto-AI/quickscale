...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  │  ON THE CRITICAL PATH             │  validation AUTHORIZED      │  TRULY GREEN — PLAN REVIEW FIRST
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **yes** — SA117e-2 closed; scoped plan review is the first action | **yes** — Track 3 has exclusive-capacity priority after plan approval | **yes** — no order gate | ✅ **yes** | ✅ **yes** — real progress |
...
**Track 3 is truly green with SA117e-3 at its head and is on the critical path; Track 1's parallel leg is paused at a recorded-partial checkpoint** whose `F-005` candidate has implementation-agent evidence only. Track 1 is blocked on `SA112A-RESET-001` — a supported control-plane reset that actually clears the persisted `qg_runs=3/2` / `review_cycles=2/2` / `authorized_retries=1/1` counters; another authorization-only retry cannot, having now failed twice (detail on the SA112a ticket). Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. SA117e-3 needs its own scoped plan review (`F-002`, track-local) before any host-service mutation, and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path and **truly green** — head advances to SA117e-3 after SA117e-2 closed 2026-08-03. SA117e-1 and SA117e-2 both produced evidence/closeout documentation only, with no source delta; SA117e-2's complete five-command ordered validation passed against the maintainer-approved `1333eb4a…` replacement baseline, and its fresh full-scope closeout review returned `STATUS: ok`, closing `SA117E-VAL-002` (narrative in [CHANGELOG.md](../../CHANGELOG.md)). Track 1 is not running a live parallel leg — it is paused at a recorded-partial checkpoint blocked on `SA112A-RESET-001` — so Track 3 is the only truly green track. `SA117E1-REV-001` (high/blocking, explicit-absence lease safety) and `SA117E1-REV-002` (medium/blocking, unbound publication authorization) remain owned by SA117e-4; plan-review `F-002` remains the separate medium/blocking host-PostgreSQL lifecycle requirement on SA117e-3. **SA112a runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not run SA117e-3's host-PostgreSQL work before its scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  *(why →* `apply` with any module has zero end-to-end coverage — `test_e2e_development_workflow.py:276` plans with modules skipped — so this skew class has never been exercised*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 closed 2026-08-03** (full-scope historical review, evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). Two of its `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. `SA117E1-REV-003`, `CR-SA117E-005`, and `CR-SA117E-006` are resolved.
    ...
    - [x] **SA117e-2 — Complete the rebaselined ordered source validation.** `Tier 2 · deps: SA117e-1 ✓ closed` — local, service-free
      **Closed 2026-08-03.** The complete five-command ordered validation remained green against `SA117_BASELINE_REF=1333eb4a…` with no source delta or rerun; the fresh full-scope independent closeout review over this roadmap and [CHANGELOG.md](../../CHANGELOG.md) returned `STATUS: ok` and closed `SA117E-VAL-002`. Per-command evidence, digests, test counts, and `SA117E2-RESET-001` history are in the changelog.
