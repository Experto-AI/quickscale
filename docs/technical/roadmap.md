...
──────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA128 (structural parity check)         SA115 (e2e xdist; deps: none)  SA117e (review + push splits) ◄─ next
  │  deps: none · replaces SA122a        │  validation AUTHORIZED       │  deps: SA117a/b/c ✓ (all closed)
  ▼  SECOND BLOCKED CHECKPOINT            │  cannot finish → SA112f      │  human-confirmed public push
SA129 (superuser probe; Tier 2)         │  cannot merge  → SA112e      │  (scope tooling → v88 SA124)
...
**Diagram checkpoint (2026-07-30, second pass).** Track 3 begins at SA117e — SA117a/b/c are closed and merged, so no closed node remains on the diagram. Track 1 begins at SA128's **second blocked checkpoint**; no executable/test delta is retained, and the next action is another clean continuation of SA128.
...
**Track 1 is internally serial and now entirely off the critical path.** SA128 is at its second blocked checkpoint, SA129 and SA130 remain queued behind its next clean continuation, and SA122b then carries the SA112e merge bound. The SA128/SA122b governance chain is off-path; SA129 and SA130 are also off-path, non-release-blocking product defects. Track 1's release blocker (SA127) is closed.
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28; re-costed 2026-07-30) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 leads with **blocked** SA128, followed by SA129, SA130, and SA122b (SA127 ✓ closed), while Track 3 has SA117e ahead of SA112a. SA128's fresh continuation has capped again with no executable delta retained, so taking the move would delay the next SA128 continuation rather than preempt live work. Reusing Track 1 still requires no fourth worktree, but it would delay the whole remaining Track 1 queue.
...
- **Track 1 (SA128 → SA129 → SA130 → SA122b) — start: YES · finish: NO · merge: YES. NOT truly green. At second blocked checkpoint.**
  - **Can start — YES.** SA128 has `deps: none`, no open product decision, and service-free validation; its latest fresh continuation exhausted the review cap and retained no executable/test delta, so another clean continuation is required. SA129 and SA130 remain queued; SA122b remains last with its stated SA112e merge bound.
  - **Can finish — NO (not yet). Review blocker, clearable on this track alone.** SA128 still carries nine high findings from its checkpoint and the current checker is explicitly non-authoritative; nothing outside Track 1 is required to clear them. SA129, SA130, and SA122b do not discharge those findings.
  ...
  - **On the critical path? NO — Track 1's release blocker (SA127) is closed.** SA128/SA122b are governance work; SA129/SA130 are non-release-blocking product defects. All Track 1 work is therefore off-path filler relative to the release. `SA112A-TRACK-002` weighs a critical-path node (SA112a) against the full Track 1 queue.
...
- **Track 3 (SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path.**
  - **Can start — YES.** SA117a/b/c are closed and merged; SA117e has no unmet dependency and no open product decision.
...
**Net.** The Track 3 critical path is **SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1's release blocker is resolved; blocked SA128 is followed by SA129, SA130, and SA122b, all off-path, with SA122b retaining the SA112e merge bound. Track 2 remains merge-gated.

**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but at the cost of delaying blocked SA128's next continuation and its successors) or leave it behind SA117e. `SA127-ORDER-001` and `SA122A-AUTH-001` are both resolved. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying blocked SA128's next continuation and its queued SA129 → SA130 → SA122b successors; it changes no track's three states. **Default while undecided: SA112a stays on Track 3; Track 1 resumes SA128 → SA129 → SA130 → SA122b through a fresh continuation.** Every individual ticket records **Decisions needed: none**.
