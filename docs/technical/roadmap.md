...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA122b-3 → -4 → -5                     SA115 (e2e xdist; deps: none) SA117e-4 ▶ → -5
  │  Umbrella, split by consumer      │                             │
  │  Make · sh · ci · publish · e2e   │  validation AUTHORIZED      │  ORDER-001 resolved (b)
  │  checker authoritative (SA128 ✓)  │  cannot finish → SA112d     │  plan-review cap + human push gate
...
**Critical path:** `SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path; SA112a is closed and merged, so SA112b's Track 1 precondition is already satisfied). **SA117e-5 is closeout and sits *off* the critical path** — SA112b's other precondition is the *pushed* splits, delivered by SA117e-4, so SA112b does not wait for the umbrella to close. Track 2 is entirely off it, and the whole remaining Track 1 queue is off it.
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-4 | **no** — needs a maintainer **continuation instruction** to open a new plan/review round; then `F-001`, `F-004`, `F-005`, `F-006` must clear before any source edit (**decision-clearable**, then track-internal) | **no** — the four plan blockers, both executable findings, fresh twelve-row push confirmation, and post-push acceptance remain | **yes** — no merge-order gate | no | ✅ **yes** — blocked head is on it |
...
**Track 3 is plan-blocked at SA117e-4 on the critical path.** The maintainer ratified the explicit empty-expectation lease and digest-bound bind-and-consume dispositions, but two plan-review passes ended `STATUS: partial` and the selected disposition was recorded partial delivery. Two gates now stand in front of implementation, and they are different in kind: **a maintainer continuation instruction (a decision, clearable today)** and then **four blocking plan findings (track-internal work, no upstream dependency)**. The fresh twelve-row confirmation remains a separate execution-time gate, and SA117e-4 still carries the relocated public-split harness gate. **Track 1 is truly green at SA122b-3** — SA122b-2's functional evidence and full-scope review are accepted, with no open blocker or decision — but it is off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear.
...
**Status:** on the critical path and **blocked at SA117e-4**. SA112b's provisioning precondition is satisfied by SA112a's merge; its split-publication precondition waits on SA117e-4. The maintainer dispositions for `SA117E1-REV-001` and `SA117E1-REV-002` are ratified, but their combined implementation remains forbidden: the ticket needs a maintainer continuation instruction to reopen plan/review, and four plan findings must then clear. `SA117E3-PUBLIC-ANALYTICS-001` remains open, and the twelve-row confirmation is obtained fresh at push time.
...
**Handoff entry point.** SA117e-4 carries the [ratified dispositions and current blocked checkpoint](#ratified-dispositions--plan-review-cap). Lease research is complete and both maintainer choices are durable in [CHANGELOG.md](../../CHANGELOG.md); a clean continuation starts with an explicit maintainer continuation instruction, then resolves the four open plan-review findings to `STATUS: ok` — not by re-asking those decisions or editing source directly.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1, SA117e-2, and SA117e-3 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-4` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-4 — Tag, human-confirmed split push, and post-push parity.** `Tier 2 · deps: SA117e-3` · **HUMAN-GATED — outward-facing, irreversible** — satisfies SA112b's precondition
      ...
      **Decisions ratified before implementation (2026-08-05):** (a) resolve `SA117E1-REV-002` by binding and one-time consumption of digest-bound authorization evidence; (b) resolve `SA117E1-REV-001` with the explicit empty-expectation form `--force-with-lease=refs/heads/<branch>:`. Official Git 2.43.0 documentation/source and a disposable local bare-remote matrix covered absent, present, fresh-tracking, stale-tracking, and concurrent-writer cases. **Neither decision authorizes a push** — the twelve-row human confirmation is obtained fresh at execution time.
      *(why →* the only irreversible, outward-facing step on the umbrella, isolated so it carries exactly one gate and no unrelated work*)*
      ...
      #### Ratified dispositions — **plan-review cap**

      > The two maintainer dispositions are ratified, but ratification is not plan approval and does not authorize source edits or a push. Two plan-review passes ended `STATUS: partial`; the maintainer selected recorded partial delivery, so a later continuation needs a fresh explicit plan/review authorization and must preserve the current finding ledger.

      **Ratified `SA117E1-REV-001` contract:** use `--force-with-lease=refs/heads/<branch>:` for an `ABSENT` expectation; do not accept bare `--force-with-lease`. Bare lease derives its expectation from local tracking state: a fresh tracking ref can silently authorize overwriting an existing branch, while a stale tracking ref can spuriously reject creation of a genuinely absent branch. A never-fetched but present branch fails closed with `stale info` on Git 2.43.0. The explicit empty expectation is tracking-independent and the server checks it under the ref lock, so a concurrent writer is rejected without clobbering its ref.

      **Ratified `SA117E1-REV-002` contract:** bind authorization to the exact verified twelve-row matrix digest and consume it once; do not defer the relied-upon gate to SA124. The verifier and publisher must prove they act on the same frozen matrix and fail closed on drift or reuse.

      **Compounding note the handoff must not lose.** The weak alternatives were declined: the public mutation must be protected by both exact Git leases and digest-bound one-time authorization in addition to the fresh twelve-row human confirmation.

      **Open plan-review blockers — all four must be resolved to `STATUS: ok` before any source edit.** Baseline for rollback is `v87` `ba34f594`; prior-round history is in [CHANGELOG.md](../../CHANGELOG.md).
      - `F-001` (**high**, security boundary) — name deterministic hook/barrier points, exact durable restart images, same-claim recovery, and replay/contender rejection for every interruption boundary.
      - `F-004` (**medium**, scope) — assign every tracked file and private artifact to a phase-local modify/create allowlist and mark read-only phases.
      - `F-005` (**high**, completeness) — name and freeze the authoritative twelve-module inventory and make every downstream stage consume that same digest.
      - `F-006` (**medium**, resilience) — arm a concrete pre-write rollback transaction to `ba34f594`, covering tracked files, refs, authorization artifacts, local-tag deletion, and clean/no-tag verification.

      `SA117E1-REV-001`, `SA117E1-REV-002`, and `SA117E3-PUBLIC-ANALYTICS-001` remain unimplemented. **A new plan/review round requires an explicit maintainer continuation instruction** — the recorded-partial disposition did not carry one forward.
    ...
    **Carried plan-review ledger.** Four findings remain blocking — `F-001`, `F-004`, `F-005`, `F-006` — all carried by SA117e-4 and listed there. `F-002` and `F-003` are resolved; `SA117E3-EXEC-001` was granted and consumed; `SA117E3-ORDER-001` is resolved (option b). Full history is in [CHANGELOG.md](../../CHANGELOG.md).
...
**No track-sequencing or worktree-assignment question is open; the latest rebalancing review (2026-08-06, cleaning pass) applied no move — the ninth consecutive pass to reach that conclusion.** Every open v87 ticket carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), and SA112a — the one node worth parallelizing onto Track 1 — has closed and merged. The remaining off-path work (SA122b-3…5) is a single serial chain; moving any of it onto Track 3 would push filler ahead of the critical path.
...
**Open maintainer decisions — exactly one, and it is procedural, not design.** No repository-design question is open: the explicit empty-expectation lease and digest-bound bind-and-consume dispositions for SA117e-4 are ratified and recorded in [CHANGELOG.md](../../CHANGELOG.md). What is open is the **SA117e-4 continuation instruction** — the recorded-partial disposition explicitly ended the authorization to run further plan/review rounds, so Track 3's critical-path head cannot restart until the maintainer issues one. Everything after that (`F-001`, `F-004`, `F-005`, `F-006`) is track-internal work with no upstream dependency. `SA117E3-ORDER-001` is resolved (option b), `SA117E3-EXEC-001` was granted and consumed, and SA117e-3 is closed. The fresh twelve-row push confirmation and SA96-PUBLISH remain execution-time human gates obtained at the outward-facing action and are never pre-granted.
