...
**Integration baseline (SA82).** The SA82 unquarantined `make test-integration` gate is the accepted baseline for the remaining restricted-role cluster. Under it the closed modules are blog (SA83), forms (SA85), listings (SA86), orgs (SA77), and notifications (SA79) — see [CHANGELOG.md](../../CHANGELOG.md). **CRM (SA84, Track 1) is now complete** (see completed entry under Track 1). One red remains open: a **blog fixture-finalizer regression (SA95, Track 2)** that SA93's broad run re-surfaced after SA83 had closed blog green (see SA95 under Track 2).
...
- [ ] **SA95 — Blog fixture-finalizer regression.** `Tier 2 · Track 2 · deps: none` — blog was closed green by SA83 but SA93's broad run re-surfaced `pytest-django` fixture-finalizer failures; full brief in the SA95 ticket under Track 2 below (reassigned from Track 1 to run in parallel; SA84 has since completed).
...
- [ ] **SA93 — Fold the e2e lane into the green-gate definition of done.** `Tier 1 · Track 3 · deps: blog+CRM integration green (previously unstated cross-track join prerequisite)`
  ...
  **Pending/Blocking:**
  ...
  - **SA93-BLOCK-001 (high/blocking):** blog integration shards fail with `pytest-django` fixture-finalizer errors, tracked as **SA95** (Track 2). CRM's restricted-role fixture block (**SA84**, Track 1) is now complete — see the SA84 completion entry under Track 1.
  ...
  **Clean continuation:** fix CR-SA93-REV-002/004 → land SA95 (blog) fixes on `v87` (SA84 CRM already landed) → resync `wt-track3` → rerun exact `make ci-e2e` → verify both E2E suites execute and `e2e.yml` is green on `v87` → independent final review → mark SA93 complete.
...
> **Prior Track 2 work is complete** (SA88b forms diagnosis 2026-07-14; SA86 listings 2026-07-15; GATE-lint / GATE-typecheck / GATE-check-suite 2026-07-15; **SA94 react-only theme + Barrier B review 2026-07-16, STATUS ok** — ledger in [CHANGELOG.md](../../CHANGELOG.md)). The GATE-quality / SA91 / SA93 closeout items were reassigned to the freed Track 3. **SA95 (blog regression) was reassigned here 2026-07-17** to run in parallel with Track 1's SA84 (since completed) — it is `deps: none`, independent of CRM, and shortens the SA84+SA95 cross-track join that gates SA93.

- [ ] **SA95 — Fix the blog restricted-role fixture-finalizer regression.** `Tier 2 · Track 2 · deps: none`
...
- **GATE-quality** (done 2026-07-15, see [CHANGELOG.md](../../CHANGELOG.md)) and **SA93** (fold e2e into the green-gate) were also reassigned here; SA93 is defined in the green-gate section above and remains open (blocked checkpoint on the SA84/SA95 cross-track prerequisite; SA84 is now complete — only SA95 remains).
...
- **Track 2 — original implementation complete (Barrier B STATUS ok); now carries SA95 (blog).** SA94 (react-only theme) removed `showcase_html` across 8 phases; resolved SA94-PLAN-CALLER-001, CR-SA94-REV-A-001/002/003, CR-SA94-REV-B-001/002 (CR-SA94-REV-A-004 low/advisory); eight pre-existing baseline gaps accepted. **SA95 (blog fixture-finalizer regression) was reassigned here 2026-07-17** — `deps: none`, ready, triage-first. SA84 (Track 1) has since completed, so SA95 is the sole remaining per-module gate. Track 2's other work is merge-ready; SA95 is its one open ticket.
- **Track 3 — PARTIAL (SA91 complete; SA93 blocked on cross-track work).** Finding 1 (DR persistence port, SA89a+SA89b), all four GATEs, and **SA91** (parallel integration worker pool) are complete. SA91 retains **CR-SA91-REV-006** (low/advisory, throughput only). **SA93** (e2e in green-gate) is a blocked checkpoint needing the deterministic CR-SA93-REV-002/004 fixes (no decision), then cannot complete until the blog (**SA95**, Track 2) shard is green on `v87` — a cross-track dependency. The CRM (**SA84**, Track 1) cross-track prerequisite is now met (SA84 complete). **SA89B-CR-004 (low/advisory)** remains independent, not gating.

**Net — no remaining maintainer decisions. SA92, SA91, and SA84 all completed; Track 1 fully complete; Track 2's original scope ready for merge (SA94 Barrier B passed).** Track 1 → SA84 complete (no remaining tickets). Track 2 → land SA95 (blog) independently. Track 3 → SA93's deterministic fixes, now with the SA84 cross-track prerequisite met. The squash-migrations decision and bounded guardrail strategy are recorded in [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92); reasoning trail in [CHANGELOG.md §SA88/§SA92](../../CHANGELOG.md).
