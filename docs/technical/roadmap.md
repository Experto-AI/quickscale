...
**Tracks 3 and 1 are both truly green and both on the critical path**, so the two parallel legs are live again: Track 3 runs SA117e-2, Track 1 runs SA112a's `F-005` correction under `SA112A-HANDOFF-003`. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
