...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Blocked checkpoint (2026-07-27 — recorded partial delivery after the plan-review cap; task remains open).**
  ...
  - **Pending / blocking:** five v87 findings remain. `SA117A-CR-002` (**medium**, test gap) — SA117a's update rollback regression does not explicitly prove `_sync_module_dependencies` remains uncalled, so SA117a stays open despite `SA117-CR-001` being resolved by the reviewed production correction. `SA117-CR-003` (**high**, security boundary) — blank direct-CLI origins can bypass the pre-mutation public-source gate → **SA117b**. `SA117-CONT-PR-003` (**high**, resilience) — interruption can leak the locally owned PostgreSQL validation container before cleanup is armed → **SA117b**. `SA117-CONT-PR-006` (**medium**, completeness) — the external Git-control package lacks a secure literal bootstrap and its final staged-index emptiness assertion is not filename-safe → **SA117b**. `SA117-CR-006` (**high**, completeness) — the advertised lock-drift route does not compare complete baseline lock semantics → **SA117c**. `SA117-CR-005` (**medium**, breaking change) remains **deferred to v88 as SA124** (`SA117-DEC-004`) and is not a v87 blocker.
  ...
  **Serial handoff contract (inherited from SA112).** Execute exactly one child at a time on Track 3. Each child names its complete file allowlist, commands, expected exits/artifacts, rollback, and focused validation before implementation; compares staged names against that allowlist after `git add -A`; obtains independent change review over its own slice; merges back to `v87`; then the next starts from a fresh sync. A child may stop with evidence and no source delta. `SA117` closes only after SA117a–SA117e are all complete.
  ...
  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: none · blocks SA112b` — **critical path**
    ...
    **Recorded partial checkpoint (2026-07-27 — change-review cap; task remains open).**
    - **Done:** The scoped plan gate returned `STATUS: ok`. The four-file correction makes the lockstep parser accept only exact canonical three-component ASCII-decimal versions, rejects padded, zero-padded, signed, Unicode-digit, empty-component, malformed, and unequal manifest/core spellings with the dedicated message, and preserves both caller implementations. Regression coverage exercises managed-wiring regeneration, standalone/apply embed fail-stop behavior, update rollback, and the real all-twelve `_regenerate_managed_wiring_for_apply → step_regenerate_wiring → regenerate_managed_wiring` path. Latest validation is green: 42 targeted tests; the separate all-twelve JUnit proof; 4,606 core/CLI unit tests with 1 skip; 12/12 version and manifest parity; lint, typecheck, and diff checks. Independent review explicitly resolved `SA117-CR-001` and confirmed production correctness, caller parity, breaking-change safety, and scope.
    - **Pending / blocking:** `SA117A-CR-002` (**medium**, test gap) — `test_update_rolls_back_noncanonical_manifest_version` asserts no state sync or commit but lacks an explicit `_sync_module_dependencies` spy and `assert_not_called()`. The current production sequence is correct; the remaining blocker is regression-proof completeness. The second review-fix cycle reduced the ledger from two medium blockers to one but reached the review cap, so no further autonomous fix cycle ran.
    - **Decisions needed:** none. A fresh tests-only continuation may add the missing dependency-sync spy, rerun the focused and full quality chain, and obtain full-scope independent `STATUS: ok`; only then may SA117a be checked and Track 3 advance to SA117b.
...
SA121 (baseline monotonicity)     SA115 (e2e xdist; deps: none)  SA117a (exact lockstep) ◄─ next
  (authorized; runnable)           │  (validation AUTHORIZED)     ▼
...
- **Track 1 — CLEAN TO CONTINUE; NO OPEN DECISIONS (2026-07-26 audit intake; off the critical path).** SA114/SA116/SA120 closed and the track was held in reserve; it now carries **SA121**, **SA122a**, and **SA122b**. `SA121-DEC-001` (2026-07-27) authorized SA121's focused tests/documentation-only continuation, so SA121 and SA122a are both runnable, need no PostgreSQL/Docker, and may run concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e. **Next action:** SA121's focused CR-003/005/006 cycle (nearest to closure), then SA122a. The publish-context disposition that used to sit inside SA122a step 3 is pre-resolved by `SA122-DEC-001`, so the track has no mid-ticket decision either.
- **Track 3 — SA117a RECORDED PARTIAL CHECKPOINT; ONE TEST-GAP BLOCKER; NO OPEN DECISIONS (on the critical path).** The original executable candidate at `43d9b8fc` remains recorded partial delivery. SA117a's scoped plan gate passed, its exact canonical lockstep production correction and four-file regression slice are implemented and fully validated, and independent review resolved `SA117-CR-001`; the task remains unchecked because the review-cap pass left `SA117A-CR-002` (medium, test gap) open for one missing update dependency-sync non-continuation assertion. **Next action: a fresh tests-only SA117a continuation** adding that spy, rerunning validation, and obtaining full-scope `STATUS: ok`; SA117b must not start first. `SA117-DEC-004` still defers scope meta-tooling to v88 as SA124. Do not publish splits (SA117e), start SA112, or treat the candidate as release-ready until the serial SA117 chain and SA117e's full-scope review complete. SA96-PUBLISH (human-only) still holds until SA112f closes the umbrella. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).
