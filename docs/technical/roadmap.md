...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: none`) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2, independent — but **authorization-blocked** at its 2026-07-27 recorded partial checkpoint (see below); a maintainer must approve one focused tests/documentation-only cycle before it can continue.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Tier 2 · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Blocked checkpoint (2026-07-27 — recorded partial delivery after the plan-review cap; task remains open).**
  ...
  - **Pending / blocking:** The executable candidate still carries `SA117-CR-001` (**high**, correctness) — padded manifest versions can pass literal lockstep; `SA117-CR-003` (**high**, security boundary) — blank direct-CLI origins can bypass the pre-mutation public-source gate; `SA117-CR-005` (**medium**, breaking change) — scope CLI/help/Make caller contracts disagree about required candidate paths; and `SA117-CR-006` (**high**, completeness) — the advertised lock-drift route does not compare complete baseline lock semantics. The continuation plan also remains blocked by `SA117-CONT-PR-003` (**high**, resilience) — interruption can leak the locally owned PostgreSQL validation container before cleanup is armed — and `SA117-CONT-PR-006` (**medium**, completeness) — the external Git-control package lacks a secure literal bootstrap and its final staged-index emptiness assertion is not filename-safe.
  - **Decisions needed:** No product or architecture decision is open. Before clean continuation, a maintainer must explicitly authorize another focused plan/review cycle for `SA117-CONT-PR-003` and `SA117-CONT-PR-006` (or provide an accepted literal plan resolving both). That plan must receive independent `STATUS: ok` before any source edit. The full executable delta, including `43d9b8fc`, must still receive full-scope independent `STATUS: ok` before this checkbox is checked, splits are published, SA112 starts, or SA117 is treated as release-ready.
...
- [ ] **SA121 — Enforce the shrink-only baseline against the merge base.** `Tier 2 · Track 1 · deps: none · blocks any further baseline edit`
  ...
  **Blocked checkpoint (2026-07-27 — recorded partial delivery after the change-review cap; task remains open).**
  ...
  - **Decisions needed:** No product or architecture decision is open. Before clean continuation, a maintainer must explicitly authorize another focused **tests/documentation-only** cycle for `SA121-CR-003`, `SA121-CR-005`, and `SA121-CR-006`. That cycle must preserve the reviewed runtime/source delta, run the full focused and quality gates, and receive full-scope independent `STATUS: ok` before this checkbox is checked or a completion entry is added to `CHANGELOG.md`.
...
──────────────────────────────────   ──────────────────────────────   ─────────────────────────────────
SA121 (baseline monotonicity)         SA115 (e2e xdist; deps: none)    SA117 (RELEASE BLOCKER) ◄─ next
SA122a (gate registry) ──┐             │                                │  stamp + assert + push splits
  │                      │             │                                ▼
  │                      │             │                              SA112a → b → c → d → e → f
...
SA121 and SA122a have `deps: none`, but SA121 is authorization-blocked at its recorded partial checkpoint until another focused tests/documentation-only cycle is approved. SA122a remains independently unblocked; only SA122b carries an ordering bound.
...
**Parallelism.** The open tickets now occupy all three tracks: SA117 → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and the audit-derived governance chain SA121 / SA122a → SA122b on Track 1. No rebalancing move is available that would speed the critical path:
...
- **Track 1's own serialization is authorization-shaped, not capacity-shaped.** SA121 and SA122a share no files (SA121's remaining cycle is tests/documentation-only over `scripts/check_quality.sh`'s test suite and `quality_tools.md`; SA122a is a new registry + checker) and have no ordering constraint between them, so Track 1 is not actually blocked as a track: **SA122a may proceed now**, and SA121 resumes independently whenever its focused cycle is authorized. Only `decisions.md` is touched by both (SA122a records its publish-context disposition there), and that surface is already in the conditionally-shared closeout set covered by the Merge procedure.

- **Track 1 is now active and does not touch the critical path.** SA121's candidate edits `scripts/check_quality.sh` + the baseline gate but is authorization-blocked at the recorded partial checkpoint above; SA122a is purely additive (new registry + checker) and remains independently unblocked. None of these files appear in any SA112 child's allowlist or in SA117's stamp/assert scope, and none need PostgreSQL or Docker — so authorized Track 1 work can run fully concurrent with Track 3, including during SA112's exclusive-infra legs. SA122b is the single exception and is bounded below.
...
- **Track 1 — CLEAN TO CONTINUE ON SA122a; SA121 AUTHORIZATION-BLOCKED (2026-07-26 audit intake; off the critical path).** SA114/SA116/SA120 closed and the track was held in reserve; it now carries **SA121**, **SA122a**, and **SA122b**. SA121 has `deps: none` and needs no PostgreSQL/Docker, but its recorded partial checkpoint requires explicit authorization for another focused tests/documentation-only cycle before clean continuation. SA122a remains independently unblocked and may run concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e. **Next SA121 action:** authorize the focused CR-003/005/006 continuation. **Independent Track 1 action:** SA122a may proceed now.
- **Track 3 — BLOCKED AT SA117 PLAN REVIEW AFTER RECORDED PARTIAL DELIVERY (on the critical path).** The executable candidate at `43d9b8fc` is merged to `v87` by explicit maintainer instruction as a blocked preservation checkpoint, not as review approval. The checkpoint above records four executable blockers (`SA117-CR-001`, `003`, `005`, `006`) plus two remaining plan blockers (`SA117-CONT-PR-003`, `006`). **Next action:** obtain explicit authorization for one focused plan/review continuation that resolves the two plan blockers; only a plan-review `STATUS: ok` permits source edits. Then resolve the four executable findings, rerun the full gate, and obtain full-scope `Adaptive-change-review STATUS: ok` over the complete candidate. Do not publish splits, start SA112, check SA117, or treat the candidate as release-ready before that approval. SA96-PUBLISH (human-only) still holds until SA112f closes the umbrella. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Net.** One critical path — **SA117 → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 3's next action is **SA117**; SA112a follows it, and no SA112 umbrella implementation is authorized. Track 2 is implementation-ready but validation-paused pending explicit future maintainer approval (SA115-DEC-001 step 3: **Keep unauthorized**), merge after SA112. Track 1 runs the audit-derived governance chain: **SA121** is recorded-partial and authorization-blocked, **SA122a** is independently unblocked and is Track 1's next runnable action, and **SA122b** merges after SA112e. The 2026-07-27 rebalancing pass found **no safe move**: the only candidate (SA122a → the idle Track 2) is blocked by `wt-track2`'s unmerged SA115 commits, and Track 1 needs no relief anyway because SA121 and SA122a are file-disjoint and can be worked in either order.
