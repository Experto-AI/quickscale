...
- [ ] **SA59 (umbrella) — Stop auto-priming `QUICKSCALE_ALLOW_BYPASSRLS=1` in the test-unit path — blocked (split into SA59.1–SA59.4).** `Tier 2 → split · Track 1 · deps: none`
  ...
  **Split (2026-07-11):** Per roadmap policy — a checklist item sized Tier 3 (7-item blocker ledger) must be split before implementation. Four sub-slices defined below, each Tier 1–2. The umbrella remains blocked until all four are complete.
  ...
  - [x] **Investigation and planning (completed 2026-07-10):** The initial blanket-removal draft was rolled back before landing; the unit/integration gate-split decision above was adopted as the path forward. Confirmed that `publish.yml` invokes `scripts/test_unit.sh` without a PostgreSQL service or a NOBYPASSRLS/NOSUPERUSER role; removing the blanket export would fail the publish workflow. Worktree-only implementation artifacts were retained but remain unmerged, unreviewed, and blocked.

  **Unmerged worktree state (Track 1 only — not integrated into v87):** The following implementation artifacts exist only in the Track 1 worktree. They have not been merged into v87, are unreviewed, and are not safe to merge due to the open blockers distributed across SA59.1–SA59.3 below. No SA59 implementation is integrated into any release.
  ...
  - Blanket `QUICKSCALE_ALLOW_BYPASSRLS=1` removed from Makefile `test-unit` target and `scripts/test_unit.sh`. Module test suites stripped from `test-unit` (now DB-free core + CLI only). `scripts/test_integration.sh` created. `make test-integration` target added. `make test` updated to run both unit and integration. ci.yml `test` job split into separate unit/integration steps. PostgreSQL 18 service, database creation, and `quickscale_test_role` setup added to publish.yml `test` job. Gate-split decision record still pending — not yet added to decisions.md.
  ...
  - [ ] **SA59.1 — Validation harness + coverage plumbing.** `Tier 1 · Track 1 · deps: none`
    Build the DB-free unit gate and the PostgreSQL integration gate, split the CI workflows, and wire coverage artifact production and upload. Inherits three blockers from the umbrella ledger:
    - **F-SA59-E2E-008** — Integration runner must exclude e2e tests from normal CI. `scripts/test_integration.sh` clears module addopts via `-o "addopts="` instead of explicitly excluding `@pytest.mark.e2e` tests. This can pull Playwright-based end-to-end tests into the CI integration gate. E2E tests belong under a dedicated `make test-e2e` target, not the module integration run.
    - **F-SA59-PARITY-009** — `make ci` / local validation parity. The `make ci` target and `scripts/check_ci_locally.sh` run `make test-unit` only, missing the integration gate entirely. Either wire `make test-integration` into the local CI flow or stop documenting it as CI-local parity.
    - **F-SA59-COV-004** — Module coverage artifacts not persisted; upload parity and thresholds incomplete. The current worktree already persists core/CLI coverage XML (`quickscale_core/coverage.xml`, `quickscale_cli/coverage.xml`) via `scripts/test_unit.sh`'s `persist_coverage_xml()` and uploads them to Codecov in `ci.yml`. However, `scripts/test_integration.sh` cleans up its per-module coverage XML instead of persisting it, and no Codecov upload step covers per-module artifacts. The remaining gap is persisted per-module coverage artifacts, upload/publish parity for module suites, and enforced minimum thresholds across all suites.
    *Target files (SA59.1 share):* `Makefile:321-327`; `scripts/test_unit.sh:365-366` (split into unit/integration paths); `scripts/test_integration.sh` (persist module coverage XML; explicitly exclude e2e-marked tests instead of clearing addopts); `.github/workflows/ci.yml:234-351` (`test` job — add the NOBYPASSRLS role, matching `isolation-conformance`'s existing postgres-service shape); `.github/workflows/publish.yml` (`test` job — add a postgres service + NOBYPASSRLS role for the new integration gate); `scripts/test_isolation_conformance.sh:50` (existing role-creation pattern to reuse, not duplicate).
  ...
  - **Blocked state:** Implementation is held in the Track 1 worktree only. Not safe to merge into v87 until all sub-slice blockers are resolved. User selected stop rather than authorize the required administrator-side grant path for F-SA59-ROLE-006 (SA59.3).
  - **Continuation note (2026-07-11):** Billing's canonical restricted-role suite is green (216 passed, 1 explicit bypass skip). Social test context work is unreviewed and incomplete (106/108 passed in the last restricted-role run); two social restricted-role boundary tests still need the direct-role adaptation pattern. The true cross-organization UPDATE test awaits an explicit decision between a single bypass-RLS mark, a production RLS-write expansion, or accepting a delete/create approximation. No unreviewed continuation changes are merged to v87.
