...
- [ ] **SA13.1 — Delete the dead context API and gate the rest.** `Tier 2 · Track 1 · deps: none · RISK LEVEL: medium`
  ...
  *Acceptance:* `resolve_public_org_context` is gone; the three primitives are renamed with a leading underscore but remain importable; the new lint script runs in warn-only mode and lists the pre-migration callsite census (arch-audit Finding 3: `tenant_context` 26 + `org_scope` 13 + `set_current_org_for_context` 5 = 44) without failing CI.

  > **SA13.1 discovery note (2026-07-03):** After syncing `wt-track1` with `v87`, discovery mapped the SA13.1 scope. `resolve_public_org_context` confirmed has **0 module+core src callers** per the Finding 3 census (internal/test references remain); the task's delete scope is limited to the definition and intra-orgs callsites. The SA9.6 import-linter pattern to reuse is `scripts/check_module_core_imports.py`. **Blocking issue — developer decision needed:** the task text says the new warn-only gate should target *only the newly privatized primitives* (the three underscored renames), while the acceptance text says it should "list the pre-migration callsite census (arch-audit Finding 3: `tenant_context` 26 + `org_scope` 13 + `set_current_org_for_context` 5 = 44)" — a broader census that includes all current usage of the public names before they become private. These two scopes differ, and the acceptance text's 44 figure does not appear in the current codebase as a measured line count from any existing check. Implementation is **paused** until the intended scope is clarified: should the lint gate warn on use of the three privatized primitives only, or on the full pre-migration callsite census (arch-audit Finding 3: `tenant_context` 26 + `org_scope` 13 + `set_current_org_for_context` 5) that the acceptance line describes?
...
- [ ] **SA15.3 — Derive the human-readable registry as a generated artifact.** `Tier 1 · Track 2 · deps: SA15.2`
  ...
  *Files:* `tenancy.py`, `decisions.md`.
  *Acceptance:* the registry overview is generated from model markers; a test asserts the derived view matches the (soon-to-be-retired) literal registry; `decisions.md`'s hand-edited count assertion is removed.
