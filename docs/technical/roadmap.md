...
> **Track status (2026-07-06):** Track 1 — **5 open items** (SA35, SA39, SA41, SA45, SA47). Track 2 — **5 open items** (SA21.2 ready; SA43, SA37, SA38 chained; SA40 independent). Track 3 — **4 open items** (SA36, SA42, SA44, SA46). All three tracks have queued work — 5/5/4 parallelism.
...
───────────────────────────────           ───────────────────────────────────         ───────────────────────────
SA39 (deps: none)                         SA40 (deps: none)                           SA36 (deps: none) ─────┐
SA45 (deps: none)                         SA21.2 (deps: SA21.1 complete, SA36*)       SA46 (deps: none)      │
SA35 (deps: none)                         SA43 (deps: none)                           SA44 (deps: none)      │
SA41 (deps: none)                         SA37 (deps: SA43)                           SA42 (deps: none)      │
SA47 (deps: SA35, SA41 — soft sequence)   SA38 (deps: SA43)                           ▲                        │
                                                ▲                                      │                        │
...
> **SA34 — complete.** Added `python manage.py createcachetable` as a new deploy step in `start.sh.j2` after the migrate step. The SA21.1 `CACHES` fallback (`DatabaseCache` on `django_cache_table`) is never created by `migrate`, so a Redis-less deployment would previously 500 on the first throttled request. Updated step numbering from 6 to 7 steps.
> **SA34 follow-up (CR-SA34-001/002):** `createcachetable` is now gated behind `[[ -z "${REDIS_URL:-}" ]]` so it only runs for DatabaseCache deployments (Redis manages its own tables). Invocation also clears `RUNTIME_DATABASE_URL=""` so the superuser `DATABASE_URL` is used (the runtime role cannot run DDL), matching the migrate step's existing pattern. Added focused template regressions for both guards. Full detail in [CHANGELOG.md](../../CHANGELOG.md).
