...
- [ ] **SA167 — Standardize module wiring on one adapter style and one app-declaration authority.** `Post-v88 · Tier 2 · deps: SA151 (social registration), SA118 (manifest projection)`
  ...
  - **The documented rationale for the split is stale.** [implementation_contract.md §Manifest Adapter Architecture](implementation_contract.md#manifest-adapter-architecture) describes core-side adapters as *"compatibility fallback … used when the module-owned adapter is not importable (bundled/installed contexts)"*. That context no longer exists: `refresh_managed_adapters()` (`entry_point.py:255-286`) raises `ImproperlyConfigured` when a managed module's manifest is present but its adapter is not importable, per the AF7 fail-hard decision. The nine core-side adapters are therefore not serving the purpose the contract states they serve.
  ...
  - **Direction.** Module-owned is the Django-idiomatic side: a reusable app owns its `AppConfig` and declares its own requirements; the framework core does not enumerate its plugins. Standardizing means moving the nine core-side adapters into their modules, making `derivation.wiring_projections` the sole `apps` authority read from each module's own manifest, and either wiring `django_apps:` to that projection or removing the key.
  **Acceptance:** every shipped module owns its adapter under `quickscale_modules/{name}/src/quickscale_modules_{name}/adapter.py`, with `MANAGED_ADAPTER_ORIGINS` covering the full inventory and no per-module block remaining in `entry_point.py`; exactly one declarative authority produces `spec.apps` and it is read from the module's own `module.yml` for every module; `django_apps:` is either derived from that authority or deleted from all manifests and the schema, with no key parsed-but-unread; a conformance test fails when a module ships models or a migration without declaring its app; the implementation contract's adapter section is rewritten to describe the actual discovery path and the retired bundled-without-source context; generator emission parity is unchanged, proving the refactor is behaviour-preserving; `make quality` is no worse than found.
  **Shared conflict surface:** `quickscale_core/src/quickscale_core/manifest/entry_point.py`, `quickscale_core/src/quickscale_core/manifest/{schema,loader}.py`, every `quickscale_modules/*/module.yml`, every module `adapter.py`, `docs/technical/implementation_contract.md`, SA90 emission-parity fixture. **Do not start before SA118 merges** — both change what a manifest projects into wiring.
