...
SA128 (structural parity check)         SA115 (e2e xdist; deps: none)  SA117e (review + push splits) ◄─ next
  ...
  ▼  CONTINUATION IN PROCESS             │  cannot finish → SA112f      │  human-confirmed public push
...
**Diagram checkpoint (2026-07-30, second pass).** Track 3 begins at SA117e — SA117a/b/c are closed and merged, so no closed node remains on the diagram. Track 1 begins at SA128, whose continuation is **in process on `wt-track1`** (decision-recording commit `c4e6560d`, not yet merged to `v87`).
...
> **Re-verified on 2026-07-30 (second pass).** SA117a/b/c are closed and fully merged; SA117e is Track 3's next action. Branch state re-measured: `wt-track3` carries **zero** commits unmerged into `v87`; **`wt-track1` carries `c4e6560d` (SA128 decision-recording, documentation only) — Track 1 is in process**; `wt-track2` still carries `5193f198` + `5b5de830` and remains closed to new work.
>
...
**Net.** The Track 3 critical path is **SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1's release blocker is resolved; in-process SA128 is followed by SA129, SA130, and SA122b, all off-path, with SA122b retaining the SA112e merge bound. Track 2 remains merge-gated.
...
**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of preempting **in-process** SA128 and delaying its queued SA129 → SA130 → SA122b successors; it changes no track's three states. **Recommendation: decline while SA128 is live. Default while undecided: SA112a stays on Track 3; Track 1 continues SA128 → SA129 → SA130 → SA122b.** Every individual ticket records **Decisions needed: none**.
