...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-1 → -2 → -3 → -4 → -5
  ...
  │  AUTHORIZED — executable now      │  cannot finish → SA112f     │  review · source · infra ·
...
| **Track 1** — SA112a | **yes** — `SA112A-HANDOFF-001` authorized 2026-08-01 | **yes** — `deps: none`, service-free | **yes** — no order gate | ✅ **yes** | ✅ **yes** — real progress |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Both truly green tracks are on the critical path, and they are the two parallel legs of it** — Track 3 advancing `SA117e-1 → -4` and Track 1 delivering SA112a, whose merge is SA112b's other precondition. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **`SA112A-HANDOFF-001` — AUTHORIZED, 2026-08-01. SA112a is executable; no gate remains open on it.** The maintainer explicitly authorized **one fresh `Adaptive-implement` attempt for the exact four-file scope** above, to be passed to the control plane as `user_authorization` (the prior refusal was a persisted `blocked_retries=1/1` ledger, not a design objection). **The grant is one attempt, not standing** — if this attempt ends without a checked box, it records its retrospective per [Rules every ticket inherits](#rules-every-ticket-inherits) and requires a fresh authorization. It restores the handoff budget only; every other gate stands unchanged — the executable slice still needs full-scope review `STATUS: ok` before closeout, and the closeout still needs its own final review. **Reuse the approved plan above verbatim** — do not re-plan unless `VERSION`, `scripts/smoke_install.sh`, or its mapped caller topology has changed. No executable/test file has changed yet; no syntax, focused, smoke, negative-control, parity, lint, type, executable-review, or completion-review evidence is claimed. `F-003` (documentation consistency) is **closed** by the four-file `Files` line above and the Track-topology allowlist.
...
**Status:** **head is SA112a, and the track is now truly green — can start (`SA112A-HANDOFF-001` authorized 2026-08-01), can finish (`deps: none`, four-file disjoint allowlist, service-free validation), can merge (no merge-order gate; SA112a is itself SA112b's upstream).** It is on the critical path, so this is real progress, not filler. The approved plan in its task block is reused verbatim. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
3. **`SA112A-HANDOFF-001` — AUTHORIZED, and SA112a stays Track 1's head.** One fresh `Adaptive-implement` attempt is granted for SA112a's exact four-file scope; the grant is per-attempt, not standing, and waives no review gate. The **reordering question is resolved in favour of Option A**: off-path SA128a stays queued behind SA112a rather than consuming the serial worktree, because SA128a's consumer SA122b-1 is independently gated behind SA128d, so queueing it delays no release property while starting it would push a critical-path node behind filler. **Effect: Track 1 becomes truly green on the critical path**, running in parallel with Track 3's SA117e children.

Track 1 has completed SA112a planning and **is now cleared to dispatch implementation** under the `SA112A-HANDOFF-001` grant. Track 3 is preserved at **SA117e-1**, whose historical review ran but whose documentation closeout remains open. Plan-review `F-002` blocks SA117e-3's host-service mutation pending that child's scoped plan-review `STATUS: ok`; `SA117E1-REV-001` and `SA117E1-REV-002` separately block SA117e-4's public split push. Those standing decisions are distinct from the push confirmation and later SA96-PUBLISH, which remain **execution-time** human gates obtained at the outward-facing action rather than pre-granted authorization.
...
- **Current state.** SA112a's fresh scoped plan passed independent plan review (`STATUS: ok`) and is recorded verbatim in its task block; no executable delta exists yet. Implementation is **authorized and dispatchable** (`SA112A-HANDOFF-001`, 2026-08-01) — one attempt, reusing the approved plan verbatim.
