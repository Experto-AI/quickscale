...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **Open at the second recorded-partial checkpoint (2026-08-03) — the task remains unchecked and is not SA112b-ready.** Delivered work, validation evidence, `F-003`'s resolution, and the reviewed-tip identity gap from the post-review Ruff hooks are recorded in [CHANGELOG.md](../../CHANGELOG.md). What remains:
    ...
    - **Decision needed (maintainer's, not upstream work):** a fresh **`SA112A-HANDOFF-003`** authorization for one focused correction/validation/review cycle. Both review passes ended at highest blocking severity **medium**, count **1**; the maintainer selected recorded partial delivery and merge at the cap, so the per-attempt ledger is spent, not the design.
