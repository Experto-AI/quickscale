...
The green-gate join (SA96-GATE), the installed-wheel discovery/resolver chain (SA109 ✓, SA110 ✓, SA113 ✓, SA111a ✓, SA111b ✓), the Track 2 frontend de-specialization chain (SA104 → SA108 ✓), the Track 1 re-verification chain (SA114 ✓, SA116 ✓), and SA120 (quiet `check` parity) are closed; detail lives in [CHANGELOG.md](../../CHANGELOG.md). Eight tickets remain open (grouped as the seven entries below) — SA117 (now the four serial children SA117a, SA117b, SA117c, SA117e, split on 2026-07-27 as `SA117-DEC-003`) is a release blocker from the 2026-07-26 diagnostic spike, and SA125/SA121–SA122b are the gate-governance chain that keeps Track 1 active.
...
4. **SA125** (retire the file line-ceiling gate, Track 1, `deps: none`) — **Track 1's next action, and it gates two other continuations.** `SA125-DEC-001` (2026-07-28) retires per-file `max_lines` enforcement and keeps per-function complexity enforcement. Tier 1, mostly deletion. Landing it first dissolves three of SA117a/SA117b's five quality blockers and narrows SA121's remaining test matrix.
5. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: SA125` — shared files, must be serial) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2. Its authorized continuation is preserved as recorded partial delivery; SA121 remains open on `SA121-CR-003`, `SA121-CR-006`, and `SA121-QG-001`, but **carries no open decision** — `SA121-DEC-002` authorized the minimal parser correction, and QG-001 now clears via SA125 plus Track 3's `SA117-QG-REMED`.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **`SA117-QG-REMED` — quality-ceiling remediation. `Track 3` · `deps: SA125` (narrowed 2026-07-28 by `SA125-DEC-001`).** `SA121-QG-001` originally recorded five exceeded ceilings introduced by `43d9b8fc`. **SA125 retires the file line-ceiling gate entirely, dissolving three of the five**; only the two complexity findings remain, and each is already inside its child's declared allowlist.
  ...
  - **Ordering:** SA125 lands on Track 1 and merges to `v87` **before** SA117a's continuation starts, so Track 3 syncs a tree where only the CC finding remains. Each child must still leave `make quality` no worse than it found it and clear its own share before its slice review; per SA121's shrink-only rule, **raising a complexity ceiling is not an acceptable remedy** — reduce the branching, or record a structured waiver once SA121 ships the waiver ledger.
  - Conflict surface: none beyond each child's existing allowlist. SA117a/SA117b no longer touch `scripts/quality_baseline.json` at all (the new-file baseline entry for `git_utils.py` is moot once `large_files` is gone), so the file is owned solely by SA125 on Track 1.

  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: SA125 (continuation only) · blocks SA112b` — **critical path**
    > **Deps note.** SA117a's *original* scope had `deps: none` and its already-merged partial delivery was executed under that. The `SA125` dependency applies **only to the remaining continuation**, which must not start until SA125 merges to `v87` — otherwise it inherits three quality blockers that SA125 deletes.
...
Track 1 holds the gate-governance work: it changes how "green" is decided, not what the generator emits, and touches **no** Track 3 release-path file. `TA62`/SA120 is closed (see [CHANGELOG.md](../../CHANGELOG.md)), leaving **SA125**, **SA121**, and **SA122a/SA122b** — the latter two from [arch-audit.md](../../arch-audit.md) Findings 12 and 11.

**Ordering on this track (one worktree, serial):** **SA125 → SA121 → SA122a**, with SA122b merge-gated behind SA112e.
- **SA125 first.** It retires the file line-ceiling gate (`SA125-DEC-001`), which dissolves three of the five `SA121-QG-001` regressions outright and narrows `SA121-CR-003`'s remaining test matrix. It also unblocks Track 3's SA117a continuation, so it is the one Track 1 ticket with a downstream consumer on the critical path.
- **SA121 second.** It shares four files with SA125 and must rebase onto the narrowed surface rather than run concurrently. Per the arch-audit fix-order section, Finding 12 should land **before any further `quality_baseline.json` adjustment** — SA125 is precisely that adjustment, which is why it goes first and carries its own decision record instead of arriving as an unexplained baseline edit.
...
- [ ] **SA125 — Drop `large_files` from the quality gate; keep `complexity`.** `Tier 1 · Track 1 · deps: none · blocks SA121 continuation + SA117a continuation`
  ...
  **Decision (`SA125-DEC-001`, 2026-07-28): per-file line ceilings are retired; per-function cyclomatic-complexity ceilings stay.** The two metrics sit in the same `scripts/quality_baseline.json` under the same enforcement and have been treated as equally authoritative. They are not.
  ...
  - **Not a weakening of fail-hard policy.** [§fail-hard-principle](./decisions.md#fail-hard-principle) governs runtime behavior on absent or invalid configuration; this is a governance-metric retirement, not a runtime fallback. The advisory `LARGE_FILE_WARNING_LINES`/`LARGE_FILE_ERROR_LINES` reporting may be retained as a **non-blocking diagnostic** — a tripwire that prompts a review question — but must not gate `make quality`.

  1. Remove the `large_files` section from `scripts/quality_baseline.json` (41 entries) and its blocking enforcement in `scripts/check_quality.sh`, leaving `complexity` (151 entries), `dead_code`, and `duplication` intact.
  2. Narrow `scripts/check_quality_baseline_monotonicity.py` and `scripts/test_quality_baseline_monotonicity.py` to the surviving sections — the SA121 helper must no longer parse or diff `max_lines` keys.
  3. Update `docs/technical/quality_tools.md` and record the rule in `docs/technical/decisions.md` (new §file-size-metric-policy, cross-referenced from the shrink-only policy so the two do not contradict).
  4. Decide and record whether the advisory large-file *report* is retained as non-blocking output or removed outright; either is acceptable, but the choice must be explicit rather than incidental.

  - Verify: `make quality` no longer fails on any file line count and exits 0 on the three previously-breaching files at current HEAD; the two complexity findings (`_perform_module_embed` 22, `_update_single_module` 18) **still fail**, proving the complexity half is untouched; the monotonicity helper rejects a complexity-ceiling increase and no longer recognizes `max_lines` keys; a fixture with a 2,000-line file passes the gate; `quality_tools.md` and `decisions.md` agree with the implementation.
  ...
  **Sequencing (maintainer decision, 2026-07-28): SA125 runs first on Track 1, and Track 3's SA117a continuation waits for it.** SA125 is a deletion sized Tier 1. Landing it before SA117a shrinks that child's quality share from three line reductions plus one complexity fix to the complexity fix alone, and avoids a 124-line `loader.py` refactor against a ceiling being deleted. It also shrinks SA121's own remaining work, since `SA121-CR-003`'s large-file separator-key cases stop existing. The cost is a short pause at the head of the critical path; the alternative — Track 3 proceeding CC-only in parallel — was rejected because it leaves two tracks mid-change on the same gate definition and keeps `make quality` red longer.

  **File overlap with SA121 — must be serial on Track 1.** SA125 and SA121's continuation both edit `scripts/check_quality.sh`, `scripts/check_quality_baseline_monotonicity.py`, `scripts/test_quality_baseline_monotonicity.py`, and `quality_tools.md`. They are one coherent unit of gate-policy work and **must not** be split across worktrees or run concurrently. SA125 first; SA121's continuation then rebases onto the narrowed surface. `SA121-DEC-001`'s prohibition on SA121 touching `scripts/quality_baseline.json` is unaffected — SA125 owns that file, SA121 still does not.
...
- [ ] **SA121 — Enforce the shrink-only baseline against the merge base.** `Tier 2 · Track 1 · deps: none · blocks any further baseline edit`
  ...
  **Recorded partial delivery (2026-07-27, two merged checkpoints — `3485e209` plus the tests/documentation continuation; delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)). Task remains open.** The merged candidate carries the merge-base monotonicity helper, a separate empty waiver ledger, `make quality` integration, policy/operator documentation, and focused coverage, with `scripts/quality_baseline.json` unchanged and `scripts/quality_waivers.json` empty. The merges are recorded partial delivery only — not a waiver, review approval, or permission to check SA121.
  ...
  - **Pending / blocking:** `SA121-CR-003` (**high**, completeness) — `complexity:file.py:::func` is accepted as an `orphan` (the extra colon is swallowed into the symbol) instead of rejected as `malformed`, so the new test is false-green; unknown-prefix, empty-component, missing/extra `::`, nonliteral duplication key, and unsafe-waiver-path cases still need exact malformed-waiver exit-1 and malformed-baseline exit-2 envelope assertions. `SA121-CR-006` (**medium**, test gap) — acceptance-critical sites still use filtered/subset, `>= 1`, `any(...)`, or non-empty-message assertions instead of complete ordered records, exact counts, and exact envelopes. `SA121-QG-001` (**high**, validation) — `make quality` exits 2 on five `v87` runtime regressions outside Track 1. **Resolution is now split:** `SA125-DEC-001` dissolves the three line-ceiling findings by retiring that gate, and the two surviving complexity findings stay with Track 3's `SA117-QG-REMED`. SA121 needs no disposition decision either way. Note `SA121-CR-003`'s remaining matrix shrinks once SA125 lands — the extra large-file separator-key cases stop existing — so **run SA125 first and rebase this continuation onto the narrowed surface**. `SA121-REV-META-001` (**low**, advisory) asks future evidence to distinguish commands run separately after an `&&` chain stops.
...
**Current status — `make quality` is RED (recorded 2026-07-27 as `SA121-QG-001`; rescoped 2026-07-28 by `SA125-DEC-001`).** The exit criteria above are not currently met. Five ceilings are exceeded, and they now clear by two different routes:
- **Three dissolve with the gate.** `module_commands.py` 1642 > 1608, `manifest/loader.py` 686 > 562, and unbaselined `git_utils.py` at 509 are file line ceilings, retired by **SA125** on Track 1. No code is reduced for them.
- **Two are remediated.** `_perform_module_embed` CC 22 > 20 (SA117a) and `_update_single_module` CC 18 > 17 (SA117b) are complexity findings, kept and owned by Track 3's `SA117-QG-REMED`.

All five originated with the SA117 preservation checkpoint `43d9b8fc` (recorded partial delivery, unapproved). Until both routes land, no track may claim the green-gate milestone, and SA121 cannot close because its own acceptance requires `make quality` green.
...
──────────────────────────────   ────────────────────────────   ─────────────────────────────────
SA125 (drop line ceilings) ◄─ next SA115 (e2e xdist; deps: none) SA117a (exact lockstep)
  │  (Tier 1; mostly deletion)     │  (validation AUTHORIZED)     ▲   + CC 22→20 share only
  ├──────────── unblocks ──────────┼──────────────────────────────┘
  ▼                                │                             ▼
SA121 (baseline monotonicity)      │                             SA117b (publish-path safety)
  │ (partial; DEC-002; deps SA125) │                              ▼   + CC 18→17 share
...
**Track 1 is now internally serial and has one downstream consumer.** SA125 (`deps: none`) runs first; SA121 follows because it shares four files with it; SA122a follows those; SA122b carries the SA112e merge bound. The new edge worth noticing is **SA125 → SA117a**: a Track 1 ticket now gates a Track 3 critical-path child. That is deliberate — SA125 deletes three of SA117a's five quality blockers, so waiting for it is strictly cheaper than remediating them.
...
**One temporary upstream node (2026-07-28).** `SA125` (Track 1) now precedes `SA117a`, making the effective near-term head `SA125 → SA117a → …`. It is not a permanent member of the chain: it is a Tier 1 deletion that removes three of SA117a's five quality blockers, so it shortens the chain's remaining work by more than the pause it costs. Once SA125 merges, the critical path reverts to the sequence above.
...
- **Track 1 is serial by file overlap, and SA125 briefly feeds the critical path.** SA125 and SA121's continuation share `scripts/check_quality.sh`, both monotonicity scripts, and `quality_tools.md` — one coherent unit of gate-policy work that must not be split across worktrees. SA122a is independent of both (a new, purely additive registry + checker touching no SA112/SA117 allowlist and needing no PostgreSQL/Docker), so in principle it could run concurrently — but Track 1 is one worktree, and a fourth worktree would buy no release time since SA122a is not on the critical path. SA125 *is* momentarily on it, via `SA125 → SA117a`; it is Tier 1 and mostly deletion, so the pause is short and pays for itself by deleting three SA117a blockers. Only `decisions.md` is conditionally shared, and the Merge procedure covers it. SA122b is the single further ordering exception, bounded below.
...
**Conflict surface.** The shared closeout files are `CHANGELOG.md` and `docs/technical/roadmap.md`, plus `docs/technical/decisions.md` when policy or acceptance evidence changes (SA122a writes its publish-context disposition there; SA121's continuation touches quality-policy prose). All three are covered by the Merge procedure above: the `git merge v87` before every merge-back forces conflicts to resolve on the track branch, preserving both sides. Two executable surfaces are also shared and already bounded: `.github/workflows/e2e.yml`'s path list between SA112e and SA115-CI-001 (serialized by the `merge after SA112` bound), and `scripts/quality_baseline.json`, where SA117a/SA117b may add a first-time `git_utils.py` entry while SA121's continuation is contractually forbidden from touching that file at all (`SA121-DEC-001`) — so the two cannot collide and no additional procedure is required.

### Track readiness (2026-07-28)

- **Track 1 — DECISION-CLEAR AND RUNNABLE NOW; briefly feeds the critical path.** The track carries **SA125**, **SA121**, **SA122a**, and merge-gated **SA122b**, run serially in that order. **Next action: SA125** — retire the file line-ceiling gate per `SA125-DEC-001` (Tier 1, mostly deletion), then merge to `v87` promptly, because Track 3's SA117a continuation is waiting on it. SA121 follows on the narrowed surface (it shares four files with SA125 and must not run concurrently); it stays a partial checkpoint on CR-003/CR-006 with **no open maintainer decision**, and still cannot check its box until `make quality` is green — which now needs SA125 plus Track 3's two complexity fixes. `SA122-DEC-001` already resolves SA122a's publish-context disposition. Nothing on this track needs PostgreSQL/Docker, so it runs concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e.
