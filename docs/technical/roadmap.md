...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA139 ◐ (recorded partial;             SA115a ◐ (local delta green;   SA136d ◐ (corrected seal tooling;
  │  validation-blocked)                  blocked by SA139-QG-001)
  │  ── unblocks ──────────────────►  │  no shared file, no gate     │  review+validation-blocked)
...
| **Track 1** — SA139 (head) | **yes** — reviewed root-anchor implementation is in place and the import-disambiguation scope expansion is granted | **no** — parity and both umbrella commands remain blocked before E2E until `SA139-QG-001` is fixed; no decision remains, only implementation | **yes** — no merge-order gate; recorded-partial preservation writes no shared executable file | no — validation-blocked | no — off-path, but it is the only ticket that unblocks another track |
...
**Truly green today: none.** **SA136d remains on the critical path and review+validation-blocked**: full-scope checkpoint review resolved F-006 but retained F-004 and late-surfaced F-007/F-008, while an npm registry timeout stopped the final ordered gate before `make quality`; the tooling is not release-authorized. **SA139 is fully unblocked to attempt**: its scope expansion is granted, and the remaining work is ordinary implementation. It stays off-path but remains the board's one unblocking move, converting Track 2's SA115a from "cannot finish" to finishable once it merges and SA115a reruns its two umbrella commands. SA122b-5's merge bound is a **hard upstream dependency** only SA112e can clear.

**Open maintainer decisions: none.** On 2026-08-09 the maintainer explicitly selected the SA136d recorded-partial checkpoint merge despite unresolved high-severity F-007; SA136d remains unchecked, and the decision grants no seal or release authorization. A future correction cycle still requires fresh authorization. The board's prior decisions remain applied: `SA136D-QG-001` is dispositioned as **reviewed acceptance** under the inherited "no worse than found" rule, with the repair ticketed separately as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); and SA139 has its import-disambiguation scope expansion. All settled topology decisions are recorded in [CHANGELOG.md](../../CHANGELOG.md).
...
- [ ] **SA139 — Repair coverage-policy gate-target derivation so `--e2e` reaches E2E.** `Tier 2 · deps: none` — **no merge gate; unblocks Track 2's SA115a**
  ...
  The reviewed root-anchor implementation is in place, but parity and both umbrella commands remain blocked before E2E by `SA139-QG-001`: during whole-repo collection, `quickscale_core/tests/test_e2e_xdist_fixtures.py:13` imports `conftest` ambiguously and resolves to `quickscale_cli/tests/conftest.py`. The defect is in the owning validation scope, not in SA115a's reviewed executable delta.
  ...
  - **Recorded-partial checkpoint (2026-08-08; task remains unchecked).** **Done:** root-anchor implementation, 87 focused tests, Ruff check/format-check, unchanged quality baseline, and executable review `STATUS: ok` at confidence `95`. **Pending-Blocking:** `SA139-QG-001` (**medium**, validation) — `quickscale_core/tests/test_e2e_xdist_fixtures.py:13` imports `conftest` ambiguously and resolves to `quickscale_cli/tests/conftest.py` during whole-repo collection; constrained parity is 6 passed / 2 collection errors; `make ci-e2e` exits 2 after coverage-policy completion with 4807 passed / 2 skipped / 1 collection error; `./scripts/check_ci_locally.sh --e2e` exits 1 at the same collection error after backups 323 passed / 2 skipped and coverage 92.99%. `make check` matched the pre-edit 4753 passed / 1 skipped / 1 error; `make quality` counts matched pre-edit exactly (dead 5, high complexity 148, critical 22, large 22, very large 21, duplication 0, warning regressions 0, critical regressions 1). **Decisions-needed:** none — the scope expansion below is granted; apply it, then rerun the exact validation commands. No completion claim is made.
  - **Scope expansion granted (2026-08-08 maintainer decision).** Fix the ambiguity **at the importing test** — `quickscale_core/tests/test_e2e_xdist_fixtures.py:13` — by making its `conftest` import unambiguous rather than hardening the derivation helper further. Rationale: the root-anchor implementation already passed executable review `STATUS: ok`; the residual failure is a genuine import-resolution ambiguity in a different file, and resolving it at its source is smaller and more honest than teaching the helper to tolerate an ambiguous module name. **This does not relax the SA128 oracle** — no tolerance or exception logic may be added to make a failing context read green, exactly as every SA122b child inherits.
...
**Rebalancing verdict: no relocation available; one new ticket (SA140).** Nothing is movable — SA136f/SA117e-4 are dependency-bound to SA136d, SA112b–f are one serial evidence chain, SA139 → SA122b-5 is in-track ordered, SA140 is evidence-bound behind SA112b, and Track 2 stays closed to unrelated work. Docker/PostgreSQL remains serialized with Track 3 priority when SA115a reruns its umbrella gates.

**Open track-topology decisions: none, and no open maintainer decision remains.** The maintainer explicitly selected SA136d's recorded-partial checkpoint merge with unresolved F-007 while withholding all seal/release authorization; a future correction cycle requires fresh authorization. `SA136D-QG-001` and SA139's scope expansion remain dispositioned and alter no topology. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
