...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; F-002 closed;        SA115a ◐ (acceptance-blocked   SA136d ✓ (reviewed + merged)
  │  full reruns infra-failed;         │   by SA141; only ticket      │
  │                                  │   on this track)             ▼
...
**Historical-overview note:** the SA136d-specific clauses in the critical-path paragraph immediately above and in the authorization paragraphs below retain the pre-DC-58 recorded-partial state for audit. The authoritative current state is the final SA136d closeout under Track 3: DC-60 approved reviewed tip `96858daf4394c42a0ef0c7be781dba84e3981064`, named `wt-track3` advanced to it, and `v87` integrated it as merge `47165663cfbe8292d60b1be8c85002f2fc70a3e7`. The live critical-path head is now SA136f.
...
|---|---|---|---|---|---|
| **Track 3** — SA136f (head; SA136d closed) | **yes** — all SA136a–e dependencies are closed | **yes** — documentation-only closeout scope is defined | **yes** — no merge-order gate | **yes** — SA136d is reviewed and integrated; SA136f is startable | ✅ **yes** |
| **Track 1** — SA141 (head) | **no — third decision required** — the second grant closed F-002 but consumed both quality runs without an exit-0 full gate | **no** — the first full run ended on two external registry failures; after registry probes recovered, the second ended on one Core Poetry-install timeout | **yes, checkpoint only** — no merge-order gate; the reviewed recorded-partial delta may merge, but SA141 remains a prerequisite for SA112f | **no** — validation-gated | no — off-path, but a hard green-gate prerequisite |
...
**Truly green today: Track 3.** SA136d is reviewed and integrated, so SA136f is the live documentation-only head. SA141 consumed its second grant: F-002 is closed, but both full-gate runs ended on external dependency failures and a third continuation now needs a maintainer decision after cache remediation. **SA122b-5**'s merge bound is clearable only by SA112e, and **SA115a** cannot finish until SA141 closes. The release-wide green gate remains unclaimed.

**Open maintainer decisions: one — whether to authorize a third SA141 full-gate cycle after the Poetry/package caches are restored.** History: the maintainer merged SA136d's recorded-partial checkpoint with F-007 unresolved (2026-08-09), then authorized SA136d's correction cycle and SA141's parallel start (2026-08-10), both of which ended at their caps in recorded-partial state. **The maintainer then granted a second continuation authorization for SA136d and SA141 together (2026-08-10).** SA141 consumed that grant by closing F-002 and running the full gate twice; neither run exited 0 because external dependency access failed at different sites.

**Scope of the second grant.** It opens exactly one further plan/implementation/validation/review cycle per ticket, on the retained tips, running concurrently under the infra-serialization rule below. It does **not** authorize any seal command, remote ref mutation, tag push, publication, or completion claim — those remain behind SA117e-4's execution-time human gate. It does not waive any review: SA136d's items still change production behaviour, so [Proportionality](#rules-every-ticket-inherits) still requires a scoped plan review before implementation, and that plan must close plan-review F-004 first. If either cycle again reaches its cap without a checked box, it returns here for a third decision rather than self-extending. Every other blocker is a hard upstream dependency or review finding, not a pending choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); SA139's scope expansion is implemented, reviewed, and closed. The remaining human gates are execution-time gates obtained at outward-facing actions, not pending decisions.

**Infra serialization (not a track constraint).** SA112's and SA115a's e2e lanes, SA117e-4's `apply` verification, and any `make ci`/`make ci-e2e` rerun all need the same PostgreSQL server, Docker daemon, and ports. The `QS_CI_PARALLEL`/`QS_E2E_PARALLEL` knobs namespace lanes *within* one invocation, not across worktrees — **only one track exercises PG/Docker at a time, and Track 3 has priority when its next service-backed leg is active.** SA136d's ordered run is complete, so SA141 may use the shared capacity until SA117e-4 or a later Track 3 validation leg starts; abandon or restart lower-priority work rather than queue an active critical-path leg.
...
**Current SA136d closeout checkpoint (authoritative through DC-58):** the clean candidate is `sa136d-sync-r3-a1` at `SYNC_COMMIT d117b1bae0d6aaacc7ddc93349200935231bc338`, with parents retained functional `be4b360b13254e1615d0daed1528e2713af5b348` and repaired `v87` `2891c4c8bfc00a859d0ceb7eb2bb851d2f0fe8a7`, and tree `865c2479abf36d011b86fa4421b42d4eaf55d213`. The retained functional commit was approved by DC-42; repair commit `12b15bcd6908ea6c29e2005e4e5187eb17ba7ef9` was independently approved by DC-54. DC-58 accepted the focused `publish87`/`git-utils225`/`beta50` evidence and the complete ordered chain: lint, typecheck, unit, integration, frontend, PostgreSQL, and `check` passed; coverage was core 90.45%, CLI 90.94%, integration mean 94.40%; quality was dead 5, high 148, critical 22, large 22, very-large 21, duplication 0, warning 0, critical 1, with the repaired target omitted and the sole accepted SA140 tuple 56/55. No new skips, tracked delta, baseline/waiver/threshold change, or outward action occurred. `SA136D-QG-003` is resolved by this accepted repair/reconstruction/full-chain evidence. The final full-scope independent review and named `wt-track3`/`v87` integration remain process steps and are not claimed here; the historical terminal attempt `8eed6f6f...` and old frozen v87 remain historical/immutable.

**Negative-action record:** no seal command, remote-ref mutation, tag, push, publication, PyPI action, SA136 umbrella closure, release completion, or merge-back was performed or claimed in this checkpoint.

**Final SA136d closeout (authoritative after integration):** DC-60 returned `STATUS: ok` over repair, retained seal functionality, and closeout docs at reviewed tip `96858daf4394c42a0ef0c7be781dba84e3981064` (tree `ae65dc6cc4daa8c15bf10f8113400a451f693808`) with no blocking or advisory findings. Named `wt-track3` fast-forwarded from `be4b360b13254e1615d0daed1528e2713af5b348` to that exact tip without a restoration commit. Clean repaired `v87` `2891c4c8bfc00a859d0ceb7eb2bb851d2f0fe8a7` then merged the named branch as `47165663cfbe8292d60b1be8c85002f2fc70a3e7`, with parent 1 the repaired-v87 tip, parent 2 the reviewed tip, and merged tree `ae65dc6cc4daa8c15bf10f8113400a451f693808`. `SA136D-QG-003`, F-007, F-008, and executable F-004 are resolved; SA136d is closed and the critical-path head advances to SA136f. No seal command, remote-ref mutation, tag, push, publication, PyPI action, SA136 umbrella closure, or release completion occurred.

The following status paragraph and detailed ledger retain the pre-DC-58 recorded-partial history; they are not the current evidence state above.

**Status:** on the critical path and **cleared to start — the maintainer granted a second continuation authorization on 2026-08-10**. **Head: SA136d** (add the seal phase and Make targets), resuming from the retained corrected implementation tip `355f65a1`; no correction code landed in the previous attempt, which capped during plan review. Prerequisites remain closed and the 2026-08-07 client-side check-then-act re-scope remains ratified. The earlier fresh focused plan passed independent review and all implementation slices landed; the first executable review retained F-004/F-006, and their corrections are committed with focused and full test evidence. The later full-scope checkpoint review resolved F-006, kept executable F-004 open, and late-surfaced F-007 (high: requested seal version is not bound to repository `VERSION`) plus F-008 (medium: same-target lightweight tags violate the idempotent predicate). The second ordered quality run also stopped during `make check` while fetching uncached frontend dependencies, leaving `make quality` pending under `SA136D-QG-002`. The latest correction plan closed three plan-review findings but capped with plan-review F-004 (high: every committed retry branch needs a terminal failure disposition and fresh-branch reconstruction rule). The retained commands must not be used to seal release refs until both the plan gate and executable/validation blockers close. SA136d is the umbrella's last open non-closeout child and the only unsatisfied member of SA136f's dependency set. SA112b's provisioning precondition is already satisfied; its split-publication precondition waits on SA117e-4, which waits on SA136.
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — critical path; **blocks SA117e-4**
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [x] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed` — complete ordered validation accepted by DC-58; final full-scope review DC-60 passed; reviewed tip `96858daf` merged to `v87` as `47165663`
    ...
    **Seal-atomicity requirement re-scoped (settled; rationale in [CHANGELOG.md](../../CHANGELOG.md)).** Server-transactional atomicity is **withdrawn as a requirement**. The seal is enforced client-side as check-then-act with a fail-closed compensating verification, and the residual window is documented rather than engineered away. This unblocks the ticket on stock Git against GitHub.com; no platform migration is required.
    ...
    - **Retained tip and open ledger (task remains unchecked).** The corrected implementation tip is `355f65a1`; what landed, what passed, and the review history are in [CHANGELOG.md](../../CHANGELOG.md). Four blockers remain open, and no release claim is made.
      ...
      - **Authorization state — CONTINUATION AUTHORIZED 2026-08-10 (second grant).** The first 2026-08-10 grant was consumed by two capped plan-review rounds without implementation; the maintainer has now granted one further cycle on the retained tip. `SA136D-QG-001` stays **accepted** (reviewed, 2026-08-08) under the inherited "no worse than found" rule; no ceiling is raised and SA140 owns the repair. The grant covers plan → implement → validate → review only. It covers **no** seal command, remote ref mutation, tag push, publication, or completion claim. Reaching the cap again without a checked box returns SA136d to the maintainer for a third decision.
      - **Carry-forward plan (this cycle's scope).** Implement the items below on the retained tip, revalidate, and obtain review. Because items 1–3 change production behaviour, [Proportionality](#rules-every-ticket-inherits) requires a scoped plan review over this slice before implementation — and that plan must first close plan-review F-004 (item 0). Order matters: fix F-007 first among the code items, since it is the only one that can mint a wrong-identity irreversible tag.
        0. **Plan-review F-004 (high, completeness).** The plan's rollback phase must state that **every** committed synchronization/retry branch whose later validation or review fails is terminal, and that each further authorized attempt reconstructs on a previously unused local branch from retained `TRACK3_PRESYNC`, merges frozen `V87_SYNC`, and passes the existing parent/ancestry/exact-tree assertions before review.
        ...
        Continuation starts from the retained corrected tip (`355f65a1`), not from a fresh branch. Allowlist is unchanged. No approved `plan_carryover` exists — the latest plan never reached `STATUS: ok`. Until every item above closes, the seal commands remain **not release-authorized**.
...
- [ ] **SA141 — Diagnose and repair the two Core E2E Poetry-install failures.** `Tier 2 · deps: none` · **blocks SA112f, SA115a acceptance, and the green gate**
  ...
  - **Retained repair and open ledger (task remains unchecked).** The diagnosis (ambient `VIRTUAL_ENV`/`POETRY_ACTIVE` inheritance under `poetry run pytest`), the isolation seam in `poetry_env.py`, and the complete second-continuation evidence are recorded in [CHANGELOG.md](../../CHANGELOG.md). `_run_repo_poetry_command` remains the intentional ambient repository-only exception. No green-gate claim is made.
    ...
    - **Authorization state — SECOND GRANT CONSUMED.** Both `qg_runs` were used without an exit-0 full gate, so the recorded-partial default applies and SA141 returns to the maintainer. No plan carryover exists — the medium-tier plan was not plan-reviewed and every planned slice landed. Advisory F-003 remains discharged by recording `apply_command.py` as serialized SA141-then-SA140 work.
...
**The milestone is unclaimed, and has two named prerequisites.** [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance) has closed every named environment-isolation defect, but its two raised-watchdog full reruns ended on external dependency failures (first two CLI registry failures, then one Core Poetry-install timeout), so `SA141-QG-001` remains open. `make quality` separately exits 2 pending [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). No single clean rerun has covered all four commands, and `make ci` in particular has not been rerun. **SA112f owns the pre-SA140 installed-wheel/E2E acceptance; after SA140 lands, SA96-PUBLISH owns the final four-command join before any publication action.**
...
**Open track-topology decisions: none. One non-topology maintainer decision remains:** whether to authorize a third SA141 quality-gate cycle after cache remediation. The 2026-08-10 second-grant continuations for SA136d and SA141 (see [Dependency & parallelization overview](#dependency--parallelization-overview)) were authorization choices, not topology ones — the two share no file and sit on different tracks, which is exactly the two-track concurrency this topology was built to allow. Every other remaining “no” in the readiness table is a hard dependency. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
