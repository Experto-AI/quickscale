...
- [ ] **SA129 — Carry the superuser answer on an explicit sentinel line.** `Tier 2 · deps: none · Track 1 head`
  ...
  - Open: the sentinel delta is implemented and its production correctness approved, but the complete-argv test still derives its script expectation from the observed argv instead of a literal independent script, and four handler branches still need byte-exact stdout/stderr plus consistent create-command presence/absence assertions. The fresh-project `apply` proof is **waived, not passed** (Track 3 held the shared infrastructure) and remains visible residual risk. A continuation corrects only the test gap, revalidates, and obtains full-scope `STATUS: ok`.
