...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  ...
  - **Blocked checkpoint (2026-07-23; not complete):**
    - **Done:** Synced Track 1 to `v87` at `556f7510`; fixed gate drift outside the SA113-owned surface; added focused migration-seam coverage; and obtained clean ordered evidence: `make check` exit 0, `make quality` exit 0 with zero baseline regressions, `make ci` exit 0, and `make ci-e2e` exit 0 after removing only stranded run-specific `e2e_cli_test_*` containers. Final evidence: core 95.35%, CLI 90.43%, 92.89% equal-weight package mean, all 90 files ≥80%, 94.41% module-integration mean, Core E2E 35 passed, CLI E2E 29 passed, and `QUARANTINE_TICKETS` empty. No SA113-owned file changed.
    - **Pending / blocking:** `SA114-REV-001` (**high**, correctness) — in-place migration still does not require `validateQuickScaleSeam.ts` during donor preflight and can silently skip the dependency; `SA114-REV-002` (**medium**, completeness) — modules-only `make check` passes an empty `SECTIONS` value to `test-unit`, which falls back to default core/CLI suites; `SA114-REV-004` (**medium**, test gap) — runtime-branding E2E assertions must verify the exact `projectName` binding and recursively prove the generated project literal is absent from frontend source consumers. `SA114-REV-003` is resolved. Independent change-review pass 2 remains `STATUS: partial`; SA114 stays unchecked.
    - **Decisions needed:** None. Clean continuation requires implementing only the three blocking findings above, rerunning their focused checks plus the four ordered gates, and obtaining an independent `STATUS: ok` review before marking SA114 complete.
...
- **Track 1 — SA114 blocked checkpoint, off the critical path.** The four ordered gates are green with empty quarantine, but independent review still has three blocking findings (`SA114-REV-001`, `SA114-REV-002`, `SA114-REV-004`); the ticket remains unchecked. No product decision is open — clean continuation is the bounded fix/validate/re-review sequence recorded in the task block. Heavy `ci`/`ci-e2e` legs remain serialized against Track 3's PG/Docker usage, and SA113-surface fixes remain deferred to Track 3.
...
**Net.** Track 2 is complete; Track 1 has one independent off-path ticket at a blocked checkpoint (SA114, gate re-verification). Track 3 is **not** done: the installed-wheel resolver crash reopened engineering work that SA109/SA110 did not cover. The critical path is now **SA113 → SA111a → SA112 → SA96-PUBLISH (human)**, all on Track 3; SA114's gates are green, but its three recorded review blockers must close before the Track 1 ticket can be completed. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the recorded squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
