...
- [ ] **SA112F-QG-001 — Obtain a pressure-free ordered acceptance verdict.** `Acceptance handoff · Track 3 · owner: service-capable acceptance operator · deps: none open · blocks SA112f`
  - **Goal/current state:** produce one valid serial-then-default acceptance verdict and distinguish corroborated external pressure from a reproducible product or harness defect. The focused repair candidate traced the generated-project install hang to Poetry's parallel installer: a retained `quickscale apply --no-docker` process had waited on its child `poetry install` since 2026-08-18, and that child had exactly 29 threads (Poetry's 28-worker default on this 24-core host plus its coordinator), slept in `futex_do_wait`, and had made no progress for more than 32 hours. The failed Core install likewise stopped during concurrent package operations without a network, OOM, or service marker. `build_isolated_poetry_env()` now forces `POETRY_INSTALLER_PARALLEL=false`, with regressions proving ambient and caller overrides cannot re-enable the wedging pool while per-worker cache overrides survive. Candidate evidence at `/tmp/quickscale-sa112f-qg-001-repair.9CgW6C` passed focused tests and the ordered campaigns: serial exit 0 in 2,578 seconds (Core 35, CLI 36), then default exit 0 in 1,213 seconds at four xdist workers per lane (Core 35, CLI 36), 1,365 seconds / 52.9% faster, with all four Compose scopes empty and no Poetry/QuickScale process left behind. This is not yet the acceptance verdict because preflight recorded the unreviewed repair as local changes; independent repair review/merge and a clean-tip repetition of the preflight and both campaigns remain required before checking this item.
