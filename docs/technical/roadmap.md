...
- [x] **SA102 — Add `--devtools` to every CI lint/typecheck invocation (TA61).** `Tier 1 · Track 1 (rebalanced from Track 3, 2026-07-19) · deps: none · assistant-executable · no blockers`
  SA99 brought `quickscale_devtools` under the Makefile *default* lint/mypy sections. Explicit `--devtools` is now present at all ten scoped CI/local lint and typecheck sites: `.github/workflows/ci.yml:336,348,547,551`, `.github/workflows/publish.yml:100,108`, and `scripts/check_ci_locally.sh:188,253,375,390`. The original task inventory said five sites; the refreshed inventory found ten, and that discrepancy was corrected without introducing blockers.
  ...
  - Verify: aggregate `make lint -- --core --cli --modules --devtools` and `make typecheck -- --core --cli --modules --devtools`, plus scoped `make lint -- --cli --devtools` and `make typecheck -- --cli --devtools`, all pass. A temporary Ruff violation in `quickscale_devtools/src` failed the lint and local-CI paths, and the probe was removed.
...
**CI-gate remediation from the 2026-07-19 sweep (SA103; SA102 landed on Track 1).** Two audit findings landed on the CI/release plumbing. **SA102** (TA61, trivial) was **rebalanced to Track 1 on 2026-07-19** and now covers all ten scoped lint/typecheck invocation sites; it is file-disjoint from SA101 (quality-baseline source) and from SA103 (SA102 edits lint/typecheck *invocation flags*; SA103 adds a *frontend* job), so the split is conflict-free. **SA103** stays here: it touches `ci.yml`/`publish.yml`/`Makefile` umbrellas + `scripts/check_ci_locally.sh`, is file-disjoint from the Track 2 frontend workstream (template tree) and from SA101, and **must land before the SA96-GATE four-command join re-runs** since it tightens what "green" means.
...
SA102 ✓ ── devtools CI gate (TA61,     TP3a ✓ → TP3b (E2E concurrency;          SA93 ✓ (e2e in green-gate)
  Tier 1, ten invocation sites;         rebalanced from Track 1; off crit.)    SA100 ✓ (TA58/TA59 theme preflight)
  landed on Track 1, 2026-07-19)        SA104 ── FE static-source move (Fin.10   SA103 ── frontend-proof gate (TA60/red
...
**Open work only:** Track 1 `TP2b` (validation-blocked) → `TP4`, Track 2 `TP3b` (after completed `TP3a`) + the frontend chain `SA104 → SA105/SA106` (arch Finding 10), Track 3 `SA101` (clears BLK-002; on the critical path) + `SA103` (2026-07-19 audit sweep; feeds SA96-GATE), and the release join
`SA96-GATE → SA96-PUBLISH`. The two TP chains run in parallel with no source-file overlap
(`Makefile`/`pyproject.toml` vs `scripts/test_e2e.sh` + `quickscale_cli/tests/test_e2e_development_workflow.py`); the frontend chain (SA104+) owns the `quickscale_core` generator + `showcase_react` template tree only, disjoint from both the TP work and the SA102/SA103 CI surfaces, so the remaining workstreams run concurrently. The TP and frontend chains are off the release critical path; **SA103 feeds the gate** (arch red flag — land before the SA96-GATE join re-runs). None may regress any gate's pass/fail set or coverage thresholds. `TP4` is docs-only and lands after
