...
SA128a → b → c → d (parity check)    SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  ...
  ▼  Make · Bash · YAML · contracts   │  validation AUTHORIZED      │  PARTIAL — PRE-USE REVIEW BLOCKED
                                      │  cannot finish → SA112f     │  F-003…F-010 + RESET-001
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **no** — `SA117E3-RESET-001`; pre-use correction handoffs exhausted the infrastructure and authorized retry allowances | **no** — `F-003`–`F-010` remain blocking and no executable hash has `STATUS: ok` | **yes** — recorded-partial merge authorized; no order gate | no | ✅ **yes** — real progress, blocked on provider/control-plane reset |
| **Track 1** — SA128a | **yes** — `deps: none`, no order gate; SA112a's QG/review completed under the authorized `SA112A-RESET-001` | **yes** — SA128a's own two-file scope carries no persisted cap | **yes** — no order gate | ✅ **yes** | no — SA112a was Track 1's only critical-path node and is complete |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is paused at a recorded-partial checkpoint on the critical path.** SA117e-3's plan review returned `STATUS: ok`, but the first full-file pre-use review rejected the ephemeral harness/wrapper bytes with `F-003`–`F-010`; two correction handoffs then returned empty, leaving `infra_retries=1/1` and the user-authorized retry allowance spent. `SA117E3-RESET-001` must restore a write-capable provider and clear that persisted scope before the approved plan resumes. **Track 1's SA112a is complete:** its exact four-file executable slice passed ordered QG (DC-52, `STATUS: ok`) and full-scope review (DC-53, `STATUS: ok`, resolving its `F-005`) under the authorized `SA112A-RESET-001`; Track 1 advances to SA128a off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear. SA117e-4 still needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path at a **recorded-partial checkpoint** — SA117e-3 remains the head after SA117e-2 closed 2026-08-03. SA117e-3's scoped plan review returned `STATUS: ok` and closed plan finding `F-002`; Phase 1 froze a clean `fbde0a82…` baseline and the original running/ready `quickscale-ci-postgres` container without mutating it. The worktree later resynchronized to current `v87` `f5de6836…` while reconciling this checkpoint, so Phase 1 must be revalidated before continuation. The first pre-use full-file review returned `STATUS: partial` on eight blocking implementation findings (`F-003`–`F-010`), and repeated correction handoffs returned empty, so `SA117E3-RESET-001` blocks clean continuation. No harness, interruption proof, negative control, `make ci-e2e`, tag, push, or publication executed. Track 1's SA112a is complete and its merge satisfies SA112b's Track 1 precondition; Track 1 advances to SA128a. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4. Do not execute SA117e-3 runtime artifacts before a fresh full-scope pre-use review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 closed 2026-08-03** (full-scope historical review, evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). Two of its `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. `SA117E1-REV-003`, `CR-SA117E-005`, and `CR-SA117E-006` are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2` — needs exclusive Docker/PG
      ...
      **Current checkpoint (`SA117E3-HANDOFF-001`, 2026-08-03; task unchecked; no executable approval or host mutation).**
      ...
      - **Decisions needed:** `SA117E3-RESET-001` — restore a functioning write-capable subagent/provider and perform a supported reset of the persisted SA117e-3 changed-file scope, whose `infra_retries=1/1` and user-authorized retry allowance are spent after two empty correction handbacks. Then resume the approved plan at Phase 2 with `F-003`–`F-010` carried verbatim; do **not** re-plan unless the scope or repository topology changed. This is an execution-capacity reset, not a waiver of any finding or review gate.
...
**Open decisions — three, all the maintainer's; one blocks Track 3 today.** `SA117E3-RESET-001` requires restoration of a functioning write-capable provider plus a supported reset of the SA117e-3 changed-file scope after `infra_retries=1/1` and the user-authorized retry allowance were spent on empty correction handbacks; continuation then reuses the approved plan above at Phase 2 with `F-003`–`F-010`. The other two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. `SA112A-RESET-001` was granted 2026-08-03 and its ordered QG (DC-52) and full-scope review (DC-53) both returned `STATUS: ok`; `SA117E2-RESET-001` was also granted and is no longer open. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
