...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Open findings and their owning child (no decision pending).** All SA117a/SA117b findings are resolved (see [CHANGELOG.md](../../CHANGELOG.md)). SA117c's recorded partial delivery resolves `SA117-CR-006`; its remaining blocker is `SA117C-CR-012` (**high**, correctness — exhaustive Python `__version__` binding rejection is incomplete). `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker.
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - **SA117a and SA117b — CLOSED (2026-07-28 / 2026-07-29).** Closeout evidence is in [CHANGELOG.md](../../CHANGELOG.md). Forward-binding residue: `SA117-CR-006` is owned by SA117c below; nothing else carries over.
  ...
  - [ ] **SA117c — Make the lock-drift route compare complete baseline semantics.** `Tier 2 · Track 3 · deps: SA117b ✓`
    ...
    **RECORDED PARTIAL DELIVERY (2026-07-29; explicitly authorized for preservation; task remains open).**
    - **Done:** `make sa117-lock-diff` now drives one lock-structure comparison plus broad fail-closed inventory validation over an immutable baseline SHA and a root-identified candidate: 55 required files, 66 canonical version values, all twelve permitted module-lock version leaves normalized, every other lock difference reported as drift, malformed/partial inputs fail closed on the covered forms, and evidence is deterministic, atomic, and stale-safe. Direct/Make caller parity, legacy weak-route rejection, nested-root rejection, structural YAML/TOML parsing, broad Python-binding validation, output/input collision safety, and evidence-cleanup behavior are covered; exhaustive Python binding validation remains explicitly excluded by the blocker below. Accepted validation before terminal review: Ruff check/format, `make version-check`, direct/Make evidence parity, **33 focused tests**, **76 combined tests**, exact 55/66 evidence, allowlist, and `git diff --check` all passed. Review explicitly resolved `SA117-CR-006`, `SA117C-CR-007` through `SA117C-CR-011`, and `SA117C-CR-013`.
    - **Pending / blocking:** `SA117C-CR-012` (**high**, correctness) remains. The Python version oracle rejects the common reassignment/deletion forms but does not exhaustively reject every valid binding: dotted imports such as `import __version__.submodule`, unprovable star imports, and Python 3.13 type-parameter bindings can still rebind `__version__` after the validated literal. Terminal delta review therefore returned `STATUS: partial`; SA117c is not release-ready, SA117e must not start, and this checkbox stays unchecked. `ACR-META-001` is **low/advisory**: preserve the exact literal final-QG command rather than prose placeholders in the next handoff.
    - **Decisions needed:** none. Clean continuation is narrow: complete the context-aware `ast.Import` / `ast.ImportFrom` and `TypeVar` / `ParamSpec` / `TypeVarTuple` binding inventory, add the corresponding controlled-exit/no-evidence regressions, rerun the authoritative focused/full quality gate, and obtain full-scope independent review `STATUS: ok` before checking SA117c or starting SA117e.
...
──────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA128 (structural parity check)         SA115 (e2e xdist; deps: none)  SA117c (lock-drift) ◄─ next
...
**Diagram checkpoint (2026-07-29).** Track 3 begins at SA117c (SA117b closed and merged); Track 1 begins at SA128's blocked checkpoint (SA127 closed, SA122a closed as superseded by `SA122A-AUTH-001`).
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now leads with blocked SA128, followed by SA129, SA130, and SA122b (SA127 ✓ closed), while Track 3 has SA117c and SA117e ahead of SA112a. Reusing Track 1 still requires no fourth worktree, but it would delay the whole remaining Track 1 queue.
...
> **Re-verified on 2026-07-29 (second pass).** SA117b is closed and fully merged; SA117c is Track 3's next action. `wt-track1` and `wt-track3` carry **zero** commits unmerged into `v87`; `wt-track2` still carries `5193f198` + `5b5de830` and remains closed to new work.
>
...
- **Track 3 (SA117c → SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path.**
  ...
  - **Next action: SA117c.** Do not publish splits, start SA112, or treat the umbrella candidate as release-ready.
  - **Process note (`SA117-DEC-005`, 2026-07-28).** The **proportionality rule** in the handoff contract binds the remaining children: documentation/test-only deltas go straight to implementation and one change review, while SA117c–SA117e production work retains its scoped plan gates.
