...
**Critical path:** `SA136a → SA136c/SA136d → SA136f → SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. SA136e is on the umbrella but not on the longest chain — the remaining parallelizable leg (its independent sibling SA136b is now closed with reviewed evidence). **SA117e-5 is closeout and sits *off* the critical path** — SA112b's precondition is the *sealed* splits delivered by SA117e-4, so it does not wait for the umbrella to close. SA112a is closed and merged, so SA112b's provisioning precondition is already satisfied. Track 2 is entirely off the path, and the whole remaining Track 1 queue is off it.
...
**Track 3 remains truly green and on the critical path.** The 2026-08-06 loop/seal ratifications dissolved the SA117e-4 plan cap: `F-001` and `F-006` no longer describe a real hazard (an interrupted republish is rerun, not recovered), and `F-004` is re-derived from a much smaller allowlist. **SA136b is closed with reviewed completion evidence** (functional commit `d00ffe68…`, green QG, full-scope review DC-94 `STATUS: ok`, `F-002` resolved — see [CHANGELOG.md](../../CHANGELOG.md)); its earlier recorded-partial attempt remains preserved at functional commit `999c025b`. Track 3's head therefore advances to **SA136c**, whose sole dependency SA136a is merged and which has no open decision; SA136e stays a pending independent sibling, and SA136d now depends on the completed SA136a+SA136b pair. **Track 1 remains truly green at SA122b-3** but is off the critical path, so it is filler relative to the release. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear.
...
- **`scripts/check_sa117_scope.py` and `scripts/version_tool.sh`** — written by SA136b, which is now closed, so both return to unowned in v87; `check_sa117_scope.py` remains owned by v88's SA124, which is not open in v87, so there is no concurrent writer.

`quickscale_cli/.../module_commands.py` and `module_output.py` pass from unowned to SA136c-owned; `quickscale_core/.../utils/git_utils.py` returns to unowned now that SA136a is merged; `scripts/publish_module.py` passes from SA136b's now-closed inventory ownership to SA136d (seal phase) alone. `quickscale_core/tests/test_e2e_full_workflow.py` and `scripts/quality_baseline.json` remain owned by no open v87 ticket. No other executable surface is shared, so no additional procedure is required.

---

## Track 3 — Core/CLI plumbing, release path

**Status:** on the critical path and **unblocked to resume**. The head is **SA136c**: SA136b is closed with reviewed completion evidence (functional commit `d00ffe686ac1abb0d15c28953bd059e5dab68211`; QG green; full-scope review DC-94 `STATUS: ok`; `F-002` resolved), SA136c's sole dependency SA136a is merged, and no open decision blocks it. SA112b's provisioning precondition is satisfied by SA112a's merge; its split-publication precondition now waits on SA117e-4, which in turn waits on SA136.
...
**Handoff entry point.** Start **SA136c** (retarget `embed` onto the version tag): SA136a is merged, SA136b is closed with reviewed completion evidence, and no upstream dependency or open decision blocks the child. It is production behaviour + public CLI surface, so scoped plan review is required before implementation. The loop/seal contract SA136 implements is recorded in [CHANGELOG.md](../../CHANGELOG.md) (ratified 2026-08-06) and becomes normative in `decisions.md` at SA136f.
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — critical path; **blocks SA117e-4**
  ...
  **SA136a is closed and merged** — `git_utils.py` now carries `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, and refspec-explicit `push_tag`; every later child consumes them (evidence in [CHANGELOG.md](../../CHANGELOG.md)). Residual `F-005` is a low advisory only.
  ...
  - [x] **SA136b — Derive the authoritative twelve-module inventory.** `Tier 2 · deps: none` — **closed with reviewed completion evidence** (functional commit `d00ffe686ac1abb0d15c28953bd059e5dab68211`; evidence in [CHANGELOG.md](../../CHANGELOG.md)). `authoritative_module_names()` and `AUTHORITATIVE_MODULE_COUNT = 12` fail hard on placeholder/count drift; publish listing, version propagation, and SA117 scope enumeration all derive from that contract; and the `scripts/publish_module.py` direct single-module selector now requires authoritative-inventory membership before release/origin prompts or any split/push path, so `teams` and an unapproved thirteenth directory fail before mutation. QG green (focused pytest 7 passed; `make version-check` all 12 modules at 0.87.0; `make check` Core 2,689 passed / 1 skipped / 90.33% and CLI 2,043 passed / 90.78%; all declared gates plus supplemental Ruff/MyPy); full-scope Adaptive-change-review **DC-94 returned `STATUS: ok`** with empty findings, resolving `F-002`. No new blockers or decisions were found for SA136b; the earlier recorded-partial attempt at `999c025b` and its review history remain in [CHANGELOG.md](../../CHANGELOG.md).
