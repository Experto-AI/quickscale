...
- **W1 — retain SA165 (#22).** A fresh terminal SA165-R1 returned **blocking** on stale
  `OPERATIONS.md` manifest hashes, and that finding is **refuted by measurement**: the rebaseline it
  reported missing is present in the tree, inside the reviewed range, and green. The blocking grade
  was an artefact of the four-file candidate, which excluded `sa90_emission_manifests.json` from the
  patch the reviewer could see. **No repair is owed and no scope decision is open.** The next action
  is a fresh terminal SA165-R1 over a **five-file** candidate that binds the fixture as reviewable
...
**No maintainer decision is open anywhere in the v88 queue.** The exact-manifest repair-scope
decision recorded on 2026-09-05 is **withdrawn as moot**: the repair it would have authorized is
already in the tree. See [the refutation](#sa165-blocking-finding-refuted) in SA165's body — the
three `OPERATIONS.md` hashes were rebaselined in the same commit that changed the template, the
three parity variants are green, and freshly rendered bytes hash to the fixture value. There is no
work to scope, so neither option A nor option B is chosen; both described a repair that does not
exist.

The one standing grant remains **`EV-8`**: exactly one replacement
`QS_E2E_INTEGRATION_REF=v88 make ci-e2e` verdict
...
|---|---|---|
| SA160, SA161 | generator templates + **SA90 emission-parity fixture**, `docs/others/tech-audit.md` | emitted output changes; they are the fixture's only open editors |
...
| SA165 | `quickscale_core/.../state_schema.py`, `scripts/test_isolation_conformance.sh`, `quickscale_core/tests/test_generator/test_generator.py`, `OPERATIONS.md.j2`, plus `sa90_emission_manifests.json` **as bound review context, not as an edit** | retained product discharge; the fixture joins the frozen set because its `OPERATIONS.md` rebaseline is the same logical change as the template edit, and omitting it produced a false stale-hash finding. **Documentation surfaces moved to SA179 2026-09-05**, which keeps the candidate free of the files that record its own review |
...
- `sa90_emission_manifests.json` — **SA161 then SA160 are its only open editors**, an ordered pair
...
- [ ] **SA165 — Discharge the tech-audit watch items that carry an action.** `Band C · Tier 3 · W1 · merge #22 · deps: none`
  ...
  **State (measured 2026-09-05): retained Phase D candidate; the fresh product-only SA165-R1's one
  blocking finding is refuted by measurement; a fresh five-file SA165-R1 and the final-candidate
  release verdict are outstanding.**
  The retained A-C product object is unchanged. Neither the historical six-file review nor the
  single prior release run may be rebound: both predate the settled candidate and close neither
  SA165 nor its four audit notes. Their evidence, and the pre-R1 reconciliation trail, are archived
  in [CHANGELOG.md](../../CHANGELOG.md). `EV-8` remains unspent. Lane state is remeasured at action
  time rather than persisted here.

  <a id="sa165-blocking-finding-refuted"></a>
  **The blocking finding is refuted; the candidate widens to five files.** The fresh terminal
  SA165-R1 over the four-file patch graded blocking, reporting that `OPERATIONS.md.j2` adds rendered
  warning text while the three manifest records retain the pre-change `OPERATIONS.md` hash. **That is
  not what the tree contains.** Commit `a14ea029` — the SA165 product commit itself — changed the
  template *and* rebaselined all three hashes from `c8e6c725…` to `94165c64…` in one change, inside
  the reviewed range `5a7ee965..69f39e2b`. Measured on this branch: the three
  `test_generated_tree_matches_manifest` variants pass, and a freshly generated `react_default` tree
  renders `OPERATIONS.md` to exactly `94165c648f7b631bdcf53182fe06bceaf2037c9ebc2646dfeca556ece9ce0a40`
  with the warning text present.
  **The cause is the narrowing, not the product.** `OPERATIONS.md.j2` and its three fixture hashes are
  one logical change; the four-file candidate cut the fixture out of the patch, so a reviewer holding
  only that patch saw a template edit with no accompanying rebaseline and correctly inferred
  staleness from incomplete evidence. The fix is to the candidate, not the code: the frozen set
  becomes **five files**, adding `quickscale_core/tests/fixtures/sa90_emission_manifests.json` so the
  rebaseline travels with the template edit it justifies. SA165 still writes no new bytes to that
  fixture, and SA161 → SA160 remain its only open editors.

  **The frozen set, after both corrections.** It is the retained product object
  `573a57a34301e6a91971a7845095bd913bebd5e1` as integrated, plus the emission fixture as read-only
  context. The six documents that *record* a review — `CHANGELOG.md`, `docs/index.md`,
  `docs/others/tech-audit.md`, `docs/technical/roadmap.md`,
  `docs/technical/v88_ticket_context.md`, and
  `quickscale_core/tests/test_v88_ticket_context_consistency.py` — stay out of it and belong to
  **SA179 (#35, W1)**, so recording an SA165 result cannot invalidate it. The emission fixture stays
  *in* it, so a reviewer cannot mistake a landed rebaseline for a missing one. The two corrections
  pull in opposite directions and both are needed: exclude what records the review, include what the
  reviewed bytes depend on.

  **Pending, in order.** (1) Sync W1 to current `v88`, re-establish the candidate binding over the
  **five-file** set, and materialize a base-to-tip patch that includes
  `sa90_emission_manifests.json`; obtain a fresh independent terminal SA165-R1 over it, together with
  the three `test_generated_tree_matches_manifest` variants and
  `test_operations_md_warns_generated_credentials_are_local_only` as green evidence. That review ends
  its root run. (2) In a later root run, revalidate the fresh review binding and execute
  `FROZEN-CHECK`. (3) Spend **`EV-8`** — the single
  ...
  **Constraints on the remainder.** Reviewed plan authority `EV-6` still governs `FROZEN-CHECK` and
  `EV8-CLOSEOUT`, but its green fresh-R1 entry precondition is unmet, so neither phase is dispatchable
  and the next root run must terminate at the fresh five-file SA165-R1. That run writes no product
  bytes: the candidate is already settled and the refuted finding needs no repair. Retain product
  object
  ...
  **Shared conflict surface:** `quickscale_core/src/quickscale_core/schema/state_schema.py`, `scripts/test_isolation_conformance.sh`, `quickscale_core/tests/test_generator/test_generator.py`, `quickscale_core/.../templates/OPERATIONS.md.j2`, `docs/others/tech-audit.md`, plus `quickscale_core/tests/fixtures/sa90_emission_manifests.json` as bound review context that SA165 does not edit.
