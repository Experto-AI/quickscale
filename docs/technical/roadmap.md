...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  Umbrella acceptance only: SA117a, SA117b, and SA117c are closed; SA117e remains open and required. Child tickets own all implementation; **do not execute this parent block as a monolithic handoff.**

  **Split (`SA117-DEC-003`, 2026-07-27): SA117 was Tier 3 and was split into the four serial children SA117a → SA117b → SA117c → SA117e. SA117a closed 2026-07-28 (`SA117-DEC-005`), SA117b closed 2026-07-29 after `SA117B-REVB-003` received full-scope documentation review `STATUS: ok`, and SA117c closed 2026-07-30 after exhaustive Python-binding validation and full-scope review `STATUS: ok`; SA117e remains.** SA117a carried what SA112b's evidence validity depends on and has delivered it; SA117b made pushing refreshed splits safe; SA117c supplies the local comparison gate; SA117e pushes refreshed splits and proves the published ones match the core. The former SA117d (scope meta-tooling) is **deferred to v88 as SA124** (`SA117-DEC-004`). Sizing rationale and the measured candidate breakdown are in [CHANGELOG.md](../../CHANGELOG.md).
  ...
  **Open findings and their owning child (no decision pending).** All SA117a/SA117b/SA117c findings are resolved (see [CHANGELOG.md](../../CHANGELOG.md)). SA117c delivered the exhaustive Python-binding inventory, resolving `SA117-CR-006` and `SA117C-CR-012`. `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker.
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - **SA117a and SA117b — CLOSED (2026-07-28 / 2026-07-29).** Closeout evidence is in [CHANGELOG.md](../../CHANGELOG.md). Forward-binding residue: `SA117-CR-006` was owned by SA117c (now resolved); nothing else carried over.

  - [x] **SA117c — Make the lock-drift route compare complete baseline semantics.** `Tier 2 · Track 3 · deps: SA117b ✓`
    ...
    *(why →* this is the gate that supplies the local comparison prerequisite for proving the splits serving `apply` match the core about to be published — SA117e's post-push verification completes that proof*)*

    **COMPLETE (2026-07-30).** Functional commit `46a2f186` delivered the exhaustive Python-binding inventory over `ast.Import`, `ast.ImportFrom`, `TypeVar`, `ParamSpec`, and `TypeVarTuple` — all recognized as possible `__version__` rebindings and rejected with controlled exit 2 and no evidence output. Valid non-binding references (local-shadow, re-`import`-as, `getattr` read, `__all__` export) are tolerated without false rejection. Stale evidence is removed before every fresh run. The lock-structure comparison and fail-closed inventory from the partial-delivery checkpoint are preserved and re-validated as part of this delta.
    - **Validation:** All quality gates passed on the executable delta: **50 focused tests**, **93 combined tests**, Ruff check/format, `make version-check` (all 12 modules at `0.87.0`), `make quality` exit 0, and `git diff --check`. Exhaustive Python-binding proof confirmed across the full `ast.Import`/`ast.ImportFrom` and type-parameter (`TypeVar`/`ParamSpec`/`TypeVarTuple`) binding matrix. Caller parity verified: direct and `make` routes produce identical evidence. No breaking change to the public CLI surface or to any existing Make target.
    - **Finding lifecycle:** `SA117C-CR-012` — **resolved**. (`SA117-CR-006`, `SA117C-CR-007`–`SA117C-CR-011`, and `SA117C-CR-013` were previously resolved in the partial checkpoint.) Full-scope source Adaptive-change-review returned `STATUS: ok` at confidence 97 with **no new source findings or blockers**.
    - **Documentation review (2026-07-30):** Full-scope docs review of the SA117c closeout found `SA117C-DOC-CR-001` (current-state prose still described SA117c as open/forward-binding) and `SA117C-DOC-CR-002` (wording conflated SA117c's local comparison gate with SA117e's published-split proof). The follow-up correction in this edit singularizes SA117e as the sole remaining child in current-state prose and reserves published-parity proof for SA117e. SA117e remains Track 3's next action.
...
**Ordering on this track (one worktree, serial):** **SA128 (second blocked checkpoint) → SA129 → SA130 → SA122b**, with SA122b merge-gated behind SA112e. **SA129 and SA130 remain behind SA128's clean continuation:** both are file-disjoint from SA128's checker/test allowlist, from SA122b's registry consumers, and from each other (SA129 is `quickscale_cli` CLI code, SA130 is a `quickscale_core` generator template), so nothing forces a rebase in any direction, and both slot ahead of SA122b, which cannot land first regardless. SA129 before SA130 is arbitrary — they are independent and may swap.
- **SA128 is Track 1's current blocked action.** The 2026-07-30 fresh continuation completed discovery, an approved two-phase plan, two implementation/QG rounds, and two full-scope reviews; the second review remained `STATUS: partial` with severity/count unchanged at high and 9 → 9. Every executable/test candidate was restored byte-identically to the synced start before this checkpoint, so the merged SA122a checker remains non-authoritative. Finding 11 still needs centralization **before or as part of** the SA112e/`SA115-CI-001` path-list edits. The registry those edits write into is **already merged and usable** (SA122a's kept half); SA128 must make the checker trustworthy before SA122b may consume it.
- **SA129 then SA130 remain queued after SA128.** Both are product defects, not gate governance — they are on Track 1 only because that is the serial worktree with capacity. Both Tier 2, `deps: none`, independent of each other; see the tickets below.
...
- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Tier 2 · Track 1 · deps: none · replaces SA122a's checker`
  ...
  **RECORDED BLOCKED CHECKPOINT (2026-07-29; no executable delta retained; task remains open).**
  - **Done:** Track 1 was clean, synced to `v87`, and service-free prerequisites were ready. The maintainer ratified `SA128-DEC-001` through `SA128-DEC-003`: no Bubblewrap or text/regex fallback; delegate semantics to GNU Make/Bash; keep all six inert adversarial forms; enforce repository-contained inputs; preserve direct exit 1 / Make exit 2 for the five publish omissions; keep the checker standalone until SA122b; and restart only from the unchanged baseline and stable ledger. Two bounded fix/re-review rounds and one explicitly authorized post-cap re-plan were completed. Every rejected checker/test candidate was restored byte-identically before this checkpoint, so **no SA128 executable or test change is merged** and the existing checker remains non-authoritative.
  - **Pending / blocking:** `SA128-CR-001` (**high**, correctness — effective Make recipes/delegation remain unobserved); `SA128-CR-002` (**high**, correctness — local serial/parallel execution and join semantics remain unobserved); `SA128-CR-003` (**high**, security-boundary — publish Bash and E2E YAML need actual/structural semantics); `SA128-CR-004` (**high**, completeness — hosted commands, `needs`, and stage topology remain unproved); `SA128-CR-005` (**high**, completeness — the candidate-only arbitrary-DAG fix was reverted with the rejected delta and must be reimplemented); `SA128-SEC-006` (**high**, security-boundary — all consumed paths, including `--registry`, need one containment contract); `SA128-CR-007` (**high**, correctness — missing/malformed/ambiguous inputs must fail with exit 2); `SA128-CR-008` (**high**, resilience — output bounds and process-group cleanup need literal failure semantics); and `SA128-TEST-009` (**high**, test-gap — the full adversarial/lifecycle matrix, focused MyPy zero, independent QG, and final review remain absent).
  - **Decisions needed:** none. A fresh continuation must start from the unchanged SA122a baseline, supply a literal current-source execution oracle (including all 16 publish `run` values and all 10 bound hosted `run` values), resolve every stable finding above, run the exact focused and repository gates, and obtain full-scope independent review `STATUS: ok` before checking SA128, starting SA129, or authorizing SA122b.

  **SECOND RECORDED BLOCKED CHECKPOINT (2026-07-30; no executable delta retained; task remains open).**
...
- [ ] **SA129 — Carry the superuser answer on an explicit sentinel line.** `Tier 2 · Track 1 · deps: none · queues behind blocked SA128`
...
──────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA128 (structural parity check)         SA115 (e2e xdist; deps: none)  SA117c (lock-drift) ✓
  │  deps: none · replaces SA122a        │  validation AUTHORIZED       │  deps: SA117b ✓
  ▼  BLOCKED CHECKPOINT                  │  cannot finish → SA112f      ▼
SA129 (superuser probe; Tier 2)         │  cannot merge  → SA112e     SA117e (review + push splits) ◄─ next
...
**Diagram checkpoint (2026-07-30).** Track 3 begins at SA117e (SA117b ✓, SA117c ✓, both merged); Track 1 begins at SA128's blocked checkpoint (SA127 closed, SA122a closed as superseded by `SA122A-AUTH-001`).
...
**Track 1 is internally serial and now entirely off the critical path.** SA128 is at a blocked checkpoint, SA129 and SA130 remain queued behind its clean continuation, and SA122b then carries the SA112e merge bound. The SA128/SA122b governance chain is off-path; SA129 and SA130 are also off-path, non-release-blocking product defects. Track 1's release blocker (SA127) is closed.

**Critical path:** `SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. SA117b and SA117c are closed; SA117e is Track 3's next action.

**Parallelism.** The open tickets occupy all three tracks: SA117e → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and blocked SA128 → SA129 → SA130 → SA122b on Track 1. **The repository runs exactly three worktrees; a fourth is not created** (`SA112A-TRACK-001`, resolved 2026-07-28 — see Standing decisions).
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now leads with blocked SA128, followed by SA129, SA130, and SA122b (SA127 ✓ closed), while Track 3 has SA117e ahead of SA112a. Reusing Track 1 still requires no fourth worktree, but it would delay the whole remaining Track 1 queue.
  ...
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* Track 1's queue head. That head is blocked **SA128**, with SA129 and SA130 queued before SA122b, so the move would delay the full off-path queue to advance a critical-path node; it no longer trades against a release blocker.
...
> **Re-verified on 2026-07-30.** SA117b and SA117c are closed and fully merged; SA117e is Track 3's next action. `wt-track1` and `wt-track3` carry **zero** commits unmerged into `v87`; `wt-track2` still carries `5193f198` + `5b5de830` and remains closed to new work.
>
...
**Tier census (2026-07-28): no open ticket is Tier 3.** SA112e/SA96-PUBLISH are Tier 1; every other open ticket is Tier 2. SA126 was the completed Tier 1 remediation ticket. The two historical Tier 3s were already split — SA117 into SA117a/b/c/e (`SA117-DEC-003`) and SA122 into SA122a/SA122b. SA128 is Tier 2. SA117a was re-split in the fifth pass on empirical grounds rather than a Tier reassignment: it was declared Tier 2 but had produced three review caps with zero source delta while carrying two unrelated concerns, so its production refactor moved to SA126 (`SA126-DEC-001`), leaving a single test-file assertion.

Each track reports **three independent states**. A track is **truly green** only when all three are yes.

- **Track 1 (SA128 → SA129 → SA130 → SA122b) — start: YES · finish: NO · merge: YES. NOT truly green.**
  - **Can start — YES.** SA128 has `deps: none`, no open product decision, and service-free validation, but its last continuation exhausted the review cap and landed only this blocked checkpoint. A fresh literal continuation is required. SA129 and SA130 remain queued; SA122b remains last with its stated SA112e merge bound.
  - **Can finish — NO. Review blocker.** SA128 retains nine high blocking findings and the current checker is explicitly non-authoritative. SA129, SA130, and SA122b do not discharge those findings.
  - **Can merge — YES.** A corrected SA128 carries no merge-order gate. SA122b alone is order-gated behind **SA112e** (shared `.github/workflows/e2e.yml` path list) — a hard dependency that only that upstream work clears.
  - **On the critical path? NO — Track 1's release blocker (SA127) is closed.** SA128/SA122b are governance work; SA129/SA130 are non-release-blocking product defects. `SA112A-TRACK-002` weighs a critical-path node (SA112a) against the full Track 1 queue.
...
- **Track 3 (SA117c ✓ → SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path.**
  - **Can start — YES.** SA117b and SA117c are closed; no product decision is open.
  - **Can finish — YES.** SA117e carries explicit acceptance and no unresolved pre-start decision.
  - **Can merge — YES.** SA117c had no merge-order gate and is closed; SA117e retains its human-confirmed public-push step.
...
**Net.** The Track 3 critical path is **SA117c ✓ → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1's release blocker is resolved; blocked SA128 is followed by SA129, SA130, and SA122b, all off-path, with SA122b retaining the SA112e merge bound. Track 2 remains merge-gated.

**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but at the cost of delaying blocked SA128 and its successors) or leave it behind SA117e. `SA127-ORDER-001` and `SA122A-AUTH-001` are both resolved. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying blocked SA128 and its queued SA129 → SA130 → SA122b successors; it changes no track's three states. **Default while undecided: SA112a stays on Track 3; Track 1 resumes SA128 → SA129 → SA130 → SA122b through a fresh continuation.** Every individual ticket records **Decisions needed: none**.
