...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: none`) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2, independent.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Tier 2 · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Blocked checkpoint (2026-07-27 — review cap reached; task remains open).**
  - **Done:** A local, uncommitted Track 3 candidate stamps the twelve module metadata sets to `0.87.0`, adds embed/wiring lockstep enforcement and lease-safe split-publication tooling, and has repeatedly passed the repository quality gates (including smoke-install, coverage, and PostgreSQL integration). It is intentionally **not merged** because independent review has not approved the executable delta.
  - **Pending / blocking:** `SA117-CR-001` (**high**, correctness) — padded manifest versions can still pass literal lockstep; `SA117-CR-003` (**high**, security boundary) — blank direct-CLI origins can bypass the pre-mutation public-source gate; `SA117-CR-005` (**medium**, breaking change) — scope CLI/help/Make caller contracts disagree about required candidate paths; `SA117-CR-006` (**high**, completeness) — the advertised lock-drift route does not yet compare complete baseline lock semantics.
  - **Decisions needed:** none. Continuation requires a new scoped plan and independent approval for the four findings above. Keep this checkbox unchecked; do not publish splits, start SA112, or merge the local executable candidate before that review returns `STATUS: ok`.
...
- [x] **SA120 — Make `make check QUIET=1` gate-equivalent to `make check`.** `Tier 1 · Track 1 · deps: none`

  The Makefile advertised `make check QUIET=1 - Same as check, quiet on success` (`Makefile:12,137`) but wrapped the only `lint-frontend` call in `if [ -z "$(QUIET)" ]` (`Makefile:833-834,918-927`). `make -n check QUIET=1` scheduled no frontend command; `make -n check` did. Because `v87` pushes do not trigger hosted CI (`.github/workflows/ci.yml:3-7`), an ESLint/TypeScript error in the generated React source could stay green through the documented agent pre-commit gate until a PR to `main` or the publish workflow.

  **Implementation:** normal and quiet check now share the same Node/pnpm availability guard and `lint-frontend` dispatch after the shared repository gates. Quiet mode uses collision-safe temporary capture, remains silent on success, replays diagnostics exactly once on failure, and preserves nonzero exit status. Policy coverage in `scripts/test_ci_coverage_policy.py` records recursive-target dispatch, compares membership against the normal `check` target, and asserts forced `lint-frontend` failure propagation in quiet mode.

  **Resolved findings:**
  - `SA120-REV-002` (medium, blocking): absent-tool tests now deterministically provide the opposite tool so pnpm absence cannot false-green.
  - `SA120-REV-003` (medium, blocking): quiet success/replay coverage proves exact silence and exactly-once diagnostics; collision-safe capture replaces the fixed `frontend_lint_log.txt`.
  - `SA120-REV-006` (low, advisory): the unreachable duplicate helper body in `scripts/test_ci_coverage_policy.py` has been removed.

  **Validation evidence:** 79/79 policy tests passed; 4,542 unit tests passed; `make check QUIET=1` completed green on its first run; normal dry-run exited 0 and contained `lint-frontend`. Full-scope independent review returned `STATUS: ok` with no new findings and all three carried IDs resolved. No unresolved SA120 blockers or decisions remain.
  ...
  *(why →* tech-audit `TA62` — the documented quiet pre-commit gate was false-green, and the active integration branch was outside the hosted CI push trigger*)*
...
SA121 and SA122a have `deps: none` and may run in any order or concurrently within Track 1 (SA120 complete); only SA122b carries an ordering bound.
...
**Parallelism.** The open tickets now occupy all three tracks: SA117 → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and the audit-derived governance chain SA121 / SA122a → SA122b on Track 1 (SA120 complete). No rebalancing move is available that would speed the critical path:

- **Track 1 is now active and does not touch the critical path.** SA120 edited `Makefile` + `scripts/test_ci_coverage_policy.py` (complete); SA121 edits `scripts/check_quality.sh` + the baseline gate; SA122a is purely additive (new registry + checker). None of these files appear in any SA112 child's allowlist or in SA117's stamp/assert scope, and none need PostgreSQL or Docker — so Track 1 can run fully concurrent with Track 3, including during SA112's exclusive-infra legs. SA122b is the single exception and is bounded below.
...
- **Track 1 — ACTIVE (2026-07-26 audit intake; off the critical path).** SA114/SA116/SA120 closed and the track was held in reserve; it now carries **SA121**, **SA122a**, and **SA122b** (SA120 complete). SA121 and SA122a have `deps: none` and need no PostgreSQL/Docker, so they may start immediately and run concurrently with Track 3's exclusive-infra legs. Suggested order: SA121 (unblocks any further baseline edit) → SA122a (registry, land before SA112e writes its paths) → SA122b (after SA112e). **Next action:** **SA121** (SA120 complete).
- **Track 3 — BLOCKED AT SA117 INDEPENDENT REVIEW (on the critical path).** The local executable candidate remains uncommitted and unmerged after the review cap; the SA117 checkpoint above records four blocking findings (`SA117-CR-001`, `003`, `005`, `006`). **Next action:** start a new scoped continuation for those findings, rerun the full gate, and obtain `Adaptive-change-review STATUS: ok`. Do not publish splits or start SA112 until SA117 closes. SA96-PUBLISH (human-only) still holds until SA112f closes the umbrella. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Net.** One critical path — **SA117 → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 3's next action is **SA117**; SA112a follows it, and no SA112 umbrella implementation is authorized. Track 2 is implementation-ready but validation-paused pending explicit future maintainer approval (SA115-DEC-001 step 3: **Keep unauthorized**), merge after SA112. Track 1 runs the audit-derived governance chain (SA120 complete); **SA121 → SA122a** are unblocked now, **SA122b** merges after SA112e.
