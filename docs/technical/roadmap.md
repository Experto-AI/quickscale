...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  ...
  │  PARTIAL — HANDOFF-003 consumed    │  cannot finish → SA112f     │  review · source · infra ·
  ▼  F-005 still blocking — re-auth  │  cannot merge  → SA112e     │  PUSH(human) · closeout
...
**Status:** on the critical path; **head is SA117e-2, and this track is truly green — can start, can finish, and can merge on its own work alone**. Track 1 is not running a live parallel leg: its `SA112A-HANDOFF-003` attempt ended at a recorded-partial checkpoint (unapproved `F-005` candidate, QG infrastructure failure, no approving `STATUS: ok` review — DC-44 returned `STATUS: partial`), so Track 3 is the only truly green track. **SA117e-2 is the next pending Track 3 child, dependency-satisfied but unexecuted — its ordered commands have not run.** `SA117E-VAL-001` is resolved (baseline `43d9b8fc` approved 2026-08-01). Open downstream blockers: `SA117E1-REV-001` (high) and `SA117E1-REV-002` (medium) on SA117e-4; plan-review `F-002` (medium, host-PostgreSQL lifecycle) on SA117e-3. **SA112a runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not start SA117e-3 before its scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  *(why →* `apply` with any module has zero end-to-end coverage — `test_e2e_development_workflow.py:276` plans with modules skipped — so this skew class has never been exercised*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **Standing baseline (`SA117E-VAL-001` resolved 2026-08-01).** The approved, immutable lock-diff baseline is `43d9b8fc0abd1c3193a3358f41dee0e09a3c106c` — the first internally consistent 0.87.0 lockstep checkpoint. **`SA117_BASELINE_REF` is an invocation variable** (`Makefile:895`, defaulted empty), so rebaselining is a command-line change requiring **no source delta**; any tolerance or exception logic added to make the lock diff pass is out of scope and rejected. Rationale and prior reviewed progress are in [CHANGELOG.md](../../CHANGELOG.md).
