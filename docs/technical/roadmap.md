...
SA140 ──► SA96-PUBLISH: build/TestPyPI ──► exact-tip green gate ──► publish-or-hold
...
**Longest open chain:** `SA140 → SA96-PUBLISH (build/TestPyPI → exact-tip green gate → publish-or-hold; if authorized: exact-tip core-tag push → hosted publish/PyPI)`. `SA112F-QG-001`, `SA112f`, and the `SA112` umbrella closed on 2026-08-20. `SA140` is the next executable ticket and repairs the apply-path quality regression; `SA96-PUBLISH` then owns TestPyPI, the four-command gate on one clean exact tip, the human publication decision, and hosted-release verification.

**Why this chain stays serial:** `SA112f` owned ordered lifecycle acceptance and umbrella closeout, and its evidence boundary is now settled; `SA140` changes the same apply behavior and must rerun the accepted lifecycle suites before the final gate. `SA96-PUBLISH` cannot precede the repaired quality gate.

**Parallelism result:** with `SA112f` closed, the exclusive PostgreSQL/Docker slot is **free** and `SA140` can start on Track 3 today with no upstream gate. The `SA140` Track 1 split is still a maintainer decision, but its trade-off has shifted: the pro that the slot is "busy anyway" no longer applies, while the con of a second live apply-path writer now buys only overlap with Track 3's own `SA140` work. Tracks 1 and 2 remain idle, and `SA96-PUBLISH` must consume the exact `SA140` tip. See the maintainer decision below. Absent that authorization, no ticket tag, per-module gate list, dependency edge, or merge-order statement changes. The only cross-track conflict surface is the shared closeout-file set named in the execution rules, and the sync-before-merge-back procedure explicitly covers it.
...
| **Track 3 — SA140** | **yes** — blocker `SA112f` closed 2026-08-20; no open ticket or plan gate remains | **yes** — the refactor, its focused tests, and its gate legs are all track-local; the exclusive service slot is now free | **yes** — no cross-track merge-order gate | **yes** | Critical path; real progress, not filler |
...
**Truly-green open tickets:** `SA140` is the only one. It can start, finish, and merge on Track 3 with no upstream gate now that `SA112f` is closed and the exclusive service slot is free. It is on the critical path and therefore represents real release progress, not filler. No off-path filler ticket is assigned.
...
|---|---|---|---|---|
| **SA140 (T3)** | **yes** — blocker `SA112f` closed 2026-08-20; the exclusive service slot is free. The Track 1 implementation split remains an open maintainer decision about *where* it runs, not whether it can start | **yes** — repair and validation are track-local | **yes** — no cross-track order gate | Critical path |
...
**No maintainer decision is needed to start the next v87 action.** `SA112f` closed on 2026-08-20 and `SA140` is startable on Track 3 now. Two decisions are nonetheless open: the `SA140` parallelism authorization immediately below, and the publish-or-hold gate further down. The remaining v87 release decision is later: publish or hold v0.87.0 after the exact-tip green gate. The retained local core tag predates the remaining release work, so `SA96-PUBLISH` must rebind it locally if needed and prove it resolves to the exact green-gated integrated tip before any push. The push starts hosted publication and makes the version permanent; no decision can bypass `SA140`.

**`SA140` parallel-implementation authorization — decision available now, and its trade-off shifted on 2026-08-20.** `SA140` is a bounded complexity refactor of `apply_command.py`; the release-wide quality gate cannot be green until it lands. With `SA112f` closed, `SA140` is no longer gated behind anything and the exclusive service slot is free, so this is now a question of *which track runs it*, not whether its implementation leg can start early. The maintainer owns this call; the options below are restated against the new state and the prior recommendation is left standing pending that call.
...
- **`SA140`:** no blocker remains — `SA112f` closed on 2026-08-20 with its evidence boundary settled, so the refactor may start, finish, and merge on Track 3. The only open question is the Track 1 split decision above, which now concerns track placement rather than an early unblock.
...
1. Hand `SA140` to the Track 3 implementation worker now; `SA112f` closed on 2026-08-20 and no upstream gate remains. Its contract below is already executable; behavior drift returns to the same ticket, never to a raised ceiling or waiver.
2. Hand `SA96-PUBLISH` to a maintainer session only after `SA140` merges. Its contract below separates reversible TestPyPI/local-tag work, the exact-tip green gate, the publish-or-hold decision, and the irreversible core-tag push. No other current v87 ticket is blocked or partially implemented without an executable next action.
...
- [ ] **SA140 — Reduce `_execute_apply_steps_locked` below its complexity ceiling.** `Tier 1 · Track 3 · deps: none open (SA112f closed 2026-08-20) · blocks SA96-PUBLISH`
