...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; repair landed;       SA115a ◐ (acceptance-blocked   SA136d ◐ (plan-review capped;
  │  full rerun timed out;            │   by SA141; only ticket      │  no correction code landed)
  │  recorded partial)                │   on this track)             ▼
  │                                  │                          SA136f
  ▼                                  │                              │
SA122b-5 ◐ (merge-gated)             │                              ▼
  │  e2e paths (incl. xdist paths)   │                          SA117e-4 → -5
  │  + blocking checker; closes -b   │                       loop · seal · PUSH(human)
  │                                  │                              │
  ├──── merge before SA112f ─────────┘                              │
  └──── merge after SA112e ─────────────────────────────────────────┤
                                                                    ▼
...
**Critical path:** `SA136d → SA136f → SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA140 → SA96-PUBLISH` — ten remaining legs, now **blocked at SA136d's plan-review gate** after the maintainer selected a recorded-partial checkpoint on 2026-08-10. **SA141 is a parallel green-gate prerequisite**, started the same day, that must merge before SA112f but is not itself a link in that chain, so it runs concurrently on Track 1 rather than lengthening the path. **SA140 joined the path on 2026-08-08**: accepting `SA136D-QG-001` per-ticket does not satisfy the green gate, which requires an exit-0 `make quality`, so the repair is v87 scope sequenced after SA112f. The earlier SA136d executable attempt is retained as **recorded partial delivery**: its approved correction plan was implemented, focused/full suites and the first complete ordered quality run were green under the accepted quality-vector rule, and the first executable review kept F-004/F-006. Their follow-up corrections are committed; the later full-scope checkpoint review resolved F-006 but retained F-004 and late-surfaced F-007 (**high**, seal VERSION identity) plus F-008 (**medium**, lightweight-tag idempotence). Independently, the second ordered quality run stopped during `make check` on uncached frontend dependency registry timeouts before `make quality`. The 2026-08-10 correction continuation then stopped **before implementation** after two authorized plan-review rounds left one revision-induced high finding open in the synchronization rollback plan. The seal commands therefore remain unauthorized and SA136d stays open. SA136a–c and SA136e are closed, so **SA136d is the only unsatisfied member** of SA136f's `deps: SA136a–SA136e` set — the whole umbrella now hinges on that one child. **SA117e-5 is closeout and sits *off* the critical path**: SA112b's precondition is the *sealed* splits delivered by SA117e-4, not the umbrella's closure. Track 2 and SA122b-5 are entirely off-path filler; SA141 is off-path but not filler — the green gate cannot be claimed without it. The green-gate milestone is governed by the four-command join below and is not claimed here.
...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **no** — the authorized plan-review rounds are spent and the latest plan remains `STATUS: partial`; a fresh continuation must first close the plan-review F-004 rollback finding | **no** — implementation never opened, so the executable F-007/F-008/F-004 and `SA136D-QG-002` ledger is unchanged | **yes, checkpoint only** — no merge-order gate; this recorded-partial status delta may merge | **no** — plan-gated | ✅ **yes** |
| **Track 1** — SA141 (head) | **no** — the executable repair and focused validation are retained, but `qg_runs=2/2`; a fresh continuation needs explicit authorization | **no** — F-001 awaits independent closure and the post-fix full `make ci-e2e` rerun timed out before its final root-cleanliness oracle | **yes, checkpoint only** — no merge-order gate; the recorded-partial delta may merge, but SA141 remains a prerequisite for SA112f | **no** — validation/review-gated | no — off-path, but a hard green-gate prerequisite |
...
**Truly green today: none.** SA136d remains the critical-path head but is plan-gated after the 2026-08-10 recorded-partial decision. SA141 has also reached recorded partial delivery: its root-venv isolation repair and focused evidence are retained, but the post-fix full E2E run hit the quality-agent watchdog and final review remains open. **SA122b-5**'s merge bound is clearable only by SA112e, and **SA115a** cannot finish until SA141 closes. The release-wide green gate remains unclaimed.

**Open maintainer decisions: none.** On 2026-08-09 the maintainer selected SA136d's recorded-partial checkpoint merge despite unresolved high-severity F-007, withholding all seal and release authorization. **On 2026-08-10 the maintainer authorized the SA136d correction cycle and the parallel start of SA141, then selected another recorded-partial checkpoint when two authorized plan-review rounds still left a high rollback-completeness finding open.** Those decisions authorize only the recorded branch integrations described here — no seal command, remote ref mutation, publication, or completion claim. Every other blocker is a hard upstream dependency or review finding, not a pending choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); SA139's scope expansion is implemented, reviewed, and closed. The remaining human gates are execution-time gates obtained at outward-facing actions, not pending decisions.

**Infra serialization (not a track constraint).** SA112's and SA115a's e2e lanes, SA117e-4's `apply` verification, and any `make ci`/`make ci-e2e` rerun all need the same PostgreSQL server, Docker daemon, and ports. The `QS_CI_PARALLEL`/`QS_E2E_PARALLEL` knobs namespace lanes *within* one invocation, not across worktrees — **only one track exercises PG/Docker at a time, and Track 3 has priority.** Abandon or restart an SA115a run rather than make a critical-path leg queue behind it. **This rule is live as of 2026-08-10**, with SA136d and SA141 authorized to run concurrently: they share no file, but SA136d's ordered revalidation chain and SA141's E2E reproduction both need the same server, daemon, and ports. **SA136d holds priority** — abandon or restart an SA141 run rather than queue the critical-path leg behind it. SA136d's own frontend-dependency warm-up (`SA136D-QG-002`) is not service-backed and may proceed at any time.
...
**Status:** on the critical path and **plan-gated after a recorded-partial decision on 2026-08-10**. **Head: SA136d** (add the seal phase and Make targets), retained at the existing corrected implementation tip; no correction code landed in the latest attempt. Prerequisites remain closed and the 2026-08-07 client-side check-then-act re-scope remains ratified. The earlier fresh focused plan passed independent review and all implementation slices landed; the first executable review retained F-004/F-006, and their corrections are committed with focused and full test evidence. The later full-scope checkpoint review resolved F-006, kept executable F-004 open, and late-surfaced F-007 (high: requested seal version is not bound to repository `VERSION`) plus F-008 (medium: same-target lightweight tags violate the idempotent predicate). The second ordered quality run also stopped during `make check` while fetching uncached frontend dependencies, leaving `make quality` pending under `SA136D-QG-002`. The latest correction plan closed three plan-review findings but capped with plan-review F-004 (high: every committed retry branch needs a terminal failure disposition and fresh-branch reconstruction rule). The retained commands must not be used to seal release refs until both the plan gate and executable/validation blockers close. SA136d is the umbrella's last open non-closeout child and the only unsatisfied member of SA136f's dependency set. SA112b's provisioning precondition is already satisfied; its split-publication precondition waits on SA117e-4, which waits on SA136.
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed`
    ...
    **Seal-atomicity requirement re-scoped (settled; rationale in [CHANGELOG.md](../../CHANGELOG.md)).** Server-transactional atomicity is **withdrawn as a requirement**. The seal is enforced client-side as check-then-act with a fail-closed compensating verification, and the residual window is documented rather than engineered away. This unblocks the ticket on stock Git against GitHub.com; no platform migration is required.
    ...
    - **Recorded-partial checkpoint (2026-08-08; task remains unchecked).** `F-001`–`F-008` are the stable executable review ledger; F-006 is resolved by independent review, while the open findings below remain blocking and no release claim is made.
      - **Done / partial improvements:** The fresh plan passed independent review, then Track 3 committed runner-local tag peeling/non-interactive creation, equal-tree identity reuse with branch-race anchoring, pre-armed cleanup/error precedence, pre-bootstrap CLI interaction validation, actual GNU Make dispatch tests, and hermetic real/delegating-Git ref-integrity proofs in `b35e99e6`, `e760d878`, `89f20f10`, `f8a0b69a`, `258afb3b`, and `355f65a1`. The first complete ordered quality run passed 75 focused publish tests, 38 focused Git tests, lint, typecheck, unit tests (Core 2725 passed / 1 skipped at 90.43%; CLI 2060 passed at 90.94%), full tests (7247 passed / 87 expected skips / 12 deselected; module mean 94.40%), and `make check`; `make quality` matched the accepted pre-edit vector exactly (dead 5, high 148, critical 22, large 22, very large 21, duplication 0, warning regressions 0, critical regressions 1). The second ordered run again passed focused tests, lint, typecheck, unit tests (4795 passed / 1 skipped), and full tests (7257 passed / 87 skipped / 12 deselected) before `make check` stopped on npm registry timeouts. Full-scope checkpoint review resolved F-006 after verifying the real/delegating-Git, CLI/Make, multi-module, fault, and ref-integrity matrix; it also confirmed that this checkpoint consistently preserves an unchecked, unauthorized state. No real remote ref, GitHub publication, or PyPI publication occurred.
      - **Pending-Blocking:** `F-004` (**medium**, resilience) — cleanup catches both exception types and preserves a primary failure, but no no-primary path surfaces a runner `OSError` cleanup failure. `F-007` (**high**, correctness; `late-surfaced: true`) — `--seal`/`--seal-all` and their Make callers do not bind the requested version to the authoritative repository `VERSION`, allowing irreversible tags under the wrong release identity. `F-008` (**medium**, correctness; `late-surfaced: true`) — same-target lightweight tags fail the required idempotent seal/status predicate instead of resolving directly when no peeled record exists. `SA136D-QG-002` (**medium**, validation) — the second autonomous quality run timed out fetching uncached no-social frontend dependencies during `make check`; `make quality` did not run. Cache/provide those dependencies, fix the three executable findings, then rerun the complete ordered chain and one full-scope review. The retained commands remain **not release-authorized**.
      - **Decisions needed: none.** On 2026-08-09 the maintainer explicitly selected “Stop here, record Done / Pending-Blocking / Decisions-needed, and merge the checkpoint” with high-severity F-007 still unresolved — authorizing only the recorded-partial branch merge, with no seal or publication authorization implied. **On 2026-08-10 the maintainer granted the fresh authorization that decision required, opening the correction cycle scoped above.** SA136d remains unchecked and every blocker above remains open until that cycle closes. `SA136D-QG-001` remains **resolved 2026-08-08 as reviewed acceptance** under the inherited "no worse than found" rule; no ceiling is raised and SA140 still owns the repair.
      - **Carry-forward plan — CONTINUATION AUTHORIZED 2026-08-10.** The maintainer authorized this correction cycle. **Scope of the authorization:** implement the four items below on the retained tip, revalidate, and obtain review. It does **not** authorize any seal command, remote ref mutation, tag push, publication, or completion claim — those stay behind SA117e-4's execution-time human gate. Because items 1–3 change production behaviour, [Proportionality](#rules-every-ticket-inherits) still requires a scoped plan review over this slice before implementation; the authorization clears the *gate*, not the review. No slice of the *approved* plan remains unimplemented — this is the derived remediation direction for the three blockers, recorded so the next attempt starts from it rather than re-deriving it. Order matters: fix F-007 first, since it is the only one that can mint a wrong-identity irreversible tag.
        1. **F-007 (high, seal VERSION identity).** Bind the requested seal version to the authoritative repository `VERSION` at the single point where `--seal`/`--seal-all` parse it, not at each Make caller — the Make targets pass `VERSION=…` through the `scripts/publish_module.sh` wrapper, so a wrapper-level check would leave the Python entrypoint independently reachable. Reject a mismatch fail-closed before any remote read, with the error naming both versions, mirroring the embed-side mismatch error SA117 step 2 already ships. `_check_release_authoritative` needs no change: it gates on a tag at local HEAD and is orthogonal to which version string was requested.
        2. **F-008 (medium, lightweight-tag idempotence).** The idempotent no-op branch of the seal predicate resolves the target tag through peeling; a same-target *lightweight* tag has no peeled record and currently fails the predicate instead of resolving. Fall back to the direct ref target when peeling yields nothing, in both the seal path and the `sealed@<version>` `--status` column, so the two agree. Keep the fail-closed behaviour for a tag pointing **elsewhere** — this widens what counts as "already at the intended commit", never what counts as movable.
        3. **F-004 (medium, resilience).** Add the missing no-primary-failure path: when cleanup raises a runner `OSError` and no earlier failure exists, that cleanup failure becomes primary and is surfaced. The existing precedence rule (a primary failure stays primary) is already correct and must not change; this only fills the uncovered branch, plus its focused test.
        4. **`SA136D-QG-002` (validation).** Warm or vendor the no-social frontend dependency set *before* the run — the timeout was a cold npm registry fetch inside `make check`, not a code defect. Then rerun the **complete ordered chain from the top** (focused publish + Git tests → lint → typecheck → unit → full → `make check` → `make quality`); a partial rerun does not discharge the gate. `make quality` must match the accepted vector exactly (dead 5, high 148, critical 22, large 22, very large 21, duplication 0, warning regressions 0, critical regressions 1) — `SA136D-QG-001` stays accepted and [SA140](#sa140--quality-ceiling-repair-on-the-apply-path) still owns the repair.
        5. **One full-scope review** over the whole retained slice, then merge-back per the reviewed-tip identity rule.
        Continuation starts from the retained corrected tip (`355f65a1`), not from a fresh branch. Allowlist is unchanged. Until all four close, the seal commands remain **not release-authorized**.
      - **Recorded-partial planning checkpoint (2026-08-10; no implementation; task remains unchecked).**
        - **Done:** Track 3 was clean, fast-forwarded to `v87` at `01ad8675610fcca3a2a1dd71c5efebf0074f1c03`, and remained unchanged through dependency, environment, planning, and review work. Python 3.14.6, Poetry 2.4.1, Node 24.19.0, pnpm 11.18.0, PostgreSQL 18, Docker 29.1.3, Git 2.43.0, and both frontend caches were ready. A fresh six-phase correction plan preserved the existing allowlist, complete ordered gate, quality-vector oracle, trusted `GitRunner`, and prohibition on every seal/remote/tag/push/publication operation. Across the capped reviews, plan findings F-001 through F-003 were explicitly resolved. No product/test file changed, no product test ran, and no outward operation occurred.
        - **Pending-Blocking:** The earlier executable ledger is unchanged: executable F-007 (**high**, VERSION identity), executable F-008 (**medium**, lightweight-tag idempotence), executable F-004 (**medium**, cleanup `OSError` precedence), and `SA136D-QG-002` (**medium**, complete ordered rerun) all remain open. Separately, plan-review F-004 (**high**, completeness; distinct from executable F-004) remains open: Phase 6 must state that **every** committed synchronization/retry branch whose later validation or review fails is terminal, and that each further authorized attempt reconstructs on a previously unused local branch from retained `TRACK3_PRESYNC`, merges frozen `V87_SYNC`, and passes the existing parent/ancestry/exact-tree assertions before review.
        - **Decisions needed:** none. At the second cap the maintainer selected “Stop here, record Done / Pending-Blocking / Decisions-needed, and merge the checkpoint.” A later continuation requires fresh explicit authorization and a focused re-plan/re-review of plan-review F-004 before any implementation. The latest plan never received `STATUS: ok`, so no approved `plan_carryover` exists and no implementation authority crosses this checkpoint.
...
- [ ] **SA115a — Guard clamp and local xdist validation.** `Tier 2 · deps: none` — items 1–3; **no `e2e.yml` edit**, no merge-order gate
  ...
  - **Recorded-partial checkpoint (2026-08-08; SA115a remains open).** **Done:** phases 1–2 remain committed on `wt-track2` (`5193f198`, reconciled at `5b5de830`); the guard clamp and deterministic explicit-worker regression are implemented; 15 focused tests, Bash syntax, and Ruff pass. The serial E2E run exited 0 in **15m32.404s** (Core 34 passed / 1 npm-timeout skip; CLI 29 passed); the parallel rerun exited 0 in **14m43.545s** (Core 35 passed; CLI 29 passed), **48.859s faster**. Three healthy per-worker PostgreSQL containers had distinct project names/ports, and back-to-back cleanup left no scoped container or volume. Final full-scope review returned **`STATUS: ok`** at confidence **96**, resolving `F-001` and `F-002` and approving this recorded-partial merge. The former coverage-policy blocker `SA115A-QG-001` is closed by SA139. **Sole remaining blocker: [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance)** — its executable repair and focused xdist evidence are retained, but its post-fix full E2E run and final review remain open. No full E2E green or SA115a defect claim is made. **Advisory:** `F-003` (low, consistency) asks `docs/technical/validation_policy.md` to document `QS_E2E_XDIST_WORKERS` and the coupled guard clamp; it stays outside this ticket's allowlist. The speedup remains provisional and gets one confirmation re-measure at SA112f, not a re-review or merge gate.
...
- [ ] **SA141 — Diagnose and repair the two Core E2E Poetry-install failures.** `Tier 2 · deps: none` · **blocks SA112f, SA115a acceptance, and the green gate**
  ...
  - **Recorded-partial checkpoint (2026-08-10; task remains unchecked).** Baseline and synced closing ref are `9473c5fa2feba92f9dc70cecd1bd946065a1876c`; no SA141 commit existed when this checkpoint was authored. The retained delta classifies three files as product (`poetry_env.py`, `apply_command.py`, `module_commands.py`) and four as tests (`test_module_commands.py`, `test_apply_command_extended.py`, `test_e2e_full_workflow.py`, `test_generated_project_runtime.py`).
    - **Done:** Diagnosed ambient `VIRTUAL_ENV`/`POETRY_ACTIVE` inheritance under `poetry run pytest` as the shared xdist race that repointed root editable module installs into pytest temporary projects. The delta centralizes Poetry environment isolation in Core and propagates it through apply, module install, and generated-project test callers without skips or weakened assertions. Pre-edit focused reproduction passed 4 tests with both shared and isolated caches. Quality pass 1 restored the root environment, passed 12 focused tests and both exact ticketed xdist nodes, kept all root dry-run oracles clean, and completed `make ci-e2e` with Core 35 passed, CLI 29 passed, and empty quarantine. After review identified F-001, the two missing callers were fixed; focused closure evidence passed 3 apply tests, 1 collectstatic test, 2 exact xdist nodes, Ruff, production MyPy, and `git diff --check`. Final full-scope review explicitly resolved F-001 and confirmed `_run_repo_poetry_command` remains the intentional ambient repository-only exception.
    - **Pending-Blocking:** `F-002` (**medium**, completeness; `late-surfaced: true`) — project generation's Poetry lock, Railway generated-project lock check/fix, and beta-migration recipient lock/install still inherit the ambient environment. Each must use the shared isolation contract and gain focused env-capture regressions while repository-only probes remain ambient. `SA141-QG-001` (**medium**, validation) remains open because quality pass 2's static, coverage, integration, focused, xdist, and pre/post-focused root checks passed, but its `make ci-e2e` process was terminated by SIGTERM at the 1,200,000 ms watchdog during E2E; the final post-CI root oracle therefore did not run. A fresh authorized cycle must fix F-002, rerun the complete full gate with a longer watchdog, execute the final root oracle, and obtain one full-scope review.
    - **Decisions needed:** none. The Adaptive cap default selected this recorded-partial merge at `qg_runs=2/2` and `review_cycles=2/2`; any further implementation/validation/review cycle requires fresh explicit authorization. Advisory F-003 was applied mechanically by recording `apply_command.py` as serialized SA141-then-SA140 work. This post-review ledger reconciliation is deferred to the next full-scope pass with local `git diff --check` evidence. No plan carryover is recorded because the medium-tier plan was not plan-reviewed and all planned implementation slices landed.
...
**The milestone is unclaimed, and has two named prerequisites.** [SA141](#sa141--core-e2e-poetry-install-failures-block-umbrella-acceptance) has repaired the root-venv pollution seam and produced one complete pre-F-001 `make ci-e2e` pass plus post-fix focused/xdist evidence, but its post-fix full rerun hit the quality-agent watchdog during E2E and final review remains open. `make quality` separately exits 2 pending [SA140](#sa140--quality-ceiling-repair-on-the-apply-path). No single clean rerun has covered all four commands, and `make ci` in particular has not been rerun. **SA112f owns the clean closeout rerun** and requires SA140 and SA141 to have landed; SA96-PUBLISH requires that evidence before release.
...
**Rebalancing verdict: no relocation available; one new ticket (SA141) placed on the idle track.** Nothing existing is movable — SA136f/SA117e-4 are dependency-bound to SA136d, SA112b–f are one serial evidence chain, SA122b-5 carries its `merge after SA112e` bound, SA140 is evidence-bound behind SA112b, and Track 2 stays closed to unrelated work. The one available parallelism gain was extracting the two Core E2E failures — previously carried as unowned prose on two tickets that were forbidden to fix them — into **SA141** and homing it on Track 1, which is otherwise idle behind a merge gate. Docker/PostgreSQL remains serialized with Track 3 priority whenever SA141 or SA115a exercises it.

**Open track-topology decisions: none, and no open maintainer decision remains.** The maintainer selected SA136d's recorded-partial checkpoint merge with unresolved F-007 while withholding all seal/release authorization (2026-08-09), then authorized the correction cycle and the parallel SA141 start (2026-08-10) — which is the two-track concurrency this topology was built to allow, since SA136d and SA141 share no file and sit on different tracks. Every remaining “no” in the readiness table is a hard dependency, not a pending choice. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
