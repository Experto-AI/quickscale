...
**Status:** on the critical path. Head is SA133 (SA135 complete); SA117e follows. **SA112a now runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not publish splits, start SA112b, or treat anything as release-ready until SA117e closes.
...
- [ ] **SA133 — Fix the three Core E2E blockers.** `Tier 2 · deps: SA135 ✓ closed` — release-blocking; restores the green-gate milestone's reachability
...
**The milestone is unclaimed.** The exact four-command join remains unproved: SA135 cleared the pre-existing three-react SA90 Dockerfile fixture drift (**QG-SA90-BASELINE**, functional commit `6a779dbb`; `make check QUIET=1` exit 0, `make quality` exit 0 with zero warning/critical regressions), but SA133's ordered broad rerun has **not yet been run**, so no `make ci-e2e` result exists at the post-SA135 baseline and nothing is claimed green before that rerun. The 31 passed / 4 failed Core E2E result behind `SA133-BLK-01` (stale DRF assertion), `SA133-BLK-02` (unpublished-core lock), and `SA133-BLK-03` (self-contradicting Python assertions) is the **historical original diagnosis**, not a current observation: SA133's functional delta is in place (functional commit `167ddd23`; recorded partial; task remains unchecked), and its acceptance rerun is now owed — rerun `make ci-e2e` → `make check QUIET=1` → `make quality` and obtain the final full-scope review (expect `make ci-e2e` exit 0 with Core E2E at 35 passed / 0 failed). **SA133 is the work that restores this milestone's reachability**; SA112f owns the eventual clean closeout rerun, and SA96-PUBLISH requires that evidence before release.
