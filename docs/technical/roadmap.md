...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  ...
  │  TRULY GREEN — HANDOFF-003        │  cannot finish → SA112f     │  Umbrella, split by risk class
  ▼  granted; F-005 correction        │  cannot merge  → SA112e     │  review · source · infra ·
                                      │                             │  PUSH(human) · closeout
...
**Both critical-path tracks are truly green.** Track 3 closes SA117e-2 with the fresh review granted by `SA117E2-RESET-001`; Track 1 runs SA112a's `F-005` correction under `SA112A-HANDOFF-003`. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear, and it is off the critical path. After SA117e-2 receives independent approval, SA117e-3 still needs its own scoped plan review (`F-002`, track-local) before any host-service mutation, and SA117e-4 needs the two maintainer decisions recorded in its task block.
