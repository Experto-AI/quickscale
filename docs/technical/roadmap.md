...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; Core E2E             SA115a ◐ (acceptance-blocked   SA136d ◐ (corrected seal tooling;
  │  poetry-install repair;          │   by SA141; only ticket      │  correction cycle authorized)
...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **yes (authorized 2026-08-10)** — the correction cycle is open; the recorded carry-forward plan for F-007/F-008/F-004 and `SA136D-QG-002` is executable from the retained tip, after its scoped plan review | **yes, on its own track** — all four blockers are inside its existing allowlist and need no other track's output; unproven until the cycle closes | **yes** — no merge-order gate; a further recorded-partial merge remains available if the cycle stops early | **yes** (execution-gated, not dependency-gated) | ✅ **yes** |
...
**Truly green today: SA136d and SA141**, and both are **in flight in parallel as of 2026-08-10** — SA136d on Track 3 is the critical-path head and therefore the only one making **real progress** toward the release; SA141 on Track 1 is **off-path but not filler**, since the green gate is unreachable without it. The two remaining tracks stay blocked by hard upstream dependencies: **SA122b-5**'s merge bound is clearable only by SA112e, and **SA115a** cannot finish until SA141 lands. The release-wide green gate remains unclaimed and no full E2E greenness is claimed anywhere.

**Open maintainer decisions: none.** On 2026-08-09 the maintainer selected SA136d's recorded-partial checkpoint merge despite unresolved high-severity F-007, withholding all seal and release authorization. **On 2026-08-10 the maintainer authorized the SA136d correction cycle and the parallel start of SA141.** That authorization covers implementation, revalidation, and review only — it grants no seal command, remote ref mutation, publication, or completion claim. Every other blocker is a hard upstream dependency, not a pending choice. `SA136D-QG-001` remains dispositioned as reviewed acceptance with its repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); SA139's scope expansion is implemented, reviewed, and closed. The remaining human gates are execution-time gates obtained at outward-facing actions, not pending decisions.
...
**Status:** on the critical path and **in flight — correction cycle authorized 2026-08-10**. **Head: SA136d** (add the seal phase and Make targets), resuming from the retained corrected tip `355f65a1` against the recorded carry-forward plan; the authorization covers implementation, revalidation, and review only, and no seal or remote mutation is authorized. Prerequisites remain closed and the 2026-08-07 client-side check-then-act re-scope remains ratified. A fresh focused plan passed independent review and all implementation slices landed; the first executable review retained F-004/F-006, and their corrections are committed with focused and full test evidence. The later full-scope checkpoint review resolved F-006, kept F-004 open, and late-surfaced F-007 (high: requested seal version is not bound to repository `VERSION`) plus F-008 (medium: same-target lightweight tags violate the idempotent predicate). The second ordered quality run also stopped during `make check` while fetching uncached frontend dependencies, leaving `make quality` pending under `SA136D-QG-002`. The retained commands must not be used to seal release refs until those review and validation blockers close. SA136d is the umbrella's last open non-closeout child and the only unsatisfied member of SA136f's dependency set. SA112b's provisioning precondition is already satisfied; its split-publication precondition waits on SA117e-4, which waits on SA136.
