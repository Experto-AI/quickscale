...
- [ ] **SA115 — Add in-lane pytest-xdist fan-out to the e2e suite.** `Tier 2 · Track 2 · deps: none · merge after SA112`
  ...
  **Done (2026-07-23 implementation; committed and reconciled 2026-07-25).** Phases 1 and 2 are implemented on `wt-track2`: a worker-unique `docker_compose_project_name` fixture and the `QS_E2E_XDIST_WORKERS` runner knob. **`SA115-DEC-001` steps 1–2 are complete:** the four previously parked files are committed (`5193f198`) and `v87` is merged into `wt-track2` (`d2fb5a1e`), so the branch is clean and current. The `scripts/test_e2e.sh` drift is reconciled — three conflicts (Environment header, `--help` list, worker-resolution vs. memory-preflight block), all additive on both sides, resolved by keeping both. Post-merge focus checks are green: `bash -n` clean, 31 focused tests pass, Ruff check/format clean. No container lifecycle or back-to-back teardown has been exercised yet.
  ...
  **Pending / blocking**
  - **Validation not yet run (does *not* require SA112).** Steps 1–3 below need only exclusive Docker/PostgreSQL, which is idle while SA112 sits in its plan-review gate. This is `SA115-DEC-001` step 3, still open for the maintainer to authorize:
    ...
    - **Settle the guard/xdist interaction while validating:** the memory preflight demotes to *serial lanes* under low RAM, but `E2E_XDIST_WORKERS` is resolved independently, so a demoted run still fans out N workers per lane. Decide whether the guard should also clamp the worker count, or whether the two knobs are correctly orthogonal. Recorded at merge time (`d2fb5a1e`); not resolved there.
  ...
  **Decisions needed:** one — `SA115-DEC-001` step 3 (authorize the heavy validation run in the current idle infra window; see the decision block at the end of this document). Steps 1–2 are done. Only the *merge* is mechanically gated by SA112; the earlier "wait for SA112, do nothing" posture was too strong.
...
2. ~~**Reconcile against current `v87`.**~~ **DONE 2026-07-25** (`d2fb5a1e`). Three additive conflicts in `scripts/test_e2e.sh` resolved keeping both sides; `bash -n`, 31 focused tests, and Ruff all green afterwards. The eventual post-SA112 merge should now be near-empty.
