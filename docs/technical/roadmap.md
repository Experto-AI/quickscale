...
- [x] **SA29 — Rebuild storage's config-delivery contract: env-var indirection for secrets.** `Tier 2 · Track 1 · deps: none`
  ...
  *Findings:* CR-SA29-003 resolved (storage module wiring spec propagates backend/bucket/region/endpoint as non-secret derived settings). CR-SA29-002 **remains as accepted/blocking** — operator docs still need to explicitly include `media_url` among `quickscale.yml`/apply-owned non-secret storage settings. The user chose to stop further fix cycles on this finding and proceed with merge-back. SA30 can still proceed on the fixed storage contract. The env-var portability classification already handles `STORAGE_` as a sensitive-token match via the existing `NON_PORTABLE_ENV_CONTAINS` list — no classification update needed. The generated `settings/modules.py` now imports `os` only when a `__QS_ENV__:*` marker is present, keeping the preamble minimal for modules without credential references.
