...
- [ ] **SA88a.1 — Reaching-definition joins for redirected & cross-scope writes.** `Tier 1 · Track 1 · deps: none (foundation; builds on the merged blocked partial)`
  ...
  **Blocked checkpoint (2026-07-15; maintainer-selected stop-and-merge at the review cap).**
  - **Done:** predecessor-seeded branch/module joins, owner-cell redirect handling, `try`/`finally` transfer, unified canonical-call checks, and direct proof/control coverage landed and validated green (MyPy, orgs Ruff/format, 9 focused proofs, 139 conformance tests, retained-role orgs 1000 passed/11 skipped); runtime tenancy and ContextVar/GUC seams unchanged. Detail in [CHANGELOG.md §SA88](../../CHANGELOG.md).
