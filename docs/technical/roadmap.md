...
4. **SA122a → SA122b** (gate-topology registry, Track 1) — arch Finding 11. Release assurance is four hand-synchronized inventories; `TA62` and `SA115-CI-001` are its latest paid drift instances. SA122a builds the registry + parity checker (deps: none) and is **currently blocked on maintainer decision `SA122A-AUTH-001`** after a `STATUS: partial` terminal review; SA122b migrates the consumers and **merges after SA112e**, which owns the last manual e2e path-list append.
...
**Ordering on this track (one worktree, serial):** **SA127 → SA122a → SA122b** (SA121 closed), with SA122b merge-gated behind SA112e.
- **SA127 is Track 1's next action and the release-blocking entry on this track.** Added 2026-07-29; `deps: none`, file-disjoint from SA122a, service-free validation. It is the only Track 1 ticket that is not governance filler. **`SA127-ORDER-001` is RESOLVED (2026-07-29, second pass): SA127 runs first.** The only cost of reordering was preempting in-process SA122a — and SA122a's recorded partial delivery is **blocked on a maintainer authorization decision** (`SA122A-AUTH-001` below), so it cannot proceed today regardless. Reordering therefore delays nothing and was applied as a pure speedup.
- **SA122a follows, once its continuation is authorized.** Finding 11 should be centralized **before or as part of** the SA112e/`SA115-CI-001` path-list edits, so SA122a should land early enough that those edits have a registry to write into; SA122b's consumer migration waits for SA112e so the e2e path list is rewritten once.
- **OPEN DECISION `SA122A-AUTH-001` (2026-07-29): how to continue SA122a.** Its partial delivery merged at `73c153b2` with terminal full-scope review `STATUS: partial` and three unresolved findings; the recorded checkpoint requires an explicit maintainer choice between (a) another narrowly scoped remediation + review cycle, (b) replacement of the checker, or (c) revert of the groundwork. Until that choice is made SA122a **cannot start**, and the registry must not be treated as authoritative by SA112e, `SA115-CI-001`, or SA122b.
...
- [ ] **SA122a — Declarative gate registry + parity checker.** `Tier 2 · Track 1 · deps: none`
  ...
  **BLOCKED — recorded partial delivery (2026-07-29; functional commit `73c153b2`; full closeout evidence in [CHANGELOG.md](../../CHANGELOG.md)).** The additive groundwork is merged (seven-entry JSON registry, standalone parity checker, `make check-gate-parity`, docs, focused suite; the tree reports exactly the five expected publish omissions and 112 focused tests pass), but terminal full-scope review returned `STATUS: partial` — preservation only, not a waiver or completion claim. **Live blockers:** `SA122A-CR-003` (**high**, security-boundary — deceptive/malformed workflow and Bash forms can false-green; E2E parsing is not uniformly structural); `SA122A-CR-002` (**high**, correctness — exact binding/stage, dependency-cycle, Make-delegation, and E2E-ownership invariants incomplete); `SA122A-QG-001` (**medium**, completeness — adversarial/exact-stream proof missing, two focused MyPy errors in the E2E extractor); `SA122A-CR-004` (**low/advisory**, `.PHONY` wording). **Decision needed: `SA122A-AUTH-001`** (Track 1 section above) — no continuation may start without it, and independent full-scope review `STATUS: ok` is required before checking SA122a, starting SA122b, or treating the registry as authoritative.
...
- [ ] **SA127 — Short-circuit managed wiring on an empty module selection.** `Tier 1 · Track 1 · deps: none · Track 1's next action`
  ...
  **Ordering.** Track 1 is one worktree and serial, and **SA127 is now first** (`SA127-ORDER-001`, 2026-07-29): SA122a's merged partial delivery is blocked on maintainer decision `SA122A-AUTH-001`, so putting the release blocker first delays nothing. SA127 is file-disjoint from SA122a's registry + parity checker, so nothing forces a rebase in either direction, and it also precedes SA122b, which is merge-gated behind SA112e and cannot land first anyway.
...
SA122a (gate registry) ⛔ blocked        │                              │  human-confirmed public push
  │  SA122A-AUTH-001 (your call)        │                              │  (scope tooling → v88 SA124)
  ▼                                     │                              │
...
**Diagram checkpoint (2026-07-29, second pass).** Track 3 begins at SA117c (SA117b closed and merged); Track 1 begins at release blocker SA127, with SA122a parked behind decision `SA122A-AUTH-001`.
...
**Track 1 is internally serial and partly release-critical.** SA121 is closed; release blocker SA127 is next, SA122a follows once `SA122A-AUTH-001` is decided, and SA122b then carries the SA112e merge bound. The SA122 governance chain remains off-path, but SA127 must land independently of Track 3 before release.
...
- **`SA127-ORDER-001` — RESOLVED 2026-07-29: Track 1 runs SA127 → SA122a → SA122b.** Applied as a pure speedup, not surfaced as a decision, because its only cost disappeared. Independence checks pass (both `deps: none`; SA127's two-file allowlist — `quickscale_cli/src/quickscale_cli/utils/module_wiring_manager.py`, `quickscale_cli/tests/test_module_wiring_manager_manifest.py` — is disjoint from SA122a's registry/checker files; neither needs PostgreSQL/Docker; neither is part of the other's logical change). The prior objection was that SA122a was *in process* and should land before SA112e/`SA115-CI-001`; but SA122a's partial delivery is itself blocked on maintainer decision `SA122A-AUTH-001`, so it is not advancing and no work is parked by putting the release blocker first. Conflict surface: shared closeout files only (`CHANGELOG.md`, `docs/technical/roadmap.md`), covered by the Merge procedure's `git merge v87`-before-merge-back step.
- **OPEN DECISION `SA122A-AUTH-001` (2026-07-29) — how to continue SA122a's blocked partial delivery.** The gate-registry groundwork merged at `73c153b2` with terminal full-scope review `STATUS: partial` and three live findings (`SA122A-CR-003` high/security-boundary false-green parsing, `SA122A-CR-002` high/correctness incomplete invariants, `SA122A-QG-001` medium/completeness plus two focused MyPy errors). The recorded checkpoint forbids continuation without explicit maintainer authorization. Options: **(a) authorize one more narrowly scoped remediation + review cycle** — cheapest if the parser is salvageable, but this is the same lane that already terminated at `STATUS: partial`; **(b) replace the checker** with a structural parser (parse workflows/Make through real YAML/Make semantics rather than text forms), which is what `SA122A-CR-003` is really asking for and fits the house fail-closed style, at the cost of discarding the current extractor; **(c) revert the groundwork** and re-ticket Finding 11 from scratch, which restores a clean tree but loses the registry SA112e and `SA115-CI-001` were meant to write into. **Fit with prior decisions:** `SA122-DEC-001` already ratified publish as a full-coverage context and the registry is additive-only, so (b) preserves every ratified rule while fixing the mechanism; (c) contradicts nothing but forfeits the sequencing benefit the roadmap has protected for three passes. **Recommendation: (b)**, scoped as a fresh ticket with a structural-parsing acceptance criterion. **States it unblocks:** Track 1's *can start* for SA122a specifically (Track 1 as a whole can already start via SA127); it does not change *can finish* or *can merge* for any track.
...
- **Track 1 (SA127 → SA122a → SA122b; SA121 closed) — start: YES · finish: YES · merge: YES. TRULY GREEN via SA127, which is a release blocker; SA122a alone is decision-blocked.**
  - **Can start — YES, via SA127.** SA127 has `deps: none`, a two-file allowlist, and service-free validation, so Track 1's next action is executable today. **SA122a on its own cannot start** — its merged partial delivery is gated on maintainer decision `SA122A-AUTH-001`; that is a decision blocker, not a hard upstream dependency. SA122b follows with its stated SA112e merge bound.
  - **Can finish — YES.** SA127 reaches a checked box using only Track 1 work: its acceptance is an installed-wheel `plan`/`apply` with modules skipped plus two focused tests, none of which consumes another track's output. (SA122a's *finish* is gated by the same `SA122A-AUTH-001` decision as its start; SA122b retains its stated SA112e merge bound.)
  - **Can merge — YES.** SA127 and SA122a carry no merge-order gate. SA122b alone is order-gated behind **SA112e** (shared `.github/workflows/e2e.yml` path list) — a hard dependency that only that upstream work clears.
...
**Net.** The Track 3 critical path is **SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1 independently carries release blocker **SA127**, now first in its queue; both chains must close for release. Track 2 remains merge-gated.

**Two open decisions: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but now at the cost of delaying release blocker SA127) or leave it behind SA117e — and **`SA122A-AUTH-001`** — how to continue SA122a's blocked partial delivery. `SA127-ORDER-001` is resolved (SA127 first). `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: two** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** (sequencing-only) — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying release blocker SA127; it changes no track's three states. **`SA122A-AUTH-001`** (content) — authorize one more remediation cycle, replace the checker, or revert the groundwork; it is the sole thing standing between SA122a and *can start*. **Defaults while undecided: SA112a stays on Track 3; Track 1 works SA127 and SA122a stays parked.** Every other individual ticket still records **Decisions needed: none**.
