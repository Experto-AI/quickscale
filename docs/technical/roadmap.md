...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-1 → -2 → -3 → -4 → -5
  ...
  │  plan approved; handoff blocked   │  cannot finish → SA112f     │  review · source · infra ·
...
**Critical path:** `SA117e-1 → -2 → -3 → -4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path), with **SA112a on Track 1 running in parallel** and joining as a merge-order precondition of SA112b. **SA117e-5 is closeout and sits *off* the critical path** — SA112b's precondition is the *pushed* splits, delivered by SA117e-4, so SA112b starts as soon as `-4` merges and does not wait for the umbrella to close. Track 2 is entirely off it; Track 1 is on it only for SA112a.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **State.** SA117a/b/c are closed; **SA117e is the sole open child, itself split into SA117e-1 … SA117e-5** on 2026-08-01 because it sized Tier 3. SA117a's local stamp/assert is merged, SA117b made pushing refreshed splits safe, and SA117c supplies the local comparison gate that SA117e-4's post-push verification consumes. The original executable candidate at `43d9b8fc` is on `v87` as recorded partial delivery, **unapproved at umbrella scope** — SA117e is therefore a correction-and-review effort over merged-but-unapproved code, not a greenfield build. SA117e-1 performed the required full-scope historical review over the complete delta including `43d9b8fc`; it returned `STATUS: partial`, and the current findings, owners, decisions, and clean-continuation requirements are recorded on that still-open child below. The former SA117d (scope meta-tooling) is deferred to v88 as **SA124**; its merged code stays in place unadvertised, gating nothing.
  ...
  *(why →* `apply` with any module has zero end-to-end coverage — `test_e2e_development_workflow.py:276` plans with modules skipped — so this skew class has never been exercised*)*
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **`SA117E-VAL-001` RESOLVED — baseline `43d9b8fc` APPROVED (2026-08-01).** `43d9b8fc0abd1c3193a3358f41dee0e09a3c106c` replaces the internally inconsistent `97404f3e…` (VERSION `0.87.0` carrying 60 pre-lockstep module values) that made ordered command 4 exit 2. It is the first internally consistent 0.87.0 lockstep checkpoint and is immutable. **`SA117_BASELINE_REF` is an invocation variable** (`Makefile:895`, defaulted empty), so rebaselining is a command-line change and requires **no source delta** — any tolerance or exception logic added to make the lock diff pass is out of scope and rejected. Prior reviewed progress (plan review `ok`, correction `b11c24c5` reviewed `ok`, source validation, reviewed-but-unexecuted harness) is in [CHANGELOG.md](../../CHANGELOG.md).
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
    ...
    - [ ] **SA117e-1 — Full-scope historical review of the complete executable delta.** `Tier 2 · deps: none` — evidence-only
      ...
      **Recorded partial checkpoint (2026-08-01; task remains unchecked; no source delta).** **Done:** Track 3 started clean at `b22bfca3`, fast-forwarded to `v87` `fc82ba64`, reused the existing Poetry environment, and discovered the authoritative 106-path SA117 review set. Adaptive-change-review DC-9 reviewed the complete historical delta including `43d9b8fc` and `b11c24c5` at tip `fc82ba6421353fd960c21e6ab8ab264b8c049250` and returned **`STATUS: partial`**. Stamp/assert and manifest caller parity were substantially sound; `CR-SA117E-005` is resolved because the evidence distinguishes reviewed from executed work. The SA117e-2 handoff below is improved with full, copyable command lines, but `CR-SA117E-006` remains open until a fresh documentation closeout review approves them. No source/test file, tag, split ref, package registry, Docker resource, or PostgreSQL service changed; no SA117e-2 command, harness, `make ci-e2e`, public push, parity proof, or installed apply ran.

      **Pending-Blocking:** DC-9's scope-local `F-001`–`F-004` are remapped here to avoid collision with the older plan-review `F-001`/`F-002` ledger: `F-001` → `SA117E1-REV-001` (**high**, security-boundary; `quickscale_core/src/quickscale_core/utils/git_utils.py:636-680`) — an `ABSENT` expectation currently emits bare `--force-with-lease` rather than proving explicit remote absence; owner **SA117e-4**. `F-002` → `SA117E1-REV-002` (**medium**, completeness; `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`) — publication authorization is not bound to exact verified evidence or consumed by the publisher; owner **SA117e-4**. `F-003` → `SA117E1-REV-003` (**low/advisory**, command consistency) — candidate correction is the literal SA117e-2 handoff below; `CR-SA117E-006` remains open pending review. `F-004` → `SA117E1-REV-004` (**low/advisory**, deferred scope inventory) — owner **SA124**; no whole-file certification is claimed for absent `quickscale/src/quickscale/_version.py`. SA117e-1 remains unchecked pending a fresh full-scope documentation closeout review; SA117e-2 must not start first.

      **Decisions needed:** before SA117e-4 implementation, choose whether `SA117E1-REV-002` is resolved by binding and one-time consumption of digest-bound authorization evidence or by de-advertising/deferment of that gate under SA124; before `SA117E1-REV-001` implementation, verify and ratify the exact supported Git syntax/behavior for a branch-scoped lease that requires remote absence, including bare-remote race and stale-tracking-state cases. No decision authorizes a push: SA117e-4 still requires its fresh execution-time twelve-row human confirmation.
    ...
    **Plan-review-cap checkpoint (2026-08-01; recorded partial delivery; no ticket closed).** **Done:** Track 3 started clean at `074061ca`, fast-forwarded to `v87` `6d0da500`, and discovery rebound the then-monolithic SA117e continuation. Two bounded plan-review passes reduced the blocking ledger from high+medium (2) to medium (1). `F-001` is resolved at plan-detail level and carried by SA117e-4. No source file, tag, split ref, package registry, Docker resource, or PostgreSQL service changed. **Pending-Blocking:** `F-002` (**medium**, resilience) is carried by SA117e-3 and must close in that child's scoped plan review before host-service mutation. **Decisions needed:** none. The five-child split supersedes the monolithic execution plan; SA117e-1 remains the executable next task. Advisories `CR-SA117E-005` and `CR-SA117E-006` remain open and are assigned to SA117e-1. Full checkpoint history is in [CHANGELOG.md](../../CHANGELOG.md).

    **Current checkpoint supersession:** the SA117e-1 recorded-partial block above is authoritative for the later historical-review findings. It does not rewrite the older plan-review `F-001`/`F-002`; the `SA117E1-REV-*` IDs are a separate ledger. SA117e-1 remains the next task, and its checkbox stays open.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **Implementation-handoff checkpoint (2026-08-01; recorded partial delivery; no ticket closed).** **Done:** Track 1 was clean, fast-forwarded to `v87` `fc82ba6421353fd960c21e6ab8ab264b8c049250`, and its Bash/Make/Poetry 2.4.1 `.venv` environment was ready. Discovery produced validated snapshot `sa112a-topology-v1`. A fresh plan closed `SA112A-PLAN-003`; plan-review pass 1 resolved it but found `F-001` (terminal-success oracle), and planner-owned revision plus pass 2 returned `STATUS: ok`, resolving both findings with no open design question. **Pending-Blocking:** `SA112A-HANDOFF-001` (**medium**, orchestration) — the Adaptive control plane refused the exact four-file implementation handoff because a persisted prior block already consumed `blocked_retries=1/1`; the user declined the one human-authorized override in this run. `F-003` (**medium**, consistency) — the ticket's legacy `Files` line and Track-topology independence list still enumerate only the three shell files, while the approved plan and handoff scope correctly include `scripts/test_installed_wheel_venv.py`; the next continuation must add that test path to both sites and obtain a fresh full-scope documentation review. The checkpoint review cap ended `STATUS: partial` at **medium / 1 → medium / 1**; no executable/test file changed, and syntax, focused, smoke, negative-control, parity, lint, type, executable review, and completion review were not run. **Decisions needed:** before the next continuation can dispatch `Adaptive-implement`, the maintainer must explicitly authorize one fresh attempt for this outstanding four-file block so that authorization can be passed as `user_authorization`; otherwise SA112a stays pending. Reuse the approved plan above — do not create another plan unless `VERSION`, `scripts/smoke_install.sh`, or its mapped caller topology has changed.
...
**Status:** **head is SA112a, whose scoped plan is approved but whose implementation handoff is blocked pending explicit maintainer authorization for one fresh Adaptive attempt** — see `SA112A-HANDOFF-001` in the task block. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**No track-sequencing or worktree-assignment question is open. Three standing implementation decisions are open:** `SA112A-HANDOFF-001` requires explicit maintainer authorization for one fresh Adaptive implementation attempt before Track 1 can dispatch SA112a; SA117e-1 records the explicit-absence Git lease contract for `SA117E1-REV-001` and bind/consume versus approved de-advertise/defer disposition for `SA117E1-REV-002` before SA117e-4 implementation. The earlier authorization decisions were resolved on 2026-08-01:

1. **`SA117E-VAL-001` — RESOLVED, baseline `43d9b8fc` approved.** Track 3's *can start* is cleared; the resumption at ordered command 4 is now owned by SA117e-2.
2. **`SA112A-AUTH-006` — GRANTED and consumed successfully.** SA112a re-entered plan/review and received `STATUS: ok`; this resolved the plan gate but does not override the later persisted implementation-handoff block.

Track 1 has completed SA112a planning but **cannot currently start implementation** until `SA112A-HANDOFF-001` is authorized. Track 3 is preserved at **SA117e-1**, whose historical review ran but whose documentation closeout remains open. Plan-review `F-002` blocks SA117e-3's host-service mutation pending that child's scoped plan-review `STATUS: ok`; `SA117E1-REV-001` and `SA117E1-REV-002` separately block SA117e-4's public split push. Those standing decisions are distinct from the push confirmation and later SA96-PUBLISH, which remain **execution-time** human gates obtained at the outward-facing action rather than pre-granted authorization.

**SA117e's Tier 3 split (2026-08-01) is a sizing correction, not a topology change.** It adds no track, moves no ticket between worktrees, and creates no new shared writer; the whole umbrella stays on Track 3 and its children run serially there under the existing handoff contract. Its one board-level effect is a **shortening**: SA112b's precondition resolves to SA117e-**4** rather than the whole umbrella, so the closeout child `-5` leaves the critical path.
...
- **Current state.** SA112a's fresh scoped plan passed independent plan review (`STATUS: ok`) and is recorded verbatim in its task block; no executable delta exists. Implementation awaits explicit maintainer authorization for one fresh Adaptive handoff attempt because the persisted exact-scope retry ledger is exhausted (`SA112A-HANDOFF-001`).
...
**SA132's three remediation decisions are resolved** (2026-07-31): scope → SA132 closed on its authorized scope with the `make ci-e2e` clause retired, and its three discovered blockers ticketed and closed as SA133; lock strategy → path/`develop` core dependency during the E2E, never publish-to-satisfy-a-gate; assertion alignment → 3.14 and latest DRF are the ratified policy, so the *assertions* move, never the pins. Full rationale is in [CHANGELOG.md](../../CHANGELOG.md).
