...
4. **SA121** (quality-baseline monotonicity gate, Track 1, `deps: none`) — arch Finding 12. `quality_baseline.json` is both measurement and waiver, so raising it legalizes the regression the shrink-only policy forbids. Tier 2, independent. Its authorized continuation is preserved as recorded partial delivery; SA121 remains open on `SA121-CR-003`, `SA121-CR-006`, and `SA121-QG-001`, but **carries no open decision** — `SA121-DEC-002` (2026-07-27) authorized the minimal parser correction, and QG-001's remediation is Track 3's `SA117-QG-REMED`.
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **`SA117-QG-REMED` — quality-ceiling remediation for the five `43d9b8fc` regressions. `Track 3` (assigned 2026-07-27).** `SA121-QG-001` recorded five exceeded quality ceilings that `43d9b8fc` introduced. They are **not** a separate ticket and **not** Track 1 work: every affected file is already inside an SA117 child's allowlist, so remediating them anywhere else would create the exact shared-file merge hazard the serial-handoff contract exists to prevent.
  - **SA117a** owns `module_commands.py` (1642 > 1608), post-SA117a `manifest/loader.py` (686 > 562; the incoming pre-SA117a checkpoint measured 624), and `_perform_module_embed` (CC 22 > 20) — all three are in its declared allowlist.
  - **SA117b** owns `git_utils.py` (unbaselined at 509) and `_update_single_module` (CC 18 > 17) — both are in its declared allowlist.
  - Each child must leave `make quality` no worse than it found it and must clear its own share before its slice review. Per SA121's shrink-only rule, **raising a ceiling in `scripts/quality_baseline.json` is not an acceptable remedy** — reduce the code, or record a structured waiver once SA121 ships the waiver ledger. Baselining `git_utils.py` for the first time is an addition, not an increase, and is permitted with a stated rationale.
  - Conflict surface: none beyond each child's existing allowlist; `scripts/quality_baseline.json` is touched by SA117a/SA117b only if a new-file baseline entry is added, and SA121's continuation is contractually forbidden from editing that file, so the two cannot collide.
  ...
  - [ ] **SA117a — Make the lockstep version comparison exact.** `Tier 2 · Track 3 · deps: none · blocks SA112b` — **critical path**
    ...
    **Recorded partial checkpoint (2026-07-27 — change-review cap; task remains open; delivered scope and evidence in [CHANGELOG.md](../../CHANGELOG.md)).**
    - **Pending / blocking:** `SA117A-CR-002` (**medium**, test gap) — `test_update_rolls_back_noncanonical_manifest_version` asserts no state sync or commit but lacks an explicit `_sync_module_dependencies` spy and `assert_not_called()`. The production sequence is correct; this non-continuation property is not regression-proven. Plus SA117a's `SA117-QG-REMED` share: `module_commands.py` 1642 > 1608 lines, `manifest/loader.py` 686 > 562 lines, `_perform_module_embed` CC 22 > 20. `make quality` stays red until those ceilings are cleared without raising the baseline.
    - **Next action (no decision pending):** a fresh scoped continuation adds the missing dependency-sync spy, clears all three quality regressions, reruns the focused and full quality chain, and obtains full-scope independent `STATUS: ok`; only then may SA117a be checked and Track 3 advance to SA117b.
  ...
  - [ ] **SA117b — Close the split-publication safety and resilience gaps.** `Tier 2 · Track 3 · deps: SA117a` — **critical path**
    ...
    - Verify: blank, unset, and whitespace-only origins each fail closed with a named error; a SIGINT injected between container creation and first use leaves no container or volume behind; a staged path containing spaces/newlines/quotes is handled correctly by the emptiness assertion; the bootstrap refuses an untrusted source; **its `SA117-QG-REMED` share is cleared** — `git_utils.py` is baselined or reduced and `_update_single_module` CC ≤ 17, with no baseline ceiling raised.
...
**Current status — `make quality` is RED at the reconciled Track 3 checkpoint (recorded 2026-07-27 as `SA121-QG-001` / `SA117-QG-REMED`).** The exit criteria above are not currently met. Five runtime ceilings are exceeded: `module_commands.py` 1642 > 1608 lines, post-SA117a `manifest/loader.py` 686 > 562 lines (the pre-SA117a checkpoint measured 624), unbaselined `git_utils.py` at 509 lines, `_perform_module_embed` CC 22 > 20, and `_update_single_module` CC 18 > 17. All five originated with the SA117 preservation checkpoint `43d9b8fc` (recorded partial delivery, unapproved); SA117a's exact-version correction subsequently increased the loader count without changing ownership. They are **Track 3–owned**, not Track 1 defects — Track 1 merely discovered them while running the gate. Remediation is assigned to the SA117 children that already own those files (see `SA117-QG-REMED` below); until it lands, no track may claim the green-gate milestone, and SA121 cannot close because its own acceptance requires `make quality` green.
...
SA121 (baseline monotonicity)     SA115 (e2e xdist; deps: none)  SA117a (exact lockstep) ◄─ next
  (partial; DEC-002 authorized)     │  (validation AUTHORIZED)     ▼   + SA117-QG-REMED share
...
SA121 and SA122a both have `deps: none`. SA121 is a recorded partial checkpoint with three blocking findings, but is **decision-clear** as of `SA121-DEC-002`: its parser correction may proceed, and its remaining external blocker (`SA121-QG-001`) is owned by Track 3's `SA117-QG-REMED`. Track 1 may run SA121's continuation or SA122a; only SA122b carries an ordering bound.
...
- **Track 1 has two runnable tickets, no open decision, and stays off the critical path.** SA121 and SA122a share no files — SA121's continuation is the strict-key parsing seam in `scripts/check_quality.sh` plus its focused tests and `quality_tools.md`; SA122a is a new, purely additive registry + checker whose files appear in no SA112 or SA117 child allowlist and which needs no PostgreSQL/Docker. They are independent, but Track 1 is one worktree and runs them in sequence; `SA121-DEC-002` unblocked SA121's parser correction, so either may go first, and SA121 is the better choice because it is nearly closed and its only remaining external blocker (`SA121-QG-001`) clears on Track 3's schedule. A fourth worktree would let them run truly concurrently but would buy no release time, since neither is on the critical path. Only `decisions.md` is conditionally shared between them, and that surface is covered by the Merge procedure. SA122b is the single ordering exception, bounded below.
...
- **Track 1 — DECISION-CLEAR; SA121 AND SA122a BOTH RUNNABLE (off the critical path).** The track carries **SA121**, **SA122a**, and merge-gated **SA122b**. SA121 is merged only as a partial checkpoint (CR-003, CR-006, QG-001 blocking) but has **no open maintainer decision** — `SA121-DEC-002` authorized the minimal parser correction and QG-001's remediation is Track 3's `SA117-QG-REMED`; `SA122-DEC-001` already resolves SA122a's publish-context disposition. **Next action:** SA121's continuation under `SA121-DEC-002`, then SA122a. SA121 goes first because it is nearly closed, though it cannot check its box until `SA117-QG-REMED` lands and `make quality` is green again. Neither needs PostgreSQL/Docker, so both run concurrently with Track 3's exclusive-infra legs; SA122b still waits for SA112e.
