...
**Status:** on the critical path. The head is **SA117e-4**; SA146 is closed, but corrected-source execution is blocked until a text-complete plan receives independent plan-review `STATUS: ok`. The exact-SHA branch loop already completed for all twelve modules; nothing immutable has been created.
...
- [x] **SA146 — Let `refresh_managed_adapters` see freshly embedded module sources.** `Tier 1 · deps: none` · **unblocks SA117e-4** · **closed 2026-08-14 (Option 1); executable validation and review passed**
  ...
  **Blast radius is exactly three modules and it is closed.** `MANAGED_ADAPTER_ORIGINS` is populated at exactly three sites — `entry_point.py:376` (billing), `:516` (crm), `:1428` (social). No other module has an `adapter.py`. The reported `social` failure and the latent `billing`/`crm` failures are the complete set; there is no fourth symptom waiting.

  **Approach — Option 1, approved.** Retry the import once, only when both conditions hold: the package root `quickscale_modules_<name>` is not already importable, **and** `<active base path>/<name>/src` exists. Perform the retry with that `src` directory prepended to `sys.path`, and remove it again in a `finally`.
  - **Not a fail-hard weakening.** The rule is unchanged: a present manifest still demands a real, importable adapter, and an adapter without `get_manifest_adapter` still raises. Only the *search path* widens, and only to the embedded location the AF7 decision already names as supported ("monorepo and embedded `modules/` contexts" — the wording in all three adapter docstrings). A missing dependency, a syntax error, or a broken submodule still raises exactly as today.

  **Plan-review findings — four, all resolved before approval.**
  1. **Feasibility confirmed (was the review's kill risk).** The retry only works if the three adapter modules are importable in the *CLI's own interpreter*, which has no project dependencies installed. All three import only `quickscale_core.*`, `typing`, and `__future__` — verified: zero Django imports, zero third-party imports. Had any adapter imported Django, Option 1 would have been ineffective and Option 2 forced.
  2. **The existing fail-hard regression already proves the guard, and must stay untouched.** `TestRefreshManagedAdaptersFailure::test_raises_improperly_configured_when_adapter_not_importable` (`quickscale_core/tests/test_manifest_entry_point.py:940-986`) builds a manifest directory with **no `src/`**, so the new retry condition is false and it still raises. **Do not modify this test.** If it needs editing to pass, the implementation is wrong — stop and re-review.
  3. **`sys.modules` caching is inherited, not introduced — say so, do not try to fix it.** Once imported, the adapter stays in `sys.modules`, so a later base-path switch inside one process reuses it rather than re-resolving. That is already true today in the monorepo path and in `_restore_modules_context` (`module_wiring_manager.py:260-268`); SA146 must not change it. Widening scope to process-level adapter cache invalidation is out of bounds — record and ticket it if it ever bites.
  4. **`sys.path` must be left exactly as found.** The retry's insertion is scoped and removed in a `finally`, including on the failure path. The adapter module remaining in `sys.modules` after the path is removed is deliberate and safe: none of the three adapters imports its own submodules lazily.

  - Allowlist: `quickscale_core/src/quickscale_core/manifest/entry_point.py`, `quickscale_core/tests/test_manifest_entry_point.py`. **Two files, no more** — the three module `adapter.py` files need no edit (their docstrings already describe the embedded context as supported), and `module_wiring_manager.py` is deliberately untouched so SA112b's traceback and SA140's `apply_command.py` scope stay clean.
  - New regressions to add (all in the allowlisted test file):
    1. manifest **plus** `<name>/src` present, package not otherwise importable → adapter registers, and `sys.path` is identical before and after;
    2. manifest present, **no** `src/` → still raises `ImproperlyConfigured` (this is finding 2's existing test — assert alongside it, don't replace it);
    3. `src/` present but the adapter module raises on import (e.g. bad submodule) → still raises `ImproperlyConfigured`, and `sys.path` is still restored;
    4. package already importable → no `sys.path` mutation occurs at all (the retry never arms).
  - Verify: the four regressions above pass; the full `quickscale_core/tests/test_manifest_entry_point.py` and `quickscale_cli/tests/test_manifest_entry_point_integration.py` pass unchanged; `make version-check` and `make check QUIET=1` exit 0; `make quality` shows no new or increased ceiling breach (it still exits 2 on the standing SA140 baseline — that is expected and not SA146's); independent full-scope change review returns `STATUS: ok`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
      ...
      **Recorded partial checkpoint (2026-08-15; corrected-source planning only).**
      - **Done:** selected SA117e-4 as Track 3's next pending task; confirmed prerequisite SA146 closed; verified `wt-track3` clean at `a8b75abdd11bd7c752827132d4b3e10bd4020398`; and synced `v87` (`Already up to date`). The planning/review sequence resolved `F-008` through `F-019` on their frozen criteria. No environment setup, executable validation, implementation, local-tag change, remote-ref mutation, seal, teams deletion, PyPI action, or publication occurred.
