...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  ...
  │  TRULY GREEN — HANDOFF-003        │  cannot finish → SA112f     │  review · source · infra ·
  ▼  granted; F-005 correction        │  cannot merge  → SA112e     │  PUSH(human) · closeout
...
**Critical path:** `SA117e-2 → -3 → -4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path), with **SA112a running in parallel on Track 1 under the granted `SA112A-HANDOFF-003`** and remaining a merge-order precondition of SA112b. **SA117e-5 is closeout and sits *off* the critical path** — SA112b's other precondition is the *pushed* splits, delivered by SA117e-4, so SA112b does not wait for the umbrella to close. Track 2 is entirely off it; Track 1 is on it only for SA112a.
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-2 | **no** — the complete ordered chain is green, but mandatory full-scope closeout review is unavailable at `review_cycles=3/2` with `authorized_retries=1/1` spent | **no** — SA117e-2 cannot close without independent review | **yes** — this recorded-partial checkpoint may merge | no | ✅ **yes** — review-gated |
| **Track 1** — SA112a | **yes** — `SA112A-HANDOFF-003` granted 2026-08-03; cap counters reset | **yes** — `F-005` is one focused own-track correction | **yes** — no order gate | ✅ **yes** | ✅ **yes** — real progress |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 1 is truly green and Track 3 remains on the critical path but is review-gated.** Track 3's complete SA117e-2 ordered validation is green, but the child remains unchecked because the mandatory full-scope closeout review cannot dispatch at the spent control-plane cap; Track 1 runs SA112a's `F-005` correction under `SA112A-HANDOFF-003`. Track 3's two "no"s are **maintainer-clearable** — a fresh review budget resolves both (see [Open decisions](#track-topology--settled)) — while Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear, and it is off the critical path. After SA117e-2 receives independent approval, SA117e-3 still needs its own scoped plan review (`F-002`, track-local) before any host-service mutation, and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path; **head remains SA117e-2, and this track can merge its recorded-partial checkpoint but cannot start or finish the next child**. SA117e-1 closed 2026-08-03 (evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). SA117e-2's complete five-command ordered validation passes against the maintainer-approved `1333eb4a…` replacement baseline, but `SA117E-VAL-002` remains open because the mandatory full-scope closeout review cannot dispatch at `review_cycles=3/2` and `authorized_retries=1/1`. `SA117E1-REV-001` (high/blocking, explicit-absence lease safety) and `SA117E1-REV-002` (medium/blocking, unbound publication authorization) remain owned by SA117e-4; plan-review `F-002` remains the separate medium/blocking host-PostgreSQL lifecycle requirement on SA117e-3. **SA117e-2 remains unchecked pending a supported fresh review budget; SA117e-3 is not authorized to start.** **SA112a runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not run SA117e-3's host-PostgreSQL work before SA117e-2 closes and SA117e-3's scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1 closed 2026-08-03** (full-scope historical review, evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). Two of its `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. `SA117E1-REV-003`, `CR-SA117E-005`, and `CR-SA117E-006` are resolved.
    ...
    - [ ] **SA117e-2 — Complete the rebaselined ordered source validation.** `Tier 2 · deps: SA117e-1 ✓ closed` — local, service-free
      **Recorded-partial checkpoint (2026-08-03) — the complete five-command ordered validation is green against `SA117_BASELINE_REF=1333eb4a…` with no source delta; per-command evidence, digests, and test counts are in [CHANGELOG.md](../../CHANGELOG.md).** **Pending-Blocking:** `SA117E-VAL-002` (**high**, validation) has direct green evidence but stays open until a mandatory full-scope independent closeout review approves the completion record; that review is unavailable because the control plane is at `review_cycles=3/2` with `authorized_retries=1/1` spent. **Decisions needed:** none on repository design — the maintainer selected recorded partial delivery. A clean continuation requires a supported fresh review budget or control-plane reset; never bypass or disguise the same changed-file scope. SA117e-2 remains unchecked and SA117e-3 remains blocked.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [ ] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **`SA112A-HANDOFF-003` — GRANTED 2026-08-03; per-attempt cap counters reset.** The maintainer granted **one fresh implementation attempt, one fresh quality-gate run, and one fresh full-scope independent review** over SA112a's unchanged four-file scope, to be passed to the control plane as `user_authorization`. The prior stop was a consumed per-attempt ledger (`qg_runs=2/2`, `review_cycles=2/2`), not a design objection; the approved continuation plan above carries plan review `STATUS: ok` and is **reused verbatim**. **The grant is per-attempt and waives no gate** — the executable slice still needs `STATUS: ok`, closeout still needs its own final full-scope review, and an attempt ending without a checked box records its retrospective and needs fresh authorization. Track 1 is therefore **can start: yes, can finish: yes, can merge: yes — truly green**, and rejoins Track 3 as the second parallel leg of the critical path. SA112a's *completion* merge remains SA112b's precondition; the existing recorded-partial merge does not satisfy it.

    **Scope of the attempt — the one remaining blocker.** `F-005` (**medium**, correctness; `scripts/_installed_wheel_venv.sh` / `scripts/test_installed_wheel_venv.py`): after a nonexistent path component sets probe state false, `..` returns to an existing prefix without restoring probing, so a later directory symlink is treated as nonexistent and receives provisioning output. Cover reverse-order/repeated cancellation, root spellings, symlink-ancestor output identity, status 2, empty stdout, native caller traps, and target preservation. `F-006` remains **low/advisory** unreadable-directory hardening and is **not** part of this correction. Delivered work, validation evidence, `F-003`'s resolution, and the reviewed-tip identity gap from the post-review Ruff hooks are in [CHANGELOG.md](../../CHANGELOG.md).
...
**Status:** **head is SA112a and the track is truly green** — `SA112A-HANDOFF-003` was granted 2026-08-03 and the per-attempt cap counters are reset, so the one focused `F-005` correction/validation/review cycle can start, finish, and merge on Track 1's own work. It is on the critical path, and the earlier partial merge does **not** satisfy SA112b's dependency — only SA112a's completion merge does. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**Open decisions — three, all the maintainer's; one blocks the critical path today.** **Blocking now:** the SA117e-2 closeout **review-budget grant** — the validation evidence is green and `SA117E-VAL-002` needs only a supported fresh full-scope review, unavailable at `review_cycles=3/2` with `authorized_retries=1/1` spent. Granting a fresh review budget (or resetting the control plane) unblocks Track 3's *can finish*, which unblocks SA117e-3 and the whole critical path; the alternative is to leave Track 3 at its recorded partial and let Track 1's SA112a leg carry progress alone. There is no design alternative: bypassing or re-scoping the same changed-file set to dodge the cap is explicitly forbidden. **Not needed until SA117e-4 implements:** the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. `SA112A-HANDOFF-003` was **granted 2026-08-03** and is no longer open. All other blockers on the board are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are **execution-time** human gates obtained at the outward-facing action and never pre-granted.
