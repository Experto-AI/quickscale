...
- [ ] **SA105 — Stage 2a: de-specialize the module-availability surface via runtime config.** `Tier 2 · Track 2 · deps: SA104 ✓ · assistant-executable (maintainer decision resolved)`
  ...
  - **Self-explaining dormant files (mitigation for Option A's dead-weight smell).** Every emitted dormant module surface must carry a generated header banner so a maintainer browsing their own tree understands why it's there and that it's safe to leave — e.g. `// QuickScale: dormant module surface — inert unless the '<module>' module is enabled in quickscale.yml. Tree-shaken from the build. Safe to leave.`. Keep the banner in the verbatim-copy source (it must survive generation byte-identically across projects, so hardcode the module name per template rather than injecting it at generation time). This neutralizes the "why is this file here?" reaction at near-zero cost without breaking the byte-identical invariant.
  - Verify: generate projects with different module sets — the module-availability files are byte-identical across them; dormant module pages tree-shake/lazy-load and do not render when the flag is false; every dormant module file carries the explanatory banner; ESLint/tsc (via SA103) green.
