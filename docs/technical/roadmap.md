...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA128a → b → c → d (parity check)    SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  │  Umbrella, split by domain        │  validation AUTHORIZED      │  TRULY GREEN — RESET-001
  ▼  Make · Bash · YAML · contracts   │  cannot finish → SA112f     │  Umbrella, split by risk class
                                      │  cannot merge  → SA112e     │  review · source · infra ·
SA122b-1 → -2 → -3 → -4 → -5          │                             │  PUSH(human) · closeout
...
**Critical path:** `SA117e-2 → -3 → -4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH` (the SA133 gate precondition is satisfied and off the path). **SA112a is complete** — its exact four-file executable slice is approved at frozen HEAD `44938f7576…` (see the SA112a ticket) — and the completion merge that closes it satisfies SA112b's merge-order precondition. **SA117e-5 is closeout and sits *off* the critical path** — SA112b's other precondition is the *pushed* splits, delivered by SA117e-4, so SA112b does not wait for the umbrella to close. Track 2 is entirely off it; Track 1 was on it only for SA112a, which is now complete, so the remaining Track 1 queue is off the critical path.

**Cross-track edges — three, all merge-order only.** SA122b-5 merges after SA112e, and SA115 merges after SA112; both share `.github/workflows/e2e.yml`'s `pull_request.paths` list. **SA112a (Track 1) must complete and merge before SA112b starts (Track 3)** — SA112a is complete and its executable slice is approved, and the completion merge-back to `v87` that closes this ticket satisfies that edge; the recorded-partial checkpoints already on `v87` never did. SA112b consumes the independently approved provisioning seam, so the edge is satisfied only by SA112a's completion merge-back to `v87`, not by co-scheduling.
...
| **Track 1** — SA128a | **yes** — `deps: none`, no order gate; the previously cap-blocked SA112a QG/review dispatches completed under the authorized `SA112A-RESET-001` | **yes** — SA128a's own two-file scope carries no persisted cap | **yes** — no order gate | ✅ **yes** | no — SA112a was Track 1's only critical-path node and is complete |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is truly green (`SA117E2-RESET-001`, granted 2026-08-03) and on the critical path. Track 1's SA112a is complete:** its exact four-file executable slice passed the ordered QG (DC-52, `STATUS: ok`) and the full-scope review (DC-53, `STATUS: ok`, resolving `F-005`) under the authorized `SA112A-RESET-001` reset, and the completion merge that closes it satisfies SA112b's precondition; Track 1's head advances to SA128a, off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Shared executable surfaces.** `.github/workflows/e2e.yml`'s path list is written by SA112e, SA115, and SA122b-5 — serialized by the merge bounds above. SA112a (Track 1) was the **sole writer of its three provisioning scripts** (ticket now complete); SA112e (Track 3) merely *names* those paths in the workflow trigger list and never edits them, so the cross-track split creates no shared writer. `quickscale_core/tests/test_e2e_full_workflow.py`, `scripts/quality_baseline.json`, and `quickscale_cli/.../module_commands.py` are owned by no open v87 ticket. No other executable surface is shared, so no additional procedure is required.

---

## Track 3 — Core/CLI plumbing, release path

**Status:** on the critical path and **truly green** — head remains SA117e-2, and `SA117E2-RESET-001` (granted 2026-08-03) restores the review budget needed to close it, so the child can start, finish, and merge on Track 3's own work. SA117e-1 closed 2026-08-03 (evidence only, no source delta; narrative in [CHANGELOG.md](../../CHANGELOG.md)). SA117e-2's complete five-command ordered validation passes against the maintainer-approved `1333eb4a…` replacement baseline; `SA117E-VAL-002` stays open only until the granted fresh full-scope closeout review returns `STATUS: ok`. Track 1's SA112a is complete — its executable slice is approved and the closeout merge to `v87` follows the final six-file review — and the remaining Track 1 queue sits off the critical path, so Track 3 remains the only critical-path track. `SA117E1-REV-001` (high/blocking, explicit-absence lease safety) and `SA117E1-REV-002` (medium/blocking, unbound publication authorization) remain owned by SA117e-4; plan-review `F-002` remains the separate medium/blocking host-PostgreSQL lifecycle requirement on SA117e-3. **SA117e-2 remains unchecked until that review returns `STATUS: ok`; SA117e-3 is not authorized to start before it does.** **SA112a ran on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31) and is now complete, so Track 3's next SA112 child is SA112b. Do not run SA117e-3's host-PostgreSQL work before SA117e-2 closes and SA117e-3's scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged and SA112a's completion merge has landed, and do not treat anything as release-ready until SA117e closes at `-5`.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  *(why →* `apply`/`up` have zero installed-artifact coverage; the existing lifecycle e2e runs only from source, which cannot reproduce install-context discovery bugs*)*
  ...
  - [x] **SA112a — Extract the installed-wheel provisioner and preserve smoke parity.** `Tier 2 · deps: none · **TRACK 1**` — on the critical path
    ...
    **State (2026-08-03): COMPLETE — executable slice approved at frozen HEAD `44938f75760bee0f51aa0c289754ff4c865d966d`** (SHA256: smoke=`46af240c…`, helper=`ba36084e…`, wrapper=`72b968c3…`, tests=`0c5b3ebf…`). The ordered QG (DC-52) returned `STATUS: ok`: Bash syntax; **67 focused tests**; two positive `make smoke-install` runs with the exact six markers and all 20 probes; the missing-manifests negative control produced gate script exit **1** / GNU Make wrapper exit **2** with `ImproperlyConfigured` and no `ImportError` — the expected red control, not a failure; the expected parity exit **2** with the five publish omissions; lint, typecheck, diff, and NUL-safe exact-scope checks. The full-scope review (DC-53) returned `STATUS: ok`, resolving **F-005** (**medium**, correctness — probe-state restoration after `..` cancellation) with caller parity passed and no breaking changes or test gaps. **F-006** (**low/advisory**, unreadable-directory hardening) remains open and **out of this correction's scope**. SA112a's *completion* merge to `v87` — which follows this closeout's final six-file review — satisfies SA112b's precondition; neither the recorded-partial merges nor the `HANDOFF-004` checkpoint ever did. Full attempt history (`SA112A-HANDOFF-001` … `-004`, including the pre-dispatch `missing/../link` control findings and the `SA112A-RESET-001` reset record) is in [CHANGELOG.md](../../CHANGELOG.md).
...
**No track-sequencing or worktree-assignment question is open, and the 2026-08-03 rebalancing review applied no move:** every open v87 ticket carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), Track 3's children are one coherent serial umbrella, and SA112a was the parallelized critical-path node until it completed. The remaining off-path work (SA128a–d, SA122b-1…5) is a single serial chain whose head is dependency-blocked on its predecessor, so it cannot be spread further; moving any of it onto Track 3 would push filler ahead of the critical path.

**Standing placements.**

- **SA112a was homed on Track 1 and is now complete** (`SA112A-TRACK-003`, accepted 2026-07-31) — the sole SA112 child with no SA117e-4 bound, `deps: none`, allowlist disjoint from SA128a's and from every other open ticket, and service-free so it never contended with Track 3's infra priority. Its **completion merge to `v87` lands before SA112b starts**; SA112b consumes the *merged* seam, not a working tree. Conflict surface is the shared closeout files only, covered by the `git merge v87`-before-merge-back step; SA112e only *names* SA112a's scripts in the e2e trigger list and never edits them.
- **Track 1's queue advances past SA112a** (**SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**) — the 2026-08-01 Option A reordering kept off-path SA128a behind the critical-path node; SA112a has now completed, so SA128a becomes head.
...
**Open decisions — two, both owned by SA117e-4 and not needed until that child implements:** the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. `SA112A-RESET-001` was **granted 2026-08-03** — the ordered QG (DC-52) and full-scope review (DC-53) it unblocked both returned `STATUS: ok` — and is no longer open. `SA117E2-RESET-001` was **granted 2026-08-03** and is no longer open. All other blockers on the board are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are **execution-time** human gates obtained at the outward-facing action and never pre-granted.
