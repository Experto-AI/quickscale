...
SA128d ⛔ → SA122b-1 → … → -5           SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  ...
  ▼  F-001 blocked; needs budget      │  validation AUTHORIZED      │  fix approved; needs budget
                                      │  cannot finish → SA112d     │  stage-10 test hardcodes 5432
                                      │  cannot merge  → SA112e     │  SA117E3-EXEC-001 decision
SA122b-1 → -2 → -3 → -4 → -5          │                             │
...
|---|---|---|---|---|---|
| **Track 3** — SA117e-3 | **no** — **maintainer decision** `SA117E3-EXEC-001` | **no** — same decision; no tracked correction ran, so the stage-11 oracle and Phase 6 stay unproved | **yes** — the recorded-partial checkpoint has no order gate | no | ✅ **yes** — on the critical path |
...
**No track is truly green. Every "no" that stops work from starting today is a maintainer decision, not upstream work.** `SA117E3-EXEC-001` and `SA128D-EXEC-001` each authorize one fresh execution/review round for a correction that is already designed and approved; granting either flips that track's *can start* and *can finish* together. Track 2's two "no"s are the opposite kind — **hard upstream dependencies** that only SA112d/SA112e can clear and that no decision reaches. Both blocked heads are recorded-partial checkpoints already merged, which is why *can merge* stays yes. SA117e-4 carries two further maintainer decisions, not needed until that child implements.
...
**Status:** on the critical path, **blocked at SA117e-3** — see the [readiness table](#dependency--parallelization-overview) and the checkpoint in that child's block. Clean continuation is `SA117E3-EXEC-001`, then implementation of the already-approved three-file correction; nothing about the stage-11 banner or `make ci-e2e` may be claimed before that. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4.
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Current checkpoint (2026-08-05; recorded partial delivery; task unchecked; no tracked correction; no tag, push, or publication).** Track 3 and `v87` are synchronized at `b151073c513663f08adcd88d472564e98a18b852`. The revised Phase 5 holds scoped plan-review `STATUS: ok`; its one bounded execution observed the approved stop boundary, restored the frozen host PostgreSQL container to running/ready, and left no scratch container, port binding, or FIFO — then stopped in stage 10. Phase 2's approved harness/wrapper candidates and the prior interruption proof carry forward unchanged. Per-phase evidence paths, hashes, and test counts are in [CHANGELOG.md](../../CHANGELOG.md).
      ...
      - **The narrow correction is planned and approved, not executed.** Exact three-file allowlist (`quickscale_modules/backups/tests/test_services.py`, this roadmap, `CHANGELOG.md`); the expected port derives from `connections["default"].settings_dict["PORT"]` — the same settings contract the production command consumes — and **must not weaken or remove the stage-11 oracle**; default and redirected focused proofs, broader backups/Ruff gates, hash provenance, pre-commit full-scope review, reviewed-patch digest identity, exact merge-back, and safe negative-control resumption are all fixed in that plan, which returned plan-review `STATUS: ok` twice (resolving `F-001`, `F-002`, `ORCH-GATE-001`).
      - **Decisions needed:** `SA117E3-EXEC-001` — authorize one fresh execution/review budget round, then execute the approved plan verbatim. Nothing else blocks it; the maintainer selected stop-and-record when the plan-review allowance was spent before implementation, which is not a finding waiver or completion claim.
...
**Open decisions — four, all the maintainer's; two are live today.** `SA117E3-EXEC-001` (blocks the critical path) and `SA128D-EXEC-001` (blocks off-path Track 1) each authorize one fresh execution/review budget round for a correction whose design is already approved — neither needs a design choice, only a budget. The other two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. All other blockers are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are execution-time human gates obtained at the outward-facing action and never pre-granted.
