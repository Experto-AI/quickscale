...
SA128a → b → c → d (parity check)    SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
  │  Umbrella, split by domain        │  validation AUTHORIZED      │  TRULY GREEN — PLAN REVIEW FIRST
...
**Track 3 is truly green with SA117e-3 at its head and is on the critical path. Track 1's SA112a is complete:** its exact four-file executable slice passed the ordered QG (DC-52, `STATUS: ok`) and the full-scope review (DC-53, `STATUS: ok`, resolving `F-005`) under the authorized `SA112A-RESET-001` reset, and the completion merge that closes it satisfies SA112b's precondition; Track 1's head advances to SA128a, off the critical path. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. SA117e-3 needs its own scoped plan review (`F-002`, track-local) before any host-service mutation, and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Status:** on the critical path and **truly green** — head advances to SA117e-3 after SA117e-2 closed 2026-08-03. SA117e-1 and SA117e-2 both produced evidence/closeout documentation only, with no source delta; SA117e-2's complete five-command ordered validation passed against the maintainer-approved `1333eb4a…` replacement baseline, and its fresh full-scope closeout review returned `STATUS: ok`, closing `SA117E-VAL-002` (narrative in [CHANGELOG.md](../../CHANGELOG.md)). Track 1's SA112a is complete — its executable slice is approved and the closeout merge to `v87` follows the final six-file review — and the remaining Track 1 queue sits off the critical path, so Track 3 remains the only critical-path track. `SA117E1-REV-001` (high/blocking, explicit-absence lease safety) and `SA117E1-REV-002` (medium/blocking, unbound publication authorization) remain owned by SA117e-4; plan-review `F-002` remains the separate medium/blocking host-PostgreSQL lifecycle requirement on SA117e-3. **SA112a ran on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31) and is now complete, so Track 3's next SA112 child is SA112b. Do not run SA117e-3's host-PostgreSQL work before its scoped plan review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged and SA112a's completion merge has landed, and do not treat anything as release-ready until SA117e closes at `-5`.
