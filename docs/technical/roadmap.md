...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **yes — authorized 2026-08-10 (second grant)** — the cycle opens with a focused re-plan closing plan-review F-004 (rollback completeness) before any implementation | **no** — implementation never opened, so the executable F-007/F-008/F-004 and `SA136D-QG-002` ledger is unchanged | **yes, checkpoint only** — no merge-order gate; this recorded-partial status delta may merge | **no** — plan-gated | ✅ **yes** |
| **Track 1** — SA141 (head) | **yes — authorized 2026-08-10 (second grant)** — the retained repair and focused validation carry forward; the cycle opens at F-002 | **no** — F-001 awaits independent closure and the post-fix full `make ci-e2e` rerun timed out before its final root-cleanliness oracle | **yes, checkpoint only** — no merge-order gate; the recorded-partial delta may merge, but SA141 remains a prerequisite for SA112f | **no** — validation/review-gated | no — off-path, but a hard green-gate prerequisite |
...
**Truly green today: none — but both heads can now start.** The 2026-08-10 second-grant authorizations clear "can start" for SA136d and SA141; neither can yet finish, because their executable/validation ledgers are unchanged until the authorized cycles close. **SA122b-5**'s merge bound is clearable only by SA112e, and **SA115a** cannot finish until SA141 closes. The release-wide green gate remains unclaimed.

**Open maintainer decisions: none — both were granted on 2026-08-10.** History: the maintainer merged SA136d's recorded-partial checkpoint with F-007 unresolved (2026-08-09), then authorized SA136d's correction cycle and SA141's parallel start (2026-08-10), both of which ended at their caps in recorded-partial state. **The maintainer then granted a second continuation authorization for SA136d and SA141 together (2026-08-10).**
...
- [ ] **SA141 — Diagnose and repair the two Core E2E Poetry-install failures.** `Tier 2 · deps: none` · **blocks SA112f, SA115a acceptance, and the green gate**
  ...
  - **Retained repair and open ledger (task remains unchecked).** The diagnosis (ambient `VIRTUAL_ENV`/`POETRY_ACTIVE` inheritance under `poetry run pytest`), the isolation seam it landed in `poetry_env.py` plus its `apply_command.py`/`module_commands.py` callers, and the evidence that passed are recorded in [CHANGELOG.md](../../CHANGELOG.md). `_run_repo_poetry_command` remains the intentional ambient repository-only exception. Two blockers remain, and no green-gate claim is made.
    - **Pending-Blocking:** `F-002` (**medium**, completeness; `late-surfaced: true`) — project generation's Poetry lock, Railway generated-project lock check/fix, and beta-migration recipient lock/install still inherit the ambient environment. Each must use the shared isolation contract and gain focused env-capture regressions while repository-only probes remain ambient. `SA141-QG-001` (**medium**, validation) — quality pass 2's static, coverage, integration, focused, xdist, and pre/post-focused root checks passed, but its `make ci-e2e` was SIGTERM'd at the 1,200,000 ms watchdog during E2E, so the final post-CI root-cleanliness oracle never ran.
    - **This cycle — CONTINUATION AUTHORIZED 2026-08-10 (second grant; prior `qg_runs=2/2`, `review_cycles=2/2` reset for one cycle).** Fix F-002, rerun the complete full gate **with a watchdog above the 1,200,000 ms that terminated the last run**, execute the final root-cleanliness oracle, and obtain one full-scope review. Reaching the cap again without a checked box returns SA141 to the maintainer for a third decision. No plan carryover exists — the medium-tier plan was not plan-reviewed and every planned slice landed. Advisory F-003 is discharged by recording `apply_command.py` as serialized SA141-then-SA140 work.
...
**Open track-topology decisions: none, and no open maintainer decision remains.** The 2026-08-10 second-grant continuations for SA136d and SA141 (see [Dependency & parallelization overview](#dependency--parallelization-overview)) were authorization choices, not topology ones — the two share no file and sit on different tracks, which is exactly the two-track concurrency this topology was built to allow. Every remaining “no” in the readiness table is a hard dependency, not a pending choice. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
