...
|---|---|---|---|---|---|
| **Track 3** — SA117e-4 | **no** for implementation — both maintainer decisions are ratified, but plan review is capped with `F-001`, `F-004`, `F-005`, and `F-006` blocking | **no** — the four plan blockers, both executable findings, fresh twelve-row push confirmation, and post-push acceptance remain | **yes** — no merge-order gate | no | ✅ **yes** — blocked head is on it |
...
**Status:** on the critical path and **blocked at SA117e-4**. SA112b's provisioning precondition is satisfied by SA112a's merge; its split-publication precondition waits on SA117e-4. The maintainer dispositions for `SA117E1-REV-001` and `SA117E1-REV-002` are ratified, but their combined implementation remains forbidden because plan review capped with four blocking findings. `SA117E3-PUBLIC-ANALYTICS-001` remains open, and the twelve-row confirmation is obtained fresh at push time.
...
**Handoff entry point.** SA117e-4 carries the [ratified dispositions and current blocked checkpoint](#ratified-dispositions--plan-review-cap). Lease research is complete and both maintainer choices are durable in [CHANGELOG.md](../../CHANGELOG.md); a clean continuation starts by resolving the four current plan-review findings and obtaining `STATUS: ok`, not by re-asking those decisions or editing source directly.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    **SA117e-1, SA117e-2, and SA117e-3 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-4` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-4 — Tag, human-confirmed split push, and post-push parity.** `Tier 2 · deps: SA117e-3` · **HUMAN-GATED — outward-facing, irreversible** — satisfies SA112b's precondition
      ...
      **Recorded partial checkpoint (2026-08-05; no source delta, tag, push, or publication).**
      - **Done:** SA117e-3's dependency is satisfied; Track 3 started clean and synced to `v87` at `ba34f594`; Python 3.14.6, Poetry 2.4.1, and Git 2.43.0 were ready; official and local bare-remote research ratified the explicit empty-expectation lease; the maintainer ratified digest-bound one-time authorization; discovery mapped the push/evidence/caller/test surfaces; two planner/reviewer cycles resolved plan findings `F-002` (one canonical byte serializer) and `F-003` (successful local-tag deletion and no-tag recheck). The plan-review ledger moved from **high / 2 blocking** to **high / 4 blocking**, so no convergence or executable approval is claimed.
      - **Pending-Blocking:** `F-001` (**high**, security boundary) — name deterministic hook/barrier points, exact durable restart images, same-claim recovery, and replay/contender rejection for every interruption boundary; `F-004` (**medium**, scope) — assign every tracked file and private artifact to a phase-local modify/create allowlist and mark read-only phases; `F-005` (**high**, completeness) — name and freeze the authoritative twelve-module inventory and make every downstream stage consume that same digest; `F-006` (**medium**, resilience) — arm a concrete pre-write rollback transaction to `ba34f594`, including tracked files, refs, authorization artifacts, local-tag deletion, and clean/no-tag verification. `SA117E1-REV-001`, `SA117E1-REV-002`, and `SA117E3-PUBLIC-ANALYTICS-001` remain unimplemented/open.
      - **Decisions needed:** none for repository design. The maintainer selected `Stop here, record Done / Pending-Blocking / Decisions-needed, and merge the checkpoint`; any future plan/review round requires a new explicit continuation instruction. The fresh twelve-row confirmation remains a separate execution-time gate and was not requested or granted.
...
- [ ] **SA122b — Migrate the consumers onto the registry.** `Umbrella · deps: SA128 ✓ closed`
  ...
  **Only SA122b-5 carries the `merge after SA112e` bound** — that is the point of the per-context split. SA122b-1 – SA122b-4 merge independently instead of the whole migration idling behind five Track 3 children.
  ...
  - [x] **SA122b-2 — Derive both `check_ci_locally.sh` inventories.** `Tier 2 · deps: SA122b-1 ✓ closed` — **complete; Track 1 advances to SA122b-3**
    ...
    - **Complete (2026-08-05; functional commit `cf9c3dc2`).** QG DC-84 passed **24 local-CI tests**, **190 parity tests**, and **49 worker-pool tests**; syntax, Ruff, format, `make check QUIET=1`, and `git diff --check` were green. The parity result retains only the five expected publish omissions. Final full-scope review DC-85 returned **`STATUS: ok`**, resolved `F-001`/`F-002`, and left no advisories; no open SA122b-2 blocker or decision was discovered.
...
**Open maintainer design decisions — none.** The explicit empty-expectation lease and digest-bound bind-and-consume dispositions for SA117e-4 are ratified and recorded in [CHANGELOG.md](../../CHANGELOG.md). Track 3 is instead blocked by the capped plan-review ledger `F-001`, `F-004`, `F-005`, and `F-006`; the selected disposition is recorded partial delivery, and a future plan/review round needs a new explicit continuation instruction. `SA117E3-ORDER-001` is resolved (option b), `SA117E3-EXEC-001` was granted and consumed, and SA117e-3 is closed. The fresh twelve-row push confirmation and SA96-PUBLISH remain execution-time human gates obtained at the outward-facing action and are never pre-granted.
