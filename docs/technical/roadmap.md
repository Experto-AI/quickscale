...
- [x] **SA68 — Finish the launcher env-pair contract for the `migrate` path; delete the last argv deciders.** `Tier 2 · Track 3 · deps: soft — pair with SA59 for CI verification (see Dependency overview above)` — **Complete.**
  ...
  *Resolved follow-up — CR-SA68-004 (medium, previously blocking):*
  `docs/planning/beta-site-migration.md` previously recommended launcher-first
  rollout too broadly. The rollout guidance now explicitly distinguishes
  Redis-present vs Redis-absent conditions for the SA63/SA68 env-pair bridge,
  and the Initial catch-up checklist includes a Redis-verification step.
  Manual adoption of the env-pair bridge into the donor-owned
  ``settings/production.py`` without the paired launcher/doc updates remains
  unsafe in all cases; the updated guidance documents this along with the
  conditional safe-ordering path and clarifies that ``settings/production.py``
  is donor-owned by policy — maintainers must cherry-pick the bridge rather
  than replacing the file wholesale. *(Resolved 2026-07-11 — donor-owned
  policy clarification applied.)*
