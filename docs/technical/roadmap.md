...
SA112F-QG-001-R1 ──► SA112F-QG-001 ──► SA112f ──► SA140 ──► SA96-PUBLISH: build/TestPyPI ──► exact-tip green gate ──► publish-or-hold
...
**Longest open chain:** `SA112F-QG-001-R1 → SA112F-QG-001 → SA112f → SA140 → SA96-PUBLISH (build/TestPyPI → exact-tip green gate → publish-or-hold; if authorized: exact-tip core-tag push → hosted publish/PyPI)`. `SA112F-QG-001-R1` owns the confirmed worker-pool self-test contamination repair; its focused validation and independent review are green, and merge remains. After that merge, `SA112F-QG-001` remains the executable acceptance handoff that must produce the missing verdict. `SA112d`'s lifecycle proof and `SA143`'s generated trigger contract are already complete. `SA112f` and `SA140` merge to `v87` in that order. `SA96-PUBLISH` then owns TestPyPI, the four-command green gate on one clean, exact integrated tip, the final human-only publication decision, and verification of the tag-triggered hosted release.
...
| **Track 3 — SA112F-QG-001-R1** | **yes** — the reviewed harness repair is ready to merge | **yes** — focused validation and independent review are green | **yes** — no cross-track merge-order blocker remains | **yes** | Critical-path harness repair |
...
**Truly-green open tickets:** `SA112F-QG-001-R1`; its focused validation and independent review are green, so merge is the remaining action at the head of the longest dependency chain. `SA112F-QG-001` then resumes with its preflight but cannot finish until the named external conditions and ordered campaign are green. No off-path filler ticket is assigned.
...
| **SA112F-QG-001-R1 (T3)** | **yes** — the reviewed harness repair is ready to merge | **yes** — focused validation and independent review are green | **yes** — no cross-track merge-order blocker remains | Critical-path harness repair |
| **SA112F-QG-001 (T3)** | **no** — blocker `SA112F-QG-001-R1`; rerun preflight only after the repair is reviewed and merged | **no** — ordered serial/default acceptance evidence is not yet green | **n/a** — evidence-only child; hands its verdict to `SA112f` | Critical-path unblock handoff |
...
**No maintainer decision clears `SA112f`'s can-start or can-finish blockers; reviewed `SA112F-QG-001-R1`, stable registry access, exclusive service headroom, and accepted green evidence must clear `SA112F-QG-001`.** The next release decision is whether to publish or hold v0.87.0 after the exact-tip green gate. The retained local core tag predates the remaining release work, so `SA96-PUBLISH` must rebind it locally if needed and prove it resolves to the exact green-gated integrated tip before any push. That push triggers the hosted publish gates, PyPI upload, and GitHub release, making the version permanent. No decision can bypass `SA112f → SA140`.
...
1. Merge the independently reviewed `SA112F-QG-001-R1` focused harness repair without changing the E2E runner's production controls.
2. Return `SA112F-QG-001` to the Track 3 acceptance operator and execute its preflight, ordered commands, diagnosis branches, cleanup, and evidence contract below. A green verdict clears the hard blocker on `SA112f`; an environmental verdict keeps this handoff open; another reproducible defect updates or creates one focused repair ticket and keeps this handoff dependent on it.
3. Finish `SA112f`'s executable review and closeout-document review against the accepted evidence, then merge it. The `SA112` umbrella has no separate implementation or handoff.
4. Hand `SA140` to the Track 3 implementation worker only after `SA112f` merges. Its contract below is already executable and dependency-gated; behavior drift returns to the same ticket, never to a raised ceiling or waiver.
5. Hand `SA96-PUBLISH` to a maintainer session only after `SA140` merges. Its contract below separates reversible TestPyPI/local-tag work, the exact-tip green gate, the publish-or-hold decision, and the irreversible core-tag push. No other current v87 ticket is blocked or partially implemented without an executable next action.
...
- [ ] **SA112F-QG-001 — Obtain a pressure-free ordered acceptance verdict.** `Acceptance handoff · Track 3 · owner: service-capable acceptance operator · deps: SA112F-QG-001-R1 (SA112d + SA143 complete) · blocks SA112f`
  - **Goal and observed blockers:** determine whether the prior CLI failures are external registry/service pressure or a reproducible product/harness defect. Prior default two-worker runs reached green Core E2E but met npm-registry stalls, Docker pressure, and frontend exit 137 in CLI; serial attempts expired without a verdict. The ordered serial campaign also exposed the confirmed self-test contamination routed to `SA112F-QG-001-R1`: its outer `QS_E2E_PARALLEL=0 QS_E2E_XDIST_WORKERS=1` values leaked through `_fake_environment()`, while the same 55-test worker-pool target passed with those two variables unset. No product edit is authorized from the earlier registry/service symptoms alone.
...
- [ ] **SA112F-QG-001-R1 — Make E2E runner self-tests hermetic against outer campaign controls.** `Tier 1 · Track 3 · deps: none · blocks SA112F-QG-001`
  - **Handoff/goal:** repair only the confirmed harness seam in `scripts/test_e2e_parallel.py`. `_fake_environment()` must discard inherited `QS_E2E_PARALLEL` and `QS_E2E_XDIST_WORKERS` before `_run()` applies each test's explicit overrides, so runner-default tests are host-independent while the existing serial and explicit-xdist cases still control their child process deliberately.
  - **Allowlist/forbidden:** `scripts/test_e2e_parallel.py` plus this roadmap dependency metadata. Do not change `scripts/test_e2e.sh`, weaken or bypass its memory guard, change production lane/worker defaults, or reinterpret prior registry/service-pressure evidence as acceptance.
  - **Validation/exit:** require `env QS_E2E_PARALLEL=0 QS_E2E_XDIST_WORKERS=1 make test-integration-worker-pool`, `env -u QS_E2E_PARALLEL -u QS_E2E_XDIST_WORKERS make test-integration-worker-pool`, and the repository `make lint && make typecheck && make test` gate to exit 0. Independent review must confirm the inherited controls are removed before per-test overrides and regression coverage names the outer-campaign case. After review and merge, remove this completed repair from the live planner, retain its closeout in `CHANGELOG.md`, and rerun `SA112F-QG-001` from preflight through both ordered umbrellas; the repair alone supplies no acceptance verdict.
  - **Current evidence:** both the contaminated and clean focused invocations pass 56 tests, `make lint && make typecheck && make test` exits 0, and independent review accepted the hermetic cleanup plus explicit-override regression coverage. The child remains open only for merge; no ordered acceptance campaign or parent verdict is claimed.
  - **Failure/rollback:** a contaminated/clean mismatch or changed explicit-override behavior remains a harness defect; fix it within the allowlist and rerun from the focused target. If the seam cannot be repaired without changing the production runner, discard only this repair's candidate edits and return blocked with the required wider scope; never relax an assertion, guard, or acceptance condition.
