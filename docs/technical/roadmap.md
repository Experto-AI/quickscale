...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; F-002 closed;        SA115a ◐ (acceptance-blocked   SA136d ◐ (full-chain evidence
  │  full reruns infra-failed;         │   by SA141; only ticket      │  accepted; final review /
  │                                  │   on this track)             │  merge-back pending)
...
**Historical-overview note:** the SA136d-specific clauses in the critical-path paragraph immediately above and in the authorization/infra paragraphs below retain the pre-DC-58 recorded-partial state for audit. The current accepted repair/reconstruction/full-chain state is the DC-58 checkpoint recorded under Track 3 below; final full-scope review and named `wt-track3`/`v87` integration are still pending.
...
|---|---|---|---|---|---|
| **Track 3** — SA136d | **n/a — implementation and the complete ordered validation chain are accepted through DC-58** | **no** — final full-scope independent review and named `wt-track3`/`v87` integration remain | **yes, checkpoint only** — no merge-order gate; this docs checkpoint may merge | **no** — final review/merge-back pending | ✅ **yes** |
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — critical path; **blocks SA117e-4**
  ...
  **Available to later children** (shipped by the closed `-a`/`-b`/`-c`): `git_utils.py`'s `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, refspec-explicit `push_tag`, and `authoritative_module_names()` / `AUTHORITATIVE_MODULE_COUNT = 12` as the single fail-hard module inventory.
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a ✓ closed, SA136b ✓ closed` — implementation and complete ordered validation accepted through DC-58; final full-scope review and named `wt-track3`/`v87` integration pending
