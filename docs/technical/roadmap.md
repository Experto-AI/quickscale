...
- **STATUS (stop-at-cap, 2026-06-29):** Schema, helpers, migration rewrites, conformance gate update, and proof tests are implemented and lint-clean. Change-review reached `review_cycles=2` cap with three **blocking** findings unresolved.
  ...
  - **Decision (CR-AF12-001 — RESOLVED, 2026-06-29):** Use the existing `_ensure_rls_test_role()` restricted-role pattern in `test_tenant_table_conformance.py`. That infrastructure already handles role creation, `SELECT` grants on every enrolled tenant table, and the `SET ROLE` cursor pattern. Add the parent-`organization_id` mutation `UPDATE` inside a `SET ROLE` block — but use the **default (unrestricted) connection** (not the restricted role) to issue the `UPDATE`, since the goal is to prove the composite FK rejects the mutation at the DB constraint level regardless of role. Assert `IntegrityError` is raised. No new fixture infrastructure needed.
