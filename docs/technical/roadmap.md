...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA122b-5 ✔ (ungated; land first)    SA115a ✓ (closed;                SA117e-4 (head)
...
**Cross-track edges — none. The board has zero merge-order edges.** The former `SA122b-5 merge after SA112e` bound was **reversed on 2026-08-12**: SA122b-5 lands first and makes `.github/workflows/e2e.yml`'s `pull_request.paths` list a *derived* artifact of `scripts/gate_registry.json`, after which SA112f (which absorbed SA112e) adds its five-path tuple to the registry rather than to the YAML. This removes the shared-file conflict instead of serializing it — the two tickets stop being co-writers of the same list. See [Track topology](#track-topology--settled) for the rationale that supersedes the 2026-08-08 ordering. Track 2 is closed and gate-free. `publish.yml` has no open writer.
...
| **Track 1** — SA122b-5 (only ticket) | **yes** — its E2E path/checker slice is scoped and ready today | **yes** — no other track's output is in its acceptance criteria | **yes** — merge bound reversed 2026-08-12; it now lands *before* SA112f's trigger phase | **yes** | no — off the critical path |
...
**Truly green today: Track 1 (SA122b-5) and live Track 3 (SA117e-4); Track 2 is complete.** Track 3 is *on* the critical path, so it is real progress rather than filler; SA115a is closed off-path work. **SA122b-5 is now startable, finishable, and mergeable today** — it is off-path filler, but it is no longer blocked, and landing it early is what converts SA112f's trigger-phase YAML edit into a registry edit. The release-wide green gate remains unclaimed.
...
- **`.github/workflows/e2e.yml`** (path list) — **SA122b-5 is the last hand-editor of this list.** Once it lands, the list is generated from `scripts/gate_registry.json` and SA112f writes the registry instead, so the two tickets never co-write the YAML; afterwards the file is a derived artifact with no hand-editor at all. Until SA122b-5 merges, SA112f must not start its phase 1 — but it cannot be anyway, being four legs downstream. SA112f only *names* the merged provisioning scripts; it never edits them.
- **`scripts/gate_registry.json`** — SA122b-5 then SA112f phase 1, in that order. Different entries, append-only, no overlap: SA122b-5 registers the 26 existing paths plus the two absorbed xdist paths; SA112f appends its own five-path tuple.
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Umbrella · deps: SA117e-4 (from SA112b on)`
  ...
  **Children (2026-08-12): `-b`, `-c`, `-d`, `-f`.** The former `-e` (E2E workflow-trigger contract) folded into `-f` as its phase 1 — the merge-order reversal reduced it to a registry append that `-f`'s acceptance run must have in place anyway. The `-b`/`-c`/`-d` boundaries stay: each consumes the previous child's evidence, and collapsing them would reintroduce the speculative-fix mode the standing constraints exist to prevent.
  ...
  - [ ] **SA112f — Register the E2E trigger contract, run ordered acceptance, review, and close SA112.** `Tier 2 · deps: SA112d, SA122b-5 (merged)` — **absorbs the former SA112e (2026-08-12)**
...
Items 1–3 were local and unblocked and are the closed SA115a; item 4 (the `e2e.yml` path registration) is owned by SA122b-5 as its "absorbed xdist paths" clause and is the only remnant of SA115 still open, on Track 1.
...
**Status:** queue is **[SA122b-5](#sa122--release-assurance-is-four-hand-synchronized-gate-inventories-arch-finding-11)** (head; **ungated and startable now** — the former `merge after SA112e` bound was reversed on 2026-08-12). The SA128 parity checker is authoritative and is an input, not scope. (SA139 and SA141 are closed; see [CHANGELOG.md](../../CHANGELOG.md).)
...
**Consumer migration is one open ticket.** `SA122b-1`/`-2`/`-3`/`-4` are closed and merged: Make aggregation, both `check_ci_locally.sh` inventories, hosted CI job/`needs` generation, and publish membership already derive from the registry. **The e2e path list and the blocking-checker wiring are all that remain**, so the former `SA122b` umbrella was **folded into its sole open child on 2026-08-12** — one open unit of work does not need an acceptance wrapper, and the wrapper's only remaining function was to close when `-5` closed. The ticket keeps the `SA122b-5` identifier for traceability against the closed siblings and the audit record.

- [ ] **SA122b-5 — Derive the E2E path allowlist, make the checker blocking, and retire Finding 11.** `Tier 2 · deps: SA122b-4 ✓ closed` · **no merge gate — land this before SA112f's trigger phase** · absorbs the former SA122b umbrella

  Migrate `.github/workflows/e2e.yml:15-40`'s 26-path manual allowlist — the workflow's single `on.pull_request.paths` list — onto the registry, then make the parity checker **blocking** in CI now that every context derives.
  ...
  **Inherited constraints** (formerly the umbrella's, now this ticket's): the registry and its schema are an **input, not scope** — if a schema change is needed, stop and escalate rather than editing it; the SA128 checker is the authoritative oracle, so no tolerance or exception logic may be added to make a context read green.
  - **Merge order reversed (2026-08-12).** This ticket previously carried a `merge after SA112e` bound so that SA112e's five-path tuple would not have to be re-migrated. That reasoning is retired: the tuple's owner sits four legs downstream behind `SA117e-4 → SA112b → SA112c → SA112d`, so the bound idled Track 1 for most of the release, and a post-migration registry append is *cheaper* than the pre-migration YAML edit rather than a redo. **This ticket now lands first and owns the last hand-edit of the YAML list**; the tuple is appended to `scripts/gate_registry.json` later by SA112f phase 1 (the former SA112e). Do not reserve a slot, placeholder, or comment for it — SA112f appends its own entries when it runs.
  - **Absorbed xdist paths.** Also register `quickscale_core/tests/test_e2e_xdist_fixtures.py` and `quickscale_core/tests/conftest.py`, which are absent from the current 26 entries — a PR touching only the xdist harness would not trigger the e2e workflow that harness runs in. Same standard as the rest: exact repository-relative path strings, order preserved, `yaml.BaseLoader` regression coverage, registered in `scripts/gate_registry.json`. One ticket owns the list end-to-end. This is the last open remnant of SA115.
  - Verify: the generated `on.pull_request.paths` list reproduces the current 26 entries byte-exact and in order, plus the two absorbed xdist paths present exactly once each; **appending one path to the registry regenerates the workflow with that path in the declared position, proven by a test — this is SA112f phase 1's contract and must be demonstrated here, not deferred to it**; adding one new gate requires editing only the registry and its implementation, proven by a test that adds a gate and asserts **all five** contexts pick it up; the checker is blocking in CI and green; `make check`, `make ci`, and hosted CI stay green with unchanged effective inventories. Full-scope review returns `STATUS: ok` — closing this ticket retires arch **Finding 11** directly, with no umbrella left to close.
...
- **Track 1's head is SA122b-5**, its only open ticket, and it is **ungated**. **Merge-order reversal (2026-08-12), superseding the 2026-08-08 fold decision:** the ticket previously merged *after* SA112e so SA112e's tuple would not need re-migrating. Three facts retire that: (a) the tuple's owner (SA112e, since folded into SA112f) sits four legs downstream behind `SA117e-4 → SA112b→c→d`, so the bound idled Track 1 — the board's only filler track — for most of the release; (b) after migration, that edit is a registry append, which is *cheaper* than the pre-migration YAML edit, so nothing is "redone"; (c) the reversal **removes** the co-writer conflict rather than serializing it, since the YAML becomes a derived artifact with exactly one hand-editor. The original ordering optimized for avoiding a redo that does not exist, at the cost of a real idle. **Conflict surface: none** beyond the shared closeout files, which the mandatory `git merge v87`-before-merge-back step in [Parallel execution tracks](#parallel-execution-tracks) covers — keep both tracks' entries when resolving.
