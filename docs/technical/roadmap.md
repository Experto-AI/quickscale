...
SA112a (provisioner) ◄─ next        SA115 (e2e xdist; deps: none) SA117e-2 → -3 → -4 → -5
  ...
  │  PARTIAL — HANDOFF-003 consumed    │  cannot finish → SA112f     │  Umbrella, split by risk class
  ▼  F-005 still blocking — re-auth  │  cannot merge  → SA112e     │  review · source · infra ·
                                      │                             │  PUSH(human) · closeout
...
| **Track 1** — SA112a | **no** — `SA112A-HANDOFF-003` consumed; attempt ended unchecked; fresh authorization needed | **no** — `F-005` lacks an independent QG result and an approving `STATUS: ok` full-scope review (DC-44 returned `STATUS: partial`) | **yes** — no order gate | no | ✅ **yes** — real progress, blocked on re-authorization |
| **Track 2** — SA115 | yes (validation authorized; yields infra to Track 3) | **no** — **hard dep** on SA112d | **no** — **hard dep** on SA112e | no | no — filler |

**Track 3 is truly green (`SA117E2-RESET-001`, granted 2026-08-03) and on the critical path; Track 1's parallel leg is paused at a recorded-partial checkpoint** — the `SA112A-HANDOFF-003` attempt produced an unapproved `F-005` candidate whose authorized QG dispatch (DC-42) failed at infrastructure level and whose fresh retry was refused, so no independent QG result exists, the independent full-scope review (DC-44) returned `STATUS: partial`, and the attempt ended without a checked box. Track 1 is blocked again on a maintainer decision (`SA112A-HANDOFF-004` — restore one QG dispatch and one full-scope review dispatch) before its leg can resume. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear; no maintainer decision reaches them, and it is off the critical path, so its available work is filler. Downstream of Track 3's head, SA117e-3 additionally needs its own scoped plan review (`F-002`, track-local) and SA117e-4 needs the two maintainer decisions recorded in its task block.
...
**Status:** **head is SA112a and the track is at a recorded-partial checkpoint** — the `SA112A-HANDOFF-003` attempt ran and ended without a checked box: DC-41's `F-005` candidate is unapproved, the authorized QG dispatch (DC-42) failed at infrastructure level with its fresh retry refused, no independent QG result exists, and the independent full-scope review (DC-44) returned `STATUS: partial`. Track 1 is blocked again on a maintainer decision (`SA112A-HANDOFF-004` — restore one QG dispatch and one full-scope review dispatch) before the focused `F-005` correction/validation/review cycle can resume. It is on the critical path, and neither the earlier partial merge nor this checkpoint satisfies SA112b's dependency — only SA112a's completion merge does. Everything after it is governance filler that changes how "green" is decided, not what the generator emits. Queue: **SA112a → SA128a → SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, with only SA122b-5 merge-gated (behind SA112e). All allowlists are disjoint from one another and from every Track 3 surface, and none needs PostgreSQL or Docker; they run serially only because Track 1 is one worktree.
...
**Open decisions — three, all the maintainer's; one blocks Track 1 today.** `SA112A-HANDOFF-004` (**blocks Track 1 today**): the `SA112A-HANDOFF-003` attempt is consumed and ended without a checked box, so a fresh authorization must restore **both** one QG dispatch **and** one full-scope review dispatch — the consumed shared override ("one extra cycle on blocking findings only") cannot supply both. The other two are owned by **SA117e-4** and are not needed until that child implements: the explicit-absence Git lease contract for `SA117E1-REV-001`, and the bind-and-consume versus approved de-advertise/defer disposition for `SA117E1-REV-002`. `SA117E2-RESET-001` was **granted 2026-08-03** and is no longer open. All other blockers on the board are hard upstream dependencies. These standing decisions are distinct from the twelve-row push confirmation and SA96-PUBLISH, which are **execution-time** human gates obtained at the outward-facing action and never pre-granted.
