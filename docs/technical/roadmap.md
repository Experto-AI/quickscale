...
The green-gate join (SA96-GATE), the installed-wheel discovery/resolver chain (SA109 ✓, SA110 ✓, SA113 ✓, SA111a ✓, SA111b ✓), the Track 2 frontend de-specialization chain (SA104 → SA108 ✓), the Track 1 re-verification chain (SA114 ✓, SA116 ✓), SA120 (quiet `check` parity), SA125 (file line-ceiling retirement), SA126 (module-command complexity remediation), and SA117b (split-publication safety) are closed; detail lives in [CHANGELOG.md](../../CHANGELOG.md). The remaining open work is grouped as the six entries below — SA117 (split into serial children on 2026-07-27 as `SA117-DEC-003`, with SA117a and SA117b now closed) remains a release blocker through SA117c and SA117e, while SA128–SA122b are the remaining gate-governance chain and SA127 is a second release blocker found by manual `apply` on 2026-07-29 — SA121 is closed, which also **closes arch-audit Finding 12**. The open Track 1 order is SA127 → SA128 → SA122b (`SA127-ORDER-001` resolved 2026-07-29; `SA122A-AUTH-001` resolved 2026-07-29 as option (b), closing SA122a as superseded and opening SA128). **No open ticket is Tier 3.**
...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Open findings and their owning child (no decision pending).** All SA117a/SA117b findings are resolved (see [CHANGELOG.md](../../CHANGELOG.md)). `SA117-CR-006` (**high**, completeness — lock-drift route omits complete baseline lock semantics) → **SA117c**. `SA117-CR-005` (**medium**, breaking change) is **deferred to v88 as SA124** and is not a v87 blocker.
...
Track 1 holds the gate-governance work: it changes how "green" is decided, not what the generator emits. `TA62`/SA120, **SA125**, and **SA126** are closed (see [CHANGELOG.md](../../CHANGELOG.md)); **SA121** is closed (`SA121-DOC-CR-002` resolved), followed by **SA128/SA122b** from [arch-audit.md](../../arch-audit.md) Finding 11 (SA122a closed as superseded, `SA122A-AUTH-001`).

**Ordering on this track (one worktree, serial):** **SA127 → SA128 → SA122b** (SA121 and SA122a closed), with SA122b merge-gated behind SA112e.
- **SA127 is Track 1's next action and the release-blocking entry on this track.** Added 2026-07-29; `deps: none`, file-disjoint from SA128, service-free validation. It is the only Track 1 ticket that is not governance filler. **`SA127-ORDER-001` is RESOLVED (2026-07-29, second pass): SA127 runs first.** The only cost of reordering was preempting then-in-process SA122a, which has since closed as superseded, so the reorder parks nothing.
- **SA128 follows.** Finding 11 should be centralized **before or as part of** the SA112e/`SA115-CI-001` path-list edits. The registry those edits write into is **already merged and usable** (SA122a's kept half); SA128 makes the checker over it trustworthy, and SA122b's consumer migration waits for SA112e so the e2e path list is rewritten once.
...
- [ ] **SA127 — Short-circuit managed wiring on an empty module selection.** `Tier 1 · Track 1 · deps: none · Track 1's next action`
...
The two former complexity blockers were closed by SA126 (Track 1), so the green-gate unblock no longer sits behind the SA117 chain's plan gates. SA121 is closed and its merge-base monotonicity gate is live in `make quality`; SA127 then SA128 follow on Track 1.
...
────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA127 (zero-module apply fix) ◄─ next  SA115 (e2e xdist; deps: none)  SA117c (lock-drift) ◄─ next
  │  deps: none · RELEASE BLOCKER       │  validation AUTHORIZED       │  deps: SA117b ✓
  │  independent of SA117               │  cannot finish → SA112f      ▼
  ▼                                     │  cannot merge  → SA112e     SA117e (review + push splits)
SA128 (structural parity check)         │                              │  human-confirmed public push
  │  deps: none · replaces SA122a       │                              │  (scope tooling → v88 SA124)
  ▼                                     │                              │
...
**Diagram checkpoint (2026-07-29, second pass).** Track 3 begins at SA117c (SA117b closed and merged); Track 1 begins at release blocker SA127, then SA128 (SA122a closed as superseded by `SA122A-AUTH-001`).
...
**Track 1 is internally serial and partly release-critical.** SA121 is closed; release blocker SA127 is next, SA128 follows, and SA122b then carries the SA112e merge bound. The SA122 governance chain remains off-path, but SA127 must land independently of Track 3 before release.
...
**Parallelism.** The open tickets occupy all three tracks: SA117c–SA117e → SA112a–SA112f → SA96-PUBLISH sequentially on Track 3, SA115 on Track 2, and SA127 → SA128 → SA122b on Track 1 (SA121 and SA122a closed); SA117b and SA126 are complete. **The repository runs exactly three worktrees; a fourth is not created** (`SA112A-TRACK-001`, resolved 2026-07-28 — see Standing decisions).
...
- **`SA127-ORDER-001` — RESOLVED 2026-07-29: Track 1 runs SA127 → SA128 → SA122b.** Applied as a pure speedup, not surfaced as a decision, because its only cost disappeared. Independence checks pass (both `deps: none`; SA127's two-file allowlist — `quickscale_cli/src/quickscale_cli/utils/module_wiring_manager.py`, `quickscale_cli/tests/test_module_wiring_manager_manifest.py` — is disjoint from the registry/checker files; neither needs PostgreSQL/Docker; neither is part of the other's logical change). The prior objection was that SA122a was *in process* and should land before SA112e/`SA115-CI-001`; SA122a has since closed as superseded (`SA122A-AUTH-001`) and its successor SA128 has not started, so no work is parked by putting the release blocker first. Conflict surface: shared closeout files only (`CHANGELOG.md`, `docs/technical/roadmap.md`), covered by the Merge procedure's `git merge v87`-before-merge-back step.
- **`SA122A-AUTH-001` — RESOLVED 2026-07-29: option (b), replace the checker; ticketed as SA128.** The groundwork merged at `73c153b2` with terminal review `STATUS: partial` and three live findings (`SA122A-CR-003` high/security-boundary false-green parsing, `SA122A-CR-002` high/correctness incomplete invariants, `SA122A-QG-001` medium/completeness plus two focused MyPy errors). Option (a), another remediation cycle, was declined: the extractors decide membership by regex over lines, so each cycle closes one deceptive form and leaves the next — the compounding shape arch Finding 11 exists to remove, and the same lane already terminated at `STATUS: partial`. Option (c), revert, was declined because it discards a registry that is itself sound and that SA112e/`SA115-CI-001` are meant to write into. **(b) keeps the registry, the Make entrypoint, and `SA122-DEC-001`'s publish-as-full-coverage rule unchanged and replaces only how an inventory is observed** — Make asked for its own expansion, shell parsed to a syntax tree or dry-run with argv recording, workflow YAML already structural — with unparseable input a hard error per [decisions.md §fail-hard-principle](./decisions.md#fail-hard-principle). **SA122a is closed as superseded; SA128 owns `SA122A-CR-003`, `SA122A-CR-002`, and `SA122A-QG-001`.** Track 1's queue is SA127 → SA128 → SA122b; no track's three states changed.
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on its current queue?** Track 1 now leads with release blocker SA127, followed by SA128, while Track 3 has SA117c and SA117e ahead of SA112a. Reusing Track 1 still requires no fourth worktree, but it would delay SA127 as well as the governance queue.
  ...
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* Track 1's queue head. That head is now **release blocker SA127**, so the move would delay a release blocker to advance a different one; it no longer trades against governance filler.
...
- **Track 1 (SA127 → SA128 → SA122b; SA121 and SA122a closed) — start: YES · finish: YES · merge: YES. TRULY GREEN, and SA127 is a release blocker.**
  - **Can start — YES.** SA127 has `deps: none`, a two-file allowlist, and service-free validation, so Track 1's next action is executable today. SA128 also carries `deps: none` and service-free validation now that `SA122A-AUTH-001` is resolved. SA122b follows with its stated SA112e merge bound.
  - **Can finish — YES.** SA127 reaches a checked box using only Track 1 work: its acceptance is an installed-wheel `plan`/`apply` with modules skipped plus two focused tests, none of which consumes another track's output. SA128's acceptance is likewise self-contained — adversarial fixtures and the existing registry, no other track's output. SA122b retains its stated SA112e merge bound.)
  - **Can merge — YES.** SA127 and SA128 carry no merge-order gate. SA122b alone is order-gated behind **SA112e** (shared `.github/workflows/e2e.yml` path list) — a hard dependency that only that upstream work clears.
  - **On the critical path? PARTLY — SA127 is, the SA122 chain is not.** SA127 blocks the release independently of Track 3: installed `apply` cannot succeed for a module-less project until it lands, and no Track 3 ticket fixes it. SA128 and SA122b remain governance filler. `SA112A-TRACK-002` therefore weighs two release-critical nodes against each other rather than moving critical work onto an otherwise off-path track.
...
**Net.** The Track 3 critical path is **SA117c → SA117e → SA112a → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH (human)**. Track 1 independently carries release blocker **SA127**, now first in its queue, followed by SA128 and SA122b; both chains must close for release. Track 2 remains merge-gated.

**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (Track 3 critical-path progress, but now at the cost of delaying release blocker SA127) or leave it behind SA117e. `SA127-ORDER-001` is resolved (SA127 first) and `SA122A-AUTH-001` is resolved (option (b), ticketed as SA128). `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one, sequencing-only** (stated in full in the Parallelism section). **`SA112A-TRACK-002`** — move SA112a onto Track 1 so the Track 3 path's first SA112 node is worked now, at the cost of delaying release blocker SA127; it changes no track's three states. **Default while undecided: SA112a stays on Track 3; Track 1 works SA127 → SA128 → SA122b.** Every individual ticket records **Decisions needed: none**.
