...
**Parallelism result:** no ticket moves without maintainer authorization. `SA112F-QG-001` owns the exclusive service slot once the service window is reserved. Tracks 1 and 2 are idle, but the `SA112` umbrella has no independent action, and `SA96-PUBLISH` must consume the exact `SA140` tip. One candidate move is open for decision: `SA140`'s **service-free implementation leg** (helper extraction, focused apply tests, `make check`, `make quality`) shares no file with `SA112F-QG-001`, whose tracked-file allowlist is empty, and could run today on idle Track 1 — the same reasoning that relocated `SA143`. It is not moved unilaterally because `SA140` still carries a merge-order gate behind `SA112f`, and its `make ci`/`make ci-e2e` legs need the same exclusive service slot Track 3 holds. See the maintainer decision below. Absent that authorization, no ticket tag, per-module gate list, dependency edge, or merge-order statement changes. The only cross-track conflict surface is the shared closeout-file set named in the execution rules, and the sync-before-merge-back procedure explicitly covers it.
...
| **SA140 (T3)** | **no** — blocker `SA112f`; hard upstream ordering dependency under the status quo. Partially user-decision-clearable: authorizing the Track 1 implementation split above makes the service-free leg startable today | **yes** — once started, repair and validation are track-local | **yes** — no cross-track order gate | Critical path |
...
**No maintainer decision is needed to start the next v87 action, and no maintainer decision can clear `SA112f`'s current blockers.** A clean worktree, healthy services and resources, and accepted `SA112F-QG-001` evidence clear them. Two decisions are nonetheless open: the `SA140` parallelism authorization immediately below, and the publish-or-hold gate further down. The remaining v87 release decision is later: publish or hold v0.87.0 after the exact-tip green gate. The retained local core tag predates the remaining release work, so `SA96-PUBLISH` must rebind it locally if needed and prove it resolves to the exact green-gated integrated tip before any push. The push starts hosted publication and makes the version permanent; no decision can bypass `SA112F-QG-001 → SA112f → SA140`.

**`SA140` parallel-implementation authorization — decision available now.** `SA140` is a bounded complexity refactor of `apply_command.py`; the release-wide quality gate cannot be green until it lands.

- **Authorize the split:** hand `SA140`'s implementation leg to idle Track 1 now, running concurrently with the Track 3 acceptance campaign, and keep its `make ci`/`make ci-e2e` legs and merge-back behind `SA112f` on Track 3. **Pros:** removes `SA140`'s implementation duration from the critical path while the exclusive service slot is busy anyway; no shared file with `SA112F-QG-001`, whose tracked-file allowlist is empty; matches the precedent that moved `SA143` off a critical-path leg. **Cons:** creates a second live worktree touching apply code, so the `SA112f` acceptance evidence and the `SA140` candidate must be reconciled explicitly at merge-back rather than implicitly by serialization. This clears **`SA140` can-start** only; can-finish and can-merge stay gated on `SA112f`.
- **Keep it serial (status quo):** leave `SA140` wholly on Track 3 after `SA112f` merges. **Pros:** one apply-path writer at a time; the accepted lifecycle evidence and the refactored code are never concurrently live. **Cons:** Tracks 1 and 2 stay idle for the whole acceptance campaign and `SA140`'s duration stays on the critical path.
- **Recommendation:** keep it serial. The wall-clock saving is one bounded refactor against an approximately one-hour campaign, and `SA112`'s acceptance claim is specifically about apply-path lifecycle behavior — the one surface `SA140` rewrites. Serializing is organically consistent with the execution rule that one reviewed child runs at a time per track, and with the existing `SA112f → SA140` evidence boundary.
...
- **`SA140`:** blocker `SA112f` is a hard ordering dependency for the service-backed legs and for merge, because the refactor must preserve and rerun the lifecycle behavior just accepted. The service-free implementation leg is the one part a maintainer decision can unblock early — see the Track 1 split decision above; the recommendation is to keep it serial. Merge before `SA112f` would invalidate the evidence boundary in either case.
...
- [ ] **SA112f — Run ordered acceptance, review, and close SA112.** `Tier 2 · Track 3 · deps: SA112F-QG-001`
  - **Blocker cleared (2026-08-20):** the `SA112F-QG-001` green verdict above is available for acceptance; can-start and can-finish are now `yes` and only the reviews remain.
  - **Pending work:** consume the green `SA112F-QG-001` verdict, record the serial/default timing comparison and provisional-speedup conclusion, review executable changes first, and then review the closeout documents. Neither `SA112f` nor `SA112` may be checked before that evidence is accepted.
