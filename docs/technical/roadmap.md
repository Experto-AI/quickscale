...
- [ ] **SA92 — Squash all module migrations to a final-schema initial migration; delete the SA88 gate saga.** `Tier 2 · Track 1 · deps: none`
  ...
  **Blocked checkpoint (updated 2026-07-16; second explicit maintainer cap decision: stop, record, and merge):**
  - **Done:** all nine module migration histories are collapsed to final-schema `0001_initial` files; 35 intermediate migrations, the SA88 gate, and `operator_access_migration` are removed. Fresh-database catalog/RLS/data captures match `v87` exactly (21 FORCE-RLS tables / 42 policies, five parent UNIQUE constraints, six composite FKs, Forms four presets / 16 fields). The continuation hardened the migration-DML guardrail and replaced permissive CRM/Forms/Billing RLS and Billing partial-index fragments with exact PostgreSQL-18 comparisons and negative controls. Focused evidence is green: Ruff, orgs-source MyPy, and 29/29 restricted-role guardrail/predicate/normalizer checks under `quickscale_test_role`.
  - **Pending/Blocking:** **CR-SA90-MSQ-002 (high/blocking)** remains open after two non-converging review cycles: the handwritten scanner still has concrete false negatives for post-control-flow bindings, parameter/import scope, nested named provenance, complete RHS validation, and PostgreSQL E-string/comment lexical edges; several canaries do not discriminate the claimed behavior. **CR-SA90-MSQ-003 (medium/blocking)** remains open: non-NULL live predicate checks are fixed, but escaped-quote/outer-parenthesis normalization and positive preservation/populated-ID controls remain incomplete, so helper defects can self-confirm. SA92 stays unchecked until independent review resolves or explicitly waives both findings.
  - **Decisions needed:** before a clean continuation, choose and approve a maintainable bounded guardrail strategy instead of extending the current ad hoc Python/SQL scanner without a reviewed contract. At this cap the maintainer selected stop, record, and merge; no completion or waiver is implied.
...
- [ ] **SA94 — Remove the `showcase_html` generator theme; make `showcase_react` the sole theme.** `Tier 2 · Track 2 · deps: none`
  ...
  *Acceptance:* `grep -rn "showcase_html" quickscale_core quickscale_cli quickscale_devtools quickscale_modules docs .github` returns only historical release notes (or nothing); full test suite + SA66/SA90 emission-conformance gate green; a default-theme scaffold builds/runs and the plan/apply interactive flow no longer offers `showcase_html`; generated `views.py`/`urls.py` contain no social link-tree/embeds routes.
  *(why →* unused theme, ongoing maintenance cost; single supported frontend is `showcase_react`*)*

  **Blocked checkpoint (2026-07-16; plan-review cap):**
  - **Done:** Discovery, phased planning, dependency checks, and an implementation attempt produced focused functional, Ruff, MyPy, caller-parity, and baseline-coverage evidence. The user accepted two disclosed validation constraints for a future corrected implementation: Docker/Playwright E2E may remain **NOT RUN**, and the pre-existing aggregate core coverage deficit may be carried only after changed `generator.py` coverage is independently proven at least 90%. Independent review found the attempted executable checkpoint unsafe to land, so every product/source/test/template/workflow/devtools change was restored to the pre-SA94 `v87` tree; this roadmap checkpoint is the only landed delta.
  - **Pending/Blocking:** `SA94-QG-COVERAGE-001` (**high**) — isolate the generator-only coverage command from inherited pytest addopts and prove its JSON measures only `generator.py` at ≥90%; the attempted tree measured 76.53% aggregate core coverage versus the 76.59% pre-SA94 baseline. `CR-SA94-PLAN-001` (**high**) — restore implementation/validation → Review Barrier A → roadmap/CHANGELOG closeout → Review Barrier B → exact reviewed commit → `v87` sync, with revalidation and re-review after any post-sync tree change. `CR-SA94-REV-001` (**high**) — reject retired historical state before apply can persist config normalization. `CR-SA94-REV-002` (**high**) — validate state again after the locked authoritative/recovery refresh. `CR-SA94-REV-003` (**medium**) — repair the one-theme E2E wizard input sequence even while runtime E2E remains waived. `CR-SA94-REV-004` (**medium**) — clarify acceptance so only HTML function views/bindings are removed while supported React `TemplateView` social routes remain. Current implementation, documentation alignment, final validation, closeout, and completion review are pending; SA94 remains open.
  - **Decisions needed:** None. The user selected this blocked-checkpoint merge at the plan-review cap; clean continuation requires a focused plan revision and corrected implementation for the six blocking findings above.
...
- **GATE-quality** (done 2026-07-15, see [CHANGELOG.md](../../CHANGELOG.md)) and **SA93** (fold e2e into the green-gate) were reassigned here; SA93 is defined in the green-gate section above and remains open (blocked checkpoint).
...
- **Track 1 — BLOCKED checkpoint merged (SA92, unchecked), with SA84 and SA95 still open.** The squash-and-drop-backward-compat decision is ratified and the validated implementation is landed, but SA92 remains unchecked on CR-SA90-MSQ-002/003; CR-SA90-MSQ-005 remains advisory. No decision remains for merging this blocked checkpoint — the maintainer selected stop-and-merge at the review cap — but a bounded guardrail-strategy decision is required before implementation resumes. A future continuation must fix those review findings, rerun affected gates, and obtain clean independent review before checking SA92. SA84 (CRM fixtures) and SA95 (blog regression) remain separate open work; rerun both after the squash to confirm no failure is a migration-state artifact. The retired SA88a–e findings (CR-SA88-REV-006/007, CR-SA88A1-REV-002/003/004) close as obsoleted-by-schema-squash.
...
**Net:** Track 1 carries the merged SA92 blocked checkpoint (unchecked) plus open SA84/SA95 work — its merge decision is resolved, while its next implementation continuation requires an approved bounded guardrail strategy. Track 2 is blocked at the SA94 plan-review cap. Track 3's SA93 is blocked on Track 1 (SA84 + SA95) plus its own deterministic fixes; SA91 on Track 3 can proceed now.
