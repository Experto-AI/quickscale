...
1. **SA101** (quality-baseline remediation, Track 3) → **SA96-GATE** (green-gate join). SA93's dependency is met; SA96-GATE is the release-path join but **blocked** on quality-baseline regressions (SA96-GATE-BLK-002), whose decision is now made (Option A — remediate) and cut as **SA101**. SA101 implementation is complete (checkpoint `SA101-IMPL-001`: all 19 regressions cleared, `make quality` exit 0) but **validation blocked** on `SA101-VAL-001` (restricted-role CRM/forms module-suite RLS fixture failures — pending-pre-existing-vs-regression decision). The prior coverage blocker (SA96-GATE-BLK-001) is superseded by the restored canonical coverage evidence.
...
- **Checkpoint `SA101-IMPL-001` (2026-07-19; implementation complete — validation incomplete):** A planned, behavior-preserving remediation across nine serial phases cleared all 19 baseline regressions through file-local extraction and direct-identity alias refactors plus migration-local compaction (no baseline/threshold/gate changes). Recorded ledger progression: 19 → 16 → 14 → 11 → 9 → 8 → 6 → 4 → 2 → 0, with `make quality` reaching exit 0 and zero baseline regressions per the canonical `.quickscale/quality_report.json`. Phases 1–8 each passed their targeted test suites plus scoped `make lint`/`make typecheck`/`radon`/`wc` and the exact per-phase quality-verifier ledger (targeted test counts: P1 130, P2 82, P3 94, P4 126 + 27 provenance, P5 55, P6 381, P7 55, P8 87). Phase 9 compacted `quickscale_modules_crm/migrations/0001_initial.py` (605 → 413 lines) and `quickscale_modules_forms/migrations/0001_initial.py` (758 → 137 lines) with exact pre/post parity: deterministic JSON probes recorded identical SHA-256 signatures for operation deconstruction and forward+reapply state for both modules, reverse-to-zero was verified (no module tables/constraints/indexes/policies/RLS flags/data/migration rows remain), the SA92 squash guardrail (8 passed) and the targeted CRM/forms migration tests (21 and 32 passed) pass, and the v87 shrink-only policy was recorded in [`decisions.md`](./decisions.md) §5 (docs do not affect the gate; these migrations were remediated, not allowlisted). **Independent change-review and `make check` have not yet been run.**
  - **Blocker `SA101-VAL-001` (2026-07-19; blocking — pending decision):** The full restricted-role CRM/forms module suites (`make MODULE=crm|forms test -- --modules` under `quickscale_test_role` / `NOBYPASSRLS`) fail with RLS fixture errors (CRM: 281 passed, 3 failed, 3 errors; Forms: 206 passed, 12 deselected, 6 errors). The implementer attributes these to pre-existing test-harness assumptions under the restricted role, not to the compaction. **Decision required to continue cleanly:** either (a) confirm the failures are pre-existing by running the same two module suites on clean v87 and, if so, accept the parity + `make quality` exit 0 + targeted migration tests + SA92 guardrail evidence as the Phase 9 gate and proceed to `make check` + independent change-review; or (b) authorize an in-scope validation-only test/fixture repair (no assertion weakening, no baseline/gate change) and re-run the two module suites; or (c) treat the failures as a true SA101 regression and roll back the compaction (which would re-open WC9/WC10 and re-block SA96-GATE-BLK-002). Until one of (a)/(b)/(c) is chosen, Phase 9 validation cannot close, `make check` must not be represented as run, and SA101 remains unchecked.
  - **Findings:** Three plan-review findings (SA101-PR-001/002/003) were raised by independent `Adaptive-change-review` pass 1 and resolved by planner revision `SA101-plan-r2`; pass 2 returned `STATUS: ok` with no remaining findings. A full implementation change-review has not yet been obtained — the closeout gate (Phases 1–10 + `make quality`/`make check` + independent change-review) is not satisfied.
  - **Next action:** Resolve `SA101-VAL-001` via decision (a)/(b)/(c). If (a) or (b) lands and `make check` exits 0 with an independent full change-review `STATUS: ok`, mark SA101 `[x]` and hand back to the SA96-GATE four-command join. If (c), re-open WC9/WC10 on the roadmap.
...
TP2b ⧗ → TP4 (off crit.; TP1/TP2 ✓)   TP3a ✓ → TP3b (E2E concurrency;          SA93 ✓ (e2e in green-gate)
 (test-parallelization; off crit.)     rebalanced from Track 1; off crit.)    SA100 ✓ (TA58/TA59 theme preflight)
        ...
        SA101 ── clear BLK-002 quality regressions (Option A: remediate, no re-baseline)  Track 3 · feeds SA96-GATE
                       │  (impl complete SA101-IMPL-001: 19→0, make quality exit0, migration parity proven;
                       │   validation blocked SA101-VAL-001: restricted-role CRM/forms suites fail — decision (a)/(b)/(c) pending)
                       ▼
        SA96-GATE ── green-gate join (make check/quality/ci/ci-e2e)  deps: SA96-T1 ✓ + SA96-T2 ✓ + SA93 ✓ + SA101
...
**Open work only:** Track 1 `TP2b` (validation-blocked) → `TP4`, Track 2 `TP3b` (after completed `TP3a`), Track 3 `SA101` (clears BLK-002; on the critical path), and the release join
...
**Critical path.** Both pre-publish module sweeps are complete: **SA96-T1** (Track 1) and **SA96-T2** (Track 2). **SA93** is complete and its dependency is met; the remaining critical-path chain is **SA101** (clear the BLK-002 quality regressions, Track 3) → **SA96-GATE** (green-gate join) → **SA96-PUBLISH** (human-only). **SA101 implementation is complete (`SA101-IMPL-001`, 19→0 regressions, `make quality` exit 0, CRM/forms migration parity proven) but validation is blocked on `SA101-VAL-001`** — the restricted-role CRM/forms module suites fail with RLS fixture errors and a maintainer decision (pre-existing-confirmation / in-scope test-harness repair / rollback) is required before `make check`, full change-review, and the SA96-GATE join can proceed. **SA98** and **SA100** are complete independent audit-remediation tickets; neither blocks SA96-GATE. SA97 and SA99 are also complete (see [CHANGELOG.md](../../CHANGELOG.md)).
