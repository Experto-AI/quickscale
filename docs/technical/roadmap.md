...
- [x] **SA102 — Add `--devtools` to every CI lint/typecheck invocation (TA61).** `Tier 1 · Track 1 (rebalanced from Track 3, 2026-07-19) · deps: none · assistant-executable · no blockers`
  ...
  - File-disjoint from Track 1's own blocked TP2b (which edits the `Makefile` `test-cov` target only) and from the in-flight Track 3 SA101 (quality-baseline source) / Track 2 TP3b (`scripts/test_e2e.sh`); it landed without conflict.
...
**Open work only:** Track 1 `TP2b` (validation-blocked) → `TP4`, Track 2's frontend chain `SA104 → SA105/SA106` (arch Finding 10), Track 3 `SA101` (clears BLK-002; on the critical path) + `SA103` (2026-07-19 audit sweep; feeds SA96-GATE), and the release join
...
- **Track 1 — ACTIVE; SA102 is landed alongside the blocked TP2b and pending TP4.** **Rebalance (2026-07-19):** SA102 (TA61 devtools CI gate, Tier 1) moved here from Track 3 and its ten-site implementation is recorded above as complete. It is file-disjoint from TP2b and the in-flight Track 2/Track 3 work. **TP1** and **TP2** were accepted 2026-07-18; **TP2b** remains validation-blocked by **QG-TP2B-001** pending exact post-fix default and required-backups validation plus final independent review; **TP4** remains pending. The E2E sub-chain **TP3a/TP3b** was rebalanced to Track 2 and is complete, including the shared `_qs_jobs.sh` helper behavior and `Makefile` harness seams reconciled through the existing pre-merge `v87` synchronization procedure. All are off the SA96 release critical path and must not regress any gate's pass/fail set or coverage thresholds. Evidence for closed work is in [CHANGELOG.md](../../CHANGELOG.md).
