...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  ...
  - **Blocked checkpoint (2026-07-23; implementation and partial final-sync validation recorded — task remains open):**
    ...
    - **Decision needed (`SA114-DEC-001`):** Choose ownership for a policy-compliant reduction of `resolvers.py` to at most 1749 lines, then rerun `make quality` and independent review. The maintainer directed this Track 1 continuation to stop and merge this checkpoint rather than widen the current implementation. A baseline increase would require an explicit repository-policy change and is not authorized by this checkpoint.
...
- **Track 1 — SA114 blocked checkpoint, off the critical path.** Independent checkpoint review returned `STATUS: ok`, resolved `SA114-REV-001`, `SA114-REV-002`, `SA114-REV-004`, and `SA114-REV-005`, and approved this explicitly incomplete checkpoint for merge. On the final synced Track 1 baseline, `make check`, `make ci`, and `make ci-e2e` are green with empty quarantine, but `make quality` exposes `SA114-QUALITY-001`: pre-existing `resolvers.py` growth above its shrink-only line maximum. The ticket remains unchecked. `SA114-DEC-001` must assign ownership for a policy-compliant reduction before `SA114-SYNC-001`, the four-command exit-0 criterion, and task-completion review can close. Heavy `ci`/`ci-e2e` legs remain serialized against Track 3's PG/Docker usage, and installed-context resolver-surface fixes remain deferred to Track 3 follow-up ownership.
...
**Net.** Track 1 has one independent off-path ticket at a merge-approved blocked checkpoint (SA114, gate re-verification): independent review resolved `SA114-REV-001`, `SA114-REV-002`, `SA114-REV-004`, and `SA114-REV-005`, while `SA114-QUALITY-001`/`SA114-DEC-001` still require ownership of a policy-compliant `resolvers.py` reduction before `SA114-SYNC-001` and the final-sync four-gate criterion can close. Track 2's feature chain is complete, with one independent off-path infra ticket (SA115, e2e in-lane parallelization — merge after SA112, PG/Docker serialized with SA114). Track 3 is **not** done: SA113 and SA111a are closed, and the remaining critical path is **SA112 → SA96-PUBLISH (human)**, with optional SA111b off-path. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the recorded squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
