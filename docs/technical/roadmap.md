...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - [ ] **SA117b — Close the split-publication safety and resilience gaps.** `Tier 2 · Track 3 · deps: none (SA117a ✓ closed)` — **critical path; Track 3's next action**
    ...
    **Recorded partial delivery (2026-07-28; plan-review cap; documentation only; task remains open).**
...
- **OPEN DECISION `SA112A-TRACK-002` (2026-07-28) — move SA112a to Track 1, or keep Track 1 on governance?** This is the same SA112a parallelization declined last pass as `SA112A-TRACK-001`, but the situation changed twice: SA126 closed, and **SA117a took a fourth review cap with still zero source delta**. Track 1's remaining queue (SA122a, SA122b) is now **off the critical path**, while Track 3 is stalled at the critical-path head. Track 1 is a real existing worktree, so unlike last pass **no fourth track is required**.
  ...
  - **The cost — this is why it is your call, not an automatic speedup.** Track 1 is one serial worktree, so SA112a would run *instead of* SA122a, not alongside it. **SA121 is closed; SA122a is next**, so this move delays the next governance ticket (SA122a); SA121's arch Finding 12 remediation is already recorded above.
...
- **Track 1's runnable tickets stay on one worktree by choice, not by dependency.** SA126 and SA121's functional scope are complete; SA121's documentation closeout is current and SA122a follows, remaining file-disjoint from the completed executable SA121 scope, while SA122a adds a new registry + parity checker touching no SA112/SA117 allowlist. None needs PostgreSQL/Docker. They remain on one worktree because Track 1 is one worktree, Track 2 cannot host them (below), and a fourth worktree is declined (`SA112A-TRACK-001`). Only `decisions.md` is conditionally shared, and the Merge procedure covers it. SA122b is the single further ordering exception, bounded below.
...
- **Track 3 (SA117b → SA117c → SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path — this is the only real progress available today.**
  - **Can start — YES.** SA117b is Track 3's next action, `deps: none` now that SA117a is closed, with no open product decision. It is three findings on one coherent publication-path review unit, plus the two-line spy absorbed from SA117a whose literal diff is already written into the ticket.
  - **Can finish — YES.** Every SA117b blocker is inside its scoped repository surfaces: the publisher, verifier, shared Git controls, actual Compose owner, and their caller-parity tests. Nothing in its acceptance depends on Track 1 or Track 2 output, and it carries no quality share.
  - **Can merge — YES.** SA117b merges straight back to `v87` with no order gate; its only shared surface is the closeout files, covered by the Merge procedure.
...
**One open decision: `SA112A-TRACK-002`** — whether to work SA112a on Track 1 now (critical-path progress, but delays Track 1's SA122a action) or leave it behind SA117e. `SA112A-TRACK-001`'s *fourth-worktree* variant stays permanently declined. No open ticket is Tier 3.

**Open decisions: one — `SA112A-TRACK-002`** (stated in full in the Parallelism section): move SA112a onto Track 1 so the critical path's first SA112 node is worked now, at the cost of delaying Track 1's SA122a next action. It is a sequencing decision and changes no ticket's technical content or any track's three states. **Default while undecided: SA112a stays on Track 3.** Every individual ticket still records **Decisions needed: none**.
