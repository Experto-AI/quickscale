...
Pre-release re-verification: **SA96-T1 (Track 1) and SA96-T2 (Track 2) module sweeps are complete** — all 12 modules re-verified green in isolation on post-SA92 v87, no regression, empty quarantine. **SA93 and SA96-GATE are complete; SA109 ✓ and SA110 ✓ are complete, verified, and merged; the only remaining release-path step is SA96-PUBLISH (human-only).** See [CHANGELOG.md](../../CHANGELOG.md).

**SA96-GATE (green-gate join) is complete** — all four commands (`make check`/`quality`/`ci`/`ci-e2e`) exited 0 on a clean rerun with empty quarantine; detail in [CHANGELOG.md](../../CHANGELOG.md). Note the join runs entirely inside the monorepo and does **not** exercise the pip-installed wheel — that gap is addressed by **SA109 ✓ / SA110 ✓** (see Track 3), which are complete, verified, and merged; the only remaining release-path step is the human-only **SA96-PUBLISH**.
  ...
  (TP1/TP2/TP2b/           SA106 ✓ ── identity de-spec (2b;             │
   TP3a/TP3b/TP4)             deps SA104 ✓; complete)            SA109 ✓ ── fix installed-wheel
                              ...
                              SA108 ── migration-doc rewrite    SA110 ✓ ── installed-artifact smoke
                                  (deps SA105 ✓+SA106 ✓+SA107 ✓)        gate (deps SA109 ✓; QG
...
**Open work only:** Track 2's **SA108** (migration-doc rewrite) closes the frontend de-specialization chain downstream of SA104/SA105/SA106/SA107 ✓. Track 3 implementation is complete — **SA109 ✓** and **SA110 ✓** (QG proofs complete). The release **SA96-PUBLISH** (human-only, deps: SA96-GATE ✓ + SA109 ✓ + SA110 ✓) is the remaining step. SA105 is complete (review STATUS ok; see [CHANGELOG.md](../../CHANGELOG.md) for validation evidence); SA106 is complete (identity de-spec); SA107 is complete (independent review STATUS ok). Track 1's TP suite is **complete** (TP1/TP2/TP2b/TP3a/TP3b/TP4). The frontend chain owns the `quickscale_core` generator + `showcase_react` template tree, with SA108 additionally owning `docs/planning/beta-site-migration.md` — disjoint from the TP work and the SA103 CI surfaces — so Track 2 runs concurrently with release; only the closeout files (`CHANGELOG.md`, `roadmap.md`) are shared, covered by the Merge procedure. Tracks 1 and 2 are off the release critical path and must not regress any gate's pass/fail set or coverage thresholds.

**Critical path.** SA96-GATE is complete — all four commands exit 0 with empty quarantine (see [CHANGELOG.md](../../CHANGELOG.md)). SA109 ✓ and SA110 ✓ are complete, verified, and merged; the only remaining critical-path item is **SA96-PUBLISH** (human-only). SA109/SA110 surfaced because the green-gate runs only inside the monorepo and never exercised the pip-installed wheel, which previously crashed on import. SA109 resolved that: discovery now falls back to bundled manifests in installed-wheel contexts. SA110 closes the coverage gap: `make smoke-install` builds wheels from per-run staged copies (no source tree mutation), installs into a throwaway venv outside the source tree, and runs every non-mutating CLI command from a sanitized external workdir.
...
**Net — Track 3 complete (SA109 ✓, SA110 ✓ QG proofs complete). Track 2 (SA108) remains as the only open assistant-executable item, file-disjoint from the release path.** Track 1 is **complete** (all TP tickets closed). Track 2 = `SA105` ✓ (complete) + `SA106` ✓ (complete) + `SA107` ✓ (complete) + `SA108` (arch Finding 10 chain, off critical path). Track 3 = `SA96-GATE` ✓ (complete) → `SA109` ✓ (fix installed-wheel discovery, complete) → `SA110` ✓ (installed-artifact smoke gate, QG proofs complete). The remaining release step is **SA96-PUBLISH** (human-only, deps: SA96-GATE ✓ + SA109 ✓ + SA110 ✓). SA109/SA110 own `quickscale_core` discovery/schema + the CI publish surface, disjoint from Track 2's `showcase_react`/generator tree, so Track 2 continues in parallel with no merge hazard beyond the shared closeout files. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision-sa92) for the recorded squash/guardrail/shrink-only-quality policies; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
