...
SA122b-3 → -4 → -5                     SA115 (e2e xdist; deps: none) SA136a ✓   SA136b ▶   SA136e ▶
  ...
  │                                   │  cannot merge  → SA112e      │   SA136c    SA136d
  │                                   │                              │      └───┬────┘
  │                                   │                              │          ▼
  │                                   │                              │       SA136f (decisions.md)
  │                                   │                              │          ▼
...
**Critical path:** `SA136a → SA136c/SA136d → SA136f → SA117e-4 → SA112b → SA112c → SA112d → SA112e → SA112f → SA96-PUBLISH`. SA136b and SA136e are on the umbrella but not on the longest chain — they are the parallelizable legs. **SA117e-5 is closeout and sits *off* the critical path** — SA112b's precondition is the *sealed* splits delivered by SA117e-4, so it does not wait for the umbrella to close. SA112a is closed and merged, so SA112b's provisioning precondition is already satisfied. Track 2 is entirely off the path, and the whole remaining Track 1 queue is off it.
...
|---|---|---|---|---|---|
| **Track 3** — SA136b (recorded partial) | **yes** — the remaining direct-publish membership correction is fully specified and has no upstream dependency or open decision | **yes** — `F-002` is confined to SA136b's publish selector and direct tests | **yes** — no merge-order gate | ✅ **yes** | ✅ **yes** |
...
**Track 3 remains truly green and on the critical path.** The 2026-08-06 loop/seal ratifications dissolved the SA117e-4 plan cap: `F-001` and `F-006` no longer describe a real hazard (an interrupted republish is rerun, not recovered), and `F-004` is re-derived from a much smaller allowlist. SA136b's first implementation attempt is preserved at functional commit `999c025b`: the authoritative inventory and three enumeration consumers are green, review `F-001` is resolved, and `F-002` remains medium/blocking because direct single-module publication still accepts an existing directory without authoritative-inventory membership. Track 3's head therefore remains SA136b; its correction has no upstream dependency or open decision. **Track 1 remains truly green at SA122b-3** but is off the critical path, so it is filler relative to the release. Track 2's two "no"s are **hard upstream dependencies** that only SA112d/SA112e can clear.
...
- **`scripts/check_sa117_scope.py` and `scripts/version_tool.sh`** — written by SA136b only. `check_sa117_scope.py` is otherwise owned by v88's SA124, which is not open in v87, so there is no concurrent writer.

`quickscale_cli/.../module_commands.py` and `module_output.py` pass from unowned to SA136c-owned; `quickscale_core/.../utils/git_utils.py` returns to unowned now that SA136a is merged; `scripts/publish_module.py` to SA136b (inventory) and SA136d (seal phase) — those two are siblings on one track and are serialized by the one-child-at-a-time rule. `quickscale_core/tests/test_e2e_full_workflow.py` and `scripts/quality_baseline.json` remain owned by no open v87 ticket. No other executable surface is shared, so no additional procedure is required.

---

## Track 3 — Core/CLI plumbing, release path

**Status:** on the critical path and **unblocked to resume**. The head is **SA136b** at a recorded-partial checkpoint: its inventory derivation is implemented and green, but direct single-module publication still needs the `F-002` authoritative-membership guard before the child can close. SA112b's provisioning precondition is satisfied by SA112a's merge; its split-publication precondition now waits on SA117e-4, which in turn waits on SA136.
...
**Handoff entry point.** Resume **SA136b** at `F-002`: require the direct single-module publish selector to reject names absent from `authoritative_module_names()` before release/origin prompts or mutation, then add direct placeholder/count-drift regressions and obtain fresh full-scope approval. No upstream dependency or open decision blocks that correction. The loop/seal contract SA136 implements is recorded in [CHANGELOG.md](../../CHANGELOG.md) (ratified 2026-08-06) and becomes normative in `decisions.md` at SA136f.
...
- [ ] **SA136 — Seal module splits behind immutable version tags.** `Umbrella · deps: none` — critical path; **blocks SA117e-4**
  ...
  **SA136a is closed and merged** — `git_utils.py` now carries `resolve_split_tag`, peeled `resolve_remote_tag`, `check_remote_tag_exists`, `get_tree_sha`, `get_local_tag_commit`, force-free `create_annotated_tag`, and refspec-explicit `push_tag`; every later child consumes them (evidence in [CHANGELOG.md](../../CHANGELOG.md)). Residual `F-005` is a low advisory only.

  - [ ] **SA136b — Derive the authoritative twelve-module inventory.** `Tier 2 · deps: none`
    Closes plan blocker `F-005` with code instead of a frozen digest. Add `authoritative_module_names()` and `AUTHORITATIVE_MODULE_COUNT = 12` to `contracts/module_discovery.py`, built on `discover_shipped_module_names` (`:157-188`), rejecting any intersection with `PLACEHOLDER_MODULE_NAMES` (`:46`) and raising `ImproperlyConfigured` on count drift. Fix `scripts/publish_module.py:_list_modules` (`:102-113`), which globs directories with no `module.yml` filter and therefore yields `teams` — the concrete reason a 13th split branch exists. Retire the hardcoded copies at `scripts/version_tool.sh:16-20` and `scripts/check_sa117_scope.py:37-52` via a `--list-modules` shim.
    - Allowlist: `quickscale_core/src/quickscale_core/contracts/module_discovery.py`, `scripts/publish_module.py`, `scripts/version_tool.sh`, `scripts/check_sa117_scope.py`, and their tests.
    - Verify: `_list_modules()` returns exactly twelve names and excludes `teams`; a placeholder directory does not change the set; a thirteenth real module makes all three consumers pick it up with no edit; `make version-check` and `make check` stay green.
    **Recorded partial (2026-08-06; functional commit `999c025b`; task remains unchecked).** **Done:** `authoritative_module_names()` and `AUTHORITATIVE_MODULE_COUNT = 12` now fail hard on placeholder/count drift; publish listing, version propagation, and SA117 scope inventory derive from that contract; review `F-001` is resolved; 147 focused tests, `make version-check`, and full `make check` passed (Core 2,689 passed / 1 skipped; CLI 2,043 passed). **Pending-Blocking:** `F-002` (**medium**, correctness; `scripts/publish_module.py` direct single-module selector and direct tests) — require authoritative-inventory membership before release/origin prompts or any split/push path, proving `teams` and an unapproved thirteenth directory fail before mutation. Two full review passes ended **medium / 1 → medium / 1**, so the maintainer selected recorded partial delivery and merge-back at the cap. **Decisions needed:** none — resume with the narrow `F-002` correction and fresh validation/full-scope review; this checkpoint is not completion, review approval, or a finding waiver.
    *(why →* the module inventory is restated in ~15 places and they disagree; the publish path's copy is the one that can mis-publish*)*

  - [ ] **SA136c — Retarget `embed` onto the version tag.** `Tier 2 · deps: SA136a` — **production behaviour + public CLI surface; scoped plan review required**
  ...
  - [ ] **SA136d — Add the seal phase and its Make targets.** `Tier 2 · deps: SA136a, SA136b`
    ...
    **SA117e-1, SA117e-2, and SA117e-3 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-4` is the head but is now gated on the SA136 umbrella. **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) remains binding on SA117e-4's loop phase. **`SA117E1-REV-002`** is **discharged by design** under the loop/seal contract — see SA117e-4. **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-4 — Loop, seal, and human-confirmed publication.** `Tier 2 · deps: SA136` · **HUMAN-GATED — outward-facing** — satisfies SA112b's precondition
      **Restructured 2026-08-06 around the ratified loop/seal contract** ([SA136](#sa136--tag-sealed-split-publication)). This child no longer models the push as one irreversible unit; it executes the ordering SA136f makes normative, using the machinery SA136a–e ship.
      ...
      5. **Clean up.** Delete the stale remote `splits/teams-module` branch, which SA136b's inventory fix makes unreachable through the tooling.
...
**Status:** off the critical path, **head is SA122b-3**, and the parity checker is authoritative now that SA128 is closed and merged (evidence in [CHANGELOG.md](../../CHANGELOG.md)). The queue is **SA122b-3 → … → SA122b-5**; SA122b-2 is closed and SA122b-3 is the next/startable child. Only SA122b-5 is merge-gated behind SA112e.
...
- [ ] **SA122b — Migrate the consumers onto the registry.** `Umbrella · deps: SA128 ✓ closed`
  ...
  **SA122b-1 and SA122b-2 are closed and merged** (Make aggregation and both `check_ci_locally.sh` inventories now derive from the registry; evidence in [CHANGELOG.md](../../CHANGELOG.md)).

  - [ ] **SA122b-3 — Derive hosted `ci.yml` jobs and `needs`.** `Tier 2 · deps: SA122b-2 ✓ closed`
    ...
    - **Recorded partial delivery (2026-08-06; task remains unchecked).** **Done:** Track 1 started clean at `b8e112ee`, synced to `v87`, and implemented the maintainer-selected hybrid static generator without changing `scripts/gate_registry.json`, its schema, or the authoritative parity checker. The checked-in workflow still has the same 11 jobs, check names, steps, ordered `needs`, and parallel roots. Two quality-gate runs passed; the post-fix run covered **204 focused tests**, direct and Make generation checks, Ruff lint/format, `make check QUIET=1`, `make quality`, and `git diff --check`. The parity checker still reports exactly the five expected `publish` omissions and zero hosted diagnostics. Final full-scope checkpoint review independently closed `F-001`–`F-004`. **Pending-Blocking:** `F-005` (**medium**, correctness) — pre-generation validation requires current workflow job IDs to equal registry IDs, so a supported `bindings.ci_job` rename cannot be rendered into the generated job block and downstream `needs`; continuation must separate pre-edit marker ownership from post-generation identity and add rename/check/write/unowned-byte coverage. `F-006` (**high**, completeness) — `check-ci-gate-generation` is opt-in rather than part of a mandatory repository or hosted gate, so registry/workflow drift can merge; continuation must wire a non-recursive generation check into a mandatory path and prove stale content fails while clean content passes. **Decisions needed:** none — the maintainer selected `Stop here, record Done / Pending-Blocking / Decisions-needed, and merge the checkpoint`; the next continuation fixes `F-005`/`F-006`, reruns validation, and obtains full-scope review. This checkpoint is not executable approval, completion, a finding waiver, or permission to start SA122b-4.
...
**No worktree-assignment question is open; the latest rebalancing review (2026-08-06, second pass of the day) applied no move — the tenth consecutive pass to reach that conclusion.** SA136a's closure freed `git_utils.py` but created no new parallel leg: SA136c and SA136d both depend on it and are Track 3 siblings serialized by the one-child-at-a-time rule. Every open v87 ticket carries a track, Track 2 is closed to new work by standing rule (homing anything there drags SA115 onto `v87`), and SA112a — the one node previously worth parallelizing onto Track 1 — has closed and merged. The remaining off-path work (SA122b-3…5) is a single serial chain; moving any of it onto Track 3 would push filler ahead of the critical path.

**Standing placements.**

- **Track 1's queue is SA122b-3 → … → SA122b-5**, head at SA122b-3 — a single serial chain whose first child is startable after SA122b-2's reviewed completion.
- **SA136 is homed on Track 3 as a new sibling umbrella, not folded into SA117e-4.** The machinery is production CLI/publish code, a different risk class from the push ceremony; folding it in would make `-4` Tier 3 — the exact sizing violation that forced the SA117e split on 2026-08-01. SA136a, SA136b, and SA136e are mutually independent; SA136c and SA136d both depend on SA136a and are serialized behind it by the one-child-at-a-time rule.
...
**Open maintainer decisions — exactly one, and it is a rebalancing trade-off, not a design question.** No repository-design question is open: the loop/seal contract, the `splits/<m>-module/<version>` tag scheme, the deletion of `split-modules.yml` and the stale remote `splits/teams-module` branch, and the restructure of SA117e-4 in place were all ratified on 2026-08-06 and are recorded in [CHANGELOG.md](../../CHANGELOG.md). The one open item:

> **Should SA136e move to Track 1?** It is the only SA136 child that is Tier 1, has `deps: none`, and shares no writer with a sibling — so it is the sole candidate for genuine cross-track parallelism. Moving it would retire the `split-modules.yml` hazard concurrently with Track 3's machinery. **Cost:** Track 1's serial-handoff rule means SA136e would run *before* SA122b-3, delaying an otherwise-mergeable chain that is currently truly green. **Not applied** — a rebalancing move that delays a mergeable track's closeout is the maintainer's call. Declining it costs nothing on the critical path, since SA136e is not on it.
