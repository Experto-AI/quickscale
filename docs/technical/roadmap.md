...
| **Track 3 — SA112f** | **yes** — `SA112d`'s lifecycle proof and `SA143`'s trigger contract are complete | **no** — blocker `SA112F-QG-001`; the required default-xdist acceptance is not green | **no** — closure cannot merge before accepted evidence exists | **no** | Critical-path head; blocked in acceptance |
...
**Truly-green open tickets:** none. `SA112f` remains the head of the longest dependency chain, but `SA112F-QG-001` blocks its finish and merge states. No off-path filler ticket is assigned.
...
| **SA112f (T3)** | **yes** — prerequisites `SA112d` and `SA143` are complete | **no** — blocker `SA112F-QG-001`; repeated default-xdist acceptance runs failed | **no** — accepted evidence is required before closeout | Critical path; acceptance-blocked |
...
**No maintainer decision is needed to continue `SA112f`; its next action is the objective rerun path under `SA112F-QG-001`.** The next release decision is whether to publish or hold v0.87.0 after the exact-tip green gate. The retained local core tag predates the remaining release work, so `SA96-PUBLISH` must rebind it locally if needed and prove it resolves to the exact green-gated integrated tip before any push. That push triggers the hosted publish gates, PyPI upload, and GitHub release, making the version permanent. No decision can bypass `SA112f → SA140`.
...
1. Resolve `SA112F-QG-001` by restoring stable npm-registry access and exclusive service headroom, then rerun the serial/default-xdist comparison with a watchdog longer than 45 minutes and require `QUARANTINE_TICKETS= make ci-e2e` to exit 0. If the default two-worker failures reproduce without registry or memory pressure, route a focused E2E resource-isolation ticket rather than widening `SA112f`. If a registered trigger path is missing, stop acceptance and route a focused registry-repair ticket that regenerates the workflow; never repair the generated `.github/workflows/e2e.yml` directly.
...
- [ ] **SA112f — Run ordered acceptance, review, and close SA112.** `Tier 2 · Track 3 · deps: SA112d + SA143 complete`
  - **Completed work:** confirmed that the five generated trigger paths are present and unique; focused parity/generation, runner, cleanup, and lifecycle-support checks passed; all 20 smoke-install probes passed in 11.88s. The first default run's three named CLI failures also passed focused serial reruns (installed lifecycle 1/1 in 373.99s; development workflow 2/2 in 1,006.60s). Preserve this evidence unless the reviewed tip changes or the acceptance criteria are revised.
  - **Pending work:** obtain a green default two-worker `QUARANTINE_TICKETS= make ci-e2e` result, complete the ordered serial/default-xdist comparison, remeasure the provisional speedup once, review executable changes first, and then review the closeout documents. Neither `SA112f` nor `SA112` may be checked before that evidence is accepted.
  - **`SA112F-QG-001` (blocking, acceptance):** two default two-worker runs reached green Core E2E but failed CLI E2E in 1,889.16s and 1,698.62s amid severe npm-registry stalls and Docker service pressure, including one frontend exit 137. The later Docker-apply failure was not rerun because the second default run had already reproduced the blocking pressure. Two serial umbrella attempts exceeded 1,900s and 2,700s worker watchdogs before returning a verdict. No executable change is justified from this evidence, and the provisional xdist speedup remains unconfirmed.
  - **Decision needed to resume:** none — no product or maintainer decision is required. Resume when stable npm-registry access and exclusive service headroom are available. If the default failures reproduce without registry or memory pressure, stop and route a focused E2E resource-isolation ticket rather than widening `SA112f`.
  - **Pending continuation plan:** (1) restore stable registry access and exclusive service capacity; (2) resume at the blocked umbrella stage and run the serial/default-xdist comparison with a watchdog longer than 45 minutes, requiring the default `QUARANTINE_TICKETS= make ci-e2e` run to exit 0; (3) record the timing comparison and confirm or reject the provisional xdist speedup; (4) complete executable review, then closeout-document review, before checking `SA112f` and `SA112`. If a registered trigger path is missing at the reviewed tip, stop acceptance and route a focused registry-repair ticket that regenerates the workflow; never hand-edit `.github/workflows/e2e.yml`.
