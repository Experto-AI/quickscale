...
- [x] **TP1 — Fan out the independent static gates in `check_ci_locally.sh`.** `Tier 2 · Track 1 · deps: none`
  ...
  **Completed 2026-07-18.** Stages 2–9 now fan out with deterministic replay and aggregate failure attribution; `QS_CI_PARALLEL=0` retains the serial debug path, and stages 10–12 remain serial. The maintained TP1 suite covers individual gate failures plus TERM/INT/HUP worker cleanup. Same-host `make ci` evidence was 539.70s parallel versus 550.96s serial (2.04% faster); final `make ci` passed with all coverage thresholds intact. **Findings/blockers:** review findings CR-TP1-REV-001..003 (stale-PID signal safety, maintained test wiring, and timing evidence) are resolved; the temporary PostgreSQL validation outage was resolved by repository-supported provisioning, with no blocker remaining. Detailed evidence is in [CHANGELOG.md](../../CHANGELOG.md).

- [x] **TP2 — Add `pytest-xdist` and run the unit suites with `-n auto`.** `Tier 2 · Track 1 · deps: none`
  Unit suites (`make test-unit`, core + CLI) are the TP2 target. Per-test DB isolation already exists (`quickscale_core/tests/conftest.py` → `unique_db_name` / `per_test_db`, `qs_test_<uuid>` databases), so worker-level parallelism is safe for DB-backed unit tests; pure-unit tests are trivially safe.
  ...
  **Implementation acceptance evidence (2026-07-18; independent change-review pass 1 STATUS ok, no findings).** Root dev dependencies now include `pytest-xdist==3.8.0` through the refreshed Poetry lock, and `poetry install --with dev` installed `execnet` and `pytest-xdist`. `make test-unit` defaults to `-n auto --dist loadfile`; `PYTEST_XDIST_WORKERS=0 make test-unit` provides a true serial parity run, while `PYTEST_XDIST_WORKERS=N` pins CI/local worker count. The maintained unit target now ignores its default module section (modules remain an explicit error) so its documented core+CLI scope is honored. Core and CLI collection sets matched exactly (2,354 and 1,993 selected node IDs respectively) in serial and xdist runs. CLI serial/xdist results were both 1,993 passed with coverage/XML at 90.36%; wall clock was 56.36s serial versus 33.44s xdist. Core serial/xdist results were both 2,353 passed + 1 skipped with coverage/XML at 80.39%; wall clock was 176.03s serial versus 68.40s xdist. `--dist loadfile` is required to keep existing CLI tests that share `/tmp/proj` from contending on their advisory lock; no tests or coverage floors were weakened.

  **Acceptance determination and finding TP2-BLOCK-001 (waived/non-blocking):** the existing core package pytest configuration still enforces a 90% coverage floor, while this unchanged core unit set measures 80.39%. Both serial and xdist runs therefore exit identically at that pre-existing floor, with identical collected/result sets and unchanged coverage. TP2's exact acceptance requires serial/xdist parity, unchanged coverage, and CLI `--cov-fail-under=90`; it does not require the pre-existing core floor to become green. The core baseline is consequently a transparent non-blocking finding outside TP2, and no threshold or test contract was changed.
...
Completed history (SA92/SA84/SA86/SA93/SA94/SA95/SA96-T1/T2/SA97/SA98/SA99/SA100/SA91/TP1/TP2,
...
**Open work only:** Track 1 `TP2b` (+ `TP4`), Track 2 `TP3a → TP3b`, and the release join
