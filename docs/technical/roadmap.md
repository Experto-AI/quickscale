...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA128b → c → d (parity check)        SA115 (e2e xdist; deps: none) SA117e-3 → -4 → -5
...
**Status:** on the critical path, **unblocked and ready to resume** — head is **SA117e-3, Phase 2**, assigned to an implementation agent and not started. Eight blocking pre-use findings (`F-003`–`F-010`) are open; no harness, interruption proof, negative control, `make ci-e2e`, tag, push, or publication has executed, and no host service has been mutated. SA112b's provisioning precondition is satisfied by SA112a's merge. `SA117E1-REV-001` and `SA117E1-REV-002` remain owned by SA117e-4. Do not execute SA117e-3 runtime artifacts before a fresh full-scope pre-use review returns `STATUS: ok`, do not publish splits before SA117e-4's human gate and both SA117e-4 blockers close, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
    ...
    **SA117e-1 and SA117e-2 are closed** (evidence in [CHANGELOG.md](../../CHANGELOG.md)); `-3` is the head. Two `SA117E1-REV-*` findings remain open and are carried by their owners: **`SA117E1-REV-001`** (high, security boundary — `quickscale_core/.../utils/git_utils.py:636-680`, an `ABSENT` expectation emits bare `--force-with-lease` instead of proving explicit remote absence) and **`SA117E1-REV-002`** (medium, completeness — `scripts/verify_sa117_publication.py:199-439,498-530`, `Makefile:919-949`, `scripts/README.md:115-124`, publication authorization is not bound to the exact verified evidence nor consumed by the publisher), both owned by **SA117e-4**; **`SA117E1-REV-004`** (low, advisory) is owned by **SA124**. All other `SA117E1-REV-*`/`CR-SA117E-*` findings are resolved.
    ...
    - [ ] **SA117e-3 — Rematerialize the harness and clear `make ci-e2e` in exclusive capacity.** `Tier 2 · deps: SA117e-2 ✓ closed` — needs exclusive Docker/PG
      ...
      **Current checkpoint (`SA117E3-HANDOFF-002`, 2026-08-04; task unchecked; no executable approval or host mutation).**
      - **Standing state:** the nine-phase plan below holds plan-review `STATUS: ok` (DC-25, closing `F-002`). Phase 1 froze the source/v87 tip and the original running/ready `quickscale-ci-postgres` identity/configuration at `fbde0a82…` **without mutating it**, and must be revalidated — the worktree has since resynchronized to `v87` `f5de6836…`. The rematerialized harness/wrapper candidates were rejected by the first full-file pre-use review (DC-30, `STATUS: partial`); no reviewed executable hash exists.
      ...
      - **`SA117E3-RESET-001` granted 2026-08-04** — scope and retry counters reset; Phase 2 is assigned to an **implementation agent**. The reset restores execution capacity **only**: it waives no finding, approves no hash, and authorizes no host-service mutation. Resume at **Phase 2** with `F-003`–`F-010` carried verbatim; do **not** re-plan — scope and repository topology are unchanged.
...
- [ ] **SA128 — Rebuild the parity checker on structural parsing.** `Umbrella · deps: none`
  ...
  **Invariants every child carries, unchanged:**
  ...
  Umbrella acceptance is that all four children (SA128a ✓ closed, plus b–d) are independently reviewed and merged in order and SA128d's full-scope review returns `STATUS: ok`. SA122a's findings close with SA128d; its two MyPy errors close in SA128c and must be **gone, not baselined**. The low-severity `.PHONY` wording finding is carried, non-blocking.
  ...
  **SA128a is closed and merged** (Make-context extractor rebuilt on Make's own `-qp`/`--dry-run` semantics; evidence in [CHANGELOG.md](../../CHANGELOG.md)). Its only carry-forward is advisory `F-003`, owned by SA128d below.

  - [ ] **SA128b — Observe shell inventories by executing them.** `Tier 2 · deps: SA128a ✓ closed`
  ...
  - [ ] **SA128d — Cross-cutting contracts, full proof matrix, and umbrella closeout.** `Tier 2 · deps: SA128c` — **this is where the checker becomes authoritative**
    ...
    **Carries F-003 (medium/advisory) from SA128a:** `_communicate_bounded` resilience in `scripts/check_gate_parity.py` (bounded output/event handling, readers, residual processes, and process-group cleanup) — explicitly deferred to this child; must be resolved before the umbrella closes.
  ...
  **Attempts and why they failed.** Both monolithic attempts were reverted before any commit, with nothing dangling in reflog or stash, so their designs are lost. Every SA128 child attempt appends a row here **before** its checkpoint, so a design rejected in one domain is visible to the others.
  ...
  (Attempt 3 — the SA128a child — succeeded at full scope; it is recorded in [CHANGELOG.md](../../CHANGELOG.md), not here.)
...
- **Track 1's queue is SA128b → SA128c → SA128d → SA122b-1 → … → SA122b-5**, head at SA128b — a single serial chain, each link dependency-blocked on its predecessor.
