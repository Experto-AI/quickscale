...
**Status:** on the critical path and **executable today** — `SA117E-VAL-001` is resolved (baseline `43d9b8fc` approved 2026-08-01), so the continuation resumes at ordered command 4. **Head is SA117e-1**, the first child of the SA117e umbrella after its 2026-08-01 Tier 3 split. **SA112a now runs on Track 1** (`SA112A-TRACK-003`, accepted 2026-07-31), so Track 3's next SA112 child is SA112b. Do not publish splits before SA117e-4's human gate, do not start SA112b until SA117e-4 has merged, and do not treat anything as release-ready until SA117e closes at `-5`.
  ...
  - [ ] **SA117e — Push refreshed splits, full-scope review, and close SA117.** `Umbrella · deps: none` — critical path; contains a human-only step
    ...
    *(why →* SA117 is only actually resolved once the *published* splits match the core; everything before SA117e-4 is local*)*
    ...
    - [ ] **SA117e-4 — Tag, human-confirmed split push, and post-push parity.** `Tier 2 · deps: SA117e-3` · **HUMAN-GATED — outward-facing, irreversible** — satisfies SA112b's precondition
      Execute the mandatory release ordering as **one unit**, because a partially pushed split set is the hazard this ticket exists to prevent: tag HEAD to match `VERSION` **locally**, then **stop and obtain fresh human maintainer confirmation** — this is an execution-time gate obtained at the push, never pre-granted and never inherited from an earlier approval — then push the twelve protected `splits/*` branches. PyPI publish is **not** in scope and stays with SA96-PUBLISH. Only after the push, verify parity across public, source, and bundled manifests, and run the installed all-module `apply`.
...
Both tracks are now startable in parallel. The one remaining human gate on the board — SA117e-4's public split-push confirmation, and later SA96-PUBLISH — is an **execution-time** gate obtained at the moment of the push, not a standing decision.
