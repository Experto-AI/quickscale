...
- [x] **SA13.4 — Flip the lint gate to hard-fail (AF9 None-path hardening deferred).** `Tier 2 · Track 1 · deps: SA13.2, SA13.3 · RISK LEVEL: medium`
  Flipped `check_org_context_primitives.py` from warn-only (always exit 0) to hard-fail (exit 1 when violations exist, exit 0 on clean). Updated docstring, Makefile comment, and violation output to reflect the new hard-fail posture. All SA13.2/SA13.3 migrations are complete — zero remaining violations.
  *Files:* `scripts/check_org_context_primitives.py`, `Makefile`, `docs/technical/roadmap.md`, `CHANGELOG.md`, `quickscale_modules/orgs/src/quickscale_modules_orgs/current_org.py`.
  *Acceptance:* lint gate fails CI on any new direct use of the privatized primitives (verified by `make check-org-context-primitives` exiting 0 on clean tree and returning 1 when violations are introduced).
  *AF9 `None`-path deferral (SA13.4-AF9-DEFERRAL):* The AF9 priming wrapper's `None`-context behavior (pass-through instead of prime-to-empty on autocommit) is **explicitly deferred**. Rationale: (1) The residual desync window is narrow — the autocommit path already falls through cleanly when `org_id is None` at line 480 of `current_org.py`, and the explicit-transaction path handles `None` correctly via `org_scope(None)`. (2) Prime-to-empty on autocommit would wrap every `None`-context autocommit statement in a `transaction.atomic()` block, which has unmeasured semantic/perf impact on non-org-scoped queries (hot-path overhead, potential atomic-block nesting issues in middleware or DRF renderers). (3) The remaining desync scenario (autocommit query runs with stale GUC from a prior transaction on a reused connection) is mitigated by the per-transaction memo identity check (CR-SA42-001), which forces re-priming across transaction boundaries. Deferral revisit is warranted if evidence shows a concrete desync failure in production or CI under the restricted role.
...
- [x] **SA18.7 — Narrow `railway_utils.py`'s broad exception swallowing (complete).** `Tier 1 · Track 3 · deps: none · (why → TA10)`
  ...
  **Final follow-up (2026-07-04):** `_link_database_step()` now wraps `link_database_to_service()`'s parse-drift `ValueError` in the same descriptive non-zero CLI failure used by `_generate_domain_step()` and `_verify_deployment_step()`, so the deploy `DATABASE_URL` link seam no longer leaks uncaught Railway output-format drift. Added direct deploy-command regression coverage for that caller path. No further SA18.7 blocker remains.
  **Decision status:** closed — no product or design decision was needed for the final follow-up.
