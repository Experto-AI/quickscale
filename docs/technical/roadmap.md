...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA141 ◐ (head; Core E2E             SA115a ◐ (acceptance-blocked   SA136d ◐ (partial seal tooling;
  │  poetry-install repair;          │   by SA141; only ticket      │  review-blocked)
...
**Truly green today: SA141 only**, and it is **off the critical path but not filler** — the green gate is unreachable without it. Every other track is blocked: **SA136d** is on the critical path and **review-blocked** by six implementation findings (hard dependency — only the upstream repair work clears it; no maintainer decision is pending); **SA122b-5**'s merge bound is a **hard upstream dependency** only SA112e can clear; and **SA115a** cannot finish until SA141 lands (hard dependency, not a decision). The release-wide green gate remains unclaimed and no full E2E greenness is claimed anywhere.

**Open maintainer decisions: none.** Every blocker on this board is a hard upstream dependency that only the named work can clear — none is waiting on a maintainer choice, authorization, or plan gate. The last two decisions landed on 2026-08-08 and are applied throughout: `SA136D-QG-001` is dispositioned as **reviewed acceptance** under the inherited "no worse than found" rule, with the repair ticketed as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); and SA139's granted scope expansion is implemented, reviewed, and closed. Neither is a release authorization. The two remaining human gates (SA117e-4's twelve-row seal confirmation and SA96-PUBLISH) are **execution-time** gates obtained at the outward-facing action, not pending decisions. All prior topology decisions are settled and recorded in [CHANGELOG.md](../../CHANGELOG.md).
...
**Open track-topology decisions: none**, and **no open maintainer decision of any kind** — every "no" in the readiness table above is a hard dependency, not a pending choice. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
