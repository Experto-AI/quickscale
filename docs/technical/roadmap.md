...
- [ ] **SA117 — Tie embedded module manifests to the core release.** `Umbrella · Track 3 · deps: none · blocks SA96-PUBLISH + SA112b`
  ...
  **Standing quality rule for every Track 3 child (`SA117-QG-REMED` retired 2026-07-28, `SA126-DEC-001`; history in [CHANGELOG.md](../../CHANGELOG.md)).** Leave `make quality` no worse than you found it. **Raising a complexity ceiling is never an acceptable remedy** — reduce the branching, or record a structured waiver through the shipped waiver ledger. No complexity or line-count remediation is owed by any SA117 child: both former ceilings closed under SA126, and the line-count gate no longer exists (`SA125-DEC-001`; [decisions.md §file-size-metric-policy](./decisions.md#file-size-metric-policy)). **Do not** reintroduce a CC criterion into a child's verify list, and **do not** spend a line-reduction refactor on `module_commands.py`, `manifest/loader.py`, or `git_utils.py`. Conflict surface: `module_commands.py` is single-owner (SA126, closed); `scripts/quality_baseline.json` is owned by no open phase.
  ...
  - [ ] **SA117b — Close the split-publication safety and resilience gaps.** `Tier 2 · Track 3 · deps: none (SA117a ✓ closed)` — **implementation reviewed; recorded partial delivery 2026-07-29**
    ...
    **Recorded partial delivery (2026-07-29; functional commit `efd86d21`; final executable Review A `STATUS: ok`; task remains open).** All four executable findings — `SA117-CR-003`, `SA117-CONT-PR-003`, `SA117-CONT-PR-006`, `SA117A-CR-002` — are resolved and validated (656 tests; Ruff/format/typecheck/quality/diff-check all exit 0; live Docker owned-resource cleanup proven across SIGINT/SIGTERM/timeout). Full evidence, including the exact reproducible validation command preserved for `SA117-META-001`, is in [CHANGELOG.md](../../CHANGELOG.md).
    - **Pending / blocking:** `SA117B-REVB-003` (**medium**, consistency) — the dependency diagram duplicated SA117e. **The diagram was corrected in the 2026-07-29 cleaning pass**; what remains is the fresh full-scope documentation review that must return `STATUS: ok` before SA117b may close.
...
────────────────────────────────     ────────────────────────────   ─────────────────────────────────
SA122a (gate registry) ◄─ in process  SA115 (e2e xdist; deps: none)  SA117b closeout ◄─ next
  │  deps: none · off critical path     │  validation AUTHORIZED       │  doc review of SA117B-REVB-003
  ▼                                     │  cannot finish → SA112f      ▼
SA127 (zero-module apply fix)           │  cannot merge  → SA112e     SA117c (lock-drift completeness)
  │  deps: none · RELEASE BLOCKER       │                              │  deps: SA117b
  │  independent of SA117               │                              ▼
  ▼                                     │                             SA117e (review + push splits)
SA122b (migrate consumers)              │                              │  human-confirmed public push
...
**Diagram checkpoint (2026-07-29).** `SA117B-REVB-003` is **substantively corrected**: the diagram above states one SA117b closeout → SA117c → SA117e → SA112a chain with no duplicated node, and the closed SA121 node is removed. SA117b's checkbox stays open until the fresh full-scope documentation review returns `STATUS: ok`.
...
- **Track 3 (SA117b closeout → SA117c → SA117e → SA112a–f → SA96-PUBLISH) — start: YES · finish: YES · merge: YES. TRULY GREEN, and on the critical path.**
  - **Can start — YES.** SA117b's next action is the full-scope documentation review of the corrected dependency diagram; no product decision is open.
  - **Can finish — YES.** `SA117B-REVB-003`'s substantive correction landed in the 2026-07-29 cleaning pass; only the review remains, and it needs no other track's output.
...
- **Track 3 detail — SA117b EXECUTABLE COMPLETE; DOCUMENTATION REVIEW OPEN.** `efd86d21` resolved all four executable findings and final Review A returned `STATUS: ok`; Review B remains partial only on `SA117B-REVB-003`, whose substance is now corrected. **Next action: full-scope documentation review of the corrected diagram; then SA117c.** Do not publish splits, start SA112, or treat the candidate as release-ready.
...
**Current state after the SA117b review cap.** SA117b's executable implementation and four task findings are complete and Review A is green; `SA117B-REVB-003`'s diagram defect is corrected and only its confirming review remains. Track 3 must close that review before SA117c; no product decision is open.
