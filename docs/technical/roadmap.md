...
> **Track status (2026-07-07, fix-plan pass):** Track 1 — **0 open items, all complete** (SA48, SA49, SA50 — SA50 now complete). Track 2 — **3 open items, all ready now, no blockers** (SA52, SA53, SA54 — SA51 complete; remaining items still soft-sequenced on `backups/services.py`). Track 3 — **3 open items, all ready now, no blockers** (SA42 ready now; SA46 ready now — CR-SA46-REV-003 decision made; SA56 ready now — new this pass, no longer soft-sequenced since SA55 already landed; SA44 and SA55 complete).
...
- [x] **SA50 — Fold orgs' `JsonApiMixin`/`JsonOrganizationAccessMixin` stack into one `OrgApiBaseView`.** `Tier 2 · Track 1 · deps: none (soft-sequence after SA49 — both touch orgs surfaces; arch-audit's fix-order note recommends sequencing within one track)`
...
- [x] **SA52 — Fail hard when `manage.py` can't be resolved instead of silently falling back.** `Tier 1 · Track 2 · deps: none (soft-sequence after SA51 — same file)`
  `_get_manage_py()` (`services.py:264-278`) wrapped resolution in `except Exception: pass` and fell through to a bare `"manage.py"` literal. In a deployment where `sys.argv[0]` isn't `manage.py` (gunicorn — the production norm) and `BASE_DIR/manage.py` is absent, `Popen` succeeded but the child died instantly — for restore the artifact was stuck `STATUS_RESTORING` until the 30-minute stale threshold; for create/prune the admin reported "initiated" and nothing happened, silently. Fix: `_get_manage_py()` now raises `BackupError` when no `manage.py` is resolvable, and does so before `_atomic_claim_restore` claims the artifact (the existing call order already had the manage.py resolution before the claim — preserved). Added `TestGetManagePySA52` with 5 regression tests covering the direct function raise/success paths and all three dispatch functions (restore/create/prune), including a `no-claimed-artifact` assertion proving `STATUS_RESTORING` is never persisted on a resolution failure.
  *Files:* `quickscale_modules/backups/src/quickscale_modules_backups/services.py:264-278` (all three dispatchers call it: `:400`, `:479`, `:508`); `quickscale_modules/backups/tests/test_services.py` (TestGetManagePySA52); `quickscale_modules/backups/tests/test_admin.py` (autouse patch for existing tests).
  *Acceptance:* `_get_manage_py()` raises `BackupError` instead of returning a fallback string when resolution fails; 5 regression tests prove the raise, the success path, and that no artifact is left claimed.
  **SA52 — complete.**
