...
The green-gate join (SA96-GATE) is green with empty quarantine. The installed-context resolver crash on the release path is closed by **SA113 ✓** (bundled-manifest fallback, both call sites) and **SA111a ✓** (installed-wheel `plan` coverage, all 12 modules); both are recorded in [CHANGELOG.md](../../CHANGELOG.md) and are referenced below as `SA113 ✓` / `SA111a ✓`. The **Track 3 (release path)** remains blocked on SA112 after a diagnostic/prototype attempt found additional correctness and planning gaps; no SA112 implementation from that attempt landed (see the blocked checkpoint below). One independent verification ticket (**SA114**) runs in parallel on the otherwise-idle **Track 1**. Track 2's feature chain is complete; one independent test-infra ticket (**SA115**, e2e parallelization) is parked there to use the idle track.
...
5. **SA116** (policy-compliant `resolvers.py` line reduction, Track 1, `deps: none`) — behavior-preserving compaction of `contracts/resolvers.py` to ≤1749 lines to clear `SA114-QUALITY-001` and reclose the SA114 four-gate criterion; assigned by `SA114-DEC-001`. Disjoint from every other track's file surface — runs fully parallel with no merge hazard.
...
- [ ] **SA114 — Re-run the make gates on current `v87` HEAD and fix drift.** `Tier 2 · Track 1 · deps: none`
  ...
  - **Blocked checkpoint (2026-07-23; implementation and partial final-sync validation recorded — task remains open):**
    ...
    - **Pending / blocking:** Independent checkpoint review returned `STATUS: ok` and confirmed this blocked checkpoint is safe to merge while SA114 remains open. It resolved `SA114-REV-001` (fail-closed runtime-seam donor preflight), `SA114-REV-002` (normal/quiet section dispatch and Ruff/MyPy/pytest caller parity), `SA114-REV-004` (exact runtime `projectName` binding plus recursive frontend-source leakage checks), and `SA114-REV-005` (status surface synchronization). Focused closeout evidence for REV-002 passed 65 policy/dispatch tests and a real modules-only quiet check. `SA114-QUALITY-001` (**blocking**) — `make quality` exits 2 because pre-existing cross-track growth left `quickscale_core/src/quickscale_core/contracts/resolvers.py` at 1761 lines against its checked-in 1749-line maximum. SA114 did not change that file. Raising `scripts/quality_baseline.json` is not an available local fix: the shrink-only quality-maxima policy in `decisions.md` permits only reducing a measured maximum or leaving it unchanged. Therefore the four-command exit-0 criterion and `SA114-SYNC-001` remain open. Existing non-blocking validation notes are five baseline dead-code findings, one CLI `PytestRemovedIn10Warning`, BYPASSRLS-dependent expected skips, and one expected Core E2E npm-registry offline skip.
    - **Decision RESOLVED (`SA114-DEC-001`, 2026-07-23):** ownership for the policy-compliant reduction of `resolvers.py` to at most 1749 lines is assigned to a new dedicated ticket, **SA116**, on the idle **Track 1** (see below). `contracts/resolvers.py` has no file overlap with SA112's surface (`manifest/implications.py`, `scripts/test_e2e.sh`, `.github/workflows/e2e.yml`), so SA116 runs fully parallel to the Track 3 release critical path with zero merge hazard, keeping Track 3 focused on SA112 — mirroring the SA101 quality-remediation-as-its-own-workstream precedent. The maintainer directed this Track 1 continuation to stop and merge this checkpoint rather than widen it; SA116 carries the reduction and the `SA114-SYNC-001` / four-gate reclose. A baseline increase remains unauthorized (shrink-only policy, decisions.md §5).
...
- [ ] **SA116 — Reduce `contracts/resolvers.py` to ≤1749 lines, behavior-preserving.** `Tier 1 · Track 1 · deps: none`
  Apply file-local, behavior-preserving compaction (extraction and/or direct-identity aliasing, following the SA101 compaction precedent) to bring `contracts/resolvers.py` to at most its 1749-line checked-in maximum, with no change to public API, resolver behavior, or test outcomes. Do **not** touch `scripts/quality_baseline.json`, thresholds, or the gate. On completion, rerun the four gates and independent review to close `SA114-QUALITY-001` / `SA114-SYNC-001`.
  - **Merge hazard:** `contracts/resolvers.py` is disjoint from SA112's surface (`manifest/implications.py`, `scripts/test_e2e.sh`, `.github/workflows/e2e.yml`) and from SA115's runner surface, so SA116 runs fully parallel to the Track 3 release path with no file conflict. The only shared-closeout overlap is `CHANGELOG.md`/`roadmap.md`, covered by the Merge procedure. Sequence after the SA114 checkpoint merges (same track, same file-scope discipline — do not run SA114 and SA116 edits concurrently on `wt-track1`).
  - Verify: `make quality` exits 0; `contracts/resolvers.py` ≤ 1749 lines; `make check` still exits 0 with unchanged pass/fail set; independent change-review `STATUS: ok`.
  *(why →* the shrink-only policy admits only a compliant reduction; assigning it a dedicated idle-Track-1 ticket keeps Track 3 on the critical path while reclosing the SA114 four-gate criterion*)*
...
- [ ] **SA115 — Add in-lane pytest-xdist fan-out to the e2e suite.** `Tier 2 · Track 2 · deps: none · merge after SA112`
  ...
  **Pending / blocking**
  - **SA112 is still open.** SA115 cannot merge until SA112 lands — both edit `scripts/test_e2e.sh` and the shared CI surface, and the merge-order bound (`merge after SA112`) is documented in the SA115 task block above. After SA112 closes:
    ...
    2. Refresh/reconcile shared runner/workflow edges (`scripts/test_e2e.sh`, `.github/workflows/e2e.yml`).
...
- [ ] **SA112 — Installed-wheel lifecycle e2e lane.** `Tier 2 · Track 3 · deps: SA110 ✓ · SA111a ✓ · SA113 ✓`
  ...
  **Decisions needed**
  - The maintainer selected **stop and record/merge this blocked checkpoint** after the second capped review remained partial. A future continuation needs a new explicit authorization for another blocking-only `Adaptive-plan` → plan-review cycle, or a maintainer-supplied literal registry that closes `SA112-PR-002` and `SA112-CR-005`. Product implementation and diagnostic instrumentation remain unauthorized until that review returns `STATUS: ok`.
...
```
Track 1 (SA114 → SA116)  Track 2 (chain done → SA115)    Track 3 → release (critical path)
────────────────────   ────────────────────────────     ─────────────────────────────────
SA114 (gate re-verify   SA104 ✓ → SA105 ✓ → SA106 ✓       SA96-GATE ✓  SA109 ✓  SA110 ✓
  & fix; deps: none)      → SA107 ✓ → SA108 ✓                │
  │ (merge checkpoint)  SA115 (e2e xdist; deps: none,       │
SA116 (resolvers.py       merge after SA112)                │
  ≤1749; deps: none)     (arch Finding 10 chain closed)     SA113 ✓ ── bundled-manifest fallback
...
**Parallelism.** The remaining gating Track 3 work forms one release unit: SA112 → SA96-PUBLISH; SA111a is complete, and SA111b is optional and non-gating. **SA113** (`implications.py`) has landed (bundled-manifest fallback with fail-hard inventory validation). **SA114** is an independent open Track 1 workstream: re-verifying the make gates against current `v87` HEAD is a different intent from adding installed-context coverage, and `deps: none`. It runs in parallel with the Track 3 release chain under two explicit bounds: (1) **infra** — its heavy `make ci`/`ci-e2e` legs share the same PostgreSQL/Docker as Track 3's `smoke-install`/e2e runs and must be serialized against them (per-lane knobs don't span worktrees); (2) **fix scope** — any failure inside SA113's surface (`implications.py`, `scripts/smoke_install.sh`, the installed-wheel e2e lane) is handed to Track 3, not fixed on Track 1. With those bounds the only shared-closeout overlap across all tracks is `CHANGELOG.md`/`roadmap.md`, covered by the Merge procedure. **SA116** (Track 1, `deps: none`) sequences after the SA114 checkpoint on the same track and reduces `contracts/resolvers.py` below its shrink-only maximum to reclose the SA114 four-gate criterion; its file surface is disjoint from SA112's (`manifest/implications.py`, `scripts/test_e2e.sh`, `.github/workflows/e2e.yml`) and SA115's runner surface, so it carries no cross-track merge hazard. SA96-PUBLISH (human-only) shares no files with the assistant-executable tickets.

### Track readiness (2026-07-23)

- **Track 1 — SA114 checkpoint merge-approved; SA116 clean to start after it merges. Off the critical path.** Independent checkpoint review returned `STATUS: ok`, resolved `SA114-REV-001`, `SA114-REV-002`, `SA114-REV-004`, and `SA114-REV-005`, and approved this explicitly incomplete checkpoint for merge. On the final synced Track 1 baseline, `make check`, `make ci`, and `make ci-e2e` are green with empty quarantine, but `make quality` exposes `SA114-QUALITY-001`: pre-existing `resolvers.py` growth above its shrink-only line maximum. `SA114-DEC-001` is **RESOLVED (2026-07-23)** — the policy-compliant reduction is owned by new ticket **SA116** on this same idle track. SA114 stays unchecked until SA116 lands and recloses the four-gate criterion (`SA114-SYNC-001`). SA116 is `deps: none` and file-disjoint from Tracks 2/3, so it is clean to start once the SA114 checkpoint merges (run SA114 then SA116 serially on `wt-track1`, not concurrently). Heavy `ci`/`ci-e2e` legs remain serialized against Track 3's PG/Docker usage, and installed-context resolver-surface fixes remain deferred to Track 3 follow-up ownership.
...
- **Track 3 — open engineering work; SA112 plan review stopped at a mergeable blocked checkpoint (2026-07-23).** The green-gate join, SA109, SA110, SA113, and SA111a are closed. SA113 resolved the installed-context implication resolver crash, and SA111a proves installed-wheel `plan` coverage with all 12 modules. SA111b remains optional and non-gating. **SA112** remains pre-diagnostic: capped plan corrections reduced the open review ledger to `SA112-PR-002` (high) and `SA112-CR-005` (medium), but independent review is still `STATUS: partial`. The maintainer selected stop-and-checkpoint, so another explicit authorization is required before a further blocking-only plan/review cycle; no product implementation or diagnostic instrumentation is authorized. SA96-PUBLISH (human-only) holds until SA112 closes. Non-gating advisories remain deferred (SA91 CR-SA91-REV-006 low; SA89B-CR-004; SA93-REV-005; SA93-ADV-001..004; SA104-ADV-001; SA105-ADV-001; CR-SA106-002; SA110-ADV-001).

**Track 3 decision — RESOLVED (2026-07-21): Option A, fix-first.** The resolver fix was ticketed as **SA113** on Track 3 and landed before its coverage; SA111a then verified the installed `plan` path, making SA112 dependency-unblocked at that point. SA111b remains optional. SA112 is now checkpoint-blocked on the two remaining plan findings `SA112-PR-002` and `SA112-CR-005`; all runtime diagnosis, implementation, and validation remain pending behind a plan-review `STATUS: ok`, as recorded above. The resolver work stayed on Track 3 (not an idle Track 1/2) because it was the *head* of the dependency chain — nothing ran in parallel with it, and splitting the fix from its coverage (same resolver module, `scripts/`, e2e lane) would only have created a cross-track merge hazard. The fix follows the SA109/AF7 bundled-fallback precedent in decisions.md.

**Net.** Track 1 has one merge-approved blocked checkpoint (SA114, gate re-verification) plus its dedicated follow-on (SA116): independent review resolved `SA114-REV-001`, `SA114-REV-002`, `SA114-REV-004`, and `SA114-REV-005`, and `SA114-DEC-001` is resolved by assigning the policy-compliant `resolvers.py` reduction to **SA116** on the same idle Track 1. `SA114-QUALITY-001`/`SA114-SYNC-001` and the final-sync four-gate criterion close when SA116 lands — file-disjoint from Tracks 2/3, so no cross-track hazard. Track 2's feature chain is complete, with one independent off-path infra ticket at a merge-order-blocked executable checkpoint (SA115, e2e in-lane parallelization — merge after SA112, PG/Docker serialized with SA114); its four executable files remain uncommitted on `wt-track2` pending SA112 and the documented reconciliation/validation sequence. Track 3 is **not** done: SA113 and SA111a are closed, and the remaining critical path is **SA112 → SA96-PUBLISH (human)**, with optional SA111b off-path. See [decisions.md §Migration-Squash Decision (SA92)](./decisions.md#migration-squash-decision) for the recorded squash/guardrail/shrink-only-quality policies and §Bundled Module Inventory (AF7) for the fallback precedent SA113 follows; detailed history is in [CHANGELOG.md](../../CHANGELOG.md).
