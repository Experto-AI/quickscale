...
─────────────────────────────────   ────────────────────────────  ─────────────────────────────────
SA139 ◐ (closeout head;                SA115a ◐ (awaits SA139          SA136d ◐ (partial seal tooling;
  │  docs/merge-back pending)             merge-back; Core E2E         │  review-blocked)
  │  ── merge-back ───────────────►      blockers)                     │
  ▼                                   │  no shared file, no gate     ▼
SA122b-5 ◐ (next after SA139            │  ONLY ticket on this track SA136f
  closeout merge; merge-gated)          │
...
**Cross-track edges — one merge-order edge, no unserialized co-writes.** **SA122b-5 merging after SA112e is the only merge-order edge on the board**, and the two tickets are the only writers of `.github/workflows/e2e.yml`'s `pull_request.paths` list. **SA139 has no merge-order gate, but Track 2's SA115a still depends on SA139's merge-back; SA115a carries no other cross-track edge** — Track 2 remains gate-free apart from that dependency. `publish.yml` has no open writer.
...
| **Track 1** — SA139 (closeout head) | **yes** — implementation and granted import-disambiguation expansion are committed on `wt-track1` and executable-review-approved | **no** — documentation review and commit of this closeout remain pending | **no** — merge-back to `v87` remains pending; no merge-order gate applies | no — closeout is not formally complete | no — off-path |
| **Track 1** — SA122b-5 (next after SA139) | **no** — starts only after the SA139 closeout merges back; its E2E/checker slice is scoped and ready | **yes** — its only bound is the merge gate below, once started | **no** — **merge after SA112e** | no — merge-gated filler | no — off the critical path |
| **Track 2** — SA115a | **no** — SA139's closeout must merge back before its umbrella reruns can be accepted | **no** — both umbrella commands reach E2E but each hits a distinct out-of-scope Core E2E Poetry-install failure; no SA115a defect is identified | **yes, after SA139 merge-back** — no separate merge-order gate or shared executable-file writer | no — downstream validation-blocked pending SA139 merge-back | no — off-path filler |

**Truly green today: none** — the release-wide green gate remains unclaimed. **SA136d remains on the critical path and review-blocked** by six implementation findings; its retained partial tooling is not a completion or release claim. **SA139 remains Track 1's closeout head**: its implementation and source import disambiguation are committed and executable-review-approved on `wt-track1`, while documentation review, commit, and merge-back to `v87` remain pending. Track 2's SA115a still depends on that merge-back; its two umbrella reruns reach E2E and fail on two out-of-scope Core E2E Poetry-install defects, so no full E2E greenness is claimed. SA122b-5 is next only after the SA139 closeout merges; its merge bound is a **hard upstream dependency** only SA112e can clear.

**Open maintainer decisions: none.** The board's last two landed on 2026-08-08 and are applied below: `SA136D-QG-001` is dispositioned as **reviewed acceptance** under the inherited "no worse than found" rule, with the repair ticketed separately as [SA140](#sa140--quality-ceiling-repair-on-the-apply-path); and SA139's granted **scope expansion** to disambiguate the import at its source is implemented and executable-review-approved. Neither is a release authorization. Documentation review and merge-back for this SA139 checkpoint remain pending. All prior topology decisions are settled and recorded in [CHANGELOG.md](../../CHANGELOG.md).
...
**Status:** queue is **[SA139](#sa139--gate-target-derivation-resolves-from-the-test-working-directory) → [SA122b-5](#sa122--release-assurance-is-four-hand-synchronized-gate-inventories-arch-finding-11)**. SA139 remains the Track 1 closeout head; its documentation closeout and merge-back are pending. SA122b-5 is next only after SA139 merges back and remains **merge after SA112e**. Both are off the critical path. The SA128 parity checker is authoritative.

### SA139 — Gate-target derivation resolves from the test working directory (implementation checkpoint; documentation closeout pending)

- [ ] **SA139 — Repair coverage-policy gate-target derivation so `--e2e` reaches E2E.** `Tier 2 · deps: none` — **implementation committed and executable-review-approved on `wt-track1`; documentation closeout and merge-back pending; no merge gate; unblocks Track 2's SA115a after merge-back**

  The reviewed root-anchor implementation and the granted source-import disambiguation are committed on `wt-track1` in `06860a7248b0c4b1e1d2fbd3c665e54b08aeab9e`. `SA139-QG-001` is addressed by that implementation: whole-repo collection no longer stops on the ambiguous `conftest` import at `quickscale_core/tests/test_e2e_xdist_fixtures.py:13`. Both umbrella commands reached E2E; their exit-1 Core Poetry-install failures are recorded below as out of scope and are not SA139 defects. Documentation review, commit of this closeout, and merge-back to `v87` remain pending.
  - **Filed out of SA115a, deliberately.** `SA115A-QG-001` was real and blocking, but no SA115a file caused it and that ticket's allowlist forbade repairing it. Homing the repair here rather than on Track 2 respected the standing three-worktree rule: Track 2 accepts no new tickets. The implementation addresses the coverage-policy blocker on `wt-track1`; Track 2's acceptance remains dependent on SA139's merge-back, and the remaining Core E2E failures are separate and out of scope.
...
**Rebalancing verdict: no relocation available; one new ticket (SA140).** Nothing is movable — SA136f/SA117e-4 are dependency-bound to SA136d, SA112b–f are one serial evidence chain, SA139 remains the Track 1 closeout head and SA122b-5 follows its merge-back, SA140 is evidence-bound behind SA112b, and Track 2 stays closed to unrelated work while SA115a depends on that merge-back. Docker/PostgreSQL remains serialized with Track 3 priority when SA115a reruns its umbrella gates.

**Open track-topology decisions: none**, and as of 2026-08-08 **no open maintainer decision of any kind** — `SA136D-QG-001` is dispositioned as reviewed acceptance, and SA139's scope expansion is implemented and executable-review-approved; neither alters topology. SA139's documentation closeout and merge-back remain pending. All resolved topology decisions, the loop/seal artifact contract, and the `splits/<m>-module/<version>` tag scheme are recorded in [CHANGELOG.md](../../CHANGELOG.md).
