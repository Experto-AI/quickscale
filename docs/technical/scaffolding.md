...
- Generated end-user Django project structure (what `quickscale plan` + `quickscale apply` produces)
...
> NOTE: The `quickscale_modules/` directory shown in the Post‑MVP / monorepo examples is a workspace convention for maintainers and for users who maintain a personal monorepo. It is NOT part of the project generated by `quickscale plan` + `quickscale apply`. Generated user projects do NOT include `quickscale_modules/` by default. See `decisions.md#module-extraction-workflow` for authoritative guidance.
...
- ✅ Theme selection: `quickscale plan myproject --theme <name>` + `quickscale apply`
...
### 5.1 Base Project (Generated by `quickscale plan` + `quickscale apply`)
...
Prerequisite: Inheritance requires that `quickscale_core` is present in the client repository (for example via `git subtree add --prefix=quickscale <repo> main --squash`). This is an advanced workflow intended for users who maintain a personal monorepo or who intentionally embed core utilities — it is NOT part of the default `quickscale plan` + `quickscale apply` generated starter.
...
- CLI commands (MVP): keep the core workflow centered on `quickscale plan` + `quickscale apply`, plus the shipped development, deployment, and module-management commands already listed in the [CLI command matrix](./decisions.md#cli-command-matrix). Truly new wrapper commands (for example future batch helpers) belong in `quickscale_cli/commands/`, need corresponding tests, and should still document manual `git subtree` recovery paths for advanced users.
