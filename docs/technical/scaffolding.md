...
>
> **Default behavior:** `quickscale plan myapp` + `quickscale apply` generates ONLY standard Django structure (manage.py, settings.py, urls.py, templates/, static/, pyproject.toml).
...
```
User runs `quickscale plan myapp` + `quickscale apply`? → ❌ NO (never generated)
...
- ✅ Module management: `quickscale update`, `quickscale push`
