...
> **Audit snapshot:** 2026-07-26 · **Current reconciliation:** 2026-08-21 · **Branch:** `v88`
...
**Growth direction (from the planning surface, authoritative).** The v88 roadmap records the prioritization decision as **"neither"** — no `teams` domain work and no third generated-project updater. Nine tickets run on three tracks; the ones that touch this audit's seams are **SA124** (unify SA117 scope-tool path authority), **SA123** (add dependency-vulnerability and security static-analysis gates, *registered through `scripts/gate_registry.json`*), and **SA135** (give test suites an owned PostgreSQL lifecycle). All three land on the CI/governance layer, which is where both new findings sit.
...
**Horizon & trigger:** `now`. Two v88 tickets build directly on this layer: **SA123** ("register every new gate through the authoritative gate registry", merge #6) and **SA124** ("a test fails if any consumer is added without going through that source", merge #2). SA124's acceptance criterion is *a test* — and it will land in `scripts/test_check_sa117_scope.py`, one of the ten suites no context executes. The ticket's own guarantee is unenforceable on arrival.

**Confidence:** High. Verified by direct execution under the project interpreter and by an exhaustive absence search; the one inference (why 72 quality-baseline tests fail) is labelled below.
...
- **Executed this pass under `.venv/bin/python` (3.14.6):** `test_gate_parity.py` → **231 passed, 1 failed**; the seven publish/scope/version suites → **580 passed, 1 failed**; `test_quality_baseline_monotonicity.py` + `test_installed_wheel_venv.py` → **148 passed, 72 failed**. Total **959 passed, 74 failed** across code nothing runs.
...
- The 72 quality-baseline failures resolve to `MERGE_BASE_ERROR` where `SCHEMA_ERROR` is expected (`scripts/test_quality_baseline_monotonicity.py:4990`), i.e. the shell wrapper fails on merge-base resolution before reaching the assertion. **Environment-sensitive; needs triage, not asserted as a defect.** `test_check_sa117_scope.py:640` fails because the test shells out to bare `python` (3.12 here) against a script using PEP 758 syntax — a portability defect in the test, also environment-dependent.
...
**Target:** **SA123** — "Add dependency-vulnerability and security static-analysis gates … register every new gate through the authoritative gate registry" (v88 Track 2, merge #6). Chosen because it is the next scheduled change that stresses the governance seam, and its acceptance criteria name the registry explicitly.
...
- **Were the ten `scripts/` suites ever intended to run in CI, or are they maintainer scratch?** (`gate-suites-unexecuted`) — if they are deliberately maintainer-run-only, Finding 12 drops to a watchlist item about the two red ones. The evidence argues against that reading, but only the maintainer can settle intent.
...
- **A repository conformance test is red on `HEAD`.** `scripts/test_gate_parity.py:1090` `TestParserPrecision::test_all_twenty_one_publish_run_values_are_structural` fails: `d3d4c633` added two steps to `publish.yml` and removed two `apt-get` lines from its `Create test databases` step, and the literal oracle was never updated. Repository-content only — no environment dependence.
- **`scripts/test_check_sa117_scope.py:640` is interpreter-bound.** `test_legacy_lock_candidate_spelling_is_rejected` shells out to bare `python` rather than `sys.executable`; where `python` is not 3.14 it hits `SyntaxError` on `check_sa117_scope.py:1579`'s PEP 758 unparenthesized `except`, returning exit 1 where 2 is expected. Use `sys.executable`.
- **72 failures in `scripts/test_quality_baseline_monotonicity.py`** in this environment, resolving to `MERGE_BASE_ERROR` where `SCHEMA_ERROR` is expected (`:4990`). Needs triage to separate environment sensitivity from defect — and it should be triaged *before* Finding 12's gate is registered, so it registers green.
- **`quickscale_devtools/pyproject.toml:34` is `0.86.0` against `VERSION` `0.87.0`.** Already owned by roadmap SA137; recorded here only so it is not mistaken for undetected drift.
- **Deprecated bool inversion in a security gate.** `scripts/check_csrf_exempt_gate.py:271` uses `~val != 0` on a bool; Python removes this in 3.16. In a CSRF-enforcement gate this should be `not val`.
- **`docs/others/tech-audit.md` header still reads `Branch: v87`.** One-line staleness, already noted in the v88 ticket context.
...
- 2026-08-21 — Prior red flags: none were carried from the previous pass (the prior document recorded no Red flags section); six new entries recorded above.
