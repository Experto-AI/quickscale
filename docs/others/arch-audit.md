...
> **Audit snapshot:** 2026-07-26 · **Current reconciliation:** 2026-08-24 · **Branch:** `v88`
...
| **Gate implementations behave as specified** | 15 suites in `scripts/`; **4 wired to a target, 11 wired to nothing**, including `scripts/test_repo_source_interpreters.py` | **Convention only — unverified** | **new row; population grew 1 → 11** |
...
**Horizon & trigger:** `now`. The current census is 15 suites total, 4 wired, and 11 unwired, including `scripts/test_repo_source_interpreters.py`. Two v88 tickets build directly on this layer: **SA123** ("register every new gate through the authoritative gate registry", merge #13) and **SA124** ("a test fails if any consumer is added without going through that source", merge #11). SA124's acceptance criterion is *a test* — and it will land in `scripts/test_check_sa117_scope.py`, one of the eleven suites no context executes. The ticket's own guarantee is unenforceable on arrival.
...
- **Census of `scripts/test_*.py` — 15 suites, 4 wired, 11 orphaned.** Wired: `test_ci_coverage_policy.py` (`Makefile:611`), `test_integration_worker_pool.py` and `test_e2e_parallel.py` (`Makefile:616`), `test_ci_local_parallel.py` (`Makefile:622`). Orphaned: `test_gate_parity.py`, `test_quality_baseline_monotonicity.py`, `test_check_csrf_exempt_gate.py`, `test_publish_module.py`, `test_installed_wheel_venv.py`, `test_verify_public_module_apply.py`, `test_version_tool.py`, `test_check_sa117_scope.py`, `test_verify_sa117_publication.py`, `test_check_release_published.py`, and `scripts/test_repo_source_interpreters.py` — together covering the gate-parity checker, the CSRF AST gate, the quality-baseline monotonicity gate, the SA117 scope guard, the version tool, publication verifiers, and repository-source interpreter routing.
...
1. **Verification debt grows one suite per gate.** The current 15-suite population has eleven orphans today, up from one on 2026-07-07. SA123 adds two gates, SA124 adds a divergence test, SA152 adds a smoke gate — four more suites into a directory nothing executes.
...
- **SA92 migration-squash discovery tuple.** Trigger: another migration-bearing module is added, or the tuple omits one. The exact artifact is `quickscale_modules/orgs/tests/test_sa92_migration_squash_guardrail.py`. It is a bounded literal tripwire, not a schema-parity proof: `_CROSS_TABLE_ORG_DML` catches the common cross-table `UPDATE … SET organization_id` shape, while the catalog/policy/data parity gate is the authoritative proof. Its `_migdir()` helper reads the inert `django_apps:` key and falls back to a conventional path; its parity backstop still names the retired `v87` baseline. SA151 has produced regenerated migrations; its S4 BYPASSRLS prerequisite was discharged on 2026-08-24 (`make test-bypassrls` exits 0 at 80 passed), and terminal closure now waits only on the ticket's own S4-C/S4-D closeout. **Action:** SA164 owns the `_migdir()` correction and parity-backstop re-anchoring after SA151 closes; no SA92 implementation is performed here.
...
- 2026-08-24 — Current reconciliation: the SA155 census is **15 suites total, 4 wired, 11 unwired**, including `scripts/test_repo_source_interpreters.py`; Finding 12 remains open because the suites still lack an owning execution context. SA151 S1-S3 implementation evidence exists, including regenerated migrations, hardened AppConfig identity checks, and an independent generated-runtime identity oracle. Its S4 BYPASSRLS prerequisite was discharged on 2026-08-24 — the true cause was an entirely absent local PostgreSQL host precondition rather than a missing grant, which is direct field evidence for Finding 13 and for SA135's owned-lifecycle design — and terminal closure now waits only on SA151's own S4-C/S4-D closeout. The SA92 artifact is located and characterized as the bounded literal tripwire at `quickscale_modules/orgs/tests/test_sa92_migration_squash_guardrail.py`; SA164 owns its `_migdir()` and parity-backstop remediation after SA151 closes. Finding 4 remains deferred because its model-graph trigger is still unfired.
