...
- **Closure reconciliation:** the historical 72 quality-baseline failures were caused by the retired `v87` fallback and are closed by SA156; the former SA117 false-green/interpreter-bound test was corrected by SA157 to use `sys.executable`, resolve the tool beside the test, and assert a tool-specific signal. TA66's stale publish oracle is closed by SA158. The genuinely open current red flag is TA69, while Finding 12 remains open as the broader unexecuted-suite problem.
