...
| `container-status-substring-match` (TA70) | S4 | Correctness (shipped CLI utility) | Exact-name structured status and failure-path coverage are implemented; Phase C/release acceptance remains outstanding | Trivial ⚡ | High | still-open — A/B remediated, C unaccepted |
...
| Phase C/release evidence has not exercised the remediated E2E harness end to end | **TA70** | A/B now include deterministic failure-path tests for exact status argv/states, malformed and failed queries, exited diagnostics, scoped image cleanup, split build correctness/duration, and PostgreSQL-client mismatch. Keep TA70 open until SA170's unaccepted Phase C and both release campaigns establish release-scope cleanup and PostgreSQL before/after evidence |
