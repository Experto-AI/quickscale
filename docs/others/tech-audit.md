...
| `container-status-substring-match` (TA70) | S4 | Correctness (shipped CLI utility) | Exact-name structured status and failure-path coverage are implemented; Phase C attempted but release acceptance remains unaccepted | Trivial ⚡ | High | still-open — A/B remediated, C retained-partial |
...
- **TA70 · `container-status-substring-match`** · **Open after a retained-partial Phase C convergence; the A/B mechanism and dependency/return-141 static blockers are remediated.** `quickscale_cli/src/quickscale_cli/utils/docker_utils.py` now queries `docker ps -a` with an anchored exact-name filter, validates exactly one named row, and returns structured *absent* / *created* / *running* / *exited(code)* state while Docker invocation and malformed-result failures raise. The E2E readiness caller now accepts only *running*, fails immediately on *exited* with its code and last log lines, and surfaces query failures rather than spending the polling budget. Mocked unit/readiness coverage exercises exact argv, all states, ambiguous/malformed output, Docker failures, immediate exited-state diagnostics, query failure, timeout state reporting, and both log streams. The latest ordered serial campaign used `setsid --wait env QS_E2E_PARALLEL=0 QS_E2E_INTEGRATION_REF=v88 make test-e2e` and exited 2: Core reported 38 passed and CLI reported 52 passed / 1 failed at the installed-wheel lifecycle row after `poetry install` aborted following dependency synchronization. The concurrent `setsid --wait env QS_E2E_INTEGRATION_REF=v88 make ci-e2e` campaign was not run because the serial prerequisite was red; exact-scope cleanup and standing PostgreSQL equality passed, but the quiet transcript did not individually attest the other three frozen rows. A convergence-only focused rerun of that row and a diagnostic-copy `poetry install -vvv` both passed, without retroactively greening the campaign or identifying its unretained lower-level cause. Reviewed-plan phase C-correct is since accepted and retained at `dcfb136f5980195afd69c2c168afc81e02e118c7` — generated projects hold backend startup until the end-of-init database sentinel is observable — while phase C-release remains unaccepted. **SA170 remains open and TA70 remains open** pending a fresh ordered serial-then-concurrent campaign with cleanup and PostgreSQL equality; no completion or release-readiness claim is made.
...
| Phase C/release evidence remains incomplete after exercising the remediated E2E harness | **TA70** | A/B include deterministic failure-path tests for exact status argv/states, malformed and failed queries, exited diagnostics, scoped image cleanup, split build correctness/duration, and PostgreSQL-client mismatch. The latest ordered serial campaign exited 2 with Core 38 passed and CLI 52 passed / 1 failed at the installed-wheel lifecycle row after `poetry install` aborted following dependency synchronization; the concurrent `make ci-e2e` campaign was not run because the serial prerequisite was red. Exact-scope cleanup and standing PostgreSQL equality passed, but the quiet transcript did not individually attest the other three frozen rows. The focused row and diagnostic-copy install now pass, but only a fresh ordered serial-then-concurrent campaign with cleanup and PostgreSQL equality can close TA70 |
...
- 2026-09-01 — **TA70** `container-status-substring-match`: **still-open with A/B remediation implemented and Phase C/release evidence unaccepted**. The old unanchored-filter, display-string, blind-poll, and missing-failure-test claims no longer describe HEAD; exact-name structured status, immediate diagnostics, deterministic failure-path coverage, scoped React-image cleanup, split correctness/duration reporting, and independent PostgreSQL-client checks are present. TA70 remains owned by SA170 and is not retired until Phase C and the release campaigns are accepted.
- 2026-09-02 — **TA70** `container-status-substring-match`: **still-open after SA170 convergence retained partial**. The DRF/sqlparse vulnerability gate and all 1356 registered script tests are green. `QS_E2E_PARALLEL=0 make test-e2e` passed Core 38 and CLI 53 with exact-scope cleanup and standing PostgreSQL equality; unchanged-byte `make ci-e2e` reached stage 12 but exited 2 with 2 Core and 8 CLI failures on the concurrent generated-PostgreSQL start surface. SA170 remains open; no completion or release-readiness claim is made; TA70 remains owned by SA170.
- 2026-09-04 — **TA70** `container-status-substring-match`: **still-open after the next SA170 retained-partial checkpoint**. The ordered serial campaign used `setsid --wait env QS_E2E_PARALLEL=0 QS_E2E_INTEGRATION_REF=v88 make test-e2e` and exited 2 with Core 38 passed and CLI 52 passed / 1 failed at the installed-wheel lifecycle row after `poetry install` aborted following dependency synchronization. The concurrent `make ci-e2e` campaign was not run because the serial prerequisite was red; exact-scope cleanup and standing PostgreSQL equality passed, but the other three frozen rows lack individual pass oracles in the quiet transcript. A convergence-only focused rerun and diagnostic-copy install both passed, but did not identify the historical lower-level cause or replace the required fresh ordered serial-then-concurrent campaign. SA170 remains open; no completion or release-readiness claim is made; TA70 remains owned by SA170.
