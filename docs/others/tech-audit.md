...
- **`quickscale_devtools/pyproject.toml:34` is `0.86.0` against `VERSION` `0.87.0`.** Verified still drifted. Owned by roadmap **SA137** (merge #1); recorded here only so it is not mistaken for undetected drift. Not a finding — planning owns it.
