...
- **`_HOST_DEPENDENT_PATHS` accountability is implemented but not yet closeout-accepted.** The retained SA165 A-C product documents the `.env` rationale and the second/third-entry escalation rule. Keep this note live until a release verdict covers the settled Phase D candidate.
- **The isolation-gate skip is identity-bound in retained product bytes.** The retained SA165 A-C product narrows authorization to the two `PENDING_REMEDIATION` test identities and carries a red negative control. Keep this note live until the settled Phase D candidate receives its release verdict.
...
- **`flush_empty_consolidated_sections` now fails hard in retained product bytes.** The retained SA165 A-C product raises `StateError` for corrupt and non-mapping YAML roots before any write and preserves the original bytes. Keep this note live until the settled Phase D candidate receives its release verdict.
...
- 2026-09-05 — **TA71** `backup-lock-stale-clear-toctou`: **retired by accepted SA171.** Inode-bound reclamation and acquisition-bound release identity are archived in [CHANGELOG.md](../../CHANGELOG.md); the separate-lock question is retained only as the non-defect structural watch item above.
...
- 2026-09-05 — **SA165 Phase D retained-partial reconciliation:** the retained A-C product addresses `flush_empty_consolidated_sections`, the identity-blind isolation skip, `_HOST_DEPENDENT_PATHS`, and generated local credentials, and the six-file Phase D candidate reconciles their documentation. The sole release verdict predates the settled `CHANGELOG.md` bytes, so the four notes remain live and SA165 remains open. The stale-but-green run and exact continuation handoff are retained in [CHANGELOG.md](../../CHANGELOG.md).
