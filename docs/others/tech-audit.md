...
- 2026-08-26 — **Dependency-vulnerability and security-static-analysis tooling gaps closed by SA123's implemented gates.** The accepted design uses Trivy v0.74.0 rather than the earlier proposed `pip-audit`, because Trivy audits both committed Poetry locks directly, plus focused Bandit 1.9.4. Both are blocking registered gates; eight registry-bound hosted jobs are generated and checked, suppressions are identity-bound with owner/rationale/expiry, and negative probes cover vulnerable-lock, Bandit, checksum, archive-safety, and stale-database failure. SA123 remains open only because unrelated SA169 failures stop its repository-wide acceptance; no root merge-back is claimed.
