...
Counts: **S1: 0 · S2: 1 · S3: 4 · S4: 2.** Quick wins flagged ⚡ (Trivial/Small-effort S2/S3).
Resolved since the 2026-07-05 audit: **TA21, TA22, TA24, TA25, TA31, TA36, TA32, TA33, TA38, TA41** (see Reconciliation log).
...
- **Backups admin as orchestration engine (TA37):** already opened as arch-audit `backups-admin-orchestration-accretion`; TA37 (staleness detection) is its remaining ticket-shaped child — the CAS transition (TA36) resolved via SA20's `_atomic_claim_restore()`, but a restore-attempt *entity* is still the shape staleness detection would naturally own.
