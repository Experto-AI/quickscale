...
| Phase C/release evidence remains incomplete after exercising the remediated E2E harness | **TA70** | A/B include deterministic failure-path tests for exact status argv/states, malformed and failed queries, exited diagnostics, scoped image cleanup, split build correctness/duration, and PostgreSQL-client mismatch. The serial campaign and static release gates are green, exact-scope cleanup and standing PostgreSQL equality passed, but `make ci-e2e` exited 2 at stage 12 with 2 Core and 8 CLI concurrent generated-PostgreSQL start failures. Keep TA70 open until the full release campaign is green |
