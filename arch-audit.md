...
- 2026-07-29 — `quality-baseline-monotonicity-unenforced`: **RESOLVED** by SA121 (arch Option 1: merge-base baseline diff enforced as a blocking gate in `make quality`, with a separate structured waiver ledger). Measurement and permission are now distinct artifacts, so the snapshot can no longer authorize its own growth; the `max_lines` half had already dissolved with SA125. Closure detail and the superseded 2026-07-28 narrowing record are in [CHANGELOG.md](CHANGELOG.md).
