...
- 2026-07-29 — `quality-baseline-monotonicity-unenforced`: **RESOLVED.** SA121 landed arch Option 1 in `scripts/check_quality_baseline_monotonicity.py`, invoked as a blocking gate by `scripts/check_quality.sh:117-138`: baseline keys and ceilings are diffed against the merge base, any positive delta or new exemption fails, and the only escape is a structured waiver in the separate `scripts/quality_waivers.json` ledger (owner/reason/expiry/ceiling), currently empty. Measurement and permission are now distinct artifacts, so the snapshot can no longer authorize its own growth. The `max_lines` half of the finding had already dissolved with SA125. Closure detail is in [CHANGELOG.md](CHANGELOG.md).
- 2026-07-28 — (superseded by the line above) `quality-baseline-monotonicity-unenforced`: **still-open, narrowed.** SA125 (`SA125-DEC-001`) retired the `large_files` section and its blocking enforcement, so two of the three census increases (`module_commands.py` 1596→1608, `config_schema.py` 605→611) no longer have a gate to be self-authorized against. The structural defect is unchanged on the surviving `max_complexity` surface, where `_validate_modules_section` 11→12 remains the live instance. Recommendation, Option 1, size, and first step all stand; SA121 owns them. The fix-order requirement that Finding 12 precede any further baseline adjustment is **not** violated by SA125, which carried its own maintainer decision record rather than an unexplained numeric edit.
