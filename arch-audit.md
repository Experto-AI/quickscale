...
| 14 | Complexity/file-size maxima never ratchet upward | decision record only; current-code check trusts the mutable baseline | convention | deteriorated: 3 of 3 numeric changes since baseline creation are increases (Finding 12) |
...
| 2 | 12 | `quality-baseline-monotonicity-unenforced` | now | High | S | A mutable snapshot is both measurement and waiver authority, so raising it silently legalizes the regression that the shrink-only policy forbids. |
| 3 | 7 | `generated-file-ownership-unmodeled` | 6–18 months | High | M | The updater re-encodes generator file ownership in 138 hand-classified entries rather than consuming ownership metadata from the emission contract. |
| 4 | 2 | `deletion-invariants-per-boundary-reimplementation` | deferred (teams/new erasure path) | High | S | Last-owner deletion is structurally protected, but other domain cleanup obligations still live only in the account-delete boundary. |
| 5 | 4 | `org-model-universe-hand-enumerated` | deferred (teams) | High | M | Purge membership is checked, but the load-bearing deletion order is a manual list with only three named edges asserted. |
...
- E2E eligibility is a 26-path manual allowlist at [.github/workflows/e2e.yml](.github/workflows/e2e.yml#L13-L41). Roadmap SA115-CI-001 records two changed e2e surfaces absent from it and calls this the same defect class as SA112-CR-005 ([docs/technical/roadmap.md](docs/technical/roadmap.md#L92-L113)).
...
## Finding 12: The shrink-only quality baseline can authorize its own growth

**ID:** `quality-baseline-monotonicity-unenforced`

**Rank rationale (blast radius × likelihood):** The gate governs repository-wide complexity/size growth and was bypassed in all three numeric baseline edits only eight days after its stated contract was ratified.

**Horizon & trigger:** `now` — the next oversized function/file change can be made green by editing the same baseline file, especially during SA112/SA115 release pressure.

**Confidence:** High — the baseline was compared exhaustively from creation commit `76c5cc55` to HEAD, blame identified the raising commit, and the gate implementation was read.

**Context dependence:** `wrong-regardless` as a governance invariant: a rule stated as monotonic but mechanically enforced by a mutable self-declared ceiling is internally contradictory. Solo maintenance lowers coordination harm, not the gate's false-green behavior.

**Problem:** `quality_baseline.json` is simultaneously the regression ceiling and an editable waiver, while no gate validates the decision record's shrink-only property against history.
...
- The quality checker trusts whatever maximum is currently in the file: complexity only regresses when actual exceeds `baseline_entry["max_complexity"]` at [scripts/check_quality.sh](scripts/check_quality.sh#L818-L831). It does not read a parent/base version. *(At intake the same defect also applied to `baseline_entry["max_lines"]`; SA125 retired that half of the gate on 2026-07-28 — see the Reconciliation log. The finding stands unchanged on the surviving `max_complexity`, `dead_code`, and `duplication` sections.)*
...
**Counter-evidence:** Searched Make, scripts, workflows, tests, pre-commit, and decisions for `shrink-only`, baseline-diff, parent-baseline comparison, increase allowlists, or waiver records. Found a robust current-code measurement/shape validator and the explicit policy, but no historical monotonicity enforcement. The strongest partial protection is review and version control: increases remain visible in diffs and `git blame`; neither blocks a false-green merge.

**Why it compounds:** Each accepted increase becomes the new floor, so future growth is measured from a larger ceiling and the exception snapshot ratchets with the code. All currently grandfathered large files/complex functions build on this authority; as more entries appear, reviewers must reconstruct intent per diff and the “regression” gate increasingly reports only growth not yet rebaselined.

**Detection signal:** Positive numeric diffs in `quality_baseline.json`, a green `make quality` immediately after production complexity/file growth, or repeated “update baseline” changes. A monotonicity check should report entry key, old/new value, and the required waiver/decision reference.

**Steelman:** The three increases accompanied a legitimate installed-context/gate-drift fix, and a snapshot must sometimes move when a structural change knowingly adds code. Do not force immediate refactoring when a time-critical correctness fix requires a temporary rise. That argues for an explicit, expiring exception mechanism—not an unrecorded rewrite of a contract declared shrink-only.

**Correct shape:** Normal changes cannot increase a grandfathered maximum; exceptional increases require an explicit, reviewable, time-bounded policy object distinct from the measurement snapshot.

**Options:**

1. Add a baseline-diff gate against the merge base that rejects increases and new exemptions unless accompanied by a structured waiver with owner, reason, expiry, and ceiling. Low effort, reversible, and directly enforces the declared property.
2. Replace per-entry current maxima with stable global thresholds plus a shrink-only exception ledger whose ceilings never auto-follow code. More cleanup and migration risk, but clearer long-term semantics.
3. Generate the measurement snapshot as CI output only and keep accepted ceilings in a separate policy file. Best separation of fact from permission; larger tooling change but removes self-authorization completely.

**Recommendation:** Option 1 now, with Option 3 only if the quality tool is later redesigned. It is the smallest seam change that prevents recurrence without blocking explicit emergency exceptions. **Size:** S. **First step:** make CI compare baseline keys/ceilings to the merge base and require a structured waiver for any positive delta; use the three SA114 increases as the initial failing fixture.
...
Fix Finding 11 before or as part of SA112e/SA115-CI-001: those pending edits are the immediate characterization case for a derived gate topology, and centralizing afterward would rework both path-list changes. Finding 12 is independent and can land in parallel; it should precede any further quality-baseline adjustment. Finding 7 remains intentionally gated on consumer growth and does not block release. Findings 2 and 4 are independent of the governance work but should be designed together at `teams` kickoff: derive the model/purge universe first (Finding 4), then attach new deletion contributors to the coordinator (Finding 2), avoiding two passes over the new domain.
...
- 2026-07-28 — `quality-baseline-monotonicity-unenforced`: **still-open, narrowed.** SA125 (`SA125-DEC-001`) retired the `large_files` section and its blocking enforcement, so two of the three census increases (`module_commands.py` 1596→1608, `config_schema.py` 605→611) no longer have a gate to be self-authorized against. The structural defect is unchanged on the surviving `max_complexity` surface, where `_validate_modules_section` 11→12 remains the live instance. Recommendation, Option 1, size, and first step all stand; SA121 owns them. The fix-order requirement that Finding 12 precede any further baseline adjustment is **not** violated by SA125, which carried its own maintainer decision record rather than an unexplained numeric edit.
