...
**Horizon & trigger:** `now` — SA112e and SA115-CI-001 both need edits to the same e2e path allowlist before the pending 0.87 release; the next repository conformance gate repeats the wider local/hosted/publish coordination. *(2026-08-12: both edits are now sequenced through the derived registry — SA115-CI-001 is inside SA122b-5, which lands first, and SA112e folded into SA112f and writes the registry afterwards. The finding is unchanged and still open; only the fix ordering moved. See correction (c) below.)*
...
**Verdict:** exonerated as a one-time seam-closing migration, not a recurring feature tax. The causal split prevents speculative fixes and leaves future installed-artifact changes behind one lifecycle proof. SA112e's manual path-list station independently fed Finding 11 — *as of 2026-08-12 that station is eliminated rather than counted: SA112e folded into SA112f and now appends to the gate registry, so the chain measures five children, not six, and no manual path-list station remains.*

## Fix order and interactions

Fix Finding 11 **before** SA112e/SA115-CI-001: those pending edits are the immediate characterization case for a derived gate topology. **This is now the ratified order (2026-08-12), not merely the preferred one** — SA122b-5 centralizes the path list first and SA112e's successor writes the registry afterwards, so no path-list change is ever made twice. The earlier "before or as part of" phrasing accompanied a board that ordered these the other way round; that ordering is retired. Finding 12 is resolved (SA121). Finding 7 remains intentionally gated on consumer growth and does not block release. Findings 2 and 4 are independent of the governance work but should be designed together at `teams` kickoff: derive the model/purge universe first (Finding 4), then attach new deletion contributors to the coordinator (Finding 2), avoiding two passes over the new domain.
