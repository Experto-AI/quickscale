...
41. Provide clear approval status and next steps
...
9. Findings Summary (Pass/Issues/Blockers)
10. Detailed Quality Metrics (tables)
11. Recommendations (strengths, required changes, future considerations)
12. Conclusion with approval status
...
END-USER VALIDATION (CRITICAL)
-------------------------------

### Purpose
After technical validation (linting, tests, code review), the developer MUST verify the changes work correctly from an end-user perspective. This section ensures the feature is usable in real-world scenarios, not just passing automated tests.

### When to Perform End-User Validation
...
### End-User Validation Workflow

**STEP 1: CLEAN ENVIRONMENT SETUP**
Simulate a fresh user installation to catch environment-specific issues:
...
Test the installation process as a new user would:
...
Walk through the complete user workflow that the feature enables:
...
Ensure user-facing output is correct and helpful:
...
For features that modify files or state:
...
Follow the documentation as a new user would:
...
Check that the feature performs acceptably in real use:
...
Verify clean removal (if applicable):
...
### Recording End-User Validation Results

Document the end-user validation in the review report:
...
### When Developer Testing is Complete

Only mark the review as complete when BOTH automated validation (tests, linting) AND end-user validation have passed. Include end-user validation results in the review report with specific commands executed and results observed.

**Remember**: If you wouldn't want to release the feature based on what you experienced during end-user testing, mark it as NEEDS REVISION in the review.
