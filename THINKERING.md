...
Build generic modules (blog, listings) in `quickscale_modules/` while simultaneously testing them in a local example project (`examples/real_estate`). This creates a validation loop where modules are generic from day 1 but proven through real-world usage.
...
**Decision**: Use **Abstract Base Classes** pattern.
- `AbstractListing`: Common fields (title, status, date).
- `Property(AbstractListing)`: Concrete model in the Vertical (e.g., Real Estate project).
- `JobPosting(AbstractListing)`: Concrete model in the Vertical (e.g., Job Board project).
...
**Decision**: Use `examples/real_estate` within the monorepo.
...
**Approach**: When user runs `quickscale embed --module simple-blog`, CLI asks configuration questions and applies settings automatically.
...
### Theme Integration
**Decision**: **Zero-Style Modules + Separate Themes Directory**.

**Approach**:
- **Modules** (`quickscale_modules/`): Contain Logic (Models/Views) and **Zero-Style** templates (Semantic HTML only).
- **Themes** (`themes/`): Contain the visual styling (CSS/JS/Decorated Templates).
- **Project**: Installs the module and applies a theme.
