...
| `./scripts/publish_module.sh --publish-all` | `make publish-modules` |
...
- [check_commit_testimony.py](./check_commit_testimony.py) — fail-closed Git-range gate for non-merge commits that change hosted workflows, `scripts/gate_registry.json`, or a provisioning-station command block. Each such commit must carry a `vNN` roadmap reference in its message, bare (`v88`) or dotted (`v0.88.0`); no ticket form or changelog entry substitutes for it. Prefer `make check-commit-testimony`; CI checks the pull-request range from a full-history checkout.
