...
The generation drift gate is part of the mandatory fast pre-commit gate (F-006): `make check` invokes `poetry run python scripts/sync_ci_gate_jobs.py --check` directly (non-recursively) after the registry-bound check targets, so stale hosted job IDs, `needs` lists, or check commands fail `make check` with the deterministic diff. Both Make-driven invocations (the mandatory gate and `make check-ci-gate-generation`) pass `--registry "$(GATE_REGISTRY)"`, so Make's registry-derived target selection and every generator validation in that invocation consume the identical registry — a `GATE_REGISTRY` override can never false-green hosted generation against the default registry. Run `make check-ci-gate-generation` solo only to debug drift. To intentionally refresh the generated regions, run `poetry run python scripts/sync_ci_gate_jobs.py --write` from the repository root, then rerun `make check-ci-gate-generation` (or `make check`).
