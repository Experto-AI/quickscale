...
Local `provision_ci_postgres.sh run` lifecycles emit schema-v1 `LIFECYCLE_RECORD` JSON on stderr at child-group readiness and final cleanup. Set `QS_PROVISION_LIFECYCLE_RECORD` to an absolute regular-file path to persist the latest record atomically with mode `0600`; relative paths, symlinks, non-regular targets, and missing parent directories fail before Docker allocation.
