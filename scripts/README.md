...
- [gate_registry.json](./gate_registry.json) — strict JSON v1 declarative gate registry declaring every gating checkpoint across all five execution contexts (local-serial, local-parallel, hosted CI, publish, e2e-trigger). It currently binds six hosted jobs; publish and E2E-trigger requirements remain independently declared. Loaded and validated at startup by `check_gate_parity.py`. Only plain JSON is accepted (no YAML constructs, no trailing commas, no unquoted keys). Duplicate mapping keys are explicitly rejected at the JSON parser level before structural schema validation.
