...
- [gate_registry.json](./gate_registry.json) — strict JSON v1 declarative gate registry declaring every gating checkpoint across all five execution contexts (local-serial, local-parallel, hosted CI, publish, e2e-trigger). It currently binds eight hosted jobs, including the Trivy dependency-vulnerability and Bandit static-security gates; publish and E2E-trigger requirements remain independently declared. Loaded and validated at startup by `check_gate_parity.py`. Only plain JSON is accepted (no YAML constructs, no trailing commas, no unquoted keys). Duplicate mapping keys are explicitly rejected at the JSON parser level before structural schema validation.
...
The registered scripts gate is deliberately outside product coverage: `make check-gate-suites` runs exactly `$(PYTHON) -m pytest scripts/ -p no:cacheprovider --no-cov -q`, removes its temporary recursion sentinel, and leaves `.coveragerc` and the 90% product threshold unchanged. The eight hosted registry-bound jobs are joined by six justified unowned jobs, for fourteen hosted jobs total; `isolation-conformance` is exposed through Make for local use but remains a hosted-unowned, PostgreSQL-backed job pending later lifecycle work. It requires PostgreSQL 18, the pre-created test databases, the restricted role setup, and installed Poetry dependencies; it does not provision PostgreSQL locally. The dependency gate acquires checksum-pinned Trivy v0.74.0 assets for supported Linux and macOS x86_64/arm64 hosts (Windows uses WSL); the static gate uses Bandit 1.9.4. Unsupported native hosts and all acquisition/database failures fail closed.
