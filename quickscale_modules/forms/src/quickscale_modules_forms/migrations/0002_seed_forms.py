...
Uses historical models via ``apps.get_model()`` so this migration does not
depend on the live model class.  This avoids a column-not-found error when
the ``organization`` FK (added in 0004) does not yet exist in the database.
"""
...
# Built-in form presets — must be kept in sync with
# ``forms_seed_presets`` management command data.
_PRESETS = [
    {
        "title": "Contact",
        "slug": "contact",
...
def seed_forms(apps: Any, schema_editor: Any) -> None:
    Form = apps.get_model("quickscale_modules_forms", "Form")  # noqa: N806
    FormField = apps.get_model("quickscale_modules_forms", "FormField")  # noqa: N806
    ...
    for preset in _PRESETS:
        ...
        for field_data in preset["fields"]:
            field_defaults = {k: v for k, v in field_data.items() if k != "name"}  # type: ignore[attr-defined]
            ...
            FormField.objects.get_or_create(
                form=form,
                name=field_data["name"],  # type: ignore[index]
