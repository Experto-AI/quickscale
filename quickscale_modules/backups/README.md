...
- Admin download and validate stay local-file-only in v1. The BackupPolicy admin page exposes a guarded restore surface for either a row-backed eligible artifact already present on disk or a staff-uploaded PostgreSQL custom dump that is quarantined and resolved against trusted recorded artifact metadata before shared restore execution continues. Remote offload still does not create an admin materialization path for remote-only artifacts.
...
- Destructive restore execution is guarded. BackupPolicy-admin restore accepts either a row-backed eligible artifact already present on disk or a staff-uploaded PostgreSQL custom dump that first resolves through quarantined trusted-match validation, never materializes remote-only artifacts, and requires exact filename confirmation plus the existing environment gate; CLI restore remains available under the same guardrails.
...
- restore either a row-backed eligible artifact already present on disk or a staff-uploaded PostgreSQL custom dump from the BackupPolicy admin page when the operator satisfies exact filename confirmation plus the environment gate
- prune expired artifacts
- delete artifacts while removing private files first
- no admin materialization path for remote-only artifacts; private remote offload only happens during backup creation when `target_mode` is `private_remote`, while admin uploads are quarantined restore inputs rather than an offload workflow
...
artifacts already present on disk or a staff-uploaded PostgreSQL custom dump,
and the CLI entrypoint for either a stored artifact id, a snapshot id, or an
operator-supplied dump file path. JSON artifacts remain export-only and are
...
- Admin download and validate only work when the local file is present; admin restore also accepts a quarantined uploaded dump that must resolve to trusted recorded artifact metadata.
- Operator-supplied filesystem-path restore remains CLI-only; the admin restore surface accepts a file upload instead of an arbitrary server path.
