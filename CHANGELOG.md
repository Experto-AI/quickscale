...
- v0.87.0 — unreleased / not yet tagged — Hardening release in progress. Completed: analytics module-owned page at `/analytics/` + dashboard card wired (M0); `showcase_html` shell/nav/dashboard refresh; server-rendered social pages/routes; apply/update safety fixes; real module-embed E2E coverage; state/drift-detection hardening; publish and beta-migration tooling hardening; manifest-driven option-resolution migration (all modules); **F14 test seams** — cross-tenant isolation harness with module-wide parametrization, generated-project boot+serve runtime smoke test in default CI, behavioral-parity replacement for string-exact auth-parity tests, e2e `pragma: no cover` debt cleared; **F11 CRM isolation groundwork** — CRM implies `orgs`+`notifications` in planner/apply (11.1a), current-org access/reset contract with fail-closed behavior (11.1b), canonical solo `/crm/` + SaaS `/orgs/<slug>/crm/` route contract enforced (11.1c), nullable `organization_id` FK on Tag/Company/Contact/Stage/Deal with ContactNote/DealNote parent-derived (11.1d), Tag-first legacy-NULL-preserving partial UniqueConstraints + serializer duplicate-rejection parity (11.1d.1); **F1 manifest wiring** — all 11 catalog modules on manifest-driven wiring path (M4), `social` migrated, `module_wiring_specs.py` deleted, configurator rerouted through manifest registry (M6 — CR-M6-004 resolved, CR-M6-005 stale refs cleaned); **F2 state consolidation** — `state.yml` authoritative with legacy read-through, drift diagnostics in `quickscale status`, fail-fast advisory lock (M2), CR-005 fixed, provenance contract + helper surface groundwork (2.3a), update-path provenance persistence, config-only/non-consolidated state safeguards, project metadata preservation, abort-on-missing-authority, py.typed fix (2.3b — merged to v87). (This entry records main-branch progress only and does not imply publication.)
