...
- v0.87.0 — unreleased / not yet tagged — Hardening release. Consolidates a large hardening pass across tenant isolation, module configuration, the generator, split publication and lockstep, and disaster recovery.
  ...
  - **SA139 recorded partial (2026-08-08; remains open).** The reviewed root-anchor implementation delivered 87 focused tests, passing Ruff checks, and an unchanged quality baseline; the executable review returned `STATUS: ok` at confidence 95. Validation is blocked by `SA139-QG-001` (medium, validation): whole-repo collection resolves the ambiguous `conftest` import at `quickscale_core/tests/test_e2e_xdist_fixtures.py:13` to `quickscale_cli/tests/conftest.py`, so parity and both umbrella commands stop before E2E. Route the defect to its owning scope and rerun the exact validation; this is not a completion or release claim.
