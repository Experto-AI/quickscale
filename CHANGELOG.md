...
- v0.87.0 — unreleased / not yet tagged — Hardening release in progress. Completed: analytics module-owned page at `/analytics/` + dashboard card wired (M0); `showcase_html` shell/nav/dashboard refresh; server-rendered social pages/routes; apply/update safety fixes; real module-embed E2E coverage; state/drift-detection hardening; publish and beta-migration tooling hardening; manifest-driven option-resolution migration (all modules); **F14 test seams** — cross-tenant isolation harness with module-wide parametrization, generated-project boot+serve runtime smoke test in default CI, behavioral-parity replacement for string-exact auth-parity tests, e2e `pragma: no cover` debt cleared; **F11 CRM isolation** — groundwork (11.1) plus F11.2–F11.9 completed (org-scoped create/read isolation, serializer related-field validation, tenant-local CRM bootstrap, existing-data backfill, bulk-deal scoping, CRM admin path, and tests/hardening); **F1 manifest wiring** — all 11 catalog modules on manifest-driven wiring path (M4), `social` migrated, `module_wiring_specs.py` deleted, configurator rerouted through manifest registry (M6 — CR-M6-004 resolved, CR-M6-005 stale refs cleaned); **F2 state consolidation** — `state.yml` authoritative with legacy read-through, drift diagnostics in `quickscale status`, fail-fast advisory lock (M2), CR-005 fixed, provenance contract + helper surface groundwork (2.3a), update-path provenance persistence, config-only/non-consolidated state safeguards, project metadata preservation, abort-on-missing-authority, py.typed fix (2.3b — merged to v87); **F2 provenance** — F2.5–F2.9b completed (provenance triple persistence, branch/default-agnostic SHA proof, split-publish wrapper, tagged/versioned-source publish gate, and operator diagnostics). (This entry records main-branch progress only and does not imply publication.)
