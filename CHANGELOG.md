...
- v0.87.0 — unreleased / not yet tagged — Hardening release. Consolidates a large hardening pass across tenant isolation, module configuration, the generator, split publication and lockstep, and disaster recovery.
  ...
  - **Split publication is idempotent by design.** Split branches are mutable work artifacts, so republishing them is safe, while PyPI is irreversible — that asymmetry is what makes verification possible.
  ...
  - **Seal atomicity re-scoped.** Servers cannot make tag-sealing transactional, so the seal is client check-then-act — reread the branch against the SHA, require the tag absent or already at the intended commit, push a single explicit refspec — plus a fail-closed post-push reread; the race window is documented, not eliminated.
  - **SA136d seal tooling retained as recorded partial delivery (not release-authorized).** Track 3 retains hermetic seal mechanics, CLI/status/batch wiring, and `seal-module` / `seal-modules` / `seal-status` Make surfaces after 25 focused tests plus lint, typecheck, and full suites passed; no remote ref or publication was performed. Full-scope review returned `STATUS: partial` with six blocking implementation/test findings, and `make quality` remains blocked by the unchanged out-of-scope `apply_command.py` complexity baseline, so SA136d stays unchecked and the commands must not be used for a release. The roadmap carries the complete Done / Pending-Blocking / Decisions-needed checkpoint and marks the prior plan stale-unverified for reuse.
  ...
  - **Publication authorization binds and consumes.** Bare `--force-with-lease` cannot distinguish an absent remote from a never-fetched one, so the handoff pins an explicit remote-absence contract that is consumed once, digest-bound; the irreversible push stays human-gated with a per-row confirmation.
