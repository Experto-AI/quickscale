...
- v0.87.0 — unreleased / not yet tagged — Hardening release in progress (completed so far: refreshed the `showcase_html` shell, navigation, and dashboard structure; added server-rendered social pages/routes/nav/dashboard coverage; landed apply/update safety fixes, real module-embed E2E coverage, and state/drift-detection hardening; hardened publish and beta-migration tooling; completed the manifest-driven option-resolution migration across modules; and completed the manifest-driven wiring migration — `social` moved to the manifest path, legacy `module_wiring_specs.py` deleted, configurator flow rerouted through the manifest registry, all catalog modules now resolve through the single generic manifest path, and the two-path resolution defect from the architecture autopsy is closed. This entry records main-branch progress only and does not imply publication.)
