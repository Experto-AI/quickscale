...
- v0.87.0 — unreleased / not yet tagged — Hardening release. Consolidates a large hardening pass across tenant isolation, module configuration, the generator, and disaster recovery.
  ...
  - <a id="sa93-continuation"></a>**SA93 continuation checkpoint (2026-07-17; blocked — external remote-run evidence missing).** **Done:** exact local `make ci-e2e` is green (all 12 stages, Core 35, CLI 29, all 12 integration modules, 91.90% combined coverage, every file at least 80%, empty quarantine); the generated-manifest hashes are synchronized; independent review resolved CR-SA93-REV-005/006 after the remote workflow adopted the maintained Core+CLI runner, bounded production triggers, and honest artifact behavior. Checkpoint bookkeeping for CR-SA93-REV-007 restores the stable advisory ledger, and the roadmap now targets this anchor. **Pending/Blocking:** SA93-EVID-001 (high) — origin has no `v87` ref, no GH auth is configured, and no successful remote run exists. **Advisory:** SA93-ADV-001 pytest-10 fixture ownership; SA93-ADV-002 app-init database access/orgs teardown warnings; SA93-ADV-003 unlabelled worker-harness substage; SA93-ADV-004 non-finite coverage-threshold overrides. **Decisions needed:** no design decision; an authorized operator must choose when to push the merged `v87` ref and dispatch `e2e.yml`, then retain the green run URL/ref/SHA before SA93 can close.
