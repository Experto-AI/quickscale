...
- **Verification:** `poetry run pytest scripts/test_ci_local_parallel.py -q` — 27 passed, including overlap, deterministic replay, aggregate failure handling, serial opt-out, signal cleanup, optional frontend skips, registry-derived gates, and isolated attribution. Synthetic timing evidence was 0.582s parallel versus 2.544s serial. `bash -n scripts/check_ci_locally.sh` and `poetry run python scripts/check_gate_parity.py` pass. With PostgreSQL 18 and its client tools available, `make ci` passed all 11 stages, including required backups coverage (93.27% equal-weight core/CLI mean) and the restricted-role integration gate (94.40% module mean). The broader `scripts/test_gate_parity.py` suite still has one unrelated publish-workflow oracle mismatch; it does not touch local-CI execution or the required `make ci` gate.
- **Scope:** No TP1 blocker remains. This closeout delta did not change Makefile gate definitions, product code, xdist/unit-test behavior, or E2E concurrency; TP1's earlier Makefile work was limited to its focused regression target and `make ci` prerequisite wiring.
