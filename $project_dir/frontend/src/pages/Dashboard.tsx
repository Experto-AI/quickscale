...
import { useModules, useProjectConfig } from '@/hooks/useModules'

type ModuleCard = {
  key: keyof ReturnType<typeof useModules>
  name: string
...
const buildModuleInfo = (socialPath: string): ModuleCard[] => [
  {
    key: 'auth',
    name: 'Authentication',
    icon: UserCircle,
...
export function Dashboard() {
  ...
  const { modulePaths } = useProjectConfig()
  const installedModules = buildModuleInfo(modulePaths.social).filter((m) => modules[m.key])
  const hasModules = installedModules.length > 0

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Welcome to phase2_showcase</h1>
                ...
                <Card className="h-full">
                  ...
                  <CardContent>
                    ...
                    <Button variant="outline" size="sm" asChild>
                      <Link to={mod.href} reloadDocument={mod.reloadDocument}>
                        {mod.actionLabel ?? 'Open'} <ExternalLink className="ml-1 h-3 w-3" />
                      </Link>
                    </Button>
                  </CardContent>
                </Card>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-4">
          ...
          <Button variant="outline" asChild>
            <a
              href="https://github.com/Experto-AI/quickscale"
              target="_blank"
