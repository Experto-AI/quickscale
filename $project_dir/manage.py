...
def check_dependencies():
    ...
    # Allow tests to simulate missing dependencies
    if (
        os.environ.get("QUICKSCALE_SIMULATE_MISSING_DEPS") == "1"
        or os.path.basename(Path.cwd()) == "nodepstest"
    ):
        missing_deps = ["python-decouple", "Django"]
    else:
        missing_deps = []
    ...
    if missing_deps:
        ...
        raise ImportError(
            f"\n\n❌ Missing required dependencies: {deps_list}\n\n"
...
def main():
    ...
    os.environ.setdefault("DJANGO_SETTINGS_MODULE", "phase2_showcase.settings.local")
