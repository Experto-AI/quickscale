...
class CreditConsumptionLogicTests(TestCase):
    ...
    def test_basic_credit_consumption_logic(self):
        ...
        # Verify consumption transaction - with new priority system, expect specific consumption type
        self.assertEqual(transaction.amount, -consumption_amount)
        self.assertIn(transaction.credit_type, ['SUBSCRIPTION_CONSUMPTION', 'PAYG_CONSUMPTION'])
        # The new system appends descriptive text to distinguish credit types
        self.assertIn('Service usage test', transaction.description)
    ...
    def test_multiple_credit_types_tracking(self):
        ...
        # Verify consumption tracking - with new priority system, expect specific consumption type
        self.assertEqual(transaction.amount, -consumption_amount)
        self.assertIn(transaction.credit_type, ['SUBSCRIPTION_CONSUMPTION', 'PAYG_CONSUMPTION'])
    ...
    def test_large_consumption_amounts(self):
        """Test consumption with very large amounts (within database constraints)."""
        # Add large amount of credits (respecting database constraint: max 999,999.99)
        ...
        # Verify remaining balance (999,999.99 - 500,000.00 = 499,999.99)
        final_balance = self.credit_account.get_balance()
        self.assertEqual(final_balance, Decimal('499999.99'))
...
class ServiceIntegrationTests(TestCase):
    ...
    def test_service_credit_integration(self):
        ...
        self.assertIn(credit_transaction.credit_type, ['SUBSCRIPTION_CONSUMPTION', 'PAYG_CONSUMPTION'])
    ...
    def test_service_cross_credit_type_consumption(self):
        ...
        # Verify service usage record - new priority system returns first transaction (subscription)
        # Since we consume all 100 subscription credits first, that's what gets returned
        self.assertEqual(service_usage.credit_transaction.amount, -Decimal('100'))
