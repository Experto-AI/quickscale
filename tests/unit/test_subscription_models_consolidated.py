...
class SubscriptionModelTests(unittest.TestCase):
    ...
    def test_subscription_migration_exists(self):
        """Test that subscription migration file exists."""
        migration_dir = self.credits_app_path / 'migrations'
        if migration_dir.exists():
            migration_files = list(migration_dir.glob('*subscription*.py'))
            self.assertTrue(len(migration_files) > 0, "No subscription migration files found")
