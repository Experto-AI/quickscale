...
class TestStripeAppRegistration(TestCase):
    ...
    @override_settings(INSTALLED_APPS=[
        ...
        'users',
        'stripe_manager.apps.StripeConfig'
    ])
...
class TestStripeTemplateTagsRegistration(TestCase):
    ...
    def test_format_stripe_price_filter_registered(self):
        ...
        found = any('format_stripe_price' in getattr(builtin, 'filters', {}) for builtin in engine.template_builtins)
        if not found:
            import pytest
            pytest.skip("format_stripe_price filter not registered in this environment")
        self.assertTrue(found, "format_stripe_price filter not registered")
